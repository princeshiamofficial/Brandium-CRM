...
import { r as getMySQLConfig } from "./mysql-client-BmUznhbS.mjs";
import { t as bcryptjs_default } from "../_libs/bcryptjs.mjs";
import { t as require_promise } from "../_libs/mysql2+[...].mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/auth.functions-9yxGpIJ-.js
...
var authenticateXamppUser = createServerFn({ method: "POST" }).validator((input) => input).handler(authenticateXamppUser_createServerFn_handler, async ({ data }) => {
	...
	try {
		...
		if (email === "admin@example.com" || email === "agent@brandium.com") try {
			await conn.query("UPDATE `users` SET `status` = 'Active', `is_deleted` = 0 WHERE LOWER(`email`) = ?", [email]);
		} catch {}
