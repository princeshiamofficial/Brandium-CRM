...
//#region node_modules/.nitro/vite/services/ssr/assets/auth.functions-DIrvpG-W.js
...
var checkDatabaseHealth_createServerFn_handler = createServerRpc({
	id: "4d5d41fffb4f4633621c8b72ec909fab889396b3a21af8bf6588c991247c5649",
	name: "checkDatabaseHealth",
	filename: "src/lib/auth.functions.ts"
}, (opts) => checkDatabaseHealth.__executeServer(opts));
var checkDatabaseHealth = createServerFn({ method: "GET" }).handler(checkDatabaseHealth_createServerFn_handler, async () => {
	try {
		const config = getMySQLConfig();
		const conn = await import_promise.createConnection({
			host: config.host,
		...
		await conn.end();
		const version = String(infoRows?.[0]?.version || "Unknown");
