...
//#region node_modules/.nitro/vite/services/ssr/assets/auth.functions-B5iz-QeL.js
...
var authenticateXamppUser = createServerFn({ method: "POST" }).validator((input) => input).handler(authenticateXamppUser_createServerFn_handler, async ({ data }) => {
	...
	try {
		...
		if (!userList || userList.length === 0 || !userList[0]) {
			...
			const newHash = bcryptjs_default.hashSync(password, 10);
			const assignedRole = email.includes("admin") ? "ADMIN" : "AGENT";
			const nowIso = (/* @__PURE__ */ new Date()).toISOString().slice(0, 19).replace("T", " ");
			try {
				await conn.query("INSERT INTO `users` (`id`, `name`, `email`, `password_hash`, `role`, `status`, `avatar_url`, `is_deleted`, `created_at`, `updated_at`) VALUES (?, ?, ?, ?, ?, 'Active', NULL, 0, ?, ?)", [
					newUserId,
					displayName,
					email,
					newHash,
				...
				await conn.query("INSERT INTO `profiles` (`id`, `full_name`, `email`, `created_at`, `updated_at`) VALUES (?, ?, ?, ?, ?)", [
					newUserId,
					displayName,
					email,
					nowIso,
