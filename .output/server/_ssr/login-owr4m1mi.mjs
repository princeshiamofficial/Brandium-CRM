...
import { o as fetchMySQLUsers } from "./auth.functions-D-ttI9sF.mjs";
import { t as supabase } from "./client-45coMFCg.mjs";
import { n as useAuth } from "./auth-CgRTR6JY.mjs";
import { n as toast } from "../_libs/sonner.mjs";
import { t as AccountSuspendedModal } from "./account-suspended-modal-T4IuKKqY.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/login-owr4m1mi.js
...
function LoginPage() {
	...
	async function performLogin(targetEmail, targetPass) {
		...
		try {
			...
			const target = users.find((u) => String(u["email"] || "").toLowerCase() === targetEmail.trim().toLowerCase());
			if (target && (String(target["status"] || "") !== "Active" || Boolean(target["is_deleted"]))) {
				setSubmitting(false);
