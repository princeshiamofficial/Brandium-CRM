...
import { t as require_lib$1 } from "./lru.min.mjs";
import { t as require_umd } from "./long.mjs";
...
var require_lib = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	*/
	...
	var CONTEXT_TRIGGER = /* @__PURE__ */ new Uint8Array(128);
	...
	var format = (sql, values, stringifyObjects, timezone) => {
		...
		while (valuesIndex < length && placeholderPosition !== -1) {
			...
			if (placeholderLength === 2) escapedValue = (0, exports.escapeId)(currentValue);
				...
				if (!(Array.isArray(currentValue) || currentValue instanceof Uint8Array || currentValue instanceof Date || hasSqlString(currentValue) || isDate(currentValue)) && (isRecord(currentValue) || currentValue instanceof Map)) {
...
//#endregion
//#region node_modules/mysql2/lib/ring_queue.js
/**
...
*/
var require_ring_queue = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var RingQueue = class {
		...
		push(item) {
			...
			if (this._tail === this._head) this._grow();
			return this.length;
		}
		unshift(item) {
			if (arguments.length === 0) return this.length;
		...
		shift() {
			...
			if (head < 2 && this._tail > MIN_SHRINK_TAIL && this._tail <= this._list.length >>> 2) this._shrink();
			return item;
		}
		pop() {
			...
			if (this._head < 2 && tail > MIN_SHRINK_TAIL && tail <= capacity >>> 2) this._shrink();
			return item;
		}
		peekAt(index) {
		...
		remove(index, count) {
			...
			if (this._head === this._tail) return;
			const size = this.length;
			if (index >= size || index < -size || count < 1) return;
			if (index < 0) index += size;
			if (count === 1 || !count) return [this.removeOne(index)];
		...
		splice(index, count, ...newItems) {
			...
			if (index < 0) index += size;
			if (index > size) return;
			if (newItems.length === 0) return this.remove(index, count);
			if (index < 0) return;
			const removalCount = count === void 0 ? 1 : count;
			if (removalCount !== (removalCount | 0) || removalCount < 0) return;
			const items = this.toArray();
			let removed;
			if (removalCount === 0) {
				removed = [];
				...
				removed = void 0;
				items.splice(index, 0, ...newItems);
			} else removed = items.splice(index, removalCount, ...newItems);
			this._rebuild(items);
			return removed;
		}
		clear() {
		...
		toArray() {
			const head = this._head;
			const tail = this._tail;
			if (head <= tail) return this._list.slice(head, tail);
			const capacity = this._list.length;
			const items = new Array(this.length);
			let count = 0;
			for (let slot = head; slot < capacity; slot++) items[count++] = this._list[slot];
			for (let slot = 0; slot < tail; slot++) items[count++] = this._list[slot];
			return items;
		}
		_grow() {
			...
			if (this._head === 0) {
				...
				let count = 0;
				for (let slot = this._head; slot < capacity; slot++) grown[count++] = list[slot];
				for (let slot = 0; slot < this._tail; slot++) grown[count++] = list[slot];
				this._list = grown;
				this._head = 0;
				this._tail = capacity;
			}
		...
		_rebuild(items) {
			...
			while (items.length >= capacity) capacity <<= 1;
			this._list = new Array(capacity);
			this._mask = capacity - 1;
			this._head = 0;
			this._tail = items.length;
			for (let i = 0; i < items.length; i++) this._list[i] = items[i];
		}
	};
...
var require_column_definition = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var ColumnDefinition = class {
		...
		[Symbol.for("nodejs.util.inspect.custom")](depth, inspectOptions, inspect) {
			...
			for (const f in fiedFlags) if (inspectFlags & fiedFlags[f]) {
				if (f === "PRI_KEY") flagNames.push("PRIMARY KEY");
			...
			if (typeName === "BLOB") {
				if (this.columnLength === 4294967295) typeName = "LONGTEXT";
		...
		execute(packet, connection) {
			...
			if (packet && packet.isError()) {
				...
				if (this.queryTimeout) {
					Timers$3.clearTimeout(this.queryTimeout);
...
var require_query = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var Query = class Query extends Command {
		constructor(options, callback) {
			...
			this.namedPlaceholders = Object.prototype.hasOwnProperty.call(options, "namedPlaceholders") ? options.namedPlaceholders : void 0;
		...
		_setTimeout() {
			if (this.timeout) {
				...
				this.queryTimeout = Timers$2.setTimeout(timeoutHandler, this.timeout);
			}
		}
		_handleTimeoutError() {
			if (this.queryTimeout) {
		...
		return class BinaryRow {
			...
			next(packet, fields, options) {
				...
				for (let i = 0; i < fields.length; i++) {
					...
					let value;
					if (nullBitmaskBytes[nullByteIndex] & currentFieldNullBit) value = typeof typeCast === "function" ? typeCast(wrapNull(field), () => null) : null;
	...
	var Execute = class Execute extends Command {
		...
		start(packet, connection) {
			...
			try {
				...
				if (this.onResult) this.onResult(error);
...
var require_ssl_profiles = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	*/
	exports["Amazon RDS"] = { ca: require_lib$3().ca };
}));
//#endregion
...
var require_connection_config = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	module.exports = class ConnectionConfig {
		constructor(options) {
			...
			this.database = options.database;
			this.connectTimeout = isNaN(options.connectTimeout) ? 1e4 : options.connectTimeout;
...
var require_named_placeholders = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function createCompiler(config) {
		...
		if (config.cache !== false && !cache) cache = require_lib$1().createLRU({ max: ncache });
	...
	module.exports = class BaseConnection extends EventEmitter$5 {
		constructor(opts) {
			...
			if (!opts.config.stream) {
				if (opts.config.socketPath) this.stream = Net.connect(opts.config.socketPath);
				else {
		...
		format(sql, values, namedPlaceholders) {
			...
			if (typeof namedPlaceholders !== "undefined") opts.namedPlaceholders = namedPlaceholders;
		...
		_resolveNamedPlaceholders(options) {
			let unnamed;
			if (typeof options.namedPlaceholders === "undefined") options.namedPlaceholders = this.config.namedPlaceholders;
		...
		query(sql, values, cb) {
			...
			const rawSql = this.format(cmdQuery.sql, cmdQuery.values !== void 0 ? cmdQuery.values : [], cmdQuery.namedPlaceholders);
			cmdQuery.sql = rawSql;
		...
		getConnection(cb) {
			const _getConnection = (cb) => {
				...
				if (this.config.connectionLimit === 0 || this._allConnections.length < this.config.connectionLimit) {
					connection = new PoolConnection(this, { config: this._createConnectionConfig() });
		...
		query(sql, values, cb) {
			...
			this.getConnection((err, conn) => {
				...
				try {
					...
					conn.release();
					if (typeof cmdQuery.onResult === "function") cmdQuery.onResult(e);
					else cmdQuery.emit("error", e);
				}
			});
			return cmdQuery;
		}
