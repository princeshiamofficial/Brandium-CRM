...
```markdown
- [ ] REVISION_CONFLICT {dimension}/{plan} — required_property: {property} | conflicts with: {locked decision D-nn / GEMINI.md rule / plan constraint} | alternatives: {the agent's alternatives}
```
...
one line beginning `- [ ]`. Never append agent text verbatim, and never append a fenced block. 3. **Resolve** — present the conflict and its alternatives to the user and ask which to take
...
`required_property` still fails, and that choice belongs to the cap escalation. 4. **Close** — the workflow that wrote the line owns flipping it to `- [x]` once the resolution
has been applied, appending ` | resolved: {chosen resolution}`. Readers only read. A line left
open is a live blocker, never a stale artifact. 5. **Re-spawn** with the chosen resolution, then re-evaluate the return from the top of this
...
`/gsd-plan-review-convergence` — it runs _inside_ that loop, so invoking it would be a cycle, and
