...
invisible to the verifier — and worse, the verifier is _confidently wrong_ about it
...
edge into an explicit, checkable assertion _before_ any code exists, after which the
...
held-out backstop. The literature applies these at the _test_ layer (back of pipeline,
generating checks). The differentiated move here is **placement, not technique**: apply the
same edge taxonomy at the _spec_ layer (front of pipeline, generating requirements), with
...
that no taxonomy was _systematically applied at all_. Categories 1, 2, and 4 alone cover
...
1. **Relevance filter first.** Classify each requirement's shape, then raise only the
   ...
   meaningful: it is an edge that _applies_ and was not addressed.
...
a heuristic over prose cues, so a requirement whose wording _is_ edge-relevant but matches no
...
- **status** — `resolved | dismissed | unresolved`:
  - **resolved** — the edge is addressed; _how_ it is addressed is the verification tier.
...
enum smuggled `covered`/`backstop` — both _resolved_ — into one flat list) and lets sibling
...
_applicable_ edge is the precise signal a soft completeness gate raises.
...
that only _touch_ — does the spec say they merge? Resolving it `resolved`/`explicit` writes
...
```json edge-probe:06-resolved-mixed/expected-coverage.json
{
  ...
  "coverage": {
    ...
    "resolved": 2,
    "unresolved": 1,
    "byVerification": { "explicit": 1, "backstop": 0 }
  }
}
```
