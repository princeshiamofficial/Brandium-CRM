...
separating the test that _drives_ the fix from the check that _judges_ it. That
is this gate.
...
5. **Revert-and-reconfirm** (Agans Rule 9 — "If you didn't fix it, it ain't
   ...
   gone. Proves _this_ change is what fixed it. Must run **before** a fix is
...
verification:
  ...
  revert_and_reconfirm:
    { result: pass | fail, bug_returned_on_revert: true | false, fixed_on_reapply: true | false }
...
## Bounded subprocesses (GEMINI.md gauntlet)
...
GEMINI.md; git: 5–30s per the gauntlet). On timeout, the signal is recorded as
