...
`@.agents/gsd-core/references/edge-probe.md` (which _classifies_ non-inferable checks) and
`@.agents/gsd-core/references/prohibition-probe.md` (whose judgment-tier disposition this mirrors).
...
**non-inferable** check the correct answer is _not derivable from the spec alone_ (e.g. "does `[1,2]`
touching `[2,3]` merge?", "is a 'character' a grapheme or a code unit?"). On these the verifier _does
not know that it does not know_: measured behavior is a **confident PASS on the blind-spot check ~100%
...
1. **Exogenous, not endogenous.** The trigger is the _external tag_ (`backstop`), never the verifier's
...
2. **Routing, not diagnosis.** The verifier need not name the omitted rule (if it could, it wouldn't
   be a blind spot). In testing, verifiers abstained correctly while citing the _wrong_ edge. The
...
- **Never silent, never a hard halt.** _Interactive:_ the abstained item routes to the end-of-phase
  human checkpoint. _Autonomous (AFK):_ it produces a prominent `unverified — held-out test
...
- **Evidence strength.** N17 is n=27 verdicts (3 models × 3 conditions × 3 tasks), 1 rep —
  **direction-finding, not powered.** The blind-spot effect is large and monotone
  (100% → 67% → 17%); the two costs are clean single events (a _false_ tag made the strongest model
...
- **Explicit non-goals.** Does NOT identify the omitted rule; does NOT recalibrate decisive verdicts;
  does NOT defend against _malicious compliance_ (a self-graded review rationalizing away its own
  findings). It raises the floor on _honest_ uncertainty about non-inferable checks — that is the
  whole claim.

## Distinct from neighbours

- **vs `PRESENT_BEHAVIOR_UNVERIFIED` (#966 axis):** that is the _inferable-but-unobserved_ case — the
  truth **can** be verified from the spec but was shortcut-passed on symbol presence; the fix is to
  demand behavioral evidence. Honest-verifier is the _non-inferable_ case — the truth **cannot** be
  verified from the spec at all; the fix is to abstain and route to a held-out test. Orthogonal axes
  (insufficient _evidence_ vs insufficient _spec_); both feed the same `human_needed` sink.
