...
function refuseAnchorsAndAliases(yaml) {
  try {
    ...
    if (e instanceof AnchorDetectedSignal) {
      throw new js_yaml_cjs_1.YAMLException(
...
function restoreNullBytesDeep(value) {
  if (typeof value === "string") {
    return value.includes(NULL_BYTE_SENTINEL)
      ? value.split(NULL_BYTE_SENTINEL).join("\u0000")
      : value;
  }
  ...
  if (value && typeof value === "object") {
    ...
    for (const [k, v] of Object.entries(value)) {
      const restoredKey = k.includes(NULL_BYTE_SENTINEL)
        ? k.split(NULL_BYTE_SENTINEL).join(String.fromCharCode(0))
        : k;
...
function flattenScalarForDisplay(value) {
  ...
  if (typeof value === "object") return flattenObjectListItem(value);
...
function extractCommentChannel(yaml, orderedKeys) {
  ...
  const attach = (pathKey, comments) => {
    if (!channel) channel = { leading: Object.create(null), trailing: [] };
...
function countTopLevelKeyShapedLines(region) {
  return (0, text_lines_cjs_1.splitLines)(region).filter((line) => /^\s*[A-Za-z0-9_-]+:/.test(line))
    .length;
}
/**
...
function escapeDoubleQuotedScalar(s) {
  const dumped = (0, js_yaml_cjs_1.dump)(s, {
    schema: js_yaml_cjs_1.FAILSAFE_SCHEMA,
    forceQuotes: true,
    quotingType: '"',
    lineWidth: -1,
  });
...
const YAML_NUMERIC_RE =
  /^(?:\d[\d_]*(?:\.[\d_]*)?(?:[eE][-+]?\d+)?|0[xXbBoO][0-9a-fA-F_]+|\d[\d_]*(?::[0-5]?\d)+(?:\.[\d_]*)?)$/;
    ...
    if (Array.isArray(value)) {
      if (value.length === 0) {
        ...
        for (const item of value) {
          lines.push(
            `  - ${typeof item === "string" && (item.includes(":") || item.includes("#") || scalarNeedsDoubleQuoting(item)) ? `"${escapeDoubleQuotedScalar(item)}"` : item}`,
          );
        }
      }
      ...
      for (const [subkey, subval] of Object.entries(value)) {
        ...
        if (Array.isArray(subval)) {
          ...
          lines.push(
            `  ${subkey}: ${sv.includes(":") || sv.includes("#") || scalarNeedsDoubleQuoting(sv) || generalScalarNeedsNumericQuoting(sv) ? `"${escapeDoubleQuotedScalar(sv)}"` : sv}`,
          );
        }
      }
    } else {
      ...
      if (
        ...
        sv.startsWith("[") ||
        sv.startsWith("{") ||
        scalarNeedsDoubleQuoting(sv) ||
        generalScalarNeedsNumericQuoting(sv)
      ) {
...
function frontmatterDeepEqual(a, b) {
  ...
  if (typeof a === "object" && typeof b === "object") {
    ...
    return ak.every(
      (k) => Object.prototype.hasOwnProperty.call(bo, k) && frontmatterDeepEqual(ao[k], bo[k]),
    );
  }
  return false;
}
...
function objectListFieldWouldLoseData(content, field, newValue) {
  ...
  try {
    ...
    return (
      `frontmatter set refused — the existing "${field}" field contains a nested object-list ` +
...
function cmdFrontmatterValidate(cwd, filePath, schemaName, raw) {
  ...
  const wrongValue = (f) =>
    fm[f] !== undefined &&
    Object.prototype.hasOwnProperty.call(requiredValues, f) &&
    fm[f] !== requiredValues[f];
  ...
  output(
    { valid: missing.length === 0, missing, present, invalidValue, schema: schemaName },
    raw,
