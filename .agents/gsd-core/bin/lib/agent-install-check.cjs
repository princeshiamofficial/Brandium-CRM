...
function getAgentsDir(runtime, projectRoot) {
  ...
  if (projectRoot) {
    ...
    const localConfigDir =
      localConfigDirName === runtime_name_policy_cjs_1.NO_LOCAL_CONFIG_DIR_SENTINEL
        ? undefined
    ...
    if (!localConfigDir) {
      return node_path_1.default.join(
        (0, runtime_homes_cjs_1.getGlobalConfigDir)(resolved),
        "agents",
      );
    }
    ...
    try {
      if (
        node_fs_1.default.lstatSync(localAgentsDir).isDirectory() &&
        node_fs_1.default.lstatSync(manifestPath).isFile()
      ) {
...
function checkCodexModelPosture(runtime, projectRoot) {
  ...
  for (const entry of tomlFiles) {
    ...
    const { model, hasReasoningEffort } = (0, codex_agent_toml_cjs_1.scanTomlLines)(
      (0, codex_agent_toml_cjs_1.stripBOM)(raw),
    );
...
function checkCodexSandboxPosture(runtime, projectRoot) {
  ...
  for (const entry of tomlFiles) {
    ...
    // contain that shape.
    const found = (0, codex_agent_toml_cjs_1.scanTomlLines)(
      (0, codex_agent_toml_cjs_1.stripBOM)(raw),
    ).sandboxMode;
    if (found === null) {
...
function agentFileExists(agentsDir, name, runtime) {
  ...
  if (
    node_fs_1.default.existsSync(base) ||
    node_fs_1.default.existsSync(copilot) ||
    node_fs_1.default.existsSync(codex)
  ) {
    return true;
  }
  ...
  return (
    runtime === "kimi" &&
    node_fs_1.default.existsSync(kimiYaml) &&
    node_fs_1.default.existsSync(kimiPrompt)
  );
}
/**
...
function resolveAgentHint(name, runtime, projectRoot) {
  ...
  candidateDirs.add(
    node_path_1.default.join(
      (0, runtime_homes_cjs_1.getGlobalConfigDir)(resolvedRuntime),
      "agents",
    ),
  );
