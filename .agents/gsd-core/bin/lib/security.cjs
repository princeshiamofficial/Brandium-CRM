...
function loadTrustedGlobalRoots(config) {
  ...
  for (const entry of raw) {
    ...
    if (!node_path_1.default.isAbsolute(expanded)) continue; // reject project-relative
    ...
    if (stripTrailingSep(node_path_1.default.parse(real).root) === stripTrailingSep(real)) continue;
