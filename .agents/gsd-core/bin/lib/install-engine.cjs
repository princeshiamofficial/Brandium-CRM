...
function withInstallerPackageSource(configDir, fn) {
  ...
  const overlay = {
    ...
    existsSync: (candidate) =>
      node_path_1.default.resolve(candidate) === markerPath || base.existsSync(candidate),
    lstatSync: (candidate) =>
      node_path_1.default.resolve(candidate) === markerPath
...
function isRuntimeSurfaceSourceUnavailable(message) {
  return (
    message.startsWith("Runtime Surface source is unavailable or incomplete for ") &&
  ...
  try {
    ...
    return Object.keys(files)
      ...
      .filter(
        (entry) =>
          entry !== "" &&
          !node_path_1.default.posix.isAbsolute(entry) &&
...
function syncRuntimeSurfaceCorpus(source, destination, configDir, manifestPrefix) {
  if (
    hasExistingSymlinkBetween(node_path_1.default.resolve(configDir), destination, {
      allowOptInFollow: isSymlinkedDestOptIn(),
    })
  ) {
    throw new Error(
...
function provisionRuntimeSurfaceCorpus(layout, configDir, scope) {
  ...
  if (typeof markerFile === "string" && markerFile !== "" && required.has("commands")) {
    try {
      const markerPath = runtimeArtifactInstallPlan.assertDestWithinConfigHome(
        configDir,
        markerFile,
      );
      if (
        hasExistingSymlinkBetween(node_path_1.default.resolve(configDir), markerPath, {
          allowOptInFollow: isSymlinkedDestOptIn(),
        })
      ) {
      ...
      installFs().writeFileSync(
        markerPath,
        node_path_1.default.join(corpusRoot, "commands", "gsd") + "\n",
        "utf8",
      );
    } catch {
...
function executingPackageRoot() {
  return node_path_1.default.dirname(
    node_path_1.default.dirname(runtimeArtifactLayout.findInstallSourceRoot()),
  );
}
// ---------------------------------------------------------------------------
...
function _hostBehaviors(runtime) {
  try {
    ...
    return (
      (reg &&
        reg.runtimes &&
        reg.runtimes[runtime] &&
        reg.runtimes[runtime].runtime &&
        reg.runtimes[runtime].runtime.hostBehaviors) ||
      {}
    );
  } catch {
    return {};
...
const SKILLS_CONVERTER_REGISTRY = {
  ...
  convertClaudeCommandToKimiCodeSkill:
    runtimeArtifactConversion.convertClaudeCommandToKimiCodeSkill,
};
// ---------------------------------------------------------------------------
...
function hasExistingSymlinkBetween(root, fullPath, options = {}) {
  ...
  if (
    resolvedFullPath !== resolvedRoot &&
    !resolvedFullPath.startsWith(resolvedRoot + node_path_1.default.sep)
  ) {
    return true;
  }
  ...
  let realRoot;
  try {
    realRoot = installFs().existsSync(resolvedRoot)
      ? installFs().realpathSync(resolvedRoot)
      : resolvedRoot;
  } catch {
...
function _resolveUserArtifactStagingRoot(configDir) {
  const stagingRoot = runtimeArtifactInstallPlan.assertDestWithinConfigHome(
    configDir,
    node_path_1.default.posix.join(".gsd-staging", "user-artifacts"),
  );
  if (
    hasExistingSymlinkBetween(node_path_1.default.resolve(configDir), stagingRoot, {
      allowOptInFollow: isSymlinkedDestOptIn(),
    })
  ) {
    throw new Error(
...
function _tryResolveUserArtifactStagingRoot(configDir) {
  try {
    ...
    console.warn(
      `  [gsd] user-artifact staging unavailable for "${configDir}" (${err.message}) — proceeding without durable staging for this step.`,
    );
    return null;
  }
}
...
function _resolveDevPreferencesSkillTarget(targetDir, runtime, scope = "global") {
  ...
  if (runtime) {
    ...
    skillDir = node_path_1.default.join(
      runtimeArtifactInstallPlan.assertDestWithinConfigHome(
        installRoot,
        skillsKindEntry.destSubpath,
      ),
      stemName,
    );
  } else {
    ...
    skillDir = node_path_1.default.join(
      runtimeArtifactInstallPlan.assertDestWithinConfigHome(targetDir, "skills"),
      "gsd-dev-preferences",
    );
  }
  return {
    skillFile: node_path_1.default.join(skillDir, "SKILL.md"),
    installRoot,
    hasHomeOverride,
  };
}
...
function migrateLegacyDevPreferencesToSkill(
  ...
  if (runtime && target.hasHomeOverride) {
    testHomeGuard.assertTestHomeSandboxed(
      "migrateLegacyDevPreferencesToSkill",
      runtime,
  ...
  if (
    hasExistingSymlinkBetween(node_path_1.default.resolve(installRoot), skillDir, {
      allowOptInFollow: isSymlinkedDestOptIn(),
    })
  ) {
    throw new Error(
...
function _copyStaged(stagedDir, destDir, kind, configDir, runtime) {
  ...
  const installRoot =
    kind && typeof kind.home === "string" && kind.home !== "" ? kind.home : configDir;
  ...
  // #2393: honor GSD_ALLOW_SYMLINKED_DEST for intentional user-owned symlink layouts.
  if (
    hasExistingSymlinkBetween(node_path_1.default.resolve(installRoot), resolvedDest, {
      allowOptInFollow: isSymlinkedDestOptIn(),
    })
  ) {
    throw new Error(
  ...
  const namespacedByDir = runtimeArtifactLayout.isNamespacedByDir(
    kind.kind,
    kind.destSubpath,
    kind.prefix,
  );
  for (const entry of entries) {
    ...
    if (kind.kind === "agents") {
      ...
      destName = _agentExt ? entry.name.replace(/\.md$/, _agentExt) : entry.name;
    } else {
    ...
    installFs().copyFileSync(
      node_path_1.default.join(stagedDir, entry.name),
      node_path_1.default.join(destDir, destName),
    );
  }
}
// ---------------------------------------------------------------------------
...
function _removeGsdEntries(destDir, kind) {
  ...
  for (const entry of installFs().readdirSync(destDir, { withFileTypes: true })) {
    ...
    installFs().rmSync(node_path_1.default.join(destDir, entry.name), {
      recursive: true,
      force: true,
    });
  }
...
function _runLegacyInstallMigrations(runtime, configDir, scope = "global") {
  ...
  if (_hostBehaviors(runtime).legacyCommandsGsdInstallMigration) {
    if (installFs().existsSync(legacyCommandsGsd)) {
      ...
      if (stagingRoot !== null) {
        ...
        stagedLegacyArtifacts = userArtifactStaging.stageUserArtifacts(
          legacyCommandsGsd,
          ["dev-preferences.md"],
          stagingRoot,
        );
  ...
  if (runtime === "hermes") {
    ...
    if (installFs().existsSync(flatSkillsDir)) {
      for (const entry of installFs().readdirSync(flatSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory() && entry.name.startsWith("gsd-")) {
          installFs().rmSync(node_path_1.default.join(flatSkillsDir, entry.name), {
            recursive: true,
          });
        }
      }
  ...
  if (stagedLegacyArtifacts) {
    ...
    try {
      migrated = migrateLegacyDevPreferencesToSkill(
        configDir,
        savedLegacyArtifacts,
        runtime,
        scope,
      );
    } catch (err) {
      console.warn(
        `  [gsd] dev-preferences.md migration skipped for "${configDir}" (${err.message}) — restoring the legacy copy instead.`,
      );
    ...
    const skillTarget = migrationRefused
      ? null
      : _resolveDevPreferencesSkillTarget(configDir, runtime, scope);
    const migrationSatisfied =
      !migrationRefused &&
      (migrated || (skillTarget !== null && installFs().existsSync(skillTarget.skillFile)));
    const nothingLeftUnmigrated =
      migrationSatisfied && migratableNames.length === stagedLegacyArtifacts.names.length;
...
function _runLegacyUninstallCleanup(runtime, configDir, scope = "global") {
  ...
  const isLegacyCommandsGsd =
    _lu === true || (_lu === "global" && (0, install_scope_cjs_1.isGlobalScope)(scope));
  ...
  if (runtime === "hermes") {
    ...
    if (node_fs_1.default.existsSync(flatSkillsDir)) {
      for (const entry of node_fs_1.default.readdirSync(flatSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory() && entry.name.startsWith("gsd-")) {
          node_fs_1.default.rmSync(node_path_1.default.join(flatSkillsDir, entry.name), {
            recursive: true,
          });
        }
      }
    ...
    if (node_fs_1.default.existsSync(nestedGsdDirForUninstall)) {
      for (const entry of node_fs_1.default.readdirSync(nestedGsdDirForUninstall, {
        withFileTypes: true,
      })) {
        if (entry.isDirectory() && !entry.name.startsWith("gsd-")) {
          node_fs_1.default.rmSync(node_path_1.default.join(nestedGsdDirForUninstall, entry.name), {
            recursive: true,
          });
        }
      }
...
function installRuntimeArtifacts(
  ...
  return withInstallFs(deps.fs, () => {
    ...
    const createPlan = () =>
      runtimeArtifactInstallPlan.createRuntimeArtifactInstallPlan({
    ...
    try {
      ...
      for (const item of planResult.plan.items) {
        ...
        if (
          hasExistingSymlinkBetween(node_path_1.default.resolve(installRoot), dest, {
            allowOptInFollow: isSymlinkedDestOptIn(),
          })
        ) {
          throw new Error(
...
function installOpencodeFamilySkills(
  ...
  const dest = runtimeArtifactInstallPlan.assertDestWithinConfigHome(
    installRoot,
    skillsKindEntry.destSubpath,
  );
  // Symlink-escape guard: reject if any path component between installRoot and
  ...
  if (capabilityRegistry) {
    ...
    for (const stem of candidateStems) {
      ...
      const skillName = `${skillsKindEntry.prefix}${stem}`;
      if (!(0, external_descriptor_trust_cjs_1.isPathConfined)(skillName, dest)) continue; // defense-in-depth
      ...
      // registry view.
      installFs().writeFileSync(
        node_path_1.default.join(skillDir, installProfiles.CAPABILITY_SKILL_MARKER),
        found.capId + "\n",
        "utf8",
      );
      count++;
...
function installAgentsKindStandalone(
  ...
  const agentCtx = {
    ...
    attribution,
    targetDir,
    projectDir: projectDir ?? targetDir,
  };
  let stagedDir;
  try {
    stagedDir = agentsKindEntry.stage(resolvedProfile, agentCtx);
  } catch (err) {
    if (scope !== "global" || !isRuntimeSurfaceSourceUnavailable(err.message)) throw err;
    stagedDir = withInstallerPackageSource(targetDir, () =>
      agentsKindEntry.stage(resolvedProfile, agentCtx),
    );
  }
  const stagedAgentFiles = installFs().existsSync(stagedDir)
    ...
    : [];
  const installRoot =
    typeof agentsKindEntry.home === "string" && agentsKindEntry.home !== ""
      ? agentsKindEntry.home
      : targetDir;
  const dest = runtimeArtifactInstallPlan.assertDestWithinConfigHome(
    installRoot,
    agentsKindEntry.destSubpath,
  );
...
function installOpencodeFamilyCommands(
  ...
  for (const entry of installFs().readdirSync(srcDir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      ...
      content =
        _hostBehaviors(runtime).frontmatterDialect === "kilo"
  ...
  if (np && np.source) {
    ...
    if (installFs().existsSync(pluginSrc)) {
      ...
      const destPath = runtimeArtifactInstallPlan.assertDestWithinConfigHome(
        configDir,
        node_path_1.default.join(np.dir, np.file),
      );
      ...
      const markerOutcome = (0, commonjs_marker_cjs_1.ensureCommonJsMarker)(
        node_path_1.default.dirname(destPath),
      );
...
function _migrateLegacyOpencodeCommandDir(runtime, configDir, behaviors) {
  ...
  try {
    ...
    /* best-effort — a non-empty or otherwise-busy dir is left in place */
  }
}
// ---------------------------------------------------------------------------
...
function installOpencodeFamilyArtifacts(
  ...
  const pathPrefix = runtimeArtifactConversion._computePathPrefix({
    ...
    resolvedTarget: (0, shell_command_projection_cjs_1.posixNormalize)(
      node_path_1.default.resolve(configDir),
    ),
  ...
  const skillsWritten = installOpencodeFamilySkills(
    runtime,
    configDir,
    rawCommandsDir,
    pathPrefix,
  ...
  const agentsResult = installAgentsKindStandalone(
    runtime,
    configDir,
    scope,
    resolvedProfile,
  ...
  return {
    ...
    kinds: [
      ...
      ...(agentsResult
        ? [{ kind: "agents", sourceDir: agentsResult.sourceDir, destDir: agentsResult.destDir }]
        : []),
    ],
    cleanup: [],
...
function uninstallRuntimeArtifacts(runtime, configDir, scope, deps = {}) {
  ...
  if (runtime === "hermes") {
    ...
    if (node_fs_1.default.existsSync(nestedGsdDir)) {
      ...
      node_fs_1.default.rmSync(node_path_1.default.join(nestedGsdDir, "DESCRIPTION.md"), {
        force: true,
      });
