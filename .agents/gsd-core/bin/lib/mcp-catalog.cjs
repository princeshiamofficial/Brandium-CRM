...
function classifyUnindexedUri(uri) {
  if (uri.includes("\0")) return exports.REASON.TRAVERSAL_REFUSED;
  if (uri.startsWith("file://")) return exports.REASON.INVALID_URI;
  const normalized = uri.replace(/\\/g, "/");
  if (!normalized.startsWith("gsd-//")) {
    if (normalized.startsWith("/")) return exports.REASON.TRAVERSAL_REFUSED;
    if (WINDOWS_ABS_RE.test(normalized)) return exports.REASON.TRAVERSAL_REFUSED;
    return exports.REASON.INVALID_URI;
  }
  const match = GSD_URI_RE.exec(normalized);
  if (!match) return exports.REASON.INVALID_URI;
  const [, rootSegment, rest] = match;
  if (!RESOURCE_SEGMENTS.has(rootSegment)) return exports.REASON.UNKNOWN_ROOT;
  if (hasTraversalSegment(rest)) return exports.REASON.TRAVERSAL_REFUSED;
...
function readIndexedResource(catalog, entry) {
  ...
  if (!check.safe) {
    fail(
      exports.REASON.TRAVERSAL_REFUSED,
      `indexed resource escapes catalog root: ${entry.relPath}`,
    );
  }
  let raw;
  try {
...
function decodeCursor(cursor) {
  try {
    ...
    if (parsed && typeof parsed === "object" && Number.isInteger(parsed.i) && parsed.i >= 0) {
...
function listResources(catalog, opts = {}) {
  const pageSize =
    opts.pageSize !== undefined && Number.isFinite(opts.pageSize) && opts.pageSize > 0
      ? Math.floor(opts.pageSize)
      : exports.DEFAULT_PAGE_SIZE;
  const sorted = [...catalog.resources.values()].sort((a, b) =>
...
function getPrompt(catalog, name, args) {
  ...
  if (typeof name !== "string" || name.length === 0) {
    fail(
      exports.REASON.INVALID_PARAMS,
      `prompt name must be a non-empty string, got ${typeof name}`,
    );
  }
