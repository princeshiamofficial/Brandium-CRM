...
function buildStateFields(statePath) {
  ...
  const frontmatterCurrentPhase = (0, state_document_cjs_1.stateFieldValue)(
    frontmatter,
    body,
    "current_phase",
    null,
  );
  const legacyCurrentPhaseLabel = (0, state_document_cjs_1.stateFieldValue)(
    frontmatter,
    section ?? body,
    null,
    "Current Phase",
  ...
  const templateCurrentPhaseLabel = (0, state_document_cjs_1.stateFieldValue)(
    frontmatter,
    section ?? body,
    null,
    "Phase",
  ...
  const currentPhaseLabel = {
    value:
      frontmatterCurrentPhase.value ??
      legacyCurrentPhaseLabel.value ??
      templateCurrentPhaseLabel.value,
    scope:
  ...
  const stateStatus = (0, state_document_cjs_1.stateFieldValue)(
    frontmatter,
    section ?? body,
    "status",
    "Status",
...
function buildWorktreeHealthField(cwd) {
  try {
    const result = inspectWorktreeHealth(
      ...
      {
        execGit: shell_command_projection_cjs_1.execGit,
...
function buildPlanningTrackedField(cwd) {
  try {
    const result = (0, shell_command_projection_cjs_1.execGit)(["ls-files", "--", ".planning"], {
      cwd,
      timeout: PLANNING_TRACKED_GIT_TIMEOUT_MS,
    });
    if (result.timedOut) {
      return {
    ...
    if (result.exitCode !== 0) {
      const probe = (0, shell_command_projection_cjs_1.execGit)(
        ["rev-parse", "--is-inside-work-tree"],
        { cwd, timeout: PLANNING_TRACKED_GIT_TIMEOUT_MS },
      );
      if (probe.timedOut) {
        return {
          value: { tracked: false, ignored: false },
...
function buildRoadmapDeclaredPhasesField(roadmapPath, convention) {
  ...
  const { roadmapPhases, sentinelPhases } = (0, validate_cjs_1.buildRoadmapPhaseVariants)(
    content,
    convention,
  );
...
function buildRoadmapBracketIncoherencesField(roadmapPath, convention) {
  ...
  // to carry.
  if (!node_fs_1.default.existsSync(roadmapPath)) return { value: [], scope: SCOPE.UNREADABLE };
...
function buildResearchValidationStatusField(phasesDir, phaseDirNames, enumerationScope) {
  const value = phaseDirNames.map((dir) => {
    ...
    if (researchFile) {
      try {
        const researchContent = node_fs_1.default.readFileSync(
          node_path_1.default.join(fullPhaseDir, researchFile),
          "utf-8",
        );
...
function buildArchivedPhaseTokensField(planBase, convention) {
  ...
  try {
    ...
    if (err.code === "ENOENT") return { value: [], scope: SCOPE.COMPLETE };
    return { value: [], scope: SCOPE.UNREADABLE };
  }
  const value = [];
...
function buildPlanningSnapshot(cwd) {
  ...
  return {
    ...
    researchValidationStatus: buildResearchValidationStatusField(
      paths.phases,
      phaseDirs.value,
      phaseDirs.scope,
    ),
    ...
    currentMilestoneRoadmapPhaseIds: buildCurrentMilestoneRoadmapPhaseIdsField(
      cwd,
      paths.roadmap,
      phaseIdConvention,
    ),
    ...
    roadmapBracketIncoherences: buildRoadmapBracketIncoherencesField(
      paths.roadmap,
      phaseIdConvention,
    ),
  };
