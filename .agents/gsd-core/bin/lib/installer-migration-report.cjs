...
exports.BUNDLED_GSD_HOOK_FILES = Object.freeze(
  new Set([
...
function buildBlockedErrorMessage(blocked) {
  ...
  for (const [reason, actions] of byReason) {
    ...
    if (sample.length > 0) {
      lines.push(
        `      e.g. ${sample.join(", ")}${actions.length > sample.length ? `, ... (+${actions.length - sample.length} more)` : ""}`,
      );
    }
  }
  lines.push(
    `  resolve non-interactively by setting ${exports.RESOLUTION_ENV_VAR}=<choice> ` +
