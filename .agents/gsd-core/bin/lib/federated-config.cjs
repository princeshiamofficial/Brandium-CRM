...
function mergeFederatedConfig(input) {
  ...
  const userConfig =
    ...
    typeof input.userConfig === "object" &&
    !Array.isArray(input.userConfig)
      ? input.userConfig
      : {};
  ...
  for (const key of Object.keys(configSchema)) {
    ...
    try {
      if (isCentralKey(key)) {
        warnings.push(
          'federated-config: key "' +
            key +
            '" is still in the central config-schema (pending-migration); ' +
    ...
    if (!_isWellFormedSlice(slice)) {
      warnings.push(
        'federated-config: config slice for key "' +
          key +
          '" is malformed (missing or invalid type/default); skipping',
      );
      continue;
    }
    ...
    if (userHasKey && userValue !== undefined) {
      ...
      if (_typeMatches(userValue, sliceEntry)) {
        ...
        const typeDesc =
          sliceEntry.type === "enum" && Array.isArray(sliceEntry.values)
            ? "enum(" + sliceEntry.values.join("|") + ")"
            : sliceEntry.type;
        warnings.push(
          'federated-config: user-supplied value for "' +
            key +
            '" has wrong type or invalid enum value ' +
            "(expected " +
            typeDesc +
            ", got " +
            typeof userValue +
