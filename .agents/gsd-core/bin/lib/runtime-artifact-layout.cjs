  ...
  try {
    ...
    for (const name of entries) {
      ...
      if (childStat.isDirectory()) {
        ...
        else node_fs_1.default.accessSync(child, node_fs_1.default.constants.R_OK);
        readableFiles += 1;
      } else {
        return false;
      }
...
function isPhysicallyConfinedTo(root, candidate) {
  try {
    ...
    return (
      ...
      physicalCandidate.startsWith(physicalRoot + node_path_1.default.sep)
    );
  } catch {
    return false;
  }
...
function installedManifestIsComplete(runtimeConfigDir, required) {
  ...
  try {
    ...
    for (const prefix of prefixes) {
      ...
      const corpusRoot = node_path_1.default.resolve(
        runtimeConfigDir,
      ...
      const visit = (dir) => {
        for (const name of io.readdirSync(dir)) {
          ...
          if (stat.isDirectory()) {
            ...
            const relative = node_path_1.default
              .relative(corpusRoot, candidate)
      ...
      for (const key of expected) {
        ...
        if (!candidate.startsWith(root + node_path_1.default.sep)) return false;
        const stat = io.lstatSync(candidate);
        if (!stat.isFile() || stat.isSymbolicLink()) return false;
        if (
          installerMigrations.classifyArtifact(runtimeConfigDir, key, manifest).classification !==
          "managed-pristine"
        ) {
          return false;
        }
...
function providerHasRequiredClasses(provider, required, routed, runtimeConfigDir) {
  ...
  return (
    (!required.has("commands") || isReadableDirectory(provider.commandsRoot, routed)) &&
...
function providersShareRequiredRoots(left, right, required) {
  ...
  return (
    (required.has("commands") && physicalRootsOverlap(left.commandsRoot, right.commandsRoot)) ||
...
function markerProvider(runtimeConfigDir) {
  ...
  try {
    ...
    if (
      packaged &&
      node_path_1.default.resolve(commandsRoot) ===
        node_path_1.default.resolve(packaged.commandsRoot)
    ) {
      return packaged;
    }
    return {
      kind: "marker",
      commandsRoot,
      agentsRoot: node_path_1.default.resolve(
        node_path_1.default.dirname(commandsRoot),
        "..",
        "agents",
      ),
    };
...
function resolveSourceProvider(
  ...
  if (runtimeConfigDir && scope === "global") {
    ...
    if (
      marker &&
      !providersShareRequiredRoots(marker, installed, required) &&
      providerHasRequiredClasses(marker, required, marker.kind !== "package")
    ) {
      return marker;
    }
...
function _resolveNamedConverter(converterName, kindLabel) {
  ...
  if (typeof fn !== "function") {
    throw new Error(
      `Converter "${converterName}" is allowlisted but is not an exported function of runtime-artifact-conversion.cjs.`,
    );
  }
  return fn;
}
...
function convertedAgentsKind(destSubpath, prefix, converterName, configDir, sourceContext, scope) {
  return {
    ...
    stage: (resolved, agentCtx) => {
      ...
      const needsModelOverride =
        converterName === "convertClaudeToOpencodeFrontmatter" ||
      ...
      if (needsModelOverride) {
        ...
        const modelOverrides =
          installModelOverrideResolver.readGsdEffectiveModelOverrides(overrideTargetDir);
        const runtimeResolver =
          installModelOverrideResolver.readGsdRuntimeProfileResolver(overrideTargetDir);
        ...
        // threading (#1173).
        converter = (content) =>
          rawConverter(content, (0, install_scope_cjs_1.isGlobalScope)(scope));
      }
      ...
      return stageAgentsForRuntimeWithConverter(
        ...
        resolved,
        converter,
        (0, install_scope_cjs_1.isGlobalScope)(scope),
        agentCtx?.runtime ? agentCtx : undefined,
      );
    },
  };
    ...
    stage: (resolved, agentCtx) => {
      ...
      if (installFs().existsSync(stagedAgents)) {
        for (const entry of installFs().readdirSync(stagedAgents, { withFileTypes: true })) {
          ...
          subagents.push({
            path: (0, shell_command_projection_cjs_1.posixNormalize)(
              node_path_1.default.join("agents", entry.name),
            ),
      ...
      installFs().writeFileSync(
        node_path_1.default.join(stageDir, "gsd.yaml"),
        artifacts.root.yaml,
      );
      installFs().writeFileSync(
        node_path_1.default.join(stageDir, "gsd.md"),
        artifacts.root.prompt,
      );
      ...
      for (const artifact of artifacts.subagents) {
        installFs().writeFileSync(
          node_path_1.default.join(subagentsDir, `${artifact.name}.yaml`),
          artifact.yaml,
        );
        installFs().writeFileSync(
          node_path_1.default.join(subagentsDir, `${artifact.name}.md`),
          artifact.prompt,
        );
      }
      return stageDir;
...
function skillsKind(
  ...
  return {
    ...
    stage: (resolved) => {
      ...
      return stageSkillsForRuntimeAsSkills(
        sourceRootFor(sourceContext, "commands"),
        resolved,
        wrappedConverter,
        prefix,
        nested,
...
function convertedCommandsKind(destSubpath, prefix, converterName, sourceContext) {
  return {
    ...
    stage: (resolved) => {
      ...
      return stageCommandsForRuntimeFlat(
        sourceRootFor(sourceContext, "commands"),
        resolved,
        converter,
        prefix,
...
function resolveRuntimeArtifactLayout(runtime, configDir, scope = "global", capabilityRegistry) {
  return resolveRuntimeArtifactLayoutFromRegistry(
    getRegistry(),
    runtime,
    configDir,
    scope,
...
function resolveRuntimeArtifactLayoutFromRegistry(
  ...
  const kinds = entries.map((entry) =>
    dispatchKindEntry(entry, runtime, configDir, scope, capabilityRegistry, sourceContext),
  );
...
function isNamespacedByDir(kind, destSubpath, prefix) {
  const destLast = node_path_1.default.posix.basename(
    (0, shell_command_projection_cjs_1.posixNormalize)(destSubpath),
  );
    ...
    for (const entry of entries) {
      ...
      for (const stem of stems) {
        ...
        if (nested && routerStemSet.size > 0 && !routerStemSet.has(stem)) {
          ...
          const routerStem =
            owningRouters && owningRouters.length > 0 ? owningRouters[0] : undefined;
