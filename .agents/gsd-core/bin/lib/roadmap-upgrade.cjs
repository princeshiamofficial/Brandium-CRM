...
const MILESTONE_HEADING_RE =
  /^##\s+(?:\[[^\]]{1,200}\]\s+|Roadmap\s+|[✅🚧]\s*)?v(\d+)\.(\d+)(?:\s|:)/iu;
...
function computeMigrationPlan(cwd, options = {}) {
  ...
  const projectCode =
    typeof configData["project_code"] === "string" ? configData["project_code"] : null;
  // ── Read ROADMAP.md ───────────────────────────────────────────────────────
  let roadmapContent = "";
  try {
  ...
  for (const dirName of existingDirs) {
    ...
    const found = orderedMappings.find(
      (m) =>
        !m._used &&
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const checklistMatch = line.match(
      new RegExp(
        `^(\\s*-\\s*\\[[ x]\\]\\s*\\*{0,2}Phase\\s+)(${PHASE_NUMBER_TOKEN_SOURCE})(\\s*[:\\s*])`,
        "i",
      ),
    );
    if (checklistMatch) {
  ...
  for (const fileName of crossRefFiles) {
    ...
    for (const m of orderedMappings) {
      ...
      if (projectCode) {
        ...
        for (const oldNum of new Set([
          paddedNum + decimalSuffix,
          unpaddedNum + decimalSuffix,
          paddedNum,
          unpaddedNum,
        ])) {
...
function applyMigration(cwd, plan, options = {}) {
  ...
  let gitStatus;
  try {
    gitStatus = (0, node_child_process_1.execSync)("git status --porcelain", {
      cwd,
      encoding: "utf8",
      windowsHide: true,
      timeout: 10_000,
  ...
  try {
    ...
    for (const [filePath, backup] of fileBackups) {
      try {
        if (backup.existed) node_fs_1.default.writeFileSync(filePath, backup.content, "utf8");
        else if (node_fs_1.default.existsSync(filePath)) node_fs_1.default.unlinkSync(filePath);
      } catch {
        /* best-effort */
      }
    }
