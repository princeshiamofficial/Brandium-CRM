...
async function lookupPypi(name, version) {
  try {
    ...
    if (version !== undefined) {
      ...
      uploadTime = versionFiles.length > 0 ? (versionFiles[0].upload_time_iso_8601 ?? null) : null;
    } else {
      const urls = data.urls ?? [];
      uploadTime = urls.length > 0 ? (urls[0].upload_time_iso_8601 ?? null) : null;
    }
    const projectUrls = info.project_urls;
    const repoUrl = projectUrls?.["Source"] ?? projectUrls?.["Homepage"] ?? info.home_page ?? null;
    return {
      exists: true,
...
async function lookupCrates(name, version) {
  try {
    ...
    const downloads =
      rawDownloads != null &&
      typeof Number(rawDownloads) === "number" &&
      !isNaN(Number(rawDownloads))
...
async function checkPackages(
  { ecosystem, packages, version },
  { registry = realRegistry, clock = Date, thresholds = DEFAULT_THRESHOLDS, slopcheck = null } = {},
) {
  const results = [];
