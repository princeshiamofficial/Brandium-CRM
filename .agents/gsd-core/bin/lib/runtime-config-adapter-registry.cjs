 ...
 */
const ALLOWED_CONFIG_RUNTIMES = new Set(
  Object.entries(runtimes)
    .filter(
      ([, cap]) =>
        cap &&
        cap.runtime &&
        typeof cap.runtime["installSurface"] === "string" &&
        cap.runtime["installSurface"] !== "none",
    )
    .map(([id]) => id),
);
...
function resolveInstallPlanFromRuntimes(runtimeDescriptors, runtime) {
  ...
  if (typeof sandboxTier !== "string" || !VALID_SANDBOX_TIERS.has(sandboxTier)) {
    throw new TypeError(
      `Runtime '${runtime}' has a missing or invalid sandboxTier descriptor axis: ${JSON.stringify(sandboxTier)}`,
    );
  }
  ...
  return {
    ...
    extendedHookEvents: Array.isArray(desc["extendedHookEvents"])
      ? [...desc["extendedHookEvents"]]
      : [],
