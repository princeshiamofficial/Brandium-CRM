...
function detectProjectType(cwd) {
  ...
  const pkgRaw = (0, shell_command_projection_cjs_1.platformReadSync)(
...
function detectMonorepoWorkspaces(cwd) {
  // pnpm-workspace.yaml
  const pnpmRaw = (0, shell_command_projection_cjs_1.platformReadSync)(
    node_path_1.default.join(cwd, "pnpm-workspace.yaml"),
  );
  if (pnpmRaw) {
    const workspaces = [];
  ...
  // lerna.json
  const lernaRaw = (0, shell_command_projection_cjs_1.platformReadSync)(
    node_path_1.default.join(cwd, "lerna.json"),
  );
  if (lernaRaw) {
    try {
