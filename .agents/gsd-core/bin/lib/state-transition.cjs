...
const { extractFrontmatter, reconstructFrontmatter, stripFrontmatter, FRONTMATTER_UNPARSEABLE } =
  frontmatter;
 ...
 * test proves nothing).
 */
exports.FIELD_CLASSIFICATION = Object.freeze(
 ...
 * the builder actually emits (GEMINI.md → Generative Fix Divergence). Keys the
...
exports.FRONTMATTER_BODY_SOURCE = Object.freeze(
...
function sessionSourceExists(body, labels) {
  return (
    (0, state_document_cjs_1.stateReplaceFieldInSession)(
      body,
      labels.primary,
      labels.fallback,
      "\u0000probe",
...
function getFrontmatterBodySource(field) {
  if (!Object.prototype.hasOwnProperty.call(exports.FRONTMATTER_BODY_SOURCE, field)) return null;
...
function frontmatterKeyForBodyField(bodyField) {
  ...
  for (const key of Object.keys(exports.FRONTMATTER_BODY_SOURCE)) {
    if (exports.FRONTMATTER_BODY_SOURCE[key].some((f) => f.toLowerCase() === wanted)) return key;
  }
  return null;
}
/**
...
function getFieldClassification(field) {
  if (!Object.prototype.hasOwnProperty.call(exports.FIELD_CLASSIFICATION, field)) return null;
...
function getPreserveWhenUnchangedFields() {
  return Object.keys(exports.FIELD_CLASSIFICATION).filter(
    (field) => exports.FIELD_CLASSIFICATION[field].preservation === "preserve-when-unchanged",
  );
}
/**
...
function scanMeasuredSomething(cls, value) {
  ...
  const rec = value;
  return PROGRESS_TOTAL_KEYS.some(
    (k) => ((0, state_document_cjs_2.toFiniteNumber)(rec[k]) ?? 0) > 0,
  );
}
/**
...
function applyPreserveAlways(field, cls, ctx) {
  ...
  let next;
  if (
    cls.mergeStrategy === "progress-ratchet" &&
    ctx.deriveProgressKeys &&
    derived &&
    derivedMeasured
  ) {
    ...
    if (curatedRecord) {
      ...
      for (const [key, value] of Object.entries(curatedRecord)) {
        ...
        if (ratchetUpKeys.has(key)) {
          const derivedNum =
            typeof derivedRecord[key] === "number" ? derivedRecord[key] : -Infinity;
...
function beginPhaseCore(content, intent, deps) {
  ...
  const isAlreadyExecuting = new RegExp(
    `Executing Phase\\s+${(0, pattern_cjs_1.escapeRegex)(String(intent.phaseNumber))}\\b`,
    "i",
  ).test(currentStatus);
...
function mutateCurrentPositionForAdvance(content, fields, statusDefaults, lastActivityDefaults) {
  ...
  if (fields.status) {
    const replaced = (0, state_document_cjs_1.stateReplaceFieldIfTemplate)(
      sectionBody,
      "Status",
      statusDefaults,
      fields.status,
  ...
  if (fields.lastActivity) {
    const replaced =
      (0, state_document_cjs_1.stateReplaceFieldIfTemplate)(
        sectionBody,
        "Last Activity",
...
function advancePlanCore(content, deps) {
  ...
  const legacyOwnCurrent =
    legacyNMatch || legacyNofMMatch ? planNumberFrom((legacyNMatch ?? legacyNofMMatch)[1]) : null;
  const planOwnCurrent =
    planNofMMatch || planNMatch ? planNumberFrom((planNofMMatch ?? planNMatch)[1]) : null;
  ...
  if (legacyOwnCurrent !== null && planOwnCurrent !== null && legacyOwnCurrent !== planOwnCurrent) {
    return {
      ...
      data: {
        ...
        plan_candidates: [`Current Plan: ${legacyPlan}`, `Plan: ${planField}`],
      },
    };
  }
  const updated = [];
  ...
  if (currentPlanDisplayValue !== undefined) {
    body =
      (0, state_document_cjs_1.stateReplaceField)(body, "Current Plan", currentPlanDisplayValue) ||
      body;
  }
...
function completePhaseCore(content, intent, deps) {
  ...
  const existingPhaseField =
    ...
    (0, state_document_cjs_1.stateExtractField)(body, "Phase");
  ...
  const phaseAfter = (0, state_document_cjs_1.stateReplaceFieldWithFallback)(
    body,
    "Current Phase",
    "Phase",
    newPhaseValue,
  ...
  if (nextPhaseDisplayName) {
    const after = (0, state_document_cjs_1.stateReplaceField)(
      body,
      "Current Phase Name",
      nextPhaseDisplayName,
    );
  ...
  const statusAfter = (0, state_document_cjs_1.stateReplaceFieldWithFallback)(
    body,
    "Status",
    null,
    statusValue,
  ...
  const planAfter = (0, state_document_cjs_1.stateReplaceFieldWithFallback)(
    body,
    "Current Plan",
    "Plan",
    "Not started",
  ...
  const ladAfter = (0, state_document_cjs_1.stateReplaceField)(
    body,
  ...
  const stoppedAfter = (0, state_document_cjs_1.stateReplaceFieldInSession)(
    body,
    "Stopped At",
    "Stopped at",
    stoppedAtLine,
  ...
  if (completedRaw !== null) {
    ...
    const completedAfter = (0, state_document_cjs_1.stateReplaceField)(
      body,
      "Completed Phases",
      String(newCompleted),
    );
    ...
    if (totalPhases && totalPhases > 0) {
      ...
      const progAfter = (0, state_document_cjs_1.stateReplaceField)(
        body,
        "Progress",
        `${newPercent}%`,
      );
...
function plannedPhaseCore(content, intent, deps) {
  ...
  const {
    ...
    body: initialBody,
    reassemble,
  } = beginFrontmatterReassembly(content, deps.sourcePath);
  let body = initialBody;
  const statusDefaults = state_document_cjs_2.KNOWN_TEMPLATE_DEFAULTS["Status"];
  const lastActivityDefaults = state_document_cjs_2.KNOWN_TEMPLATE_DEFAULTS["Last Activity"];
  // Status — template-aware (preserve executor-authored values).
  const statusAfter = (0, state_document_cjs_1.stateReplaceFieldIfTemplate)(
    body,
    "Status",
    statusDefaults,
    "Ready to execute",
  ...
  if (intent.planCount !== null && intent.planCount !== undefined) {
    const result = (0, state_document_cjs_1.stateReplaceField)(
      body,
      "Total Plans in Phase",
      String(intent.planCount),
    );
    if (result) {
      body = result;
  ...
  const lastActivityAfter = (0, state_document_cjs_1.stateReplaceFieldIfTemplate)(
    body,
    "Last Activity",
    lastActivityDefaults,
    today,
  ...
  // Last Activity Description.
  const ladResult = (0, state_document_cjs_1.stateReplaceField)(
    body,
    "Last Activity Description",
...
function milestoneCompleteCore(content, intent, deps) {
  ...
  // Last Activity.
  const lastActivityAfter = (0, state_document_cjs_1.stateReplaceFieldWithFallback)(
    body,
    "Last Activity",
    "Last activity",
    today,
  ...
  // Last Activity Description.
  const ladAfter = (0, state_document_cjs_1.stateReplaceFieldWithFallback)(
    body,
    "Last Activity Description",
    null,
...
function patchCore(content, intent) {
  const { existingFm, hasFrontmatter, fmPrefix, unparseableFm } =
    beginFrontmatterReassembly(content);
...
function updateCore(content, intent) {
  ...
  if (sessionWriteLabels) {
    ...
    const replaced = (0, state_document_cjs_1.stateReplaceFieldInSession)(
      body,
      sessionWriteLabels.primary,
      sessionWriteLabels.fallback,
      intent.value,
    );
  ...
  if (result === null) {
    ...
    const bodySourceExists = sessionProbeLabels
      ...
      : (bodySource ?? []).some(
          (f) => (0, state_document_cjs_1.stateExtractField)(body, f) !== null,
        );
...
function pruneCore(content, intent) {
  ...
  pruneSectionSpan(
    (lv, text) =>
      (lv === 2 || lv === 3) &&
      /^(?:Blockers|Blockers\/Concerns|Blockers\s*&\s*Concerns)$/i.test(text),
    (body) => {
      const keep = [],
        archive = [];
      for (const line of body.split("\n")) {
...
function syncCore(content, intent, deps) {
  ...
  if (intent.totalPlansInPhase !== null) {
    const currentPlansField = (0, state_document_cjs_1.stateExtractField)(
      modified,
      "Total Plans in Phase",
    );
    if (currentPlansField && parseInt(currentPlansField, 10) !== intent.totalPlansInPhase) {
      ...
      const result = (0, state_document_cjs_1.stateReplaceField)(
        modified,
        "Total Plans in Phase",
        String(intent.totalPlansInPhase),
      );
      if (result) {
        modified = result;
        updated.push("Total Plans in Phase");
  ...
  const lastActivityResult = (0, state_document_cjs_1.stateReplaceField)(
    modified,
    "Last Activity",
    today,
  );
...
function rebuildCore(content, _intent, deps) {
  ...
  return {
    ...
    data: {
      ...
      ...(phaseInventoryScan.reason !== null
        ? { phase_inventory_scan_reason: phaseInventoryScan.reason }
        : {}),
    },
  };
}
    ...
    if (existing !== null && existing !== canonicalName) {
      ...
      if (replaced !== null) {
        ...
        log.push({
          ...
          reason:
            "frontmatter 'current_phase_name' is canonical; body 'Current Phase Name' was stale",
        });
      }
    }
  }
...
function reconcileByPhaseTable(content, deps, timestamp, log, meta) {
  ...
  log.push({
    ...
    reason:
      "phase dirs on disk are canonical; rows for missing phases dropped, missing phases added",
  });
  return candidate;
}
/**
...
function deduplicateSessionArchive(content, timestamp, log) {
  ...
  const archiveHeadings = hs.filter(
    (h) =>
      h.level === 3 &&
      h.offset >= sectionStart &&
      h.offset < sectionEnd &&
      SESSION_ARCHIVE_H3.test(h.text),
  );
...
function appendRebuildLogSection(content, entries) {
  ...
  if (sectionHeaderIdx === -1) {
    ...
    const trailer = needsLeadingBlank
      ? ["", REBUILD_LOG_SECTION, "", ...rendered]
      : [REBUILD_LOG_SECTION, "", ...rendered];
  ...
  if (
    ...
    lines[insertAt - 1].trim() !== REBUILD_LOG_SECTION
  ) {
    sep.push("");
  }
