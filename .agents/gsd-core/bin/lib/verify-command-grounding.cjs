 ...
 * not scan cost.
 */
const VERIFY_TOKEN_RE =
  /<(automated|fails_when)[^>]{0,200}>((?:(?!<(?:automated|fails_when)\b)[\s\S])*?)<\/\1>/g;
const PLACEHOLDER_STATEMENTS = new Set([
  "tbd",
  "todo",
  "n/a",
  "na",
...
function resolveVerifyCommandTarget(command, options) {
  ...
  const target = isAbs
    ? node_path_1.default.normalize(norm)
    : node_path_1.default.resolve(base, norm);
  result.form = form;
  result.rawTarget = rawTarget;
...
function harvestPriorVerifyCommands(options) {
  ...
  for (const candidate of candidates) {
    ...
    let planFiles;
    try {
      planFiles = node_fs_1.default
        .readdirSync(phaseDirPath)
        .filter((f) => PLAN_FILE_RE.test(f))
        .sort();
    } catch (err) {
      readErrors.push(toMessage(err));
      continue;
    ...
    for (const file of planFiles) {
      let text;
      try {
        text = node_fs_1.default.readFileSync(
          node_path_1.default.join(phaseDirPath, file),
          "utf-8",
        );
      } catch (err) {
        readErrors.push(toMessage(err));
