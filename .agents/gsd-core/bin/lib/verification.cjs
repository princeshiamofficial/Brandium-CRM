...
const VERIFICATION_ROUTING_TABLE = {
  ...
  human_needed: {
    status: "human_needed",
    next_action:
      "Human verification required. Complete the manual tests in the phase's *-UAT.md, then re-run the verify step until status is passed.",
  ...
  missing: {
    status: "missing",
    next_action:
      "No verification report found — the verify step never completed. Running execute-phase is safe here: it resumes at the verification gates and does not re-run plans that already have a SUMMARY.md (see #2868).",
  ...
  for (const rel of uniqueSorted) {
    ...
    try {
      ...
      if (
        realRel === "" ||
        realRel === ".." ||
        realRel.startsWith(`..${node_path_1.default.sep}`) ||
        node_path_1.default.isAbsolute(realRel)
      ) {
        return null;
      }
...
function defaultPhaseCleanCommitTimesMs(
  phaseDir,
  files,
  execGitFn = shell_command_projection_cjs_1.execGit,
) {
  if (files.length === 0) return new Map();
  const requestedPosix = files.map(toPosix);
  const logRes = execGitFn(
    ["log", "--first-parent", "--format=%ct", "--name-only", "--", ...files],
    {
      cwd: phaseDir,
    },
  );
...
function findStaleVerificationSummary(
  ...
  phaseCleanCommitTimesMs = defaultPhaseCleanCommitTimesMs,
) {
  ...
  try {
    ...
    const summaryFiles = scanPhasePlans(phaseDir).summaryFiles.slice().sort();
...
function readVerificationStatus(phaseDir, opts = {}) {
  ...
  if (declaresFingerprint) {
    ...
    isStale =
      ...
      computeCoveredDigest((0, project_root_cjs_1.findProjectRoot)(phaseDir), coveredFilesVal) !==
        coveredDigestVal ||
...
function cmdVerificationResolveFile(cwd, phaseDirArg, raw) {
  ...
  try {
    ...
    const phaseDirName = node_path_1.default.basename(phaseDir);
    const phaseToken = extractPhaseToken(phaseDirName);
    const verificationFile = resolveVerificationFile(entries, {
      allowBare: true,
      phaseToken,
      phaseDirName,
    });
...
function cmdVerificationFingerprint(cwd, phaseDirArg, files, raw) {
  ...
  if (digest === null) {
    error(
      "could not compute fingerprint — a covered file is missing, unreadable, or escapes the project root",
    );
    return;
  }
