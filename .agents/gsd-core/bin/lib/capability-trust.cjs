...
function collectReviewerLaneSurfaces(manifest, resolveHost) {
  return safeCollect(() => {
    ...
    const invoke =
      typeof invokeRaw === "object" && invokeRaw !== null && !Array.isArray(invokeRaw)
        ? invokeRaw
        : {};
    ...
    const probeIsObj =
      typeof probeRaw === "object" && probeRaw !== null && !Array.isArray(probeRaw);
 ...
 */
const INSTRUCTION_SURFACE_FIELDS = [{ field: "skills", kind: "skill" }];
/**
...
function disclosureSignature(d) {
  ...
  const mods = d.commandModules
    .map((m) => stableJson(["mod", m.family, m.module, m.router || ""]))
    .sort();
  ...
  const lanes = d.reviewerLanes
    .map((l) => {
      ...
      const declaresExtra =
        Object.keys(extra.env).length > 0 ||
        extra.defaultHost !== "" ||
  ...
  return lanes.length > 0
    ? JSON.stringify([hooks, mods, mcp, lanes])
    : JSON.stringify([hooks, mods, mcp]);
}
/**
...
function summarizeDisclosure(disclosure) {
  ...
  if (disclosure.mcpServers.length > 0) {
    lines.push(
      `  MCP servers (${disclosure.mcpServers.length}): spawned/connected by the host runtime`,
    );
    ...
    for (const s of disclosure.mcpServers) {
      ...
      if (isRemote) {
        ...
        if (hdrKeys.length > 0) {
          lines.push(
            `        headers: ${hdrKeys.map((k) => `${renderValueForPrompt(k)}=<redacted>`).join(", ")}`,
          );
        }
      } else {
      ...
      if (envKeys.length > 0) {
        lines.push(
          `        env: ${envKeys
            .map(
              (k) =>
                `${renderValueForPrompt(k)}=${renderValueForPrompt(truncateEnvValue(s.env[k]))}`,
            )
            .join(", ")}`,
        );
      }
  ...
  if (disclosure.reviewerLanes.length > 0) {
    lines.push(
      `  reviewer lane (${disclosure.reviewerLanes.length}): an external reviewer receives plan/review data on every run`,
    );
    for (const l of disclosure.reviewerLanes) {
      ...
      if (l.transport === "openai-http" || (!l.binary && l.hostConfigKey)) {
        ...
        const hostConfigKey = l.hostConfigKey
          ? renderValueForPrompt(l.hostConfigKey)
          : "(hostConfigKey?)";
        lines.push(
          `    - ${slug} -> [openai-http] ${hostConfigKey} => ${renderValueForPrompt(l.resolvedHost)}${localTag}`,
        );
        ...
        if (l.defaultHost) {
          lines.push(
            `        fallback destination declared by this capability: ${renderValueForPrompt(l.defaultHost)}`,
          );
        }
      } else {
      ...
      if (l.probeBinary && l.probeBinary !== l.binary) {
        ...
        lines.push(
          l.probeKind === "command-capability"
      ...
      if (laneEnvKeys.length > 0) {
        lines.push(
          `        env: ${laneEnvKeys
            .map(
              (k) =>
                `${renderValueForPrompt(k)}=${renderValueForPrompt(truncateEnvValue(l.env[k]))}`,
            )
            .join(", ")}`,
        );
        ...
        if (flagged.length > 0) {
          lines.push(
            `        WARNING — ${flagged.map(renderValueForPrompt).join(", ")} can make this lane run code of the capability's choosing`,
          );
        }
      }
