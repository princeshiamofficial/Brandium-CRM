 ...
 * timeout (GEMINI.md's clock-seam rule).
 *
 ...
 * the other fails a test instead of drifting quietly (GEMINI.md's Generative
 * Fix Divergence rule).
 */
...
function parseResolverDeclaration(capabilityId, raw) {
  ...
  const args = Array.isArray(invBody.args)
    ...
    : null;
  if (args === null || args.length !== invBody.args.length) return null;
 ...
 * than hanging the caller (GEMINI.md's Unbounded Subprocesses gauntlet line).
 */
