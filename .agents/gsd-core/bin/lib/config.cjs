...
const { VALID_PROFILES, getAgentToModelMapForProfile, formatAgentToModelMapAsTable } =
  modelProfiles;
// eslint-disable-next-line @typescript-eslint/no-require-imports
...
const SHIP_PR_BODY_SOURCE_RE =
  /^(ROADMAP|PLAN|SUMMARY|VERIFICATION|STATE|REQUIREMENTS|CONTEXT)\.md\s+##\s+[^\r\n#][^\r\n]*$/;
/**
...
function validateShipPrBodySections(value) {
  ...
  value.forEach((section, index) => {
    ...
    const unknownKeys = Object.keys(sectionObj).filter(
      (key) => !SHIP_PR_BODY_SECTION_KEYS.has(key),
    );
    ...
    if (typeof sectionObj["source"] === "string" && sectionObj["source"].trim() !== "") {
      const selectors = sectionObj["source"]
        .split("||")
        .map((selector) => selector.trim())
        .filter(Boolean);
      if (
        selectors.length === 0 ||
        selectors.some((selector) => !SHIP_PR_BODY_SOURCE_RE.test(selector))
      ) {
...
function buildNewProjectConfig(userChoices) {
  ...
  const hasTavilySearch = !!(
    process.env["TAVILY_API_KEY"] || node_fs_1.default.existsSync(tavilyKeyFile)
  );
  ...
  const hasPerplexity = !!(
    process.env["PERPLEXITY_API_KEY"] || node_fs_1.default.existsSync(perplexityKeyFile)
  );
...
function _setNestedValue(config, keyPath, parsedValue) {
  ...
  for (let i = 0; i < keys.length - 1; i++) {
    ...
    if (key === "__proto__" || key === "prototype" || key === "constructor") {
      error(
        "Invalid config key (prototype pollution guard): " + keyPath,
        ERROR_REASON.CONFIG_PARSE_FAILED,
      );
    }
...
function cmdConfigSet(cwd, keyPath, value, raw) {
  ...
  if (!isValidConfigKey(kp, cwd)) {
    error(
      `Unknown config key: "${kp}". Valid keys: ${[...VALID_CONFIG_KEYS].sort().join(", ")}, agent_skills.<agent-type>, features.<feature_name>, phase_commit_docs.<phase-id>`,
      ERROR_REASON.CONFIG_INVALID_KEY,
    );
  }
  ...
  if (parsedValue === null) {
    ...
    if ((0, secrets_cjs_1.isSecretKey)(kp)) {
      const maskedPrev =
        unsetResult.previousValue === undefined
          ? undefined
          : (0, secrets_cjs_1.maskSecret)(unsetResult.previousValue);
      output(
        { ...unsetResult, value: null, previousValue: maskedPrev, masked: true },
        raw,
        `${kp} unset`,
      );
      return;
  ...
  if (kp === "context_window") {
    if (
      ...
      error(
        `Invalid context_window '${val}'. Must be a positive integer (token count).`,
        ERROR_REASON.USAGE,
      );
    }
  }
  ...
  if (kp === "workflow.smart_zone_tokens") {
    if (typeof parsedValue !== "number" || !Number.isSafeInteger(parsedValue) || parsedValue < 1) {
      error(
        `Invalid workflow.smart_zone_tokens '${val}'. Must be a positive integer (token count).`,
        ERROR_REASON.USAGE,
      );
    }
  }
  ...
  if (kp === "git.protected_branches") {
    if (!isValidProtectedBranches(parsedValue)) {
      error(
        `Invalid git.protected_branches '${val}'. Must be a non-empty array of non-empty branch names.`,
      );
    }
  }
  ...
  if (kp === "review.default_reviewers") {
    const normalized = (0, review_reviewer_selection_cjs_1.normalizeConfiguredDefaultReviewers)(
      parsedValue,
    );
  ...
  const instanceLeaf = kp.match(
    /^review\.reviewer_instances\.([a-zA-Z0-9_-]+)\.(cli|model|agent)$/,
  );
  if (instanceLeaf) {
    ...
    if (field === "cli") {
      if (
        ...
        !review_reviewer_selection_cjs_1.KNOWN_REVIEWER_SLUGS.includes(parsedValue)
      ) {
        error(
          `Invalid reviewer_instances.${instanceName}.cli '${val}'. Must be a known reviewer adapter: ${review_reviewer_selection_cjs_1.KNOWN_REVIEWER_SLUGS.join(", ")}.`,
        );
      }
    } else {
...
function resolveFromRootConfig(cwd, kp) {
  ...
  let rootConfig;
  try {
    if (!node_fs_1.default.existsSync(rootConfigPath)) return { found: false, value: undefined };
...
function cmdConfigPath(cwd, _raw, workstreamContext = null) {
  ...
  const configPath =
    workstreamContext && workstreamContext.configPath
...
function cmdMigrateConfig(cwd, raw) {
  ...
  const scopedConfigPath = node_path_1.default.join(
    planningDir(cwd, ws || undefined),
    "config.json",
  );
  ...
  if (raw) {
    ...
    const declinedLines = declined.map(
      (s) =>
        `  ${s.from} → ${s.to}  SKIPPED: '${s.section}' holds a ${s.sectionType}, not an object`,
    );
