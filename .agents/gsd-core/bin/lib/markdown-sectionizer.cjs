...
function collectSection(content, headingPredicate, opts = {}) {
  ...
  const bodyStartOffset =
    bodyStartLine <= lines.length ? lineOffsets[bodyStartLine - 1] : eofOffset;
  // Slice body lines (0-based array: bodyStartLine-1 to bodyEndLine-2 inclusive)
  const bodyRaw = lines
    .slice(bodyStartLine - 1, bodyEndLine - 1)
    .join("\n")
    .trimEnd();
  ...
  return {
    ...
    bodyEnd: bodyStartOffset + body.length,
  };
}
// ─── iterateBullets ───────────────────────────────────────────────────────────
/**
...
function stripTaggedBlocks(content, tagName, allowAttributes = false, caseSensitive = false) {
  ...
  return content.replace(
    taggedBlockPattern(tagName, caseSensitive ? "g" : "gi", allowAttributes),
    "",
  );
}
// ─── replaceSection ───────────────────────────────────────────────────────────
...
function withSection(content, target, edit, opts = {}) {
  if (typeof content !== "string") return content;
  const predicate = typeof target === "function" ? target : (h) => h.text.trim() === target.trim();
