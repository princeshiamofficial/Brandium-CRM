...
function resolveDetectionArgs(args, cwd) {
  ...
  // --runtime reported `runtime_not_claude` on a perfectly ordinary the agent
  ...
  const agentSdkVersion =
    argValue(args, "--agent-sdk-version") ||
  ...
  const hostIntegration = noNested
    ? { dispatch: { nested: false, background: true } }
    : CAPABLE_HOST;
...
function readWavesManifest(wavesPath, error) {
  try {
    ...
    error(
      'could not read/parse --waves file "' +
        wavesPath +
        '": ' +
...
function cmdEmitWorkflow(args, cwd, raw, error) {
  ...
  const budget =
    typeof budgetTokens === "number" && !Number.isNaN(budgetTokens) ? budgetTokens : undefined;
