...
const capabilities = {
  "ai-integration": {
    ...
    description:
      "AI-SPEC design contract workflow for phases that build AI systems; owns the AI integration command, agents, and workflow.ai_integration_phase activation key.",
    tier: "full",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
    ...
    config: {
      "workflow.ai_integration_phase": {
        ...
        default: true,
        description:
          "Prompt for an AI-SPEC design contract before planning phases that involve AI systems.",
      },
  ...
  antigravity: {
    ...
    title: "Antigravity",
    description:
      "Google Antigravity IDE — config/settings home nested under ~/.gemini/antigravity (probed across 1.x and 2.x layouts); global skills/agents install under ~/.gemini/config, the dir AGY scans for global discovery (#3738); Gemini hook event dialect; flat skill layout; tier-1 support.",
    tier: "core",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
    ...
    config: {
      "review.models.agy": {
        ...
        default: "",
        description:
          "Model passed to the Antigravity reviewer lane. The key suffix is the lane binary/flag alias `agy`, not the slug `antigravity` — preserved verbatim so existing .planning/config.json files keep working.",
      },
      "review.max_prompt_tokens_per_reviewer.antigravity": {
        type: "number",
        default: -1,
        description:
          'Prompt-token budget for the Antigravity reviewer lane. Unset is -1, a sentinel: 0 is a legitimate value meaning "do not trim this lane", so it cannot double as "not configured". Keyed on the reviewer slug `antigravity`, not the `agy` binary alias used by review.models.agy.',
      },
      "review.timeouts.antigravity": {
        type: "number",
        default: -1,
        description:
          "Outer wall-clock timeout override (seconds) for the Antigravity reviewer lane. Unset is -1, a sentinel: 0 or a negative number is also treated as unset (a timeout has no legitimate zero/negative value), so no second sentinel is needed. Falls back to the lane's built-in timeoutFloorMs when unset.",
      },
    },
  },
  "assumption-delta": {
    ...
    description:
      "Rarely-firing advisory checkpoint that triggers when a phase makes something plural, optional, or chosen that used to be singular, required, or derived. Surfaces one identity-model question (promote the new general representation to primary, or add it alongside?) so a silent primary-key drift does not accumulate into a later user-facing bug. Non-blocking; fires only on a detected signal.",
    tier: "full",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
    ...
    config: {
      "workflow.assumption_delta": {
        ...
        default: true,
        description:
          "Enable the assumption-delta architecture checkpoint during planning. When a pluralization/optional/chosen signal is detected in the phase scope, the planner is prompted to re-ask whether the primary key / identity model still names the right thing. Advisory (non-blocking).",
      },
    },
    steps: [],
    contributions: [
  ...
  audit: {
    ...
    title: "Audit",
    description:
      "Open-artifact audit and UAT-gap audit for milestone close gates; exposes `gsd-tools audit-uat` (cross-phase UAT outstanding items) and `gsd-tools audit-open` (structured open-artifact scan across debug, tasks, threads, todos, seeds, UAT, verification, context-questions).",
    tier: "full",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
  ...
  augment: {
    ...
    title: "Augment Code",
    description:
      "Augment Code CLI — commands + nested-skill artifact layout; settings-json hook surface; the agent hook event dialect; tier-2 support.",
    tier: "core",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
  ...
  claude: {
    ...
    title: "Claude Code",
    description:
      "Anthropic Claude Code — primary development runtime; tier-1 support with full hook surface and skills-based global install.",
    tier: "core",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
    ...
    config: {
      ...
      "review.max_prompt_tokens_per_reviewer.claude": {
        ...
        default: -1,
        description:
          'Prompt-token budget for the the agent reviewer lane. Unset is -1, a sentinel: 0 is a legitimate value meaning "do not trim this lane", so it cannot double as "not configured".',
      },
      "review.timeouts.claude": {
        type: "number",
        default: -1,
        description:
          "Outer wall-clock timeout override (seconds) for the the agent reviewer lane. Unset is -1, a sentinel: 0 or a negative number is also treated as unset (a timeout has no legitimate zero/negative value), so no second sentinel is needed. Falls back to the lane's built-in timeoutFloorMs when unset.",
      },
      "review.effort.claude": {
        type: "string",
        default: "",
        description:
          "Reasoning effort for the the agent reviewer lane: minimal, low, medium, high, xhigh, max, or inherit. Unset falls back to the lane's declared review default (high); inherit emits no effort argument so the CLI's own configuration decides. An unrecognized value falls back to the lane default rather than being forwarded.",
      },
    },
  },
  "claude-orchestration": {
    ...
    commands: [
      {
        ...
        subcommands: ["detect-backend", "emit-workflow", "resolve-wave-dispatch"],
      },
    ],
    ...
    config: {
      "claude_orchestration.enabled": {
        ...
        default: false,
        description:
          "Master toggle for the the agent orchestration capability. Default-off + BETA: the Workflow-tool execution backend and the ultraplan plan-offload surface are inert unless this is true. When false, loop behaviour is byte-identical to a non-the agent runtime (inline/manual dispatch).",
      },
      "claude_orchestration.execution_backend": {
        type: "enum",
        values: ["auto", "workflow", "inline"],
        default: "auto",
        description:
          "Which execute-phase dispatch backend to use when the capability is enabled. 'auto' (default) activates the Workflow backend only when the runtime is the agent AND the Workflow tool is detected AND the Agent SDK meets claude_orchestration.min_agent_sdk_version; otherwise it falls back to inline. 'workflow' forces the Workflow backend when the tool is present AND the Agent SDK meets the floor (still fails closed to inline if the tool is absent or the SDK is too old — the floor applies in both modes). 'inline' forces today's manual one-agent-per-message dispatch regardless of tool availability.",
      },
      "claude_orchestration.min_agent_sdk_version": {
        type: "string",
        default: "0.3.149",
        description:
          "Minimum Agent SDK version required to activate the Workflow backend under execution_backend='auto'. Defaults to 0.3.149 (the release that introduced the Workflow tool). Raise to pin a higher floor; the detection seam fails closed to inline for any runtime reporting an older or unknown version.",
      },
    },
    steps: [],
    contributions: [
  ...
  cline: {
    ...
    title: "Cline",
    description:
      "Cline (VS Code extension) — global-only nested-skill layout; cline-rules hook surface (.clinerules); no hook events emitted; tier-2 support.",
    tier: "core",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
  ...
  "code-review": {
    ...
    title: "Code review",
    description:
      "Source-file code review and review-fix workflow support for completed execution work.",
    tier: "full",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
    ...
    config: {
      ...
      "workflow.code_review_point": {
        ...
        description:
          "Loop point at which the code-review step registers — execute:post reviews once per phase (default); execute:wave:post reviews once per completed wave, scoped to that wave's diff.",
      },
    },
    steps: [
      {
  ...
  codebuddy: {
    ...
    title: "CodeBuddy",
    description:
      "CodeBuddy (Tencent) — converted commands + skills artifact layout; settings-json hook surface; the agent hook event dialect; tier-2 support.",
    tier: "core",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
    ...
    runtime: {
      ...
      writesSharedSettings: true,
      permissionWriter: null,
      extendedHookEvents: ["SubagentStop", "Stop", "PreCompact", "SubagentStart"],
      hostIntegration: {
        embeddingMode: "declarative",
        commandSurface: "slash-file",
        dispatch: {
  ...
  coderabbit: {
    ...
    title: "CodeRabbit",
    description:
      "CodeRabbit CLI — cross-AI /gsd-review reviewer lane only; not a GSD install target (no runtime body, no artifacts). Reviews the working-tree diff (`coderabbit review --prompt-only`), not the source tree, and accepts neither a prompt nor a model flag; findings are down-weighted in consensus (evidenceClass: diff-only).",
    tier: "full",
    requires: [],
    engines: {
      gsd: ">=1.8.0",
    ...
    config: {
      "review.max_prompt_tokens_per_reviewer.coderabbit": {
        ...
        default: -1,
        description:
          'Prompt-token budget for the CodeRabbit reviewer lane. Unset is -1, a sentinel: 0 is a legitimate value meaning "do not trim this lane", so it cannot double as "not configured".',
      },
    },
  },
  codex: {
    ...
    description:
      "OpenAI Codex CLI — shell-var command style; per-agent sandbox tiers; config.toml + hooks.json hook surface; tier-1 support.",
    tier: "core",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
    ...
    config: {
      ...
      "review.max_prompt_tokens_per_reviewer.codex": {
        ...
        default: -1,
        description:
          'Prompt-token budget for the Codex reviewer lane. Unset is -1, a sentinel: 0 is a legitimate value meaning "do not trim this lane", so it cannot double as "not configured".',
      },
      "review.timeouts.codex": {
        type: "number",
        default: -1,
        description:
          "Outer wall-clock timeout override (seconds) for the Codex reviewer lane. Unset is -1, a sentinel: 0 or a negative number is also treated as unset (a timeout has no legitimate zero/negative value), so no second sentinel is needed. Falls back to the lane's built-in timeoutFloorMs when unset.",
      },
      "review.effort.codex": {
        type: "string",
        default: "",
        description:
          "Reasoning effort for the Codex reviewer lane: minimal, low, medium, high, xhigh, max, or inherit. Unset falls back to the lane's declared review default (high); inherit emits no effort argument so the CLI's own configuration decides. An unrecognized value falls back to the lane default rather than being forwarded.",
      },
    },
  },
  copilot: {
    ...
    description:
      "GitHub Copilot (VS Code) — markdown config format; copilot-inline hook surface; no hook events emitted; flat skill nesting (unconfirmed recursive loader); tier-2 support.",
    tier: "core",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
  ...
  cursor: {
    ...
    title: "Cursor",
    description:
      "Cursor IDE — skills-only workflow surface; hooks.json surface; the agent hook event dialect; recursive skill loader (flat nesting); tier-2 support.",
    tier: "core",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
    ...
    config: {
      ...
      "review.max_prompt_tokens_per_reviewer.cursor": {
        ...
        default: -1,
        description:
          'Prompt-token budget for the Cursor reviewer lane. Unset is -1, a sentinel: 0 is a legitimate value meaning "do not trim this lane", so it cannot double as "not configured".',
      },
    },
  },
  drift: {
    ...
    config: {
      "workflow.drift_threshold": {
        ...
        default: 3,
        description:
          "Minimum number of new structural elements (directories, barrel exports, migrations, routes) before the codebase drift gate triggers a warn or auto-remap action.",
      },
      "workflow.drift_action": {
        ...
        default: "warn",
        description:
          "Action taken by the codebase drift gate when the threshold is exceeded: warn (advisory message) or auto-remap (spawn gsd-codebase-mapper agent to refresh STRUCTURE.md).",
      },
      "workflow.schema_drift_gate": {
        type: "boolean",
        default: true,
        description:
          "Enable the drift gates at execute:wave:post. When enabled, the schema drift gate blocks verification if schema-relevant files changed during execution but no database push command was executed; the codebase drift gate (non-blocking) warns when structural additions exceed the drift_threshold.",
      },
      "workflow.plan_drift_precheck": {
        type: "boolean",
        default: true,
        description:
          "Enable the non-blocking codebase drift pre-check at plan:pre, before /gsd-plan-phase spawns the planner. When enabled, a stale STRUCTURE.md (structural additions exceeding drift_threshold) is surfaced up front as a warn-only advisory pointing to /gsd-map-codebase; it never blocks planning and never spawns the mapper agent. Separate from schema_drift_gate so autonomous/CI runs can silence the plan-time advisory while keeping the execute:wave:post gates enabled.",
      },
      "workflow.context_drift_precheck": {
        type: "boolean",
        default: true,
        description:
          "Enable the non-blocking context-drift pre-check at plan:pre, before /gsd-plan-phase reuses an existing RESEARCH.md/PATTERNS.md/VALIDATION.md/SPEC.md. Compares each artifact's effective last-changed time (git commit time, falling back to mtime for uncommitted edits) against CONTEXT.md's own — an artifact that predates CONTEXT.md's newest decision was derived from a premise that has since changed. Warn-only by default (see workflow.context_drift_action); never blocks planning on its own.",
      },
      "workflow.context_drift_action": {
        ...
        default: "warn",
        description:
          "Action taken by the context-drift gate when a stale upstream artifact is found: warn (advisory message naming the stale artifacts and how to regenerate them) or block (halt plan-phase until the artifacts are regenerated or the check is disabled).",
      },
    },
    steps: [],
    contributions: [],
  ...
  "external-job": {
    ...
    config: {
      "external_job.enabled": {
        ...
        default: false,
        description:
          "Master toggle for the async external-job producer capability. Default-off: the core loop consumes manifests whether or not this is on, but no manifest is ever written unless an executor opts in here.",
      },
      "external_job.backend": {
        ...
        default: "slurm",
        description:
          "Scheduler backend. SLURM is the first adapter; the field is the pluggability seam for future backends (LSF, PBS, Kubernetes batch). Core never interprets this value.",
      },
      "external_job.artifact_dir": {
        type: "string",
        default: "Artifacts/jobs",
        description:
          "Root for per-job artifact directories (e.g. Artifacts/jobs/<jobid>/). Avoids fixed log paths and hardcoding a cluster/project layout. Surfaced by the adapter (`slurm-adapter.cjs submit` prints the resolved value); override via GSD_EXTERNAL_JOB_ARTIFACT_DIR.",
      },
      "external_job.submit_timeout_ms": {
        type: "number",
        default: 30000,
        description:
          "Hard timeout (ms) for the scheduler submit subprocess (e.g. sbatch). Bounded per GEMINI.md unbounded-subprocess policy. Read by the adapter (env GSD_SLURM_SUBMIT_TIMEOUT_MS overrides).",
      },
      "external_job.poll_timeout_ms": {
        type: "number",
        default: 15000,
        description:
          "Hard timeout (ms) for the scheduler poll subprocess (squeue, with sacct fallback). Read by the adapter (env GSD_SLURM_POLL_TIMEOUT_MS overrides).",
      },
    },
    steps: [],
    contributions: [
      {
        ...
        produces: [".planning/async-jobs/<job>.json"],
        consumes: ["PLAN.md"],
  ...
  "gap-analysis": {
    ...
    description:
      "Proactive, non-blocking post-planning coverage report. After all PLAN.md files are generated, cross-references every REQ-ID and D-ID from REQUIREMENTS.md and CONTEXT.md against plan bodies. Emits a Source | Item | Status table. Does not block phase advancement.",
    tier: "standard",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
  ...
  gemini: {
    ...
    title: "Gemini CLI",
    description:
      "Google Gemini CLI — cross-AI /gsd-review reviewer lane only; not a GSD install target (no runtime body, no artifacts). Spawned as `gemini -p - -m <model>` with the plan piped on stdin.",
    tier: "full",
    requires: [],
    engines: {
      gsd: ">=1.8.0",
    ...
    config: {
      ...
      "review.max_prompt_tokens_per_reviewer.gemini": {
        ...
        default: -1,
        description:
          'Prompt-token budget for the Gemini reviewer lane. Unset is -1, a sentinel: 0 is a legitimate value meaning "do not trim this lane", so it cannot double as "not configured".',
      },
      "review.timeouts.gemini": {
        type: "number",
        default: -1,
        description:
          "Outer wall-clock timeout override (seconds) for the Gemini reviewer lane. Unset is -1, a sentinel: 0 or a negative number is also treated as unset (a timeout has no legitimate zero/negative value), so no second sentinel is needed. Falls back to the lane's built-in timeoutFloorMs when unset.",
      },
    },
  },
  graphify: {
    ...
    description:
      "Build, query, and inspect the project knowledge graph in `.planning/graphs/`; exposes graphify CLI subcommands (build, query, status, diff) and the /gsd-graphify skill.",
    tier: "full",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
  ...
  hermes: {
    ...
    title: "Hermes Agent",
    description:
      "Hermes Agent (NousResearch) — skills nest under skills/gsd/ category bucket; nested skill layout; settings-json hook surface; the agent hook event dialect; tier-2 support.",
    tier: "core",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
  ...
  intel: {
    ...
    description:
      "Code-intelligence store for codebase querying, diff, snapshot, and API-surface extraction; exposes `gsd-tools intel` subcommands (query, status, update, diff, snapshot, patch-meta, validate, extract-exports, api-surface) and backs `/gsd-map-codebase` and `gsd-intel-updater`.",
    tier: "full",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
  ...
  kilo: {
    ...
    title: "Kilo Code",
    description:
      "Kilo Code — XDG-based config dir; global skills at ~/.kilo/skills (separate from XDG config); flat command/ + skills artifact layout; no lifecycle hook registration; tier-2 support.",
    tier: "core",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
  ...
  kimi: {
    ...
    title: "Kimi CLI",
    description:
      "Kimi CLI (Moonshot AI) — generic agents root at ~/.config/agents; skills + kimi-agents artifact layout; native config.toml [[hooks]] bus at ~/.kimi/config.toml; background dispatch; tier-2 support.",
    tier: "core",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
  ...
  "kimi-code": {
    ...
    runtime: {
      ...
      hostIntegration: {
        ...
        dispatch: {
          ...
          backgroundDispatch: true,
          builtInSubagents: ["coder", "explore", "plan"],
          isolation: "orchestrator-worktree",
          maxConcurrency: "undocumented",
        },
        modelMode: "passive",
    ...
    config: {
      ...
      "review.max_prompt_tokens_per_reviewer.kimi-code": {
        ...
        default: -1,
        description:
          'Prompt-token budget for the Kimi Code reviewer lane. Unset is -1, a sentinel: 0 is a legitimate value meaning "do not trim this lane", so it cannot double as "not configured".',
      },
      "review.timeouts.kimi-code": {
        type: "number",
        default: -1,
        description:
          "Outer wall-clock timeout override (seconds) for the Kimi Code reviewer lane. Unset is -1, a sentinel: 0 or a negative number is also treated as unset (a timeout has no legitimate zero/negative value), so no second sentinel is needed. Falls back to the lane's built-in timeoutFloorMs when unset.",
      },
    },
  },
  "live-dom-uat": {
  ...
  "llama-cpp": {
    ...
    config: {
      "review.models.llama_cpp": {
        ...
        default: "",
        description:
          "Model requested from the llama.cpp reviewer lane; empty discovers the first model from /v1/models.",
      },
      ...
      "review.max_prompt_tokens_per_reviewer.llama_cpp": {
        ...
        default: -1,
        description:
          'Prompt-token budget for the llama.cpp reviewer lane. Unset is -1, a sentinel: 0 is a legitimate value meaning "do not trim this lane", so it cannot double as "not configured".',
      },
      "review.timeouts.llama_cpp": {
        type: "number",
        default: -1,
        description:
          "Outer wall-clock timeout override (seconds) for the llama.cpp reviewer lane. Unset is -1, a sentinel: 0 or a negative number is also treated as unset (a timeout has no legitimate zero/negative value), so no second sentinel is needed. Falls back to the lane's built-in timeoutFloorMs when unset.",
      },
    },
  },
  "lm-studio": {
    ...
    config: {
      "review.models.lm_studio": {
        ...
        default: "",
        description:
          "Model requested from the LM Studio reviewer lane; empty discovers the first model from /v1/models.",
      },
      ...
      "review.max_prompt_tokens_per_reviewer.lm_studio": {
        ...
        default: -1,
        description:
          'Prompt-token budget for the LM Studio reviewer lane. Unset is -1, a sentinel: 0 is a legitimate value meaning "do not trim this lane", so it cannot double as "not configured".',
      },
      "review.timeouts.lm_studio": {
        type: "number",
        default: -1,
        description:
          "Outer wall-clock timeout override (seconds) for the LM Studio reviewer lane. Unset is -1, a sentinel: 0 or a negative number is also treated as unset (a timeout has no legitimate zero/negative value), so no second sentinel is needed. Falls back to the lane's built-in timeoutFloorMs when unset.",
      },
    },
  },
  mempalace: {
    ...
    description:
      "Cross-session, cross-project memory: deliberate recall before discuss/plan and verbatim capture + temporal-KG sync at phase boundaries, via the MemPalace MCP server and CLI.",
    tier: "full",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
    ...
    config: {
      ...
      "mempalace.memory_mode": {
        ...
        default: "augment",
        description:
          "How MemPalace relates to GSD native memory during recall/capture. 'augment' (default): the palace is an additive layer alongside .planning/graphs/ — native memory stays authoritative. 'kg_backend': knowledge-graph queries resolve against the palace's temporal KG as the primary source, with .planning/graphs/ as fallback. 'replace': recall resolves through the palace as the source of truth, native memory as fallback. Every mode stays onError:skip and default-resilient — an unreachable palace degrades to native memory, and GSD keeps writing .planning/graphs/ so no memory is lost. Cross-mode migration of existing .planning/graphs/ into the palace is a separate, not-yet-implemented concern.",
      },
      "mempalace.wing": {
        type: "string",
        default: "",
      ...
      "mempalace.auto_capture_hooks": {
        ...
        default: false,
        description:
          "Reserved / not yet implemented: will install MemPalace's native stop/precompact Claude Code hooks for passive mid-session capture (the capability's hooks array is currently empty).",
      },
    },
    steps: [
      {
  ...
  nyquist: {
    ...
    description:
      "Validation coverage audit that maps executed work back to tests and manual-only evidence.",
    tier: "full",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
  ...
  ollama: {
    ...
    title: "Ollama",
    description:
      "Ollama local model server — cross-AI /gsd-review reviewer lane only; not a GSD install target (no runtime body, no artifacts). OpenAI-compatible HTTP transport against a user-configured `review.ollama_host` (POST /v1/chat/completions); model discovered via GET /v1/models piped through jq.",
    tier: "full",
    requires: [],
    engines: {
      gsd: ">=1.8.0",
    ...
    config: {
      "review.models.ollama": {
        ...
        default: "",
        description:
          "Model requested from the Ollama reviewer lane; empty discovers the first model from /v1/models.",
      },
      "review.ollama_host": {
        type: "string",
        default: "",
      ...
      "review.max_prompt_tokens_per_reviewer.ollama": {
        ...
        default: -1,
        description:
          'Prompt-token budget for the Ollama reviewer lane. Unset is -1, a sentinel: 0 is a legitimate value meaning "do not trim this lane", so it cannot double as "not configured".',
      },
      "review.timeouts.ollama": {
        type: "number",
        default: -1,
        description:
          "Outer wall-clock timeout override (seconds) for the Ollama reviewer lane. Unset is -1, a sentinel: 0 or a negative number is also treated as unset (a timeout has no legitimate zero/negative value), so no second sentinel is needed. Falls back to the lane's built-in timeoutFloorMs when unset.",
      },
    },
  },
  opencode: {
    ...
    title: "OpenCode",
    description:
      "OpenCode — XDG-based config dir; flat commands/ + skills artifact layout; settings-json config format; no lifecycle hook registration; tier-2 support.",
    tier: "core",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
    ...
    config: {
      ...
      "review.max_prompt_tokens_per_reviewer.opencode": {
        ...
        default: -1,
        description:
          'Prompt-token budget for the OpenCode reviewer lane. Unset is -1, a sentinel: 0 is a legitimate value meaning "do not trim this lane", so it cannot double as "not configured".',
      },
      "review.timeouts.opencode": {
        type: "number",
        default: -1,
        description:
          "Outer wall-clock timeout override (seconds) for the OpenCode reviewer lane. Unset is -1, a sentinel: 0 or a negative number is also treated as unset (a timeout has no legitimate zero/negative value), so no second sentinel is needed. Falls back to the lane's built-in timeoutFloorMs when unset.",
      },
      "review.effort.opencode": {
        type: "string",
        default: "",
        description:
          "Reasoning effort for the OpenCode reviewer lane: minimal, low, medium, high, xhigh, max, or inherit. Unset falls back to the lane's declared review default (high); inherit emits no effort argument so the CLI's own configuration decides. An unrecognized value falls back to the lane default rather than being forwarded.",
      },
    },
  },
  "pattern-mapper": {
    ...
    description:
      "Optional codebase-pattern mapping before planning; owns the pattern mapper agent and workflow.pattern_mapper activation key.",
    tier: "full",
    requires: ["research"],
    engines: {
      gsd: ">=1.6.0",
    ...
    config: {
      "workflow.pattern_mapper": {
        ...
        default: true,
        description:
          "Run the pattern mapper before planning when context or research is available.",
      },
    },
    steps: [
      {
  ...
  pi: {
    ...
    title: "pi",
    description:
      "pi (pi.dev) — bun-runtime programmatic-CLI; TS ExtensionAPI (registerCommand/registerTool/registerProvider/pi.on); single native-extension file at ~/.pi/agent/extensions/gsd.js (.js, not .cjs — pi's extension auto-discovery accepts only .ts/.js, #2470); no shared-settings hook surface; tier-2 support.",
    tier: "core",
    requires: [],
    engines: {
      gsd: ">=1.7.0",
  ...
  "profile-pipeline": {
    ...
    config: {
      "profile-pipeline.enabled": {
        ...
        default: false,
        description:
          "Enable the developer profiling pipeline commands (scan-sessions, extract-messages, profile-sample, write-profile, etc.).",
      },
    },
    commands: [
      {
  ...
  qwen: {
    ...
    title: "Qwen Code",
    description:
      "Qwen Code (Alibaba) — nested-skill artifact layout; settings-json hook surface; the agent hook event dialect; tier-2 support.",
    tier: "core",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
    ...
    config: {
      "review.max_prompt_tokens_per_reviewer.qwen": {
        ...
        default: -1,
        description:
          'Prompt-token budget for the Qwen Code reviewer lane. Unset is -1, a sentinel: 0 is a legitimate value meaning "do not trim this lane", so it cannot double as "not configured".',
      },
    },
  },
  "refactor-trigger": {
    ...
    config: {
      "refactor.trigger_enabled": {
        ...
        default: false,
        description:
          "Enable the complexity-triggered refactor hook. When true, an execute:post step evaluates the complexity of the files the phase touched and writes a scoped refactor proposal if a function crosses refactor.complexity_threshold or jumps past refactor.complexity_jump_delta. Opt-in; when false the hook never runs. Issue #1953.",
      },
  ...
  research: {
    ...
    description:
      "Optional phase research before planning; owns the phase researcher agent and workflow.research activation key.",
    tier: "standard",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
    ...
    config: {
      "workflow.research": {
        ...
        default: true,
        description:
          "Run phase research before planning when research artifacts are missing or explicitly refreshed.",
      },
    },
    steps: [
      {
  ...
  "schema-gate": {
    ...
    description:
      "Detects ORM schema-relevant files in the phase scope during planning and injects a mandatory [BLOCKING] schema push task into the plan. Prevents false-positive verification where build/types pass because TypeScript types come from config, not the live database.",
    tier: "full",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
    ...
    config: {
      "workflow.schema_push_detection": {
        ...
        default: true,
        description:
          "Enable ORM schema push detection during planning. When schema-relevant files are detected in the phase scope, a [BLOCKING] push task is injected into the plan.",
      },
    },
    steps: [],
    contributions: [
  ...
  security: {
    ...
    description:
      "Threat mitigation verification and ship-time security blocking for phases with security enforcement enabled.",
    tier: "full",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
  ...
  tdd: {
    ...
    description:
      "Injects TDD heuristics into the planner and enforces RED/GREEN gate compliance on type:tdd plans after execution. Owns workflow.tdd_mode; the --tdd CLI flag is the ephemeral override.",
    tier: "full",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
    ...
    config: {
      "workflow.tdd_mode": {
        ...
        default: false,
        description:
          "Enable TDD mode: planner annotates eligible tasks type:tdd and executor enforces RED/GREEN/REFACTOR gate sequence.",
      },
    },
    steps: [],
    contributions: [
  ...
  trae: {
    ...
    title: "Trae IDE",
    description:
      "Trae IDE — nested-skill artifact layout; no hook surface (profile-marker-only config); tier-2 support.",
    tier: "core",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
  ...
  vscode: {
    ...
    title: "VS Code",
    description:
      "VS Code — Marketplace/VSIX extension; no file-projected config directory; IDE-profile reference host (active vscode.lm model, engine-owned hook bus, sandboxed globalState/workspaceState stateIO).",
    tier: "core",
    requires: [],
    engines: {
      gsd: ">=1.7.0",
  ...
  windsurf: {
    ...
    title: "Windsurf",
    description:
      "Windsurf (Codeium) — workspace workflow artifact layout for slash commands; Cascade native hooks.json blocking hook bus (pre_write_code, pre_run_command); tier-2 support.",
    tier: "core",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
  ...
  zcode: {
    ...
    title: "ZCode",
    description:
      "ZCode (Z.ai) — desktop Agentic Development Environment for GLM-5.2; Claude-shaped nested skills at ~/.zcode/skills/<name>/SKILL.md, slash commands, named subagents, native MCP; declarative plugin surface; profile-marker install; tier-2 community support.",
    tier: "core",
    requires: [],
    engines: {
      gsd: ">=1.6.0",
...
const capabilityClusters = {
  "ai-integration": ["ai-integration-phase"],
  "code-review": ["code-review"],
  graphify: ["graphify"],
  mempalace: ["mempalace-capture", "mempalace-recall"],
  nyquist: ["validate-phase"],
  "profile-pipeline": ["profile-user"],
  security: ["secure-phase"],
