...
function applyWorktreeBaseRef(settings) {
  ...
  if (settings === null || Array.isArray(settings) || typeof settings !== "object") {
    throw new TypeError(
      `applyWorktreeBaseRef: expected a plain object, got ${settings === null ? "null" : Array.isArray(settings) ? "array" : typeof settings}`,
    );
  }
...
function resolveEffectiveBaseRef(claudeDir, deps, userClaudeDir) {
  ...
  if (
    userClaudeDir &&
    node_path_1.default.resolve(userClaudeDir) !== node_path_1.default.resolve(claudeDir)
  ) {
...
function cmdWorktreeBaseCheck(cwd, args, deps) {
  ...
  if (modeIdx !== -1) {
    ...
    if (value !== "harness-worktree" && value !== "orchestrator-worktree") {
      throw new Error(
        `worktree base-check: --mode must be harness-worktree or orchestrator-worktree, got ${JSON.stringify(value ?? null)}`,
      );
    }
    isolationMode = value;
  }
...
function cmdWorktreeSetBaseRef(cwd, _args, deps) {
  ...
  if (apply.changed) {
    ...
    const mkdirFn =
      deps?.mkdir ??
      ((p, opts) => {
        node_fs_1.default.mkdirSync(p, opts);
      });
    if (!existsSync(dir)) {
...
function evaluateWorktreeBaseDegrade(deps) {
  ...
  if (
    headIgnoredByHarness &&
    (deps?.isolationMode ?? "harness-worktree") === "orchestrator-worktree"
  ) {
    return {
      shouldDegrade: false,
      reason: "baseref-head",
  ...
  if (headIgnoredByHarness) {
    ...
    return {
      shouldDegrade: true,
      reason: "baseref-head-ignored-by-harness",
      message,
      headSha,
      forkRef,
      forkSha,
