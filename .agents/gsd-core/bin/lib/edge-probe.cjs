...
exports.SHAPE_CUES = {
  "numeric-range":
    /\b(round(ing|ed)?|threshold|max(imum)?|min(imum)?|limit|bound(ary)?|between|cap|percent|amount|price|count|number|score|rate|decimal)\b/i,
  collection:
    /\b(lists?|arrays?|sets?|items?|collections?|each|every|all|sort(ed|ing)?|merge|dedupe|group|ranges?|intervals?|overlap(ping)?)\b/i,
  text: /\b(string|text|names?|labels?|truncate|substring|char(acter)?s?|length|slug|message|unicode)\b/i,
  stateful:
    /\b(save|persist|store|update|toggle|create|delete|remove|submit|retry|apply|register|insert)\b/i,
...
function classifyShape(text) {
  ...
  for (const shape of Object.keys(exports.SHAPE_CUES)) {
    if (exports.SHAPE_CUES[shape].test(subject)) shapes.push(shape);
  }
  return shapes;
}
/**
...
exports.TAXONOMY = [
  ...
  },
  {
    ...
    shapes: ["text"],
    probe:
      "Whose definition of length/equality applies — bytes, code points, grapheme clusters, or normalized form?",
  },
  {
    id: "ordering",
  ...
  {
    id: "idempotency",
    name: "Idempotency / repetition",
    shapes: ["stateful"],
    probe: "What happens if this runs twice on the same input?",
  },
  {
    id: "concurrency",
    name: "Concurrency / effect ordering",
    shapes: ["stateful", "io"],
...
function validateRequirement(requirement) {
  ...
  if (r.text_en != null && !(typeof r.text_en === "string" && r.text_en.trim())) {
    throw new Error(
      `requirement ${requirement.id} text_en must be a non-empty string when present`,
    );
  }
}
  ...
  if (Array.isArray(requirement.shapes)) {
    ...
    for (const s of requirement.shapes) {
      if (typeof s !== "string" || !exports.VALID_SHAPES.has(s)) {
        throw new Error(
          `invalid shape ${JSON.stringify(s)} for requirement ${requirement.id} — must be one of: ${[...exports.VALID_SHAPES].join(", ")}`,
        );
      }
    }
...
if (require.main === module) {
  ...
  runMain(() => {
    (0, probe_core_cjs_1.runProbeCli)(
      (requirements, resolutions) => analyzeCoverage(requirements, resolutions),
