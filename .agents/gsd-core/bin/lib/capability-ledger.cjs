...
function isValidLedgerEntry(id, entry) {
  ...
  if (Object.prototype.hasOwnProperty.call(e, "_pending")) {
    ...
    if (pending !== undefined) {
      ...
      if (p["sharedFiles"].length > MAX_SHARED_FILES) return false;
...
function fsyncContainingDir(dest) {
  ...
  try {
    ...
    if (code !== undefined && !DIR_FSYNC_TOLERATED_ERRNOS.has(code)) {
      ...
      throw new Error(
        `Directory fsync of "${node_path_1.default.dirname(dest)}" failed (${code}); durability of the ledger ` +
...
function reconcile(runtimeDir) {
  ...
  if (ledger === null) {
    ...
    try {
      ...
      if (lstatErr.code !== "ENOENT") entryExists = true;
    }
    if (entryExists) {
