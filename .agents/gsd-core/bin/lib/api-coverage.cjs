...
const LOCAL_URL_RE =
  /^[([<"'`]*[a-z][a-z0-9+.-]*:\/\/(?:localhost|127(?:\.\d{1,3}){1,3}|0\.0\.0\.0|\[::1\])(?=[:/?#]|$)/i;
...
function scanLineTokens(line, nounRe, nounSet) {
  ...
  while ((m = tokenRe.exec(line)) !== null) {
    ...
    let trailLen = 0;
    while (
      trailLen < rawTok.length &&
      CLAUSE_BOUNDARY_CHARS.has(rawTok[rawTok.length - 1 - trailLen])
    ) {
      trailLen++;
    }
...
function detectApiIntegration(text, terms) {
  ...
  const stripped = (0, markdown_sectionizer_cjs_1.stripFencedCode)(
    text.replace(/\r\n/g, "\n"),
  ).text;
  ...
  const verbRe = hasCompoundTerms
    ? new RegExp(
        "(^|[^a-zA-Z0-9])(" +
          effective.verbs.map(pattern_cjs_1.escapeRegex).join("|") +
          ")(?=[^a-zA-Z0-9]|$)",
        "gi",
      )
    : null;
  const nounRe = hasCompoundTerms
    ? new RegExp(
        "(^|[^a-zA-Z0-9])(" +
          effective.nouns.map(pattern_cjs_1.escapeRegex).join("|") +
          ")(?=[^a-zA-Z0-9]|$)",
        "gi",
      )
    : null;
      ...
      for (const clause of clauses) {
        ...
        if (nouns.length > 0) {
          ...
          if (maxNounStart > minVerbStart) {
            ...
            hasNegatedNoun = betweenWords.some((w) =>
...
function parseCoverageMatrix(text) {
  ...
  const declMatch = NO_INTEGRATION_DECLARATION_RE.exec(
    (0, markdown_sectionizer_cjs_1.stripFencedCode)(src).text.replace(HTML_COMMENT_RE, ""),
  );
  if (declMatch) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (!row.capability) {
      ...
      if (/[|\n\r]/.test(row.capability)) {
        errors.push(
          `row[${i}]: capability contains a pipe or newline (unsupported in a table cell)`,
        );
      }
...
function renderCoverageMatrix(rows) {
  const body = rows.map((r) => `| ${r.capability} | ${r.decision} | ${r.reason} |`).join("\n");
...
if (require.main === module) {
  ...
  const verbsOverride =
    typeof verbsVal === "string" && verbsVal.length > 0 && !verbsVal.startsWith("-");
  const nounsOverride =
    typeof nounsVal === "string" && nounsVal.length > 0 && !nounsVal.startsWith("-");
  if (verbsOverride || nounsOverride) {
    termsOverride = {};
    if (verbsOverride) {
      termsOverride.verbs = verbsVal
        .split(",")
        .map((t) => t.trim().toLowerCase())
        .filter(Boolean);
    }
    if (nounsOverride) {
      termsOverride.nouns = nounsVal
        .split(",")
        .map((t) => t.trim().toLowerCase())
        .filter(Boolean);
    }
  ...
  process.stdin.on("end", () => {
    ...
    if (input.trim().length === 0) {
      cliExit.terminateNow(
        "NO_INPUT",
        wantJson ? { skipped: true, reason: "no_input" } : undefined,
      );
    }
  ...
  process.stdin.on("error", (err) => {
    ...
    cliExit.terminateNow(
      "UNAVAILABLE",
      wantJson ? { skipped: true, reason: "stdin_error" } : undefined,
    );
  });
}
