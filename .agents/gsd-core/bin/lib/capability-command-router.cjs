...
async function routeCapabilityCommand({ args, cwd, raw }) {
  ...
  const capResolveScope = (scope) => {
    ...
    if (s !== "global" && s !== "project") {
      error(
        `Invalid --scope "${s}": expected global or project`,
  ...
  const capReadStrict = () => {
    ...
    if (
      cfg &&
      cfg.capabilities &&
      Object.prototype.hasOwnProperty.call(cfg.capabilities, "strict_known_registries")
    ) {
  ...
  const capRunReconcile = (runtimeDir, lifecycle, scope) => {
    try {
      const report = lifecycle.reconcileCapabilities({
        runtimeDir,
        scope,
      ...
      if (report && Array.isArray(report.warnings)) {
        for (const w of report.warnings) {
          try {
            process.stderr.write(`capability reconcile: ${w}\n`);
          } catch {
            /* best-effort */
          }
        }
      ...
      /* best-effort crash recovery — never block the op on a reconcile failure */
    }
  };
  if (capSubcommand === "state") {
    ...
    if (!capId || capId.startsWith("--")) {
      error(
        "Missing capability id for: capability set <id>",
        ERROR_REASON ? ERROR_REASON.USAGE : undefined,
      );
    }
    // Parse --config-dir
    ...
    if (hasOn && hasOff) {
      error(
        "Conflicting flags: --on/--enable and --off/--disable cannot both be present",
        ERROR_REASON ? ERROR_REASON.USAGE : undefined,
      );
    }
    let setEnabled;
    ...
    for (let gi = 0; gi < args.length; gi++) {
      if (args[gi] === "--gate") {
        ...
        if (!gateVal || gateVal.startsWith("--")) {
          error(
            "Missing value for --gate (expected <key>=<true|false>)",
            ERROR_REASON ? ERROR_REASON.USAGE : undefined,
          );
        }
        ...
        if (eqIdx === -1) {
          error(
            `Malformed --gate value "${gateVal}": expected <key>=<true|false>`,
            ERROR_REASON ? ERROR_REASON.USAGE : undefined,
          );
        }
        ...
        if (gateBoolStr !== "true" && gateBoolStr !== "false") {
          error(
            `Malformed --gate value "${gateVal}": bool must be true or false`,
            ERROR_REASON ? ERROR_REASON.USAGE : undefined,
          );
        }
    ...
    capabilityWriter.cmdCapabilitySet(
      ...
      {
        enabled: setEnabled,
        gates: Object.keys(setGates).length > 0 ? setGates : undefined,
        runtime: setRuntime,
        scope: setScope,
      },
      raw,
    ...
    if (!spec || spec.startsWith("--")) {
      error(
        "Missing <spec> for: capability install <spec>",
        ERROR_REASON ? ERROR_REASON.USAGE : undefined,
      );
    }
    ...
    if (!all && !id) {
      error(
        "capability update requires <id> or --all",
        ERROR_REASON ? ERROR_REASON.USAGE : undefined,
      );
    }
    if (all && id) {
      error(
        "capability update: pass either <id> or --all, not both",
        ERROR_REASON ? ERROR_REASON.USAGE : undefined,
      );
    }
    const { scope, runtimeDir } = capResolveScope(capFlagValue("--scope"));
    ...
    try {
      ...
      error(
        `capability update blocked: ${err.message}`,
        ERROR_REASON ? ERROR_REASON.SDK_FAIL_FAST : undefined,
      );
    }
    ...
    if (all) {
      ...
      if (r.status === "upgraded") {
        ...
        error(
          `capability "${id}" is not installed in ${scope} scope; use: capability install`,
          ERROR_REASON ? ERROR_REASON.USAGE : undefined,
        );
        ...
        error(
          `capability update blocked: ${(r.blockReasons || ["unknown reason"]).join("; ")}`,
          ERROR_REASON ? ERROR_REASON.SDK_FAIL_FAST : undefined,
        );
      }
    }
    ...
    if (!id || id.startsWith("--")) {
      error(
        "Missing <id> for: capability remove <id>",
        ERROR_REASON ? ERROR_REASON.USAGE : undefined,
      );
    }
    const { scope, runtimeDir } = capResolveScope(capFlagValue("--scope"));
    ...
    try {
      ...
      error(
        `capability remove blocked: ${err.message}`,
        ERROR_REASON ? ERROR_REASON.SDK_FAIL_FAST : undefined,
      );
    }
    const inLedger = !!(
      removeLedger &&
      removeLedger.entries &&
      Object.prototype.hasOwnProperty.call(removeLedger.entries, id)
    );
    if (!inLedger) {
      ...
      if (
        base &&
        base.capabilities &&
        Object.prototype.hasOwnProperty.call(base.capabilities, id)
      ) {
        error(
          `"${id}" is a first-party capability and cannot be removed here; use the product uninstaller (gsd --uninstall)`,
          ERROR_REASON ? ERROR_REASON.USAGE : undefined,
        );
      }
    }
    ...
    if (res.status === "removed") {
      ...
      if (res.consentRevokeFailed) {
        process.stderr.write(
          `warning: ${res.consentRevokeWarning || `consent record for "${id}" could not be revoked; clear it with: gsd capability trust revoke ${id}`}\n`,
        );
      }
      output(
        {
      ...
      error(
        `capability "${id}" is not installed in ${scope} scope`,
        ERROR_REASON ? ERROR_REASON.USAGE : undefined,
      );
    } else {
      error(
        `capability remove blocked: ${(res.blockReasons || ["unknown reason"]).join("; ")}`,
        ERROR_REASON ? ERROR_REASON.SDK_FAIL_FAST : undefined,
      );
    }
    ...
    if (listScopeArg && listScopeArg !== "global" && listScopeArg !== "project") {
      error(
        `Invalid --scope "${listScopeArg}": must be "global" or "project"`,
        ERROR_REASON ? ERROR_REASON.USAGE : undefined,
      );
    }
    ...
    try {
      ...
      /* best-effort — list still works without the inactive annotation */
    }
    ...
    try {
      ...
      /* best-effort — list still works without the surfaced annotation */
    }
    for (const capId of Object.keys(fp)) {
      ...
      rows.push({
        ...
        scope: "first-party",
        status: "active",
        surfaced: Object.prototype.hasOwnProperty.call(surfacedById, capId)
          ? surfacedById[capId] === true
          : null,
    ...
    for (const sc of overlayScopes) {
      ...
      try {
        ...
        error(
          `capability list blocked (${sc} scope): ${err.message}`,
          ERROR_REASON ? ERROR_REASON.SDK_FAIL_FAST : undefined,
        );
      }
      ...
      for (const capId of Object.keys(ledger.entries)) {
        ...
        try {
          ...
          const raw = ledgerMod.readSmallRegularFile(
            path.join(runtimeDir, ".gsd", "capabilities", capId, "capability.json"),
            8 * 1024 * 1024,
          );
        ...
        if (typeof range === "string" && range && !semver.semverSatisfies(host, range))
          status = "incompatible";
        ...
        rows.push({
          ...
          surfaced:
            status === "active"
              ? Object.prototype.hasOwnProperty.call(surfacedById, capId)
                ? surfacedById[capId] === true
                : null
              : false,
    ...
    if (!id || id.startsWith("--")) {
      error(
        `Missing <id> for: capability ${capSubcommand} <id>`,
        ERROR_REASON ? ERROR_REASON.USAGE : undefined,
      );
    }
    ...
    capabilityWriter.cmdCapabilitySet(
      ...
      id,
      {
        enabled: capSubcommand === "enable",
        runtime: capFlagValue("--runtime"),
        scope: capFlagValue("--scope"),
      },
      raw,
    );
    ...
    if (outdatedScopeArg && outdatedScopeArg !== "global" && outdatedScopeArg !== "project") {
      error(
        `Invalid --scope "${outdatedScopeArg}": must be "global" or "project"`,
        ERROR_REASON ? ERROR_REASON.USAGE : undefined,
      );
    }
    ...
    if (asJson) {
      ...
      const widths = headers.map((h, i) =>
        Math.max(h.length, ...tableRows.map((row) => row[i].length), 0),
      );
      const fmt = (row) =>
        row
    ...
    if (trustSub === "list") {
      ...
      if (listScope && listScope !== "project") {
        error(
          `Invalid --scope "${listScope}" for trust list: only "project" consent records exist`,
          ERROR_REASON ? ERROR_REASON.USAGE : undefined,
        );
      }
      ...
      if (!id || id.startsWith("--")) {
        error(
          "Missing <id> for: capability trust revoke <id>",
          ERROR_REASON ? ERROR_REASON.USAGE : undefined,
        );
      }
      ...
      let projectRoot;
      try {
        projectRoot = projFlag ? fs.realpathSync(path.resolve(projFlag)) : capProjectRoot();
      } catch {
        projectRoot = projFlag ? path.resolve(projFlag) : cwd;
      }
