...
function fieldNameMatchesRawCell(fieldName, rawCell) {
  ...
  for (let j = 0; j <= maxOffset; j++) {
    if (canonicalizeForCaselessCompare(rawCell.slice(j, j + n)) !== canonicalFieldName) continue;
    if (/^[ \t]*$/.test(rawCell.slice(j + n))) return true;
  }
  return false;
}
/**
      ...
      if (pipeCount === 3 && trimmedEnd.endsWith("|")) {
        ...
        if (cells.length === 2 && !isTableSeparatorRow(cells[0])) {
          ...
          if (fieldNameMatchesRawCell(fieldName, rawCell)) {
            ...
            while (content[valueStart] === " " || content[valueStart] === "\t") valueStart++;
            let valueEnd = lineStart + rowClosingPipe;
            while (
              valueEnd - 1 >= valueStart &&
              (content[valueEnd - 1] === " " || content[valueEnd - 1] === "\t")
            )
              valueEnd--;
...
function stateCurrentPositionSlice(body) {
  const isCurrentPosition = (h) =>
    (h.level === 2 || h.level === 3) && h.text.trim().toLowerCase() === "current position";
  const section = (0, markdown_sectionizer_cjs_1.collectSection)(body, isCurrentPosition, {
    levelBounded: true,
  });
...
function stateReplaceField(content, fieldName, newValue) {
  ...
  if (plainPattern.test(content)) {
    return content.replace(plainPattern, (_match, prefix) =>
      joinFieldReplacement(prefix, newValue),
    );
  }
  // Pipe-table format: | FieldName | value |
...
function stateReplaceFieldInSession(content, primary, fallback, value) {
  ...
  const hasCanonicalSession =
    (0, markdown_sectionizer_cjs_1.collectSection)(content, isSession, { levelBounded: true }) !==
    null;
  const target = hasCanonicalSession ? isSession : isSessionContinuity;
  return (0, markdown_sectionizer_cjs_1.withSection)(content, target, (sectionBody) =>
    stateReplaceFieldWithFallback(sectionBody, primary, fallback, value),
  );
}
...
function isStateTemplateDefault(field, value) {
  ...
  if (!defaults) {
    ...
    const matchKey = Object.keys(exports.KNOWN_TEMPLATE_DEFAULTS).find(
  ...
  if (fieldLower === "status") {
    if (exports.KNOWN_STATUS_PATTERNS.some((p) => p.test(v))) return true;
  }
...
function stateReplaceFieldIfTemplate(content, field, knownDefaults, newValue) {
  ...
  const matchesStatusPattern =
    fieldLower === "status" && exports.KNOWN_STATUS_PATTERNS.some((p) => p.test(v));
