...
exports.CALIBRATION_SCHEMA_VERSION =
  exports.CALIBRATION_FACTOR_MAX =
  exports.CALIBRATION_FACTOR_MIN =
  exports.CONFIDENCE_HIGH_MIN_SAMPLES =
  exports.CONFIDENCE_MED_MIN_SAMPLES =
  exports.MIN_CALIBRATION_SAMPLES =
...
function deriveConfidence(sampleCount) {
  if (typeof sampleCount !== "number" || !Number.isFinite(sampleCount) || sampleCount < 0)
    return "low";
  if (sampleCount >= exports.CONFIDENCE_HIGH_MIN_SAMPLES) return "high";
  if (sampleCount >= exports.CONFIDENCE_MED_MIN_SAMPLES) return "med";
  return "low";
}
/**
...
function classifyAgainstBudget(estimate, budget) {
  ...
  if (!isPositiveFinite(budget) || !isPositiveFinite(estimate)) {
    return {
      ...
      ratio: 0,
      recommendation: null,
      budgetValid: isPositiveFinite(budget),
    };
  }
...
function computeCalibration(samples) {
  ...
  const factor = Math.min(
    exports.CALIBRATION_FACTOR_MAX,
    Math.max(exports.CALIBRATION_FACTOR_MIN, raw),
  );
...
function extractFrontmatterBlock(text, key) {
  ...
  for (let i = startIdx + 1; i < lines.length; i += 1) {
    ...
    out[m[1]] =
      /^-?\d+(?:\.\d+)?$/.test(rawValue) && Number.isFinite(asNumber)
        ? asNumber
        : rawValue.replace(/^['"]|['"]$/g, "");
  }
...
function renderEstimate(estimate) {
  const lines = ["estimate:", `  tokens: ${estimate.tokens}`];
...
function parseCalibrationDocument(raw) {
  ...
  const doc = parsed;
  if (doc["schema_version"] !== exports.CALIBRATION_SCHEMA_VERSION) return [];
