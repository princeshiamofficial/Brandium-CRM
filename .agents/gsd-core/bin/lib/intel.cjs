...
function safeReadJson(filePath, errors) {
  ...
  try {
    ...
    if (errors) {
      errors.push(
        `Could not read ${formatDiagnosticToken(filePath)}: ${formatDiagnosticToken(err?.message ?? String(err))}`,
      );
    }
    return null;
  }
  ...
  try {
    ...
    if (errors) {
      errors.push(
        `Could not parse ${formatDiagnosticToken(filePath)}: ${formatDiagnosticToken(err?.message ?? String(err))}`,
      );
    }
    return null;
  }
...
function intelApiSurface(planningDir) {
  ...
  const entries =
    data && data.entries && typeof data.entries === "object" ? Object.entries(data.entries) : [];
