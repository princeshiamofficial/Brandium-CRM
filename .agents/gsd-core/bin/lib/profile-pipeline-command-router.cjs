...
function routeExtractMessages({ args, cwd, raw, error, _pipeline }) {
  ...
  void cwd;
  p.cmdExtractMessages(projectArg, { sessionId, limit }, raw, sessionsPath).catch((e) => {
    _handlePipelineRejection(e, error);
  });
}
...
function routeProfileSample({ args, cwd, raw, error, _pipeline }) {
  ...
  p.cmdProfileSample(sessionsPath, { limit, maxPerProject, maxChars }, raw).catch((e) => {
    _handlePipelineRejection(e, error);
  });
}
