...
function deriveOpenQuestions(content, fm) {
  ...
  if (questions.length === 0) {
    const oqSection = (0, markdown_sectionizer_cjs_1.collectSection)(
      content,
      (h) => h.level === 2 && h.text.trim().toLowerCase().startsWith("open questions"),
      { levelBounded: true },
    );
    if (oqSection) {
...
function scanDebugSessions(planDir) {
  ...
  if (!node_fs_1.default.existsSync(debugDir)) return { items: [], acknowledged: 0 };
  const results = [];
  let acknowledged = 0;
  let files;
  try {
    ...
    return {
      items: [{ scan_error: true, slug: "", status: "", updated: "", hypothesis: "" }],
      acknowledged: 0,
    };
  }
  for (const entry of files) {
    ...
    let safeFilePath;
    try {
      safeFilePath = (0, security_cjs_1.requireSafePath)(filePath, planDir, "debug session file", {
        allowAbsolute: true,
      });
    } catch {
      continue;
    ...
    let hypothesis = "";
    const focusSection = (0, markdown_sectionizer_cjs_1.collectSection)(
      content,
      (h) => h.level === 2 && h.text.trim().toLowerCase().startsWith("current focus"),
      { levelBounded: true },
    );
    if (focusSection) {
...
function resolveQuickTaskSummaryFile(taskDir, dirName) {
  let summaryFiles;
  try {
    summaryFiles = node_fs_1.default
...
function scanQuickTasks(planDir) {
  ...
  if (!node_fs_1.default.existsSync(quickDir)) return { items: [], acknowledged: 0 };
  let entries;
  try {
    entries = node_fs_1.default.readdirSync(quickDir, { withFileTypes: true });
  } catch {
    return {
      items: [{ scan_error: true, slug: "", date: "", status: "", description: "" }],
      acknowledged: 0,
    };
  }
  const results = [];
  ...
  for (const entry of entries) {
    ...
    let safeTaskDir;
    try {
      safeTaskDir = (0, security_cjs_1.requireSafePath)(taskDir, planDir, "quick task dir", {
        allowAbsolute: true,
      });
    } catch {
      continue;
    ...
    if (summaryPath && node_fs_1.default.existsSync(summaryPath)) {
      let safeSum;
      try {
        safeSum = (0, security_cjs_1.requireSafePath)(summaryPath, planDir, "quick task summary", {
          allowAbsolute: true,
        });
      } catch {
        continue;
...
function scanThreads(planDir) {
  ...
  if (!node_fs_1.default.existsSync(threadsDir)) return { items: [], acknowledged: 0 };
  let files;
  try {
    files = node_fs_1.default.readdirSync(threadsDir, { withFileTypes: true });
  } catch {
    return {
      items: [{ scan_error: true, slug: "", status: "", updated: "", title: "" }],
      acknowledged: 0,
    };
  }
  ...
  for (const entry of files) {
    ...
    let safeFilePath;
    try {
      safeFilePath = (0, security_cjs_1.requireSafePath)(filePath, planDir, "thread file", {
        allowAbsolute: true,
      });
    } catch {
      continue;
...
function scanTodos(planDir) {
  ...
  if (!node_fs_1.default.existsSync(pendingDir)) return { items: [], acknowledged: 0 };
  let files;
  try {
    files = node_fs_1.default.readdirSync(pendingDir, { withFileTypes: true });
  } catch {
    return {
      items: [{ scan_error: true, filename: "", priority: "", area: "", summary: "" }],
      acknowledged: 0,
    };
  }
  ...
  for (const entry of mdFiles) {
    ...
    let safeFilePath;
    try {
      safeFilePath = (0, security_cjs_1.requireSafePath)(filePath, planDir, "todo file", {
        allowAbsolute: true,
      });
    } catch {
      continue;
    ...
    if (
      isAuditItemAcknowledged(fm, { snapshotKey: "status", currentValue: "", presenceOnly: true })
    ) {
      acknowledged++;
      continue;
    }
  ...
  if (openFiles.length > 5) {
    results.push({
      _remainder_count: openFiles.length - 5,
      filename: "",
      priority: "",
      area: "",
      summary: "",
...
function scanSeeds(planDir) {
  ...
  if (!node_fs_1.default.existsSync(seedsDir)) return { items: [], acknowledged: 0 };
  let files;
  try {
    files = node_fs_1.default.readdirSync(seedsDir, { withFileTypes: true });
  } catch {
    return {
      items: [{ scan_error: true, seed_id: "", slug: "", status: "", title: "" }],
      acknowledged: 0,
    };
  }
  ...
  for (const entry of files) {
    ...
    let safeFilePath;
    try {
      safeFilePath = (0, security_cjs_1.requireSafePath)(filePath, planDir, "seed file", {
        allowAbsolute: true,
      });
    } catch {
      continue;
...
function scanUatGaps(planDir, cwd) {
  ...
  for (const target of targets) {
    ...
    for (const file of scopeToPhase(
      ...
      let safeFilePath;
      try {
        safeFilePath = (0, security_cjs_1.requireSafePath)(filePath, planDir, "UAT file", {
          allowAbsolute: true,
        });
      } catch {
        continue;
      ...
      if (
        isAuditItemAcknowledged(fm, {
          snapshotKey: "gap_snapshot",
          currentValue: deriveUatGapSnapshotValue(status, content),
        })
      ) {
        acknowledged++;
        continue;
...
function scanVerificationGaps(planDir, cwd) {
  ...
  for (const target of targets) {
    ...
    for (const file of scopeToPhase(
      ...
      let safeFilePath;
      try {
        safeFilePath = (0, security_cjs_1.requireSafePath)(filePath, planDir, "VERIFICATION file", {
          allowAbsolute: true,
        });
      } catch {
        continue;
...
function scanContextQuestions(planDir, cwd) {
  ...
  for (const target of targets) {
    ...
    for (const file of files.filter((f) => f.includes("-CONTEXT") && f.endsWith(".md"))) {
      ...
      let safeFilePath;
      try {
        safeFilePath = (0, security_cjs_1.requireSafePath)(filePath, planDir, "CONTEXT file", {
          allowAbsolute: true,
        });
      } catch {
        continue;
      ...
      if (
        isAuditItemAcknowledged(fm, {
          ...
          currentValue: deriveOpenQuestionsDigest(questions),
        })
      ) {
        acknowledged++;
        continue;
...
function scanDeferredItems(planDir, cwd) {
  ...
  for (const target of targets) {
    ...
    if (!node_fs_1.default.existsSync(filePath)) continue;
    let safeFilePath;
    try {
      safeFilePath = (0, security_cjs_1.requireSafePath)(filePath, planDir, "deferred items file", {
        allowAbsolute: true,
      });
    } catch {
      continue;
...
function auditOpenArtifacts(cwd) {
  ...
  const uatGaps = (() => {
    try {
      ...
      return {
        items: [{ scan_error: true, phase: "", file: "", status: "", open_scenario_count: 0 }],
        acknowledged: 0,
      };
    }
  })();
  ...
  const contextQuestions = (() => {
    try {
      ...
      return {
        items: [{ scan_error: true, phase: "", file: "", question_count: 0, questions: [] }],
        acknowledged: 0,
      };
    }
  })();
  ...
  acknowledged.total =
    acknowledged.debug_sessions +
    acknowledged.quick_tasks +
    acknowledged.threads +
    acknowledged.todos +
    acknowledged.seeds +
    acknowledged.uat_gaps +
    acknowledged.verification_gaps +
    acknowledged.context_questions +
    acknowledged.deferred_items;
  return {
...
function formatAuditReport(auditResult) {
  ...
  if (!has_open_items) {
    ...
    if (acknowledged.total > 0) {
      lines.push(
        `All artifact types clear (${acknowledged.total} previously acknowledged item${acknowledged.total !== 1 ? "s" : ""} still suppressed).`,
      );
    } else {
  ...
  if (counts.debug_sessions > 0) {
    lines.push("");
    lines.push(
      `🔴 Debug Sessions (${counts.debug_sessions} open${ackSuffix(acknowledged.debug_sessions)})`,
    );
  ...
  if (counts.uat_gaps > 0) {
    lines.push("");
    lines.push(
      `🔴 UAT Gaps (${counts.uat_gaps} phases with incomplete UAT${ackSuffix(acknowledged.uat_gaps)})`,
    );
    for (const item of items.uat_gaps.filter((i) => !i.scan_error)) {
      ...
      lines.push(
        `   • Phase ${item.phase}${archived}: ${item.file} [${item.status}] — ${item.open_scenario_count} pending scenarios`,
      );
    }
  }
  ...
  if (counts.verification_gaps > 0) {
    lines.push("");
    lines.push(
      `🔴 Verification Gaps (${counts.verification_gaps} unresolved${ackSuffix(acknowledged.verification_gaps)})`,
    );
  ...
  if (counts.quick_tasks > 0) {
    lines.push("");
    lines.push(
      `🟡 Quick Tasks (${counts.quick_tasks} incomplete${ackSuffix(acknowledged.quick_tasks)})`,
    );
  ...
  if (counts.context_questions > 0) {
    lines.push("");
    lines.push(
      `🔵 CONTEXT Open Questions (${counts.context_questions} phases with open questions${ackSuffix(acknowledged.context_questions)})`,
    );
    for (const item of items.context_questions.filter((i) => !i.scan_error)) {
      const archived = item.archived_milestone ? ` (archived ${item.archived_milestone})` : "";
      lines.push(
        `   • Phase ${item.phase}${archived}: ${item.file} (${item.question_count} question${item.question_count !== 1 ? "s" : ""})`,
      );
  ...
  if (counts.deferred_items > 0) {
    lines.push("");
    lines.push(
      `🔵 Deferred Items (${counts.deferred_items} unresolved${ackSuffix(acknowledged.deferred_items)})`,
    );
  ...
  if (acknowledged.total > 0) {
    lines.push(
      `${acknowledged.total} previously acknowledged item${acknowledged.total !== 1 ? "s" : ""} also suppressed above the ${counts.total} open item${counts.total !== 1 ? "s" : ""}.`,
    );
  }
  return lines.join("\n");
}
...
function cmdAuditAcknowledge(cwd, args, raw) {
  ...
  const {
    ...
    phase,
    file,
    "archived-milestone": archivedMilestone,
    slug,
    "seed-id": seedId,
    dir: quickDir,
    filename,
    text,
  } = (0, command_arg_projection_cjs_1.parseNamedArgsOrExit)(
    args,
    {
      valueFlags: [
        "category",
  ...
  if (PHASE_SCOPED.has(category)) {
    ...
    if (!targetDir) {
      ioError(
        `no phase directory found for phase "${phase}"${archivedMilestone ? ` (archived-milestone "${archivedMilestone}")` : ""}`,
      );
    }
    const filePath = node_path_1.default.join(targetDir, file);
    const safeFilePath = (0, security_cjs_1.requireSafePath)(
      filePath,
      planDir,
      "audit acknowledge target",
      { allowAbsolute: true },
    );
    if (!node_fs_1.default.existsSync(safeFilePath)) ioError(`file not found: ${file}`);
    if (category === "deferred_items") {
      ...
      if (result.status === "not_found") ioError(`no deferred item matched --text "${text}"`);
      if (result.status === "ambiguous")
        ioError(`--text "${text}" matches more than one deferred item — text must be unique`);
      if (result.status === "already_resolved")
        ioError(
          `deferred item is already "status: resolved" — acknowledging a resolved item is a no-op`,
        );
      if (result.status === "unsupported_heading_shape") {
        ...
        ioError(
          "this deferred item's span embeds a GFM table row, so the CLI writer cannot anchor a safe write to it — edit the file directly",
        );
      }
      if (result.status === "match_verification_failed") {
        ioError(
          `internal error: matched span for --text "${text}" did not re-verify before write — refused rather than risk writing the wrong entry`,
        );
      }
    ...
    if (category === "uat_gaps") {
      ...
      snapshotKey = "gap_snapshot";
      currentValue = deriveUatGapSnapshotValue(
        (fm.status || "unknown").toLowerCase(),
        normalizedContent,
      );
  ...
  if (category === "debug_sessions") {
    ...
    safeFilePath = (0, security_cjs_1.requireSafePath)(
      node_path_1.default.join(planDir, "debug", `${slug}.md`),
      planDir,
      "audit acknowledge target",
      { allowAbsolute: true },
    );
    if (!node_fs_1.default.existsSync(safeFilePath)) ioError(`file not found: debug/${slug}.md`);
    const content = node_fs_1.default.readFileSync(safeFilePath, "utf-8");
    ...
    safeFilePath = (0, security_cjs_1.requireSafePath)(
      node_path_1.default.join(planDir, "threads", `${slug}.md`),
      planDir,
      "audit acknowledge target",
      { allowAbsolute: true },
    );
    if (!node_fs_1.default.existsSync(safeFilePath)) ioError(`file not found: threads/${slug}.md`);
    const content = node_fs_1.default.readFileSync(safeFilePath, "utf-8");
    ...
    safeFilePath = (0, security_cjs_1.requireSafePath)(
      node_path_1.default.join(planDir, "seeds", `${seedId}.md`),
      planDir,
      "audit acknowledge target",
      { allowAbsolute: true },
    );
    if (!node_fs_1.default.existsSync(safeFilePath)) ioError(`file not found: seeds/${seedId}.md`);
    const content = node_fs_1.default.readFileSync(safeFilePath, "utf-8");
    ...
    safeFilePath = (0, security_cjs_1.requireSafePath)(
      node_path_1.default.join(planDir, "todos", "pending", filename),
      planDir,
      "audit acknowledge target",
      { allowAbsolute: true },
    );
    ...
    const taskDir = (0, security_cjs_1.requireSafePath)(
      node_path_1.default.join(planDir, "quick", quickDir),
      planDir,
      "audit acknowledge target dir",
      { allowAbsolute: true },
    );
    if (!node_fs_1.default.existsSync(taskDir)) ioError(`directory not found: quick/${quickDir}`);
    ...
    if (resolvedSummaryPath) {
      ...
      safeFilePath = (0, security_cjs_1.requireSafePath)(
        node_path_1.default.join(taskDir, `${quickDir}-SUMMARY.md`),
        planDir,
        "audit acknowledge target",
        { allowAbsolute: true },
      );
    ...
    ioError(
      `unknown --category "${category}". Available: debug_sessions, quick_tasks, threads, todos, seeds, uat_gaps, verification_gaps, context_questions, deferred_items`,
    );
    return; // unreachable — ioError throws — satisfies TS control-flow analysis
  }
  const presenceOnly = category === "todos";
  const fm = createIfMissing
    ? fmForCreate
    : extractFrontmatter(node_fs_1.default.readFileSync(safeFilePath, "utf-8"), safeFilePath);
  fm.audit_acknowledged = presenceOnly
    ? { ...markerBase }
    : { ...markerBase, [snapshotKey]: currentValue };
  ...
  (0, shell_command_projection_cjs_2.platformWriteSync)(safeFilePath, newContent);
  output(
    { acknowledged: true, category, ...(presenceOnly ? {} : { [snapshotKey]: currentValue }) },
    raw,
    "true",
  );
}
