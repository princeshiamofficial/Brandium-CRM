  ...
  const body = rows
    .map((r) => {
      const tick =
        r.status === "Covered"
          ? "✓ Covered"
          : r.status === "Missing from REQUIREMENTS.md"
...
function runGapAnalysis(cwd, phaseDir, options = {}) {
  ...
  const absPhaseDir = node_path_1.default.isAbsolute(phaseDir)
    ? phaseDir
    : node_path_1.default.join(cwd, phaseDir);
  const reqPath = planningPaths(cwd).requirements;
  const reqMd = node_fs_1.default.existsSync(reqPath)
    ? node_fs_1.default.readFileSync(reqPath, "utf-8")
    : "";
  ...
  const phaseDirReadError =
    phaseDirScope === SCOPE.UNREADABLE
  ...
  const scopedPhaseDirFiles = scopeToPhase(
    phaseDirFiles,
    node_path_1.default.basename(absPhaseDir),
  );
        ...
        .map((f) => {
          try {
            return node_fs_1.default.readFileSync(
              node_path_1.default.join(absPhaseDir, f),
              "utf-8",
            );
          } catch {
            return "";
    ...
    if (items.length > 0 || ghostReqIds.length > 0) {
      const rows = sortRows([
        ...
        ...ghostReqIds.map((id) => ({
          ...
          item: id,
          status: "Missing from REQUIREMENTS.md",
        })),
      ]);
