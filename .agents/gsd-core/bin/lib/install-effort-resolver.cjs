...
function readGsdEffectiveEffortConfig(targetDir = null) {
  ...
  const homeEffort =
    homeDefaults &&
    homeDefaults.effort &&
    typeof homeDefaults.effort === "object" &&
    !Array.isArray(homeDefaults.effort)
      ? homeDefaults.effort
      : null;
  const projectEffort =
    projectConfig &&
    projectConfig.effort &&
    typeof projectConfig.effort === "object" &&
    !Array.isArray(projectConfig.effort)
      ? projectConfig.effort
      : null;
...
function resolveInstallTimeEffort(effortCfg, agentName) {
  ...
  const { AGENT_DEFAULT_TIERS, EFFORT_MANIFEST_TIER_DEFAULTS, EFFORT_MANIFEST_DEFAULT } =
