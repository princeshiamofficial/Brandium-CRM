...
exports.REASON =
  exports.VERDICT =
  exports.ANALYZABLE_EXTENSIONS =
  exports.DEFAULTS =
  exports.SCHEMA_VERSION =
  exports.PROPOSAL_SUFFIX =
  exports.BASELINE_FILE_NAME =
    void 0;
...
function stripLiterals(source) {
  ...
  while (i < len) {
    ...
    if (ch === "/" && source[i + 1] === "*") {
      ...
      if (!closed) return { ok: false, reason: exports.REASON.REFACTOR_ANALYZER_UNPARSEABLE };
      lastTokenIsValue = false;
      continue;
    }
...
function isAnalyzablePath(relPath) {
  ...
  if (!exports.ANALYZABLE_EXTENSIONS.includes(ext)) return false;
...
function parseProposal(text) {
  try {
    ...
    const resolvedAt =
      fm.resolved_at === "null" || fm.resolved_at === undefined ? null : fm.resolved_at;
    let reason = "";
    try {
