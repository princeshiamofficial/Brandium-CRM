...
const EFFORT_LEVELS = new Set(["minimal", "low", "medium", "high", "xhigh", "max", "inherit"]);
/**
...
function resolveTimeoutMs(timeoutConfigKey, floorMs, configGet) {
  const configuredSeconds =
    typeof timeoutConfigKey === "string" ? configGet(timeoutConfigKey) : undefined;
...
function resolveLanePlan(input) {
  ...
  if (!review_lane_descriptor_cjs_1.LANE_SLUG_RE.test(slug)) {
    return fail(
      exports.LANE_UNAVAILABLE.MALFORMED_LANE,
      `lane slug '${slug}' is outside the declared grammar ${String(review_lane_descriptor_cjs_1.LANE_SLUG_RE)}`,
    );
  }
  ...
  if (handler !== null && !KNOWN_HANDLERS.has(handler)) {
    return fail(
      exports.LANE_UNAVAILABLE.UNKNOWN_HANDLER,
  ...
  const floorMs =
    typeof lane.timeoutFloorMs === "number" &&
    Number.isFinite(lane.timeoutFloorMs) &&
    lane.timeoutFloorMs > 0
      ? lane.timeoutFloorMs
      : 900_000;
  ...
  if (lane.transport !== "spawn") {
    return fail(
      exports.LANE_UNAVAILABLE.UNKNOWN_TRANSPORT,
      `lane '${slug}' declares transport '${String(lane.transport)}'`,
    );
  }
  ...
  const template = Array.isArray(inv.args) ? inv.args.filter((a) => typeof a === "string") : [];
  const argv = [];
