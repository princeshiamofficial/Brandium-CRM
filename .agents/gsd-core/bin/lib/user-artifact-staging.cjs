...
function stagingKeyFor(destDir, runId) {
  const destHash = node_crypto_1.default
    .createHash("sha256")
    .update(node_path_1.default.resolve(destDir))
    .digest("hex")
    .slice(0, 16);
  ...
  const runHash = node_crypto_1.default
    .createHash("sha256")
    .update(runId)
    .digest("hex")
    .slice(0, 8);
...
// "KNOWN DEFECTS" precedent in GEMINI.md's "Unbounded Subprocesses" row);
...
function stageUserArtifacts(destDir, fileNames, stagingRoot, opts = {}) {
  ...
  for (const name of fileNames) {
    ...
    if (!isFlatName(name)) {
      throw new Error(
        `stageUserArtifacts: file name "${name}" must be a flat name — no path separator is allowed`,
      );
    }
...
function restoreStagedUserArtifacts(destDir, staged, opts = {}) {
  ...
  for (const name of staged.names) {
    ...
    const srcInStaging = runtimeArtifactInstallPlan.assertDestWithinConfigHome(
      staged.filesDir,
      name,
    );
...
function recoverOrphanedUserArtifacts(stagingRoot, configDir, opts = {}) {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      // WRITE destination, e.g. nix-darwin's `.agents` symlink). This is a
      ...
      try {
        ...
        confinedDestDir = runtimeArtifactInstallPlan.assertDestWithinConfigHome(
          configHome,
          relDest,
        );
      } catch {
      ...
      if (
        symlinkGuard.hasExistingSymlinkBetween(configHome, confinedDestDir, {
...
module.exports = {
  ...
  // #2875 defect fix: exported for a fast-check property test (GEMINI.md
