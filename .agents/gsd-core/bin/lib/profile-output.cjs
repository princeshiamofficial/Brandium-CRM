...
function generateArchitectureSection(cwd) {
  const fallbacks = buildClaudeMdFallbacks((0, runtime_slash_cjs_1.resolveRuntime)(cwd));
  const architecturePath = node_path_1.default.join(
    cwd,
    ".planning",
    "codebase",
    "ARCHITECTURE.md",
  ...
  if (!content) {
    return {
      content: fallbacks["architecture"],
...
function generateSkillsSection(cwd) {
  ...
  for (const dir of SKILL_SEARCH_DIRS) {
    ...
    if (!node_fs_1.default.existsSync(absDir)) continue;
    let entries;
    try {
    ...
    for (const entry of entries) {
      ...
      if (!node_fs_1.default.existsSync(skillMdPath)) continue;
...
function cmdWriteProfile(cwd, options, raw) {
  ...
  const templatePath = node_path_1.default.join(
    __dirname,
    "..",
    "..",
    "templates",
  ...
  if (!node_fs_1.default.existsSync(templatePath)) error(`Template not found: ${templatePath}`);
  ...
  if (!outputPath) {
    ...
    try {
      ...
      effectiveRuntime =
        (0, runtime_name_policy_cjs_1.resolveRuntimeNameFromCandidates)(
    ...
    outputPath = node_path_1.default.join(
      (0, runtime_homes_cjs_1.getGlobalConfigDir)(effectiveRuntime),
      "gsd-core",
      "USER-PROFILE.md",
    );
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  if (!outputPath) {
    ...
    const skillDir = (0, runtime_homes_cjs_1.getGlobalSkillDir)(
      effectiveRuntime,
      "gsd-dev-preferences",
    );
    if (!skillDir) {
      error(
        `Runtime "${effectiveRuntime}" does not use a skills directory; pass --output to choose a path explicitly.`,
      );
    }
  ...
  const result = {
    ...
    command_name: (0, runtime_slash_cjs_1.formatGsdSlash)(
      "dev-preferences",
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  // hardcoded .agents/GEMINI.md. Mirrors the #3163 fix applied to
  // cmdGenerateClaudeMd above; that fix diverged when it didn't propagate
  // here, leaving /gsd-profile-user writing the agent files on Codex installs.
  ...
  //   at ~/.codex/AGENTS.md. the agent global is preserved byte-for-byte (no
  ...
  //   write to GEMINI.md even if a prior the agent setup left claude_md_path).
  let config = {};
  try {
  ...
  const effectiveRuntime = (0, runtime_name_policy_cjs_1.resolveRuntimeNameFromCandidates)(
    process.env["GSD_RUNTIME"],
    config["runtime"],
  );
  ...
  if (options.global) {
    if (isClaudeRuntime) {
      ...
      targetPath = node_path_1.default.join(
        (0, runtime_homes_cjs_1.getGlobalConfigDir)(effectiveRuntime),
        node_path_1.default.basename(
          (0, runtime_name_policy_cjs_1.getProjectInstructionFile)(effectiveRuntime),
        ),
      );
    }
  } else if (options.output) {
    targetPath = node_path_1.default.isAbsolute(options.output)
      ? options.output
      : node_path_1.default.join(cwd, options.output);
  } else {
    ...
    if (!isClaudeRuntime) {
      configClaudeMdPath = (0, runtime_name_policy_cjs_1.getProjectInstructionFile)(
        effectiveRuntime,
      );
    }
    targetPath = node_path_1.default.isAbsolute(configClaudeMdPath)
      ? configClaudeMdPath
      : node_path_1.default.join(cwd, configClaudeMdPath);
  }
  let action;
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  let outputPath = options.output;
  if (!outputPath) {
    outputPath = node_path_1.default.isAbsolute(configClaudeMdPath)
      ? configClaudeMdPath
      : node_path_1.default.join(cwd, configClaudeMdPath);
  } else if (!node_path_1.default.isAbsolute(outputPath)) {
    outputPath = node_path_1.default.join(cwd, outputPath);
    ...
    output(
      {
        ...
        action: "skipped",
        reason:
          "existing instruction file has no GSD markers (hand-crafted); not overwriting. Pass --force to overwrite.",
        sections_generated: [],
        sections_fallback: [],
