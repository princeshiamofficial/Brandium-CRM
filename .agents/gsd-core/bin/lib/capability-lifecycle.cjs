...
function acquireLock(runtimeDir) {
  ...
  try {
    ...
    /* best-effort — lockMod also mkdirs */
  }
...
function readManifest(dir) {
  try {
    const raw = node_fs_1.default.readFileSync(
      node_path_1.default.join(dir, "capability.json"),
      "utf8",
    );
...
function safeRmUnder(runtimeDir, rel) {
  ...
  if (node_path_1.default.isAbsolute(rel) || rel.split(/[/\\]/).includes("..")) return false;
  let realRoot;
  try {
  ...
  try {
    ...
    else node_fs_1.default.rmSync(realTarget, { recursive: true, force: true });
    return true;
  } catch {
    return false;
  }
...
function confinedSharedFile(runtimeDir, relFile) {
  if (
    typeof relFile !== "string" ||
    !relFile ||
    node_path_1.default.isAbsolute(relFile) ||
...
function isSafeHookScriptPath(script) {
  ...
  if (node_path_1.default.isAbsolute(script)) return false;
  ...
  if (PYCACHE_SUFFIX_RE.test(node_path_1.default.basename(script))) return false;
  return true;
}
/**
...
function runnableHookCommand(absScript) {
  return JS_HOOK_EXT_RE.test(absScript)
    ? "node " + shellSingleQuote(absScript)
    : shellSingleQuote(absScript);
}
/**
...
function confinedBundleScript(capDirPath, script) {
  ...
  if (node_path_1.default.isAbsolute(script) || script.split(/[/\\]/).includes("..")) return null;
...
function reapplyCapabilitySharedEdits(args) {
  ...
  if (stripFiles.length > 0) {
    stripCapabilitySharedEdits({
      runtimeDir,
      capId,
      sharedEdits: stripFiles.map((file) => ({ file, marker: capId })),
    });
  }
  ...
  for (const relFile of sharedFiles) {
    ...
    if (mcpEntries.length > 0) {
      ...
      for (const { name, config } of mcpEntries) {
        ...
        const ownedByUs =
          typeof existing === "object" && existing !== null && existing[CAP_MARKER] === capId;
    ...
    if (hooksObj && typeof hooksObj === "object" && !Array.isArray(hooksObj)) {
      ...
      for (const event of Object.keys(ho)) {
        ...
        const kept = arr.filter(
          (e) => !(typeof e === "object" && e !== null && e[CAP_MARKER] === capId),
        );
...
function isFirstPartyCapabilityId(id) {
  try {
    ...
    return !!(
      reg &&
      reg.capabilities &&
      Object.prototype.hasOwnProperty.call(reg.capabilities, id)
    );
  } catch {
    return false;
  }
...
function checkSharedFileCount(sharedFiles) {
  ...
  if (sharedFiles.length > ledgerMod.MAX_SHARED_FILES) {
    return (
      `too many --shared-file entries: ${sharedFiles.length} exceeds the maximum of ` +
...
function resolveReviewerEgressHost(opts, manifest) {
  try {
    ...
    if (key) {
      ...
      const cfg = cfgLoader.loadConfigResolved
        ? (cfgLoader.loadConfigResolved(root).config ?? {})
        : {};
      let cur = cfg;
...
function bindProjectConsent(opts, id, integrity, manifest) {
  ...
  try {
    ...
    warnConsent(
      `could not write the consent record for "${id}" to "${storePath}": ${err.message}. ` +
  ...
  try {
    if (!lock) {
      return {
        status: "blocked",
        id: resolved.id,
        blockReasons: ["another capability operation is in progress"],
      };
    }
    ...
    if (opts.expectedId && resolved.id !== opts.expectedId) {
      return {
        ...
        id: resolved.id,
        blockReasons: [
          `source resolved to capability id "${resolved.id}" but "${opts.expectedId}" was expected; refusing`,
        ],
      };
    }
    ...
    if (ledgerMod.isUnsafeCapabilityId(resolved.id)) {
      return {
        ...
        id: resolved.id,
        blockReasons: [
          `capability id "${resolved.id}" is unsafe (prototype-pollution key or invalid kebab-case); refusing to install`,
        ],
      };
    }
    if (isFirstPartyCapabilityId(resolved.id)) {
      return {
        ...
        id: resolved.id,
        blockReasons: [
          `"${resolved.id}" is a first-party capability id and cannot be overridden by a third-party overlay`,
        ],
      };
    }
    ...
    const prior =
      existingLedger && Object.prototype.hasOwnProperty.call(existingLedger.entries, resolved.id)
        ? existingLedger.entries[resolved.id]
        : null;
    const hadDir = node_fs_1.default.existsSync(finalDir);
    const priorSharedFiles =
      prior && Array.isArray(prior.sharedEdits) ? prior.sharedEdits.map((e) => e.file) : [];
    ...
    try {
      ledgerMod.recordInstall(
        runtimeDir,
        {
          ...pendingBase,
          _pending: {
            kind: isUpgradeLike ? "upgrade" : "install",
            backupName,
            sharedFiles: candidateFiles,
          },
        },
    ...
    try {
      ...
      const sharedEdits = reapplyCapabilitySharedEdits({
        runtimeDir,
        capId: resolved.id,
        stripFiles: candidateFiles,
        applyFiles: files,
        manifest,
      });
  ...
  try {
    ...
    if (opts.expectedId && resolved.id !== opts.expectedId) {
      return {
        ...
        id: resolved.id,
        blockReasons: [
          `source for "${opts.expectedId}" now resolves to a different capability id "${resolved.id}"; refusing to upgrade`,
        ],
      };
    }
    ...
    if (ledgerMod.isUnsafeCapabilityId(resolved.id)) {
      return {
        ...
        id: resolved.id,
        blockReasons: [
          `capability id "${resolved.id}" is unsafe (prototype-pollution key or invalid kebab-case); refusing to upgrade`,
        ],
      };
    }
    ...
    const prior =
      existing && Object.prototype.hasOwnProperty.call(existing.entries, resolved.id)
    ...
    if (!prior) {
      return {
        ...
        id: resolved.id,
        blockReasons: ["capability is not installed; use install"],
      };
    }
    ...
    try {
      ledgerMod.recordInstall(runtimeDir, {
        ...prior,
        _pending: { kind: "upgrade", backupName, sharedFiles: candidateFiles },
      });
    } catch (err) {
      return { status: "blocked", id: resolved.id, blockReasons: [err.message] };
    }
...
function removeCapability(id, opts) {
  ...
  try {
    ...
    const entry =
      ledger && Object.prototype.hasOwnProperty.call(ledger.entries, id)
        ? ledger.entries[id]
        : null;
...
function backupNameMatchesId(name, id) {
  return (
    typeof name === "string" && BACKUP_NAME_RE.test(name) && name.startsWith(id + ".upgrading-")
  );
}
/**
...
function reconcileCapabilities(opts) {
  ...
  const revokeStaleConsent = (id) => {
    ...
    try {
      ...
      /* best-effort — a consent-store IO error must never abort crash recovery */
    }
  };
  // Finding 2 (HIGH): READ-ONLY corruption preflight BEFORE acquireLock. acquireLock creates
  // .gsd/capabilities and a .lock file; doing it before detecting corruption pollutes the scope
  ...
  try {
    ...
    if (ledger === null) {
      ...
      if (ledgerEntryExists) {
        report.warnings.push(
          `Capability ledger file exists but could not be parsed: ${ledgerFilePath}`,
        );
    ...
    if (ledger) {
        ...
        try {
          ...
          if (pending.kind === "install") {
            ...
            stripCapabilitySharedEdits({
              runtimeDir,
              capId: id,
              sharedEdits: candidateFiles.map((file) => ({ file, marker: id })),
            });
          ...
          if (node_fs_1.default.existsSync(backupDir)) {
            try {
              ...
              if (node_fs_1.default.existsSync(finalDir)) retryRenameSync(finalDir, discard); // (a) set the new dir aside
          ...
          const refreshed = resyncCapabilitySharedEdits({
            runtimeDir,
            capId: id,
            sharedFiles: candidateFiles,
          });
          ...
          report.warnings.push(
            `Reconcile could not roll back capability "${id}": ${entryErr.message}`,
          );
        }
      }
      ...
      if (ledgerDirty) {
        ...
        try {
          ...
          report.warnings.push(
            `Reconcile could not persist rolled-back ledger state: ${writeErr.message}`,
          );
        }
      }
    ...
    for (const name of entries) {
      ...
      if (/\.discard-\d+-\d+-[0-9a-f]+$/.test(name)) {
        try {
          node_fs_1.default.rmSync(node_path_1.default.join(root, name), {
            recursive: true,
            force: true,
          });
    ...
    try {
      ...
      /* runtimeDir unreadable — nothing to sweep */
    }
    try {
      report.ledger = ledgerMod.reconcile(runtimeDir);
    } catch {
...
function outdatedCapabilities(opts) {
  ...
  for (const id of Object.keys(ledger.entries)) {
    ...
    try {
      ...
      /* unparseable source — leave kind 'unknown' */
    }
    let peek;
    try {
