...
function resolveLoopHooks(input) {
  ...
  function isCapabilityActive(capId) {
    ...
    const state =
      capabilityStatesById instanceof Map
  ...
  for (const hook of steps) {
    ...
    const ref = typeof hook["ref"] === "object" && hook["ref"] !== null ? hook["ref"] : undefined;
  ...
  for (const hook of activeHooks) {
    if (hook.kind === "step") {
      ...
      if (hook.fragment?.inline) {
        ...
        lines.push("");
        lines.push(
          `_Step fragment path is declared but not rendered by loop-resolver: ${hook.fragment.path}_`,
        );
      }
      lines.push("");
      ...
      if (hook.fragment?.inline) {
        ...
        lines.push(
          `_Contribution fragment path is declared but not rendered by loop-resolver: ${hook.fragment.path}_`,
        );
      }
      if (hook.produces && hook.produces.length > 0) {
        lines.push(`- produces: ${hook.produces.join(", ")}`);
      ...
      if (hook.check !== undefined && hook.check !== null) {
        checkStr =
          typeof hook.check === "object"
            ? JSON.stringify(hook.check)
            : typeof hook.check === "string" ||
                typeof hook.check === "number" ||
                typeof hook.check === "boolean"
              ? String(hook.check)
              : "(complex)";
      }
      lines.push(
        `**Gate** (${hook.capId}): check=${checkStr}, blocking=${String(hook.blocking ?? false)}, onError=${hook.onError ?? "skip"}`,
      );
      if (hook.when) {
        lines.push(`- when: \`${hook.when}\``);
      }
...
function cmdLoopRenderHooks(cwd, point, raw, options = {}) {
  if (!point) {
    coreError(
      "loop render-hooks requires a <point> argument. Valid points: " + CANONICAL_POINTS.join(", "),
    );
    return;
  }
