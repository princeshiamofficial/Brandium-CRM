...
function isClosedMilestoneHeading(headingText) {
  return (
    MILESTONE_CLOSED_MARKER_PATTERN.test(headingText) &&
    !MILESTONE_ACTIVE_MARKER_PATTERN.test(headingText)
  );
}
/**
...
function bracketHeadingHasMatchingChild(headings, index) {
  ...
  for (let i = index + 1; i < headings.length; i++) {
    ...
    if (
      childMatch &&
      foldBracketId(childMatch[1]) === ownId &&
      BRACKET_PHASE_TAIL_RE.test(next.text)
    )
      return true;
...
function hasVersionedMilestones(content) {
  return /^#{1,3}\s+.*v\d+\.\d+/im.test(content);
}
...
function sliceMilestoneWindow(content, version) {
  ...
  return content.slice(
    selected.index,
    computeMilestoneSectionEnd(content, selected[0], selected.index),
  );
}
/**
...
function hasPhaseEntries(markdown, phaseIdConvention) {
  ...
  if (
    BULLET_PHASE_LINE_PATTERN.test((0, markdown_sectionizer_cjs_1.stripFencedCode)(markdown).text)
  )
    return true;
...
function collectTablePhaseRows(window) {
  ...
  for (let i = 0; i + 1 < lines.length; i++) {
    ...
    if ((0, markdown_table_cjs_1.matchTableSchema)(headerCells) !== null) continue; // canonical non-listing schema
    if (
      !(0, markdown_table_cjs_1.isDelimiterRow)(
        (0, markdown_table_cjs_1.splitTableRow)(lines[i + 1]),
      )
    )
      continue;
    for (let j = i + 2; j < lines.length; j++) {
      ...
      if (!/^999\b/.test(first)) {
        rows.push({
          id: first,
          name: cells[1] && cells[1] !== "" ? cells[1] : null,
          row: lines[j],
        });
      }
    }
...
function classifyMilestoneWindow(input) {
  const {
    ...
    headingFound,
    windowHasPhaseEntries,
    documentHasPhaseEntries,
  } = input;
  return !readable
    ? SCOPE.UNREADABLE // row 2
    : !versionResolved && !hasVersionedMilestones
      ? SCOPE.COMPLETE // row 3: free-form legacy roadmap
      : !versionResolved && hasVersionedMilestones
        ? SCOPE.UNSCOPED // row 4
        : versionResolved && !headingFound
          ? SCOPE.UNSCOPED // row 5
          : headingFound && !windowHasPhaseEntries && documentHasPhaseEntries
            ? SCOPE.TRUNCATED // row 8
...
function extractCurrentMilestoneScoped(content, cwd, ws, phaseIdConvention) {
  ...
  const summaryPattern = new RegExp(
    `<summary[^>]*>([^<]*${(0, pattern_cjs_1.escapeRegex)(version)}[^<]*)<\\/summary>`,
    "i",
  );
  ...
  if (phaseIdConvention === undefined) {
    try {
      ...
      /* unresolvable convention → preserve the legacy fallback */
    }
  }
  if (headingMatches.length === 0 && bracketScopeConvention === "bracket") {
    ...
    if (Number.isSafeInteger(milestoneInt)) {
      ...
      headingMatches = (0, markdown_sectionizer_cjs_1.tokenizeHeadings)(content)
        .filter(
          (h) =>
            h.level <= 3 && content[h.offset] === "#" && bracketMilestoneHeadingRe.test(h.text),
        )
        .map((h) => {
    ...
    if (summaryMatch) {
      ...
      if (detailsOpenIdx !== -1) {
        ...
        const detailsEnd = closingMatch
          ...
          : content.length;
        const anyMilestoneOrDetails =
          /^#{1,3}\s+(?!Phase\s+\S)(?:.*v\d+\.\d+|✅|📋|🚧|🔄)|<details/im;
        ...
        const preamble = (0, markdown_sectionizer_cjs_1.stripTaggedBlocks)(
          content.slice(0, preambleCutoff),
          "details",
        )
          // #1729: `(?:\s*\([^)\n]{0,200}\))?` tolerates a pre-colon ( ) tag (literal mirror of OPTIONAL_PHASE_TAG_SOURCE).
          .replace(
  ...
  const selected =
    selectMilestoneHeading(content, version) ??
  ...
  const preambleCutoff =
    earliestMilestoneIndex !== null ? earliestMilestoneIndex : firstMatch.index;
  ...
  const preambleBase = (0, markdown_sectionizer_cjs_1.stripTaggedBlocks)(
    beforeMilestones,
    "details",
  );
...
function withPhaseSection(content, phaseId, edit) {
  ...
  const headingRe = new RegExp(
    `^\\s*(?:\\[[^\\]]{1,200}\\]\\s*)?Phase\\s+${src}(?=[\\s:(]|$)`,
    "i",
  );
  return (0, markdown_sectionizer_cjs_1.withSection)(content, (h) => headingRe.test(h.text), edit, {
    levelBounded: false,
  });
}
...
function findRoadmapProgressTable(roadmapContent) {
  ...
  const section = (0, markdown_sectionizer_cjs_1.collectSection)(
    roadmapContent,
    isProgressHeading,
    { levelBounded: true },
  );
  const scoped = section ? section.body : roadmapContent;
  return (0, markdown_table_cjs_1.findTableWithColumns)(scoped, [
    "Phase",
    "Plans Complete",
    "Status",
    "Completed",
...
function extractMilestoneHeadingName(headingText, expectedVersion) {
  const versionMatch = expectedVersion
    ? // Anchor to the KNOWN literal version, then extend across any immediate
      ...
      headingText.match(
        new RegExp(`${(0, pattern_cjs_1.escapeRegex)(expectedVersion)}(?:[-.][A-Za-z0-9]+)*`, "i"),
      )
...
function getMilestonePhaseFilter(cwd, versionOverride, phaseIdConvention, ws) {
  ...
  try {
    ...
    if (
      !hasVersionedMilestonesGlobal &&
      hasPhaseHeadings &&
      phaseIdConvention === "milestone-prefixed"
    ) {
      console.warn(
    ...
    if (versionOverride) {
      ...
      const documentHasPhaseEntries = hasPhaseEntries(
        stripShippedMilestones(roadmapContent),
        phaseIdConvention,
      );
      if (sliced !== null) {
        versionScoped = true;
        ...
        const versionInSummary = new RegExp(
    ...
    headingConvention =
      phaseIdConvention === undefined ? resolvePhaseIdConvention(cwd, ws) : phaseIdConvention;
...
function currentMilestoneRawRanges(content, cwd) {
  ...
  if (detailsMatch) {
    ...
    details = {
      start: detailsStart,
      end: computeMilestoneSectionEnd(content, detailsMatch[0], detailsStart),
    };
  }
