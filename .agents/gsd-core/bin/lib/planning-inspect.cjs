...
function readDocument(filePath, root) {
  ...
  try {
    ...
    return {
      text: normalizeLineEndings(node_fs_1.default.readFileSync(filePath, "utf-8")),
      exists: true,
      readable: true,
    };
  } catch {
...
function parseSummaryProvenance(content) {
  ...
  const filesSection = collectSection(content, (h) =>
    /^files\s+created\/modified\s*$/i.test(h.text.trim()),
  );
  if (filesSection) {
  ...
  const deviationsSection = collectSection(content, (h) =>
    /^deviations\s+from\s+plan/i.test(h.text.trim()),
  );
...
function summaryForPlan(planFile, summaryFiles) {
  ...
  for (const candidate of summaryFiles) {
    ...
    const candidateKey = node_path_1.default
      .basename(candidate)
      .replace(/-?SUMMARY/i, "")
      .replace(/\.md$/i, "");
  ...
  return parsed.tasks.map((task) => {
    ...
    if (attributed === undefined) {
      ...
      diagnostics.push({
        ...
        subject: `${toPosix(planFile)}#${task.index}`,
        detail:
          "SUMMARY carries only a plan-level file list; task-scoped changed files are unknown.",
      });
      return {
        index: task.index,
        kind: task.kind,
    ...
    if (planned.length > 0) {
      ...
      if (!same) {
        diagnostics.push({
          ...
          subject: `${toPosix(planFile)}#${task.index}`,
          detail:
            "Planned and changed file sets disagree; both are reported verbatim, unreconciled.",
        });
      }
    }
    return {
...
function buildPlanRows(phaseDir, diagnostics, planningRoot) {
  ...
  const rows = scan.allPlanFiles.map((planFile) => {
    ...
    let provenance = null;
    if (summaryFile !== null) {
      const summaryDoc = readDocument(
        node_path_1.default.join(phaseDir, summaryFile),
        planningRoot,
      );
...
function buildPhaseGoalAndDependencies(cwd, roadmapDoc, phaseId, phaseDirLabel, diagnostics) {
  ...
  if (phaseId === null) {
    diagnostics.push({
      ...
      subject: phaseDirLabel,
      detail:
        "Phase directory name carries no recognizable phase number; ROADMAP section cannot be located.",
    });
    return {
  ...
  if (sectionBody === null) {
    diagnostics.push({
      ...
      subject: phaseDirLabel,
      detail:
        "ROADMAP.md has no section for this phase; goal and dependencies are non-answers, not empty.",
    });
    return {
      goal: { value: null, scope: SCOPE.UNSCOPED },
      dependencies: { value: [], scope: SCOPE.UNSCOPED },
...
function buildPlanningInspect(cwd) {
  ...
  if (!planningExists) {
    diagnostics.push({
      ...
      detail:
        "No .planning/ directory; every section below is an empty non-answer, not an empty project.",
    });
  }
  ...
  const orphans = snapshot.allPhaseDirNames.value.filter((dir) => !windowedSet.has(dir)).sort();
