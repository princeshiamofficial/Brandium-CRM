...
function execGitDefault(args, opts = {}) {
  return (0, shell_command_projection_cjs_1.execGit)(args, {
    ...opts,
    timeout: opts.timeout ?? DEFAULT_GIT_TIMEOUT_MS,
  });
}
function parseWorktreePorcelain(porcelain) {
  return parseWorktreeEntries(porcelain)
...
function normalizeCleanupManifestEntry(entry) {
  ...
  const allowedBases = Array.from(
    new Set([
      expectedBase,
      ...rawAllowedBases.filter((base) => typeof base === "string" && base.length > 0),
    ]),
  );
  ...
  const filesModified = (Array.isArray(e.files_modified) ? e.files_modified : []).filter(
  ...
  const declaredDeletions = (
    Array.isArray(e.declared_deletions) ? e.declared_deletions : []
...
function normalizeCleanupManifest(manifest) {
  ...
  const p = parsed;
  const rawEntries = Array.isArray(p) ? p : Array.isArray(p?.worktrees) ? p.worktrees : [];
  const seen = new Set();
  const entries = [];
...
function isSummaryArtifactRelPath(relPath) {
  ...
  return (
    normalized.startsWith(`${SUMMARY_ARTIFACT_DIR}/`) &&
...
function rescueSummaryArtifacts(worktreePath, repoRoot, deps) {
  ...
  for (const absPath of summaryPaths) {
    ...
    const relPath = (0, shell_command_projection_cjs_1.posixNormalize)(
      absPath.slice(worktreePath.length).replace(/^[/\\]/, ""),
    );
    ...
    const catFileResult = execGit(["-C", worktreePath, "cat-file", "-e", `HEAD:${relPath}`], {
      cwd: repoRoot,
    });
...
function planWaveScopeConformance(changedPaths, declaredFiles, branch) {
  ...
  for (const raw of Array.isArray(changedPaths) ? changedPaths : []) {
    ...
    const covered = prefixes.some(
      (prefix) => prefix === null || changed === prefix || changed.startsWith(`${prefix}/`),
    );
    if (covered) continue;
...
function executeWorktreeWaveCleanupPlan(plan, deps = {}) {
  ...
  for (let i = 0; i < entries.length; i += 1) {
    ...
    const branchCheck = execGit(["-C", entry.worktree_path, "rev-parse", "--abbrev-ref", "HEAD"], {
      cwd: plan.repoRoot,
    });
    ...
    const allowedBases =
      Array.isArray(entry.allowed_bases) && entry.allowed_bases.length > 0
        ? entry.allowed_bases
        : [entry.expected_base];
    ...
    const deletions = execGit(
      ["diff", "--diff-filter=D", "--name-only", `HEAD...${entry.branch}`],
      { cwd: plan.repoRoot },
    );
    ...
    if (declaredFiles.length > 0) {
      const scopeDiff = execGit(["diff", "--name-only", `HEAD...${entry.branch}`], {
        cwd: plan.repoRoot,
      });
    ...
    const { rescuedRelPaths, failures: rescueFailures } = rescueSummaryArtifacts(
      entry.worktree_path,
      plan.repoRoot,
      deps,
    );
    if (rescueFailures.length > 0) {
      blockEntry(
        result,
        "summary_rescue_failed",
        rescueFailures.map((f) => `${f.relPath}: ${f.error}`).join("; "),
      );
      continue; // #2852: isolate
    }
    const worktreeStatus = execGit(
      ["-C", entry.worktree_path, "status", "--porcelain", "--untracked-files=all"],
      { cwd: plan.repoRoot },
    );
    ...
    const dirtyLines = (worktreeStatus.stdout || "").split("\n").filter((line) => {
    ...
    const merge = execGit(
      [
        ...
        "--no-edit",
        "-m",
        `chore: merge executor worktree (${entry.branch})`,
      ],
      { cwd: plan.repoRoot },
    );
    ...
    let remove = execGit(["worktree", "remove", entry.worktree_path, "--force"], {
      cwd: plan.repoRoot,
    });
    if (!gitResultOk(remove)) {
      ...
      remove = execGit(["worktree", "remove", entry.worktree_path, "--force"], {
        cwd: plan.repoRoot,
      });
    }
    if (!gitResultOk(remove)) {
...
function cmdWorktreeCleanupWave(cwd, args = []) {
  ...
  let manifest;
  try {
    manifest = node_fs_1.default.readFileSync(
      node_path_1.default.resolve(cwd, manifestPath),
      "utf8",
    );
  } catch (err) {
...
function cmdWorktreeRecordAgent(cwd, args = [], deps = {}) {
  ...
  try {
    ...
    write(
      `${JSON.stringify({ ok: false, reason: "manifest_read_failed", hint, error: err.message }, null, 2)}\n`,
    );
    process.exitCode = 1;
  ...
  const writeFile =
    deps.writeFile || ((p, content) => node_fs_1.default.writeFileSync(p, content, "utf8"));
  writeFile(resolved, plan.manifest);
  write(
    `${JSON.stringify({ ok: true, reason: "ok", entry: plan.entry, manifest_path: resolved }, null, 2)}\n`,
  );
...
function executeWorktreeCreatePlan(plan, repoRoot, deps = {}) {
  ...
  const baseCheck = execGit(["rev-parse", "--verify", "--quiet", `${base}^{commit}`], {
    cwd: repoRoot,
  });
  ...
  const addResult = execGit(["worktree", "add", "-b", branch, worktreePath, base], {
    cwd: repoRoot,
  });
...
function cmdWorktreeCreate(cwd, args = [], deps = {}) {
  ...
  try {
    ...
    const hint =
      'Manifest is not valid JSON. The orchestrator must initialize it as {"orchestrator_root": "...", "worktrees": []} before creating agent worktrees.';
  ...
  if (Array.isArray(parsed)) {
    ...
    if (!Array.isArray(container.worktrees)) {
      const hint =
        'Manifest "worktrees" must be an array. Re-initialize as {"orchestrator_root": "...", "worktrees": []}.';
  ...
  if (!rootFlag) {
    const hint =
      "--root is required (fail-closed root confinement, #3050). Pass --root <orchestrator-root-dir> so worktree.create can verify --path resolves inside it before creating anything.";
  ...
  }
  {
    ...
    if (
      rel === "" ||
      rel === ".." ||
      rel.startsWith(`..${node_path_1.default.sep}`) ||
  ...
  if (!result.ok) {
    ...
    write(
      `${JSON.stringify({ ok: false, reason: result.reason, stderr: result.stderr }, null, 2)}\n`,
    );
    process.exitCode = 1;
  ...
  try {
    ...
    write(
      `${JSON.stringify({ ok: false, reason: "manifest_write_failed", hint, error: err.message }, null, 2)}\n`,
    );
    process.exitCode = 1;
    return { ok: false, reason: "manifest_write_failed", hint };
  }
  write(
    `${JSON.stringify({ ok: true, reason: "created", entry: recorded, cwd: result.cwd, manifest_path: resolved }, null, 2)}\n`,
  );
...
function reapOrphanWorktrees(repoRoot, deps = {}) {
  ...
  const reapMtimeGuardMs =
    deps.reapMtimeGuardMs !== undefined ? deps.reapMtimeGuardMs : REAP_MTIME_GUARD_MS;
  ...
  for (const entryName of entries) {
    ...
    const worktreePath =
      node_path_1.default.basename(resolvedGitFile) === ".git"
    ...
    let branchTip;
    {
      ...
      if (!headContent) {
        results.push({
          path: worktreePath,
          status: "skipped",
          reason: "cannot_resolve_branch_tip",
        });
        continue;
      }
    ...
    const ancestorCheck = execGit(["merge-base", "--is-ancestor", branchTip, mainTip], {
      cwd: repoRoot,
    });
    ...
    const removeResult = execGit(["worktree", "remove", gitKnownPath, "--force"], {
      cwd: repoRoot,
    });
...
function cmdWorktreeReapOrphans(cwd, deps = {}) {
  ...
  try {
    ...
    writeErr(
      `[gsd] worktree.reap-orphans failed: ${err && err.message ? err.message : String(err)}\n`,
    );
    result = [];
  }
  ...
  if (skippedCount > 0) {
    ...
    writeErr(
      `[gsd] worktree.reap-orphans: ${skippedCount} orphan(s) skipped (run with DEBUG=1 for details)\n`,
    );
  }
  write(
    `${JSON.stringify({ ok: true, reaped: result.filter((r) => r.status === "reaped").length, entries: result }, null, 2)}\n`,
  );
}
