...
const {
  ...
  isCentralConfigKey: _isCentralConfigKeyFn,
} = configSchema;
...
const { UNUSABLE_REASON: _UNUSABLE_REASON, warnUnusableInput: _warnUnusableInput } =
  unusableInputModule;
...
function _getConfigValue(parsed, key, nested) {
  ...
  if (
    ...
    typeof parsed[nested.section] === "object" &&
    parsed[nested.section] !== null
  ) {
...
function isGitIgnored(cwd, targetPath) {
  ...
  const result = (0, shell_command_projection_cjs_1.execGit)(
    ["check-ignore", "-q", "--no-index", "--", normalized],
    { cwd },
  );
...
function _warnUnknownProfileOverrides(parsed, configLabel) {
  ...
  if (policy && typeof policy === "object" && !Array.isArray(policy)) {
    ...
    if (
      ...
      !model_catalog_cjs_1.KNOWN_PROVIDERS.has(provider) &&
      !_POLICY_SENTINEL_PROVIDERS.has(provider)
    ) {
...
function _warnShadowedGlobalDefaults(globalDefaults, globalPath) {
  const shadowed = GLOBAL_DEFAULTS_RESOLUTION_KEYS.filter(
    (k) => k !== "effort" && Object.prototype.hasOwnProperty.call(globalDefaults, k),
  );
...
function _federatedConfigSchema(cwd) {
  ...
  if (typeof cwd === "string" && cwd) {
    try {
      ...
      const schema = loaderMod.loadRegistry({
...
function _readConfigFile(filePath) {
  ...
  try {
    ...
    return {
      kind: "fault",
      fault: { reason: CONFIG_REASON.CONFIG_UNREADABLE, path: filePath, code },
    };
  }
  ...
  try {
    ...
    return {
      kind: "fault",
      fault: { reason: CONFIG_REASON.CONFIG_UNPARSEABLE, path: filePath, code: "" },
    };
  }
...
function _warnUnusableConfig(fault) {
  ...
  const what =
    fault.reason === CONFIG_REASON.CONFIG_UNPARSEABLE
      ? "is not valid JSON"
...
function loadConfigResolved(cwd, options = {}) {
  ...
  const activeWorkstream = Object.prototype.hasOwnProperty.call(options, "workstream")
    ? options["workstream"]
    : options["workstreamContext"] &&
        Object.prototype.hasOwnProperty.call(options["workstreamContext"], "ws")
      ? options["workstreamContext"]["ws"]
      : process.env["GSD_WORKSTREAM"] || null;
  const ws =
    typeof activeWorkstream === "string"
      ? activeWorkstream
  ...
  if (ws) {
    ...
    try {
      ...
      const {
        parsed: rootNormalized,
        normalizations: rootNorms,
        skipped: rootSkipped,
      } = (0, configuration_cjs_1.normalizeLegacyKeys)(rootParsed);
      if (rootSkipped.length > 0) {
        _warnUnusableInput({
          reason: _UNUSABLE_REASON.CONFIG_SECTION_NOT_OBJECT,
        ...
        if (persist) {
          try {
            (0, shell_command_projection_cjs_1.platformWriteSync)(
              rootConfigPath,
              JSON.stringify(rootParsed, null, 2),
            );
          } catch {
            /* ignore */
          }
  ...
  try {
    ...
    let configDirty = false;
    {
      const {
        ...
        normalizations,
        skipped,
      } = (0, configuration_cjs_1.normalizeLegacyKeys)(fileData);
      if (skipped.length > 0) {
        _warnUnusableInput({
          reason: _UNUSABLE_REASON.CONFIG_SECTION_NOT_OBJECT,
          source: configPath,
    ...
    if (Array.isArray(currentSubRepos) && currentSubRepos.length > 0) {
      ...
      if (detected.length > 0) {
        ...
        if (JSON.stringify(sorted) !== JSON.stringify(detected)) {
          ...
          if (!(0, configuration_cjs_1.isConfigSection)(fileData.planning)) fileData.planning = {};
          fileData.planning["sub_repos"] = detected;
          configDirty = true;
        }
      }
    ...
    const parsed = rootParsed ? (_deepMergeConfig(rootParsed, fileData) ?? fileData) : fileData;
    ...
    const _baseConfig = {
      ...
      pr_strict:
        get("pr_strict", { section: "planning", field: "pr_strict" }) ?? defaults.pr_strict,
      ...
      review: {
        max_prompt_tokens:
          get("max_prompt_tokens", { section: "review", field: "max_prompt_tokens" }) ??
    ...
    const reason = fileHadKeys ? CONFIG_REASON.RESOLVED : CONFIG_REASON.CONFIGURED_EMPTY;
    ...
    if (node_fs_1.default.existsSync(planningDir(cwd, ws))) {
      ...
      try {
        return fallback({
          config: _applyFederatedOverlay(defaults, {}, cwd),
    ...
    try {
      ...
      const _globalBaseCfg = {
        ...
        research_before_questions:
          globalDefaults["research_before_questions"] ??
        ...
        granularity:
          globalDefaults["granularity"] !== undefined ? globalDefaults["granularity"] : null,
      ...
      try {
        return fallback({
          config: _applyFederatedOverlay(_globalBaseCfg, globalDefaults, cwd),
