...
const { computeHaltPropagation, buildSummaryFileIndex, isSummaryFileHalted, isSummaryFileBlocked } =
  planDependencyGraphMod;
...
function cmdPhasesList(cwd, options, raw) {
  ...
  try {
    ...
    // can tell a genuinely-empty milestone from one it could not scope.
    output(
      { directories: dirs, count: dirs.length, phase_scope: phaseScope },
      raw,
      dirs.join("\n"),
    );
  } catch (e) {
  ...
  try {
    ...
    if (node_fs_1.default.existsSync(phasesDir)) {
      ...
      const dirPattern = new RegExp(
        `^${OPTIONAL_PROJECT_CODE_PREFIX_SOURCE}${(0, pattern_cjs_1.escapeRegex)(normalized)}\\.(\\d+)`,
      );
      for (const dir of dirs) {
...
function getRoadmapModeForPhase(cwd, phaseNum) {
  const roadmapPath = node_path_1.default.join(planningDir(cwd), "ROADMAP.md");
  if (!node_fs_1.default.existsSync(roadmapPath)) return null;
  ...
  const phaseHeader = new RegExp(
    `#{2,4}\\s*Phase\\s+${escapedPhase}${OPTIONAL_PHASE_TAG_SOURCE}\\s*:`,
    "i",
  );
...
function cmdPhaseTddApplicable(cwd, args, raw) {
  ...
  const resolvedPath = node_path_1.default.isAbsolute(planPath)
    ? planPath
    : node_path_1.default.join(cwd, planPath);
...
function cmdFindPhase(cwd, phase, raw) {
  ...
  if (node_fs_1.default.existsSync(flatPhasesDir)) searchDirs.push(flatPhasesDir);
  try {
  ...
  notFound.searched_directories = searchDirs.map((searchDir) =>
    toPosixPath(
      node_path_1.default.join(
        node_path_1.default.relative(cwd, planBase),
        node_path_1.default.relative(planBase, searchDir),
      ),
    ),
  );
  for (const searchDir of searchDirs) {
    try {
      ...
      const dirMatch =
        match.match(
          new RegExp(
            `^${OPTIONAL_PROJECT_CODE_PREFIX_SOURCE}(${PHASE_NUMBER_TOKEN_SOURCE})-?(.*)`,
            "i",
          ),
...
function resolveDependencyId(dep, planMap, canonicalToId, shortFormToId) {
  ...
  if (planMap.has(lower)) return planMap.get(lower).id;
  if (canonicalToId.has(lower)) return canonicalToId.get(lower);
...
function cmdPhasePlanIndex(cwd, phase, raw) {
  ...
  const countableSummaryFiles = summaryFiles.filter(
    (f) => !isSummaryFileBlocked(node_path_1.default.join(phaseDir, f)),
  );
  ...
  for (const planFile of planFiles) {
    ...
    const halted =
      hasSummary && summaryFile !== undefined
  ...
  const { level, visited, order, unresolved } = computeDependencyLevels(
    rawPlans,
    planMap,
    canonicalToId,
    shortFormToId,
...
function describeGoalShapedTitle(description) {
  ...
  const reasons = [
    tooLong
      ? `${trimmed.length} chars (over the ${PHASE_ADD_TITLE_MAX_LEN}-char title threshold)`
      : null,
...
function assertDescriptionPreservesMilestoneScope(cwd, description, command) {
  ...
  try {
    ...
    /* unresolvable convention → treat as not-configured (base behaviour) */
  }
...
function collectSiblingWorktreePhaseNums(cwd, used) {
  ...
  for (const line of porcelain.split("\n")) {
    ...
    if (!wt || node_path_1.default.resolve(wt) === node_path_1.default.resolve(cwd)) continue;
    try {
      for (const entry of node_fs_1.default.readdirSync(
        node_path_1.default.join(wt, ".planning", "phases"),
      )) {
    ...
    try {
      const content = node_fs_1.default.readFileSync(
        node_path_1.default.join(wt, ".planning", "ROADMAP.md"),
        "utf-8",
      );
      let m;
...
function cmdPhaseAdd(cwd, description, raw, customId) {
  ...
  const { newPhaseId, dirName } = withPlanningLock(cwd, () => {
    ...
    (0, shell_command_projection_cjs_1.platformWriteSync)(
      node_path_1.default.join(dirPath, ".gitkeep"),
      "",
    );
    const dependsOn =
  ...
  const result = {
    ...
    directory: toPosixPath(
      node_path_1.default.join(
        node_path_1.default.relative(cwd, planningDir(cwd)),
        "phases",
        dirName,
      ),
    ),
  ...
  const { decimalPhase, dirName } = withPlanningLock(cwd, () => {
    ...
    if (node_fs_1.default.existsSync(phasesDir)) {
      ...
      const decimalPattern = new RegExp(
        `^${OPTIONAL_PROJECT_CODE_PREFIX_SOURCE}${(0, pattern_cjs_1.escapeRegex)(normalizedBase)}\\.(\\d+)`,
      );
      for (const dir of dirs) {
...
function renameDecimalPhases(phasesDir, baseInt, removedDecimal) {
  ...
  for (const item of toRename) {
    ...
    (0, shell_command_projection_cjs_1.retryRenameSync)(
      node_path_1.default.join(phasesDir, item.dir),
      node_path_1.default.join(phasesDir, newDirName),
    );
    renamedDirs.push({ from: item.dir, to: newDirName });
    for (const f of node_fs_1.default.readdirSync(
      node_path_1.default.join(phasesDir, newDirName),
    )) {
      if (f.includes(oldPhaseId)) {
        const newFileName = f.replace(oldPhaseId, newPhaseId);
        (0, shell_command_projection_cjs_1.retryRenameSync)(
          node_path_1.default.join(phasesDir, newDirName, f),
          node_path_1.default.join(phasesDir, newDirName, newFileName),
        );
...
function findOrphanedDisplacementName(dir, fileName) {
  ...
  if (!node_fs_1.default.existsSync(node_path_1.default.join(dir, base))) return base;
  for (let n = 2; n <= 100; n++) {
    const candidate = `${base}.${n}`;
    if (!node_fs_1.default.existsSync(node_path_1.default.join(dir, candidate))) return candidate;
  }
  return null;
}
    ...
    for (const f of node_fs_1.default.readdirSync(
      ...
      if (matchedPrefix) {
        ...
        if (node_fs_1.default.existsSync(destPath)) {
          ...
          (0, shell_command_projection_cjs_1.retryRenameSync)(
            destPath,
            node_path_1.default.join(phasesDir, newDirName, displacedName),
          );
          (0, shell_command_projection_cjs_1.retryRenameSync)(
            node_path_1.default.join(phasesDir, newDirName, f),
            destPath,
...
function updateRoadmapAfterPhaseRemoval(roadmapPath, targetPhase, isDecimal, removedInt, cwd) {
  return withPlanningLock(cwd, () => {
    ...
    content = (0, markdown_sectionizer_cjs_1.deleteSection)(
      content,
    ...
    if (progressHeadingMatch && progressHeadingMatch.index !== undefined) {
      ...
      const deleteResult = (0, markdown_table_cjs_1.deleteTableRow)(
        progressSection,
        matchRemovedProgressRow,
      );
      if (deleteResult.ok) {
    ...
    if (!isDecimal) {
      ...
      if (ordinalHeadingMatch && ordinalHeadingMatch.index !== undefined) {
        ...
        for (;;) {
          ...
          const cellResult = (0, markdown_table_cjs_1.updateTableCell)(
            ordinalSection,
            (row, index) => {
            ...
            (current) => {
              ...
              const padMatch = targetLine
                ? new RegExp(
                    `^[ \\t]*\\|(\\s*)${(0, pattern_cjs_1.escapeRegex)((0, markdown_table_cjs_1.escapeCell)(current))}(\\s*)\\|`,
                  ).exec(targetLine)
                : null;
    ...
    return (0, shell_command_projection_cjs_1.contentChangedAfterNormalize)(
      roadmapPath,
      originalContent,
      content,
    );
...
function cmdPhaseRemove(cwd, targetPhase, options, raw) {
  ...
  const roadmapPath = node_path_1.default.join(planningDir(cwd), "ROADMAP.md");
  const phasesDir = node_path_1.default.join(planningDir(cwd), "phases");
  if (!node_fs_1.default.existsSync(roadmapPath)) error("ROADMAP.md not found");
  ...
  if (targetDir && !force) {
    ...
    // without --force.
    const summaryCount = scanPhasePlans(node_path_1.default.join(phasesDir, targetDir)).summaryFiles
      .length;
    if (summaryCount > 0) {
      error(
  ...
  if (targetDir)
    node_fs_1.default.rmSync(node_path_1.default.join(phasesDir, targetDir), {
      recursive: true,
      force: true,
    });
  let renamedDirs = [];
...
function phaseDisplayNameFromRoadmap(roadmapContent, phaseNum) {
  ...
  const heading = roadmapContent.match(
    new RegExp(
      `^#{2,4}\\s*Phase\\s+${phaseEscaped}${OPTIONAL_PHASE_TAG_SOURCE}\\s*:\\s*([^\\n]+)`,
      "im",
    ),
  );
...
function analyzeRequirementsLine(rawLine) {
  ...
  const nothingDemonstrablyDropped =
    ...
    spacedRangePairs.every(
      ([a, b]) => selected.has(a.toUpperCase()) && selected.has(b.toUpperCase()),
    );
...
function formatRequirementsLineWarning(phaseNum, rawLine, analysis) {
  ...
  const skippedNames = analysis.unselectedIdShaped.filter(
    (t) => !alreadyNamed.has(t.toUpperCase()),
  );
...
function cmdPhaseComplete(cwd, phaseNum, raw) {
  ...
  const summaryCount = phaseInfo["summaries"] ? phaseInfo["summaries"].length : 0;
  ...
  try {
    ...
    error(
      `Phase ${phaseNum} cannot be completed: its plan directory is unreadable (${phaseInfo["directory"]}: ${readErr.code || readErr.message}), so plan coverage cannot be verified. Restore read access and retry — a coverage gate that passes when it cannot read the plans is no gate at all (#2648).`,
  ...
  if (unsummarizedPlans.length > 0) {
    ...
    const more =
      unsummarizedPlans.length > 20 ? ` (and ${unsummarizedPlans.length - 20} more)` : "";
    ...
    const supersededCount =
      coverageScan.planFiles.length === 0
        ? 0
        : Math.max(0, phaseInfoPlanCount - coverageScan.planFiles.length);
  ...
  try {
    ...
    for (const file of scopeToPhase(
      ...
      const content = node_fs_1.default.readFileSync(
        node_path_1.default.join(phaseFullDir, file),
        "utf-8",
      );
    ...
    for (const file of scopeToPhase(
      ...
      const content = normalizeLineEndings(
        node_fs_1.default.readFileSync(verificationFilePath, "utf-8"),
      );
      ...
      const verStatus =
        typeof verFm["status"] === "string" ? verFm["status"].trim().toLowerCase() : "";
  ...
  try {
    ...
    for (const summaryName of summaryNames) {
      const v = verifyMod.verifySummaryCore(cwd, `${phaseDirRel}/${summaryName}`, Infinity, {
        checkCommits: false,
      });
    ...
    const runPhaseCompleteTransaction = () => {
      ...
      if (node_fs_1.default.existsSync(roadmapPath)) {
        ...
        const mutateMilestonePhase = (slice) => {
          let s = slice;
          s = (0, markdown_sectionizer_cjs_1.updateBullet)(
            s,
          ...
          s = editProgressHeadingSlice(s, (scoped) => {
            ...
            const statusResult = (0, markdown_table_cjs_1.updateTableCell)(
              text,
              rowMatch,
              "Status",
              " Complete    ",
            ...
            const completedResult = (0, markdown_table_cjs_1.updateTableCell)(
              text,
              rowMatch,
              "Completed",
              (current) => (dateShape.test(current.trim()) ? current : ` ${today} `),
            );
        ...
        roadmapUpdated = (0, shell_command_projection_cjs_1.contentChangedAfterNormalize)(
          roadmapPath,
          originalRoadmapContent,
          roadmapContent,
        );
        ...
        if (node_fs_1.default.existsSync(reqPath)) {
          ...
          if (reqMatch) {
            ...
            for (const reqId of citedReqIds) {
              ...
              let tableHit = false;
              const reqUpdate = updateTraceabilityCell(
                reqContent,
                reqRowMatch,
                "Status",
                (current) => {
          ...
          const stateRawForMilestone = node_fs_1.default.existsSync(statePath)
            ? node_fs_1.default.readFileSync(statePath, "utf-8")
            : null;
          const currentMilestoneRaw = stateRawForMilestone
            ? stateExtractField(stateRawForMilestone, "milestone")
            : null;
          const currentMilestoneMajor = currentMilestoneRaw
            ? extractMajorVersion(currentMilestoneRaw)
            : null;
          const bodyReqIds = [];
          ...
          const reqIsRegisteredAnywhere = (id) => {
            ...
            const rowProbeMatch = (row) =>
              (Object.values(row)[0] ?? "").trim().toLowerCase() === id.toLowerCase();
            return updateTraceabilityCell(reqContent, rowProbeMatch, "Status", (current) => current)
              .ok;
          };
          ...
          requirementsUpdated = (0, shell_command_projection_cjs_1.contentChangedAfterNormalize)(
            reqPath,
            originalReqContent,
            reqContent,
          );
      ...
      try {
        ...
        for (const dir of dirs) {
          ...
          if (dm) {
            ...
            if (
              comparePhaseNum(dm[1], phaseNum) > 0 &&
      ...
      if (roadmapContent !== null) {
        try {
          ...
          while ((pm = phasePattern.exec(roadmapForPhases)) !== null) {
            ...
            if (
              comparePhaseNum(pmNum, phaseNum) > 0 &&
      ...
      if (roadmapNextNum !== null) {
        ...
        const diskAgrees =
          diskNextNum !== null && comparePhaseNum(diskNextNum, roadmapNextNum) === 0;
        ...
        try {
          ...
          while ((cbm = cbPattern.exec(milestoneScope)) !== null) {
            ...
            if (!isChecked && !isSentinelPhaseId(cbm[2]) && comparePhaseNum(cbm[2], phaseNum) < 0) {
              if (
                lowestOutstanding === null ||
                comparePhaseNum(cbm[2], lowestOutstanding.num) < 0
              ) {
                lowestOutstanding = {
                  num: cbm[2],
                  name: cbm[3]
      ...
      if (node_fs_1.default.existsSync(statePath)) {
        const originalStateContent =
          (0, shell_command_projection_cjs_1.platformReadSync)(statePath) || "";
        ...
        const completeResult = (0, state_transition_cjs_1.transitionCore)(
          stateContent,
          {
            kind: "completePhase",
            phaseNum,
        ...
        stateUpdated = (0, shell_command_projection_cjs_1.contentChangedAfterNormalize)(
          statePath,
