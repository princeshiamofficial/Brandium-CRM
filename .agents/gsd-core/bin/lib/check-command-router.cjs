...
function resolvePath(inputPath, projectDir) {
  return node_path_1.default.isAbsolute(inputPath)
    ? inputPath
    : node_path_1.default.join(projectDir, inputPath);
}
...
function loadPlanContents(phaseDir) {
  if (!node_fs_1.default.existsSync(phaseDir)) return [];
  ...
  // neither for.
  return scanPhasePlans(phaseDir).planFiles.map((entry) =>
    readIfExists(node_path_1.default.join(phaseDir, entry)),
  );
}
...
function buildPlanMessage(uncovered) {
  ...
  return [
    ...
    "",
    ...uncovered.map(
      (item) => `- **${item.id}** (${item.category || "uncategorized"}): ${item.text}`,
    ),
    "",
    ...
    output(
      {
        ...
        uncovered: [],
        message:
          "Decision coverage gate called without a context path argument — the caller (e.g. the plan-phase workflow) must pass the CONTEXT.md path. An empty argument is a caller error, not evidence there is nothing to check (#2770).",
      },
      raw,
      undefined,
    );
...
function readModifiedFilesContent(projectDir, summaries) {
  ...
  for (const summary of summaries) {
    ...
    for (const blockMatch of summary.matchAll(/files_modified:\s*\n((?:[ \t]*-\s+.+\n?)+)/g)) {
      const files = [...(blockMatch[1] || "").matchAll(/-\s+(.+)/g)].map((match) =>
...
function cmdDecisionCoverageVerify(projectDir, args, raw) {
  ...
  const summaryParts = node_fs_1.default.existsSync(phaseDir)
    ? scanPhasePlans(phaseDir).summaryFiles.map((entry) =>
        readIfExists(node_path_1.default.join(phaseDir, entry)),
      )
    : [];
  const haystack = [
...
function findUiSpecInDir(phaseDir) {
  if (!phaseDir || !node_fs_1.default.existsSync(phaseDir)) return "";
  try {
...
function computeUiPlanGate(projectDir, phase) {
  ...
  try {
    ...
    /* roadmap read failure → treat as empty (non-frontend) */
  }
  ...
  const hasFrontendEvidence = frontend
    ? (0, ui_frontend_evidence_cjs_1.hasStaticFrontendEvidence)(projectDir)
    : false;
  ...
  try {
    ...
    /* phase dir lookup failure → hasUiSpec=false */
  }
...
function cmdUiPlanGate(projectDir, args, raw) {
  ...
  if (!phase) {
    error(
      "ui-plan-gate requires a phase argument: check ui-plan-gate <phase>",
...
function computeUiSafetyGate(projectDir, phase) {
  ...
  try {
    const changed = (0, node_child_process_1.execFileSync)(
      "git",
    ...
    /* git unavailable or no prior commit — treat as no UI files changed */
  }
  ...
  if (block) {
    result.message =
      `UI files changed in this wave but no UI-SPEC.md exists for Phase ${phase}. ` +
...
function cmdUiSafetyGate(projectDir, args, raw) {
  ...
  if (!phase) {
    error(
      "ui-safety-gate requires a phase argument: check ui-safety-gate <phase>",
      ERROR_REASON.SDK_MISSING_ARG,
    );
    return;
  }
...
function cmdTddReviewCheckpoint(projectDir, args, raw) {
  ...
  try {
    ...
    if (result && typeof result === "object") {
      const relDir = typeof result["directory"] === "string" ? result["directory"] : "";
      if (relDir) phaseDir = node_path_1.default.resolve(projectDir, relDir);
    } else if (typeof result === "string") {
      phaseDir = result;
    }
  } catch {
    /* phase dir lookup failure */
  }
  ...
  for (const planPath of tddPlanFiles) {
    ...
    let refactor = false;
    try {
      const redCommit = (0, node_child_process_1.execFileSync)(
        "git",
        ["log", "--oneline", `--grep=^test(${planId}):`, "--", "."],
        {
          cwd: projectDir,
          encoding: "utf-8",
          maxBuffer: 1024 * 1024,
      ...
      /* git unavailable or no match */
    }
    try {
      const greenCommit = (0, node_child_process_1.execFileSync)(
        "git",
        ["log", "--oneline", `--grep=^feat(${planId}):`, "--", "."],
        {
          cwd: projectDir,
          encoding: "utf-8",
          maxBuffer: 1024 * 1024,
    ...
    try {
      const refactorCommit = (0, node_child_process_1.execFileSync)(
        "git",
        ["log", "--oneline", `--grep=^refactor(${planId}):`, "--", "."],
        {
          cwd: projectDir,
          encoding: "utf-8",
          maxBuffer: 1024 * 1024,
    ...
    output(
      {
        ...
        record: resolved,
        readError: text
          ? `record is not valid JSON: ${resolved}`
          : `record not found or unreadable: ${resolved}`,
      },
      raw,
      undefined,
    );
...
function resolvePhaseDirOrEmpty(projectDir, phase) {
  try {
    ...
    /* phase dir lookup failure → caller emits degraded payload */
  }
  return "";
}
    ...
    output(
      {
        ...
        readError:
          "verify-command-paths requires a phase argument: check verify-command-paths <phase>",
      },
      raw,
      undefined,
    );
    ...
    output(
      {
        ...
        counts: { blocker: 0, warning: 0, total: 0 },
        readError:
          "verify-failure-directions requires a phase argument: check verify-failure-directions <phase>",
      },
      raw,
      undefined,
    );
...
function buildPredicateDeps() {
  return {
    runBoundedShell(opts) {
      const r = (0, shell_command_projection_cjs_1.execTool)("sh", ["-c", opts.command], {
        cwd: opts.cwd,
    ...
    findPhaseArtifact(phaseDir, artifactSuffix) {
      if (!node_fs_1.default.existsSync(phaseDir)) return null;
      if (
        artifactSuffix === "." ||
        artifactSuffix === ".." ||
      ...
      if (
        directPath.safe &&
        node_fs_1.default.existsSync(directPath.resolved) &&
        node_fs_1.default.statSync(directPath.resolved).isFile()
      ) {
        return directPath.resolved;
      }
      const planningPath = (0, security_cjs_1.validatePath)(
        node_path_1.default.join(".planning", artifactSuffix),
        phaseDir,
      );
      if (
        planningPath.safe &&
        node_fs_1.default.existsSync(planningPath.resolved) &&
        node_fs_1.default.statSync(planningPath.resolved).isFile()
      ) {
...
function cmdApiCoverageVerifyPre(projectDir, args, raw) {
  ...
  let token =
    (0, shell_command_projection_cjs_1.posixNormalize)(phaseArg).split("/").filter(Boolean).pop() ||
    "";
    ...
    output(
      {
        ...
        detected: false,
        message:
          "api-coverage: no .planning/phases directory; gate skipped (not a GSD project layout)",
      },
      raw,
      undefined,
    );
  ...
  if (coverageFile) {
    let matrixText;
    try {
      matrixText = node_fs_1.default.readFileSync(
        node_path_1.default.join(resolvedDir, coverageFile),
        "utf8",
      );
    } catch {
...
function routeCheckCommand({ args, cwd, raw }) {
  ...
  const subcommand =
    typeof rawSubcommand === "string" ? rawSubcommand.replace(/\./g, "-") : rawSubcommand;
