...
function parseCodexAgentToml(content) {
  ...
  const { model, modelLineIndex, reasoningEffort, reasoningEffortLineIndex } = scanHeaderLines(
    lines,
    start,
    end,
  );
...
function removeLine(doc, index, which) {
  ...
  return {
    ...
    reasoningEffortLineIndex:
      which === "reasoningEffort" ? null : reindex(doc.reasoningEffortLineIndex),
  };
}
/**
...
exports.CODEX_SANDBOX_HOLDS = Object.freeze({
  ...
  "gsd-debug-session-manager":
    "declares Write/Edit; pending Codex sandbox_mode enforcement decision",
  ...
  "gsd-nyquist-auditor":
    "declares Write/Edit (YAML list-form tools:, surfaced by the list-form parse fix); pending Codex sandbox_mode enforcement decision",
});
