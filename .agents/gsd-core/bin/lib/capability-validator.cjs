...
function validateConfigSliceEntry(capId, key, slice) {
  ...
  if (typeof slice !== "object" || slice === null || Array.isArray(slice)) {
    errors.push(
      'capability "' + capId + '" config["' + key + '"]: slice must be a non-null object',
    );
    return errors;
  }
  ...
  if (!VALID_CONFIG_SLICE_TYPES.has(slice.type)) {
    errors.push(
      'capability "' +
        ...
        key +
        '"]: type must be one of ' +
        [...VALID_CONFIG_SLICE_TYPES].join(", ") +
        " (got: " +
        JSON.stringify(slice.type) +
        ")",
    );
  }
  ...
  if (!Object.prototype.hasOwnProperty.call(slice, "default")) {
    errors.push('capability "' + capId + '" config["' + key + '"]: default is required');
  } else {
      ...
      if (typeof def !== "boolean") {
        errors.push(
          'capability "' +
            ...
            '" config["' +
            key +
            '"]: default must be a boolean for type:"boolean" (got: ' +
            typeof def +
            ")",
        );
      }
      ...
      if (typeof def !== "string") {
        errors.push(
          'capability "' +
            ...
            '" config["' +
            key +
            '"]: default must be a string for type:"string" (got: ' +
            typeof def +
            ")",
        );
      }
      ...
      if (typeof def !== "number") {
        errors.push(
          'capability "' +
            ...
            '" config["' +
            key +
            '"]: default must be a number for type:"number" (got: ' +
            typeof def +
            ")",
        );
        ...
        errors.push(
          'capability "' +
            ...
            '" config["' +
            key +
            '"]: default for type:"number" must be a finite number (got: ' +
            String(def) +
            ")",
        );
      }
      ...
      if (!Array.isArray(slice.values) || slice.values.length === 0) {
        errors.push(
          'capability "' +
            ...
            '" config["' +
            key +
            '"]: type:"enum" requires a non-empty "values" array of strings',
        );
        ...
        errors.push(
          'capability "' +
            ...
            '" config["' +
            key +
            '"]: type:"enum" values array must contain only strings',
        );
      }
      if (typeof def !== "string") {
        errors.push(
          'capability "' +
            ...
            '" config["' +
            key +
            '"]: default must be a string for type:"enum" (got: ' +
            typeof def +
            ")",
        );
      } else if (
        Array.isArray(slice.values) &&
        slice.values.length > 0 &&
        !slice.values.includes(def)
      ) {
        errors.push(
          'capability "' +
            capId +
  ...
  if (typeof slice.description !== "string" || slice.description.length === 0) {
    errors.push(
      'capability "' +
        ...
        '" config["' +
        key +
        '"]: description must be a non-empty string (got: ' +
        JSON.stringify(slice.description) +
        ")",
    );
  }
...
function validateVersionEnvelope(cap) {
  ...
  if (typeof cap.version !== "string" || !SEMVER_RE.test(cap.version)) {
    errors.push(
      'version must be a semver string (e.g. "1.2.3"); got: ' + JSON.stringify(cap.version),
    );
  }
  ...
  if (cap.engines !== undefined) {
    if (typeof cap.engines !== "object" || cap.engines === null || Array.isArray(cap.engines)) {
      ...
      errors.push(
        "engines.gsd must be a semver range string; got: " + JSON.stringify(cap.engines.gsd),
      );
    }
  }
  ...
  if (cap.compatVersions !== undefined) {
    if (
      typeof cap.compatVersions !== "object" ||
      cap.compatVersions === null ||
      Array.isArray(cap.compatVersions)
    ) {
      errors.push(
        "compatVersions must be an object mapping capability versions to gsd version ranges",
      );
    } else {
      for (const [k, v] of Object.entries(cap.compatVersions)) {
        if (!SEMVER_RE.test(k))
          errors.push('compatVersions key "' + k + '" must be a semver string');
        if (!isPlausibleRange(v))
          errors.push('compatVersions["' + k + '"] must be a semver range string');
      }
    }
  }

  if (
    cap.integrity !== undefined &&
    (typeof cap.integrity !== "string" || !SHA512_INTEGRITY_RE.test(cap.integrity))
  ) {
...
function validateCapability(cap, folderId) {
  ...
  if (cap.role === "feature") {
    ...
    if (cap.reviewer !== undefined) {
      errors.push(
        'role:feature capability must not have a "reviewer" body (admissible on role runtime or reviewer only)',
      );
    }
    ...
    if (cap.taskContentResolver !== undefined) {
      errors.push(
        'role:runtime capability must not have a "taskContentResolver" body (feature-only field)',
      );
    }
    ...
    if (cap.runtime !== undefined) {
      errors.push(
        'role:reviewer capability must not have a "runtime" body (it is not an install target)',
      );
    }
...
function validateCommandEntry(capId, entry, prefix) {
  ...
  if (typeof entry.family !== "string" || entry.family.length === 0) {
    ...
    entry.family === "__proto__" ||
    entry.family === "constructor" ||
    entry.family === "prototype"
  ) {
  ...
  if (typeof entry.module !== "string" || entry.module.length === 0) {
    ...
    if (!SAFE_BASENAME.test(mod)) {
      errors.push(
        ctx +
          '.module must be a safe bare basename (pattern: /^[A-Za-z0-9._-]+\\.cjs$/, no path separators, no "..", no NUL bytes, must end in ".cjs"); got: ' +
          JSON.stringify(mod),
      );
    }
  }
...
function validateRuntimeCompat(capId, runtimeCompat) {
  ...
  const validateRuntimeArray = (field, { allowWildcard }) => {
    ...
    if (!Array.isArray(value)) {
      errors.push(
        ctx + "." + field + " must be an array of runtime ids" + (allowWildcard ? ' or ["*"]' : ""),
      );
      return;
    }
  ...
  if (runtimeCompat.notes !== undefined) {
    if (
      typeof runtimeCompat.notes !== "object" ||
      runtimeCompat.notes === null ||
      Array.isArray(runtimeCompat.notes)
    ) {
...
function validateFeatureBody(cap) {
  ...
  if (cap.commands !== undefined) {
    if (!Array.isArray(cap.commands)) {
      ...
      for (let i = 0; i < cap.commands.length; i++) {
        errors.push(
          ...validateCommandEntry(cap.id || cap.role, cap.commands[i], "commands[" + i + "]"),
        );
      }
    }
  }
    ...
    for (const key of Object.keys(cap.config)) {
      ...
      if (val === null || typeof val !== "object" || Array.isArray(val)) {
        ...
        errors.push(
          'config["' +
            key +
            '"] must have a string "type" field (e.g. "boolean", "string", "number", "enum")',
        );
      }
    }
  }
        ...
        if (typeof h !== "object" || h === null || Array.isArray(h)) {
          ...
          if (typeof h.script !== "string" || h.script.length === 0) {
            ...
            errors.push(
              "hooks[" +
                i +
                "].script must be a relative path containing only [A-Za-z0-9._/-] " +
                '(no whitespace, shell metacharacters (e.g. ; | & $ ` ( ) < > * ? newline), leading "-", ' +
                'absolute path, or ".." segment) — it contains unsafe characters: ' +
                JSON.stringify(h.script),
            );
          }
  ...
  const declaredSkills = Array.isArray(cap.skills)
    ? new Set(cap.skills.filter((s) => typeof s === "string"))
    : null;
  const declaredAgents = Array.isArray(cap.agents)
    ? new Set(cap.agents.filter((a) => typeof a === "string"))
    : null;
  ...
  if (!Array.isArray(cap.steps)) {
    ...
    for (let i = 0; i < cap.steps.length; i++) {
      errors.push(
        ...validateStep(cap.steps[i], "steps[" + i + "]", declaredSkills, declaredAgents),
      );
    }
  }
  ...
  if (cap.activationKey !== undefined) {
    if (typeof cap.activationKey !== "string" || cap.activationKey.length === 0) {
      errors.push(
        'capability "' +
          (cap.id || "(unknown)") +
          '" activationKey must be a non-empty string (got: ' +
          JSON.stringify(cap.activationKey) +
          ")",
      );
    } else if (
      cap.activationKey === "__proto__" ||
      cap.activationKey === "constructor" ||
      cap.activationKey === "prototype"
    ) {
...
function validateTaskContentResolverFields(cap) {
  ...
  if (typeof tcr !== "object" || tcr === null || Array.isArray(tcr)) {
    ...
    errors.push(
      ctx +
        " taskContentResolver must be an object (got: " +
        got +
        "). " +
  ...
  if (
    typeof tcr.trackerPrefix !== "string" ||
    tcr.trackerPrefix.length === 0 ||
    !KEBAB_RE.test(tcr.trackerPrefix)
  ) {
    errors.push(
      ctx +
        " taskContentResolver.trackerPrefix must be a non-empty kebab-case string matching " +
        String(KEBAB_RE) +
        " (got: " +
        describeValue(tcr.trackerPrefix) +
        ")",
    );
  }
  ...
  if (!Array.isArray(inv.args)) {
    ...
    for (const a of inv.args) {
      if (typeof a !== "string") {
        errors.push(
          ctx +
            " taskContentResolver.invoke.args entries must be strings (got: " +
            describeValue(a) +
            ")",
        );
      } else if (a === "{{id}}") {
    ...
    if (!hasPlaceholder) {
      errors.push(
        ctx +
          ' taskContentResolver.invoke.args must contain a "{{id}}" placeholder — ' +
  ...
  if (!isPositiveIntegerMs(inv.timeoutMs)) {
    errors.push(
      ctx +
        " taskContentResolver.invoke.timeoutMs must be a positive integer of milliseconds — " +
        "an unbounded resolver call could hang task execution indefinitely " +
        "(got: " +
        describeValue(inv.timeoutMs) +
        ")",
    );
    ...
    errors.push(
      ctx +
        " taskContentResolver.invoke.timeoutMs must not exceed " +
        TASK_CONTENT_RESOLVER_TIMEOUT_CEILING_MS +
        "ms — an unbounded-in-practice value defeats the " +
        '"bounded subprocess" design intent (got: ' +
        inv.timeoutMs +
        ")",
    );
  }
...
const VALID_CONFIG_HOME_KINDS = new Set([
  "dot-home",
  "dot-home-nested",
  "xdg",
  "generic-agents-root",
...
const VALID_HOOKS_SURFACES = new Set([
  "settings-json",
  "codex-hooks-json",
  "cursor-hooks-json",
  "copilot-inline",
...
const FEATURE_FIELDS_FORBIDDEN_ON_RUNTIME = [
  "skills",
  "agents",
  "steps",
  "contributions",
...
const VALID_INSTALL_SURFACES = new Set([
  "settings-json",
  "codex-toml",
...
const VALID_EXTENDED_HOOK_EVENTS = new Set([
  "SubagentStop",
  "Stop",
  "PreCompact",
  "FileChanged",
...
const VALID_HOST_RUNTIMES = new Set([
  "node",
  "bun",
  "sandboxed-web",
  "python",
...
const SPAWN_ONLY_INVOKE_FIELDS = [
  "binary",
  "args",
  "promptChannel",
  "outputChannel",
...
const FEATURE_FIELDS_FORBIDDEN_ON_REVIEWER = [
  "skills",
  "agents",
  "steps",
  "contributions",
...
const CLAUDE_FAMILY_EVENTS = new Set([
  "SubagentStop",
  "Stop",
  "PreCompact",
  "FileChanged",
...
function validateConfigHome(capId, ch) {
  ...
  if (ch.kind === "__proto__" || ch.kind === "constructor" || ch.kind === "prototype") {
    ...
    errors.push(
      ctx +
        ".kind must be one of: " +
        [...VALID_CONFIG_HOME_KINDS].join(", ") +
        " (got: " +
        JSON.stringify(ch.kind) +
        ")",
    );
  }
  ...
  if (!Array.isArray(ch.env)) {
    errors.push(
      ctx +
        ".env is required and must be an array of strings (got: " +
        JSON.stringify(ch.env) +
        ")",
    );
  } else {
  ...
  if (ch.probeExists !== undefined) {
    if (typeof ch.probeExists !== "string" || ch.probeExists.length === 0) {
      errors.push(
        ctx +
          ".probeExists must be a non-empty string if present (got: " +
          JSON.stringify(ch.probeExists) +
          ")",
      );
    }
  }
  ...
  if (ch.skillsHome !== undefined) {
    ...
    for (const e of skillsHomeErrors) {
      errors.push(
        e.replace('capability "' + capId + '.skillsHome" runtime.configHome', ctx + ".skillsHome"),
      );
    }
  }
...
function validateArtifactKindEntry(capId, entry, prefix) {
  ...
  if (entry.kind === "__proto__" || entry.kind === "constructor" || entry.kind === "prototype") {
    ...
    errors.push(
      ctx +
        ".kind must be one of: " +
        [...VALID_ARTIFACT_KIND_NAMES].join(", ") +
        " (got: " +
  ...
  if (entry.nesting === undefined || entry.nesting === null) {
    errors.push(
      ctx + ".nesting is required and must be one of: " + [...VALID_ARTIFACT_NESTINGS].join(", "),
    );
  } else if (!VALID_ARTIFACT_NESTINGS.has(entry.nesting)) {
    errors.push(
      ctx +
        ".nesting must be one of: " +
        [...VALID_ARTIFACT_NESTINGS].join(", ") +
        " (got: " +
        JSON.stringify(entry.nesting) +
        ")",
    );
  }
  ...
  if (entry.recursive !== undefined) {
    if (typeof entry.recursive !== "boolean") {
      errors.push(
        ctx + ".recursive must be a boolean if present (got: " + typeof entry.recursive + ")",
      );
    }
  }
  ...
  for (const scope of ["global", "local"]) {
    ...
    if (!Array.isArray(arr)) {
      ...
      for (let i = 0; i < arr.length; i++) {
        const entry = arr[i];
        errors.push(
          ...validateArtifactKindEntry(capId, entry, "artifactLayout." + scope + "[" + i + "]"),
        );
        if (
          scope === "local" &&
          typeof entry === "object" &&
...
function validateRuntimeBody(cap) {
  ...
  if (!VALID_CONFIG_FORMATS.has(r.configFormat)) {
    errors.push(
      "runtime.configFormat must be one of: " +
  ...
  if (
    r.commandStyle === "__proto__" ||
    r.commandStyle === "constructor" ||
    r.commandStyle === "prototype"
  ) {
    errors.push('runtime.commandStyle "' + r.commandStyle + '" is a reserved name');
  } else if (!VALID_COMMAND_STYLES.has(r.commandStyle)) {
    errors.push(
      "runtime.commandStyle must be one of: " +
        [...VALID_COMMAND_STYLES].join(", ") +
        " (got: " +
        JSON.stringify(r.commandStyle) +
        ")",
    );
  }
  ...
  if (
    r.hooksSurface === "__proto__" ||
    r.hooksSurface === "constructor" ||
    r.hooksSurface === "prototype"
  ) {
    errors.push('runtime.hooksSurface "' + r.hooksSurface + '" is a reserved name');
  } else if (!VALID_HOOKS_SURFACES.has(r.hooksSurface)) {
    errors.push(
      "runtime.hooksSurface must be one of: " +
        [...VALID_HOOKS_SURFACES].join(", ") +
        " (got: " +
        JSON.stringify(r.hooksSurface) +
        ")",
    );
  }
  ...
  if (r.hookEvents !== undefined) {
    if (
      r.hookEvents === "__proto__" ||
      r.hookEvents === "constructor" ||
      r.hookEvents === "prototype"
    ) {
      errors.push('runtime.hookEvents "' + r.hookEvents + '" is a reserved name');
    } else if (!VALID_HOOK_EVENTS.has(r.hookEvents)) {
      errors.push(
        "runtime.hookEvents must be one of: " +
          [...VALID_HOOK_EVENTS].join(", ") +
          " (got: " +
          JSON.stringify(r.hookEvents) +
          ")",
      );
    }
  }
  ...
  if (r.extensionEvents !== undefined) {
    if (
      r.extensionEvents === "__proto__" ||
      r.extensionEvents === "constructor" ||
      r.extensionEvents === "prototype"
    ) {
      ...
      errors.push(
        "runtime.extensionEvents must be one of: " +
          [...VALID_EXTENSION_EVENTS].join(", ") +
          " (got: " +
          JSON.stringify(r.extensionEvents) +
          ")",
      );
    }
  }
  ...
  if (
    r.sandboxTier === "__proto__" ||
    r.sandboxTier === "constructor" ||
    r.sandboxTier === "prototype"
  ) {
    errors.push('runtime.sandboxTier "' + r.sandboxTier + '" is a reserved name');
  } else if (!VALID_SANDBOX_TIERS.has(r.sandboxTier)) {
    errors.push(
      "runtime.sandboxTier must be one of: " +
        [...VALID_SANDBOX_TIERS].join(", ") +
        " (got: " +
        JSON.stringify(r.sandboxTier) +
        ")",
    );
  }
  ...
  if (!VALID_INSTALL_SURFACES.has(r.installSurface)) {
    errors.push(
      "runtime.installSurface must be one of: " +
        [...VALID_INSTALL_SURFACES].join(", ") +
        " (got: " +
        JSON.stringify(r.installSurface) +
        ")",
    );
  }
  ...
  if (typeof r.writesSharedSettings !== "boolean") {
    errors.push(
      "runtime.writesSharedSettings must be a boolean (got: " +
        JSON.stringify(r.writesSharedSettings) +
        ")",
    );
  }
  ...
  if (!Object.prototype.hasOwnProperty.call(r, "permissionWriter")) {
    errors.push(
      "runtime.permissionWriter is required (must be null or one of: " +
        [...VALID_PERMISSION_WRITERS].join(", ") +
        ")",
    );
  } else if (r.permissionWriter !== null && !VALID_PERMISSION_WRITERS.has(r.permissionWriter)) {
    errors.push(
      "runtime.permissionWriter must be null or one of: " +
        [...VALID_PERMISSION_WRITERS].join(", ") +
        " (got: " +
        JSON.stringify(r.permissionWriter) +
        ")",
    );
  }
  ...
  const configHomeKind =
    r.configHome && typeof r.configHome === "object" ? r.configHome.kind : undefined;
    ...
    if (typeof r.localConfigDir !== "string" || r.localConfigDir.length === 0) {
      errors.push(
        'runtime.localConfigDir is required and must be a non-empty string (e.g. ".claude"); ' +
          "got: " +
          JSON.stringify(r.localConfigDir),
      );
    } else if (!r.localConfigDir.startsWith(".")) {
      errors.push(
        'runtime.localConfigDir must start with "." (a dot-dir); got: ' +
          JSON.stringify(r.localConfigDir),
      );
    }
  ...
  if (!Array.isArray(r.extendedHookEvents)) {
    errors.push(
      "runtime.extendedHookEvents must be an array (got: " +
        JSON.stringify(r.extendedHookEvents) +
        ")",
    );
  } else {
    for (let i = 0; i < r.extendedHookEvents.length; i++) {
      ...
      if (typeof ev !== "string" || !VALID_EXTENDED_HOOK_EVENTS.has(ev)) {
        errors.push(
          "runtime.extendedHookEvents[" +
            i +
            "] must be one of: " +
            [...VALID_EXTENDED_HOOK_EVENTS].join(", ") +
            " (got: " +
            JSON.stringify(ev) +
            ")",
        );
  ...
  if (
    typeof r.hostIntegration !== "object" ||
    r.hostIntegration === null ||
    Array.isArray(r.hostIntegration)
  ) {
    ...
    // embeddingMode
    if (
      hi.embeddingMode === "__proto__" ||
      hi.embeddingMode === "constructor" ||
      hi.embeddingMode === "prototype"
    ) {
      errors.push(
        'runtime.hostIntegration.embeddingMode "' + hi.embeddingMode + '" is a reserved name',
      );
    } else if (
      hi.embeddingMode !== "undocumented" &&
      !VALID_EMBEDDING_MODES.has(hi.embeddingMode)
    ) {
      errors.push(
        "runtime.hostIntegration.embeddingMode must be one of: " +
          [...VALID_EMBEDDING_MODES].join(", ") +
          ' (or "undocumented") (got: ' +
          JSON.stringify(hi.embeddingMode) +
          ")",
      );
    }
    ...
    // commandSurface
    if (
      hi.commandSurface === "__proto__" ||
      hi.commandSurface === "constructor" ||
      hi.commandSurface === "prototype"
    ) {
      errors.push(
        'runtime.hostIntegration.commandSurface "' + hi.commandSurface + '" is a reserved name',
      );
    } else if (
      hi.commandSurface !== "undocumented" &&
      !VALID_COMMAND_SURFACES.has(hi.commandSurface)
    ) {
      errors.push(
        "runtime.hostIntegration.commandSurface must be one of: " +
          [...VALID_COMMAND_SURFACES].join(", ") +
          ' (or "undocumented") (got: ' +
          JSON.stringify(hi.commandSurface) +
          ")",
      );
    }
    ...
    // modelMode
    if (
      hi.modelMode === "__proto__" ||
      hi.modelMode === "constructor" ||
      hi.modelMode === "prototype"
    ) {
      ...
      errors.push(
        "runtime.hostIntegration.modelMode must be one of: " +
          [...VALID_MODEL_MODES].join(", ") +
          ' (or "undocumented") (got: ' +
          JSON.stringify(hi.modelMode) +
          ")",
      );
    }
    ...
    if (hi.hookBus === "__proto__" || hi.hookBus === "constructor" || hi.hookBus === "prototype") {
      ...
      errors.push(
        "runtime.hostIntegration.hookBus must be one of: " +
          [...VALID_HOOK_BUSES].join(", ") +
          ' (or "undocumented") (got: ' +
          JSON.stringify(hi.hookBus) +
          ")",
      );
    }
    ...
    if (hi.stateIO === "__proto__" || hi.stateIO === "constructor" || hi.stateIO === "prototype") {
      ...
      errors.push(
        "runtime.hostIntegration.stateIO must be one of: " +
          [...VALID_STATE_IO].join(", ") +
          ' (or "undocumented") (got: ' +
          JSON.stringify(hi.stateIO) +
          ")",
      );
    }
    ...
    // transport
    if (
      hi.transport === "__proto__" ||
      hi.transport === "constructor" ||
      hi.transport === "prototype"
    ) {
      ...
      errors.push(
        "runtime.hostIntegration.transport must be one of: " +
          [...VALID_TRANSPORTS].join(", ") +
          ' (or "undocumented") (got: ' +
          JSON.stringify(hi.transport) +
          ")",
      );
    }
    ...
    if (hi.runtime === "__proto__" || hi.runtime === "constructor" || hi.runtime === "prototype") {
      ...
      errors.push(
        "runtime.hostIntegration.runtime must be one of: " +
          [...VALID_HOST_RUNTIMES].join(", ") +
          ' (or "undocumented") (got: ' +
          JSON.stringify(hi.runtime) +
          ")",
      );
    }
    ...
    if (hi.effortSurface === undefined) {
      ...
      hi.effortSurface === "__proto__" ||
      hi.effortSurface === "constructor" ||
      hi.effortSurface === "prototype"
    ) {
      errors.push(
        'runtime.hostIntegration.effortSurface "' + hi.effortSurface + '" is a reserved name',
      );
    } else if (
      hi.effortSurface !== "undocumented" &&
      !VALID_EFFORT_SURFACES.has(hi.effortSurface)
    ) {
      errors.push(
        "runtime.hostIntegration.effortSurface must be one of: " +
          [...VALID_EFFORT_SURFACES].join(", ") +
          ' (or "undocumented") (got: ' +
          JSON.stringify(hi.effortSurface) +
          ")",
      );
    }
    ...
    if (typeof hi.dispatch !== "object" || hi.dispatch === null || Array.isArray(hi.dispatch)) {
      ...
      if (typeof d.namedDispatch !== "boolean" && d.namedDispatch !== "undocumented") {
        errors.push(
          'runtime.hostIntegration.dispatch.namedDispatch must be a boolean or "undocumented" (got: ' +
            JSON.stringify(d.namedDispatch) +
            ")",
        );
      }
      ...
      if (typeof d.nested !== "boolean" && d.nested !== "undocumented") {
        errors.push(
          'runtime.hostIntegration.dispatch.nested must be a boolean or "undocumented" (got: ' +
      ...
      if (typeof d.background !== "boolean" && d.background !== "undocumented") {
        errors.push(
          'runtime.hostIntegration.dispatch.background must be a boolean or "undocumented" (got: ' +
            JSON.stringify(d.background) +
            ")",
        );
      }
      ...
      if (
        d.subagentToolkit === "__proto__" ||
        d.subagentToolkit === "constructor" ||
        d.subagentToolkit === "prototype"
      ) {
        errors.push(
          'runtime.hostIntegration.dispatch.subagentToolkit "' +
            d.subagentToolkit +
            '" is a reserved name',
        );
      } else if (
        d.subagentToolkit !== "undocumented" &&
        !VALID_SUBAGENT_TOOLKITS.has(d.subagentToolkit)
      ) {
        errors.push(
          "runtime.hostIntegration.dispatch.subagentToolkit must be one of: " +
            [...VALID_SUBAGENT_TOOLKITS].join(", ") +
            ' (or "undocumented") (got: ' +
            JSON.stringify(d.subagentToolkit) +
            ")",
        );
      }
      ...
      if (d.maxDepth !== "undocumented" && (!Number.isInteger(d.maxDepth) || d.maxDepth < -1)) {
        errors.push(
          'runtime.hostIntegration.dispatch.maxDepth must be an integer >= -1 or "undocumented" (got: ' +
            JSON.stringify(d.maxDepth) +
            ")",
        );
      }
      ...
      if (!Object.prototype.hasOwnProperty.call(d, "backgroundDispatch")) {
        ...
        typeof d.backgroundDispatch !== "boolean" &&
        d.backgroundDispatch !== "undocumented"
      ) {
        errors.push(
          'runtime.hostIntegration.dispatch.backgroundDispatch must be a boolean or "undocumented" (got: ' +
            JSON.stringify(d.backgroundDispatch) +
            ")",
        );
      }
      ...
      if (d.isolation === undefined) {
        ...
        d.isolation === "__proto__" ||
        d.isolation === "constructor" ||
        d.isolation === "prototype"
      ) {
        errors.push(
          'runtime.hostIntegration.dispatch.isolation "' + d.isolation + '" is a reserved name',
        );
      } else if (d.isolation !== "undocumented" && !VALID_DISPATCH_ISOLATION.has(d.isolation)) {
        errors.push(
          "runtime.hostIntegration.dispatch.isolation must be one of: " +
            [...VALID_DISPATCH_ISOLATION].join(", ") +
            ' (or "undocumented") (got: ' +
            JSON.stringify(d.isolation) +
            ")",
        );
      }
      ...
      if (d.maxConcurrency === undefined) {
        ...
        errors.push(
          'runtime.hostIntegration.dispatch.maxConcurrency must be a positive safe integer or "undocumented" ' +
            "(got: " +
            JSON.stringify(d.maxConcurrency) +
            ")",
        );
      }
    }
  ...
  if (r.orchestratorExec !== undefined) {
    if (
      typeof r.orchestratorExec !== "object" ||
      r.orchestratorExec === null ||
      Array.isArray(r.orchestratorExec)
    ) {
      errors.push(
      ...
      if (
        oe.command === "__proto__" ||
        oe.command === "constructor" ||
        oe.command === "prototype"
      ) {
        ...
        errors.push(
          "runtime.orchestratorExec.command must be a non-empty string (got: " +
            JSON.stringify(oe.command) +
            ")",
        );
      }
      ...
      if (oe.args !== undefined) {
        if (!Array.isArray(oe.args) || !oe.args.every((a) => typeof a === "string")) {
          errors.push(
            "runtime.orchestratorExec.args must be an array of strings (got: " +
              JSON.stringify(oe.args) +
              ")",
          );
        }
      }
      ...
      if (oe.cwdFlag !== undefined && oe.cwdFlag !== null && typeof oe.cwdFlag !== "string") {
        errors.push(
          "runtime.orchestratorExec.cwdFlag must be a string or null (got: " +
            JSON.stringify(oe.cwdFlag) +
            ")",
        );
      }
      ...
      if (
        oe.promptFlag !== undefined &&
        oe.promptFlag !== null &&
        typeof oe.promptFlag !== "string"
      ) {
        errors.push(
          "runtime.orchestratorExec.promptFlag must be a string or null (got: " +
            JSON.stringify(oe.promptFlag) +
            ")",
        );
      }
      ...
      if (oe.modelFlag !== undefined && oe.modelFlag !== null && typeof oe.modelFlag !== "string") {
        errors.push(
          "runtime.orchestratorExec.modelFlag must be a string or null (got: " +
            JSON.stringify(oe.modelFlag) +
            ")",
        );
      }
    }
  ...
  if (
    r.harnessIsolationFlag !== undefined &&
    (typeof r.harnessIsolationFlag !== "string" || r.harnessIsolationFlag.length === 0)
  ) {
    errors.push(
      "runtime.harnessIsolationFlag must be a non-empty string (got: " +
        JSON.stringify(r.harnessIsolationFlag) +
        ")",
    );
  }
    ...
    if (allowedHooksSurfaces !== undefined && !allowedHooksSurfaces.has(r.hooksSurface)) {
      errors.push(
        'runtime.hooksSurface "' +
          r.hooksSurface +
          '" is not valid for installSurface "' +
          r.installSurface +
          '"' +
          " — allowed: " +
          [...allowedHooksSurfaces].join(", ") +
  ...
  if (!Array.isArray(triggerPrecedenceValue)) {
    errors.push(
      "runtime.triggerPrecedence must be an array of trigger-bearing kind names (" +
        [...VALID_TRIGGER_PRECEDENCE_KINDS].join(", ") +
        ") (got: " +
        JSON.stringify(triggerPrecedenceValue) +
        ")",
    );
...
const KNOWN_PROBE_FIELDS = new Set([
  "kind",
  "binary",
  "needle",
  "timeoutMs",
      ...
      if (omitted > 0) {
        records.push({
          ...
          message:
            ...
            '" declares ' +
            omitted +
            " further unknown runtime.hostBehaviors " +
  ...
  for (const key of Object.keys(r)) {
    ...
    records.push({
      ...
      message:
        ...
        '" reviewer.' +
        safeKey +
        " is not a known reviewer field " +
        "in this GSD version — ignored. Known fields: " +
        [...KNOWN_REVIEWER_FIELDS].join(", "),
    });
  }
  if (omittedFields > 0) {
    records.push({
      ...
      message:
        ...
        '" declares ' +
        omittedFields +
        " further unknown reviewer field(s), " +
        "not listed.",
    });
  }
  return records;
...
function validateReviewerBodyFields(cap) {
  ...
  if (typeof r !== "object" || r === null || Array.isArray(r)) {
    ...
    errors.push(
      ctx +
        " reviewer must be an object (got: " +
        got +
        "). " +
  ...
  if (!Array.isArray(r.flags)) {
    ...
    errors.push(
      ctx +
        " reviewer.flags must declare at least one flag — a lane nobody can name " +
  ...
  if (!isPositiveIntegerMs(r.timeoutFloorMs)) {
    errors.push(
      ctx +
        " reviewer.timeoutFloorMs must be a positive integer of milliseconds " +
        "(got: " +
        describeValue(r.timeoutFloorMs) +
        ")",
    );
  }
  ...
  errors.push(
    ...validateEnumField(ctx, "reviewer.evidenceClass", r.evidenceClass, VALID_EVIDENCE_CLASSES),
  );

  if (!Array.isArray(r.requiresBinaries)) {
    errors.push(
      ctx +
        " reviewer.requiresBinaries must be an array (use [] when the lane needs no " +
    ...
    for (const bin of r.requiresBinaries) {
      if (typeof bin !== "string" || bin.length === 0) {
        errors.push(
          ctx +
            " reviewer.requiresBinaries entry " +
            describeValue(bin) +
  ...
  if (
    r.modelConfigKey !== undefined &&
    r.modelConfigKey !== null &&
    (typeof r.modelConfigKey !== "string" || r.modelConfigKey.length === 0)
  ) {
    errors.push(
      ctx +
        " reviewer.modelConfigKey must be a dotted config key or null " +
        "(got: " +
        describeValue(r.modelConfigKey) +
        ")",
    );
  }
  ...
  if (
    r.timeoutConfigKey !== undefined &&
    r.timeoutConfigKey !== null &&
    (typeof r.timeoutConfigKey !== "string" || r.timeoutConfigKey.length === 0)
  ) {
    errors.push(
      ctx +
        " reviewer.timeoutConfigKey must be a dotted config key or null " +
        "(got: " +
        describeValue(r.timeoutConfigKey) +
        ")",
    );
  }
  ...
  if (
    r.effortConfigKey !== undefined &&
    r.effortConfigKey !== null &&
    (typeof r.effortConfigKey !== "string" || r.effortConfigKey.length === 0)
  ) {
    errors.push(
      ctx +
        " reviewer.effortConfigKey must be a dotted config key or null " +
        "(got: " +
        describeValue(r.effortConfigKey) +
        ")",
    );
  }
  ...
  if (
    ...
    r.defaultEffort !== null &&
    !(typeof r.defaultEffort === "string" && REVIEWER_EFFORT_LEVELS.has(r.defaultEffort))
  ) {
    errors.push(
      ctx +
        " reviewer.defaultEffort must be one of " +
        Array.from(REVIEWER_EFFORT_LEVELS).join("/") +
        " or null " +
        "(got: " +
        describeValue(r.defaultEffort) +
        ")",
    );
  }
  ...
  if (
    ...
    errors.push(
      ctx +
        " reviewer.defaultEffort is declared without an effortConfigKey, so the level " +
  ...
  if (
    r.promptBudgetKey !== null &&
    (typeof r.promptBudgetKey !== "string" || r.promptBudgetKey.length === 0)
  ) {
    errors.push(
      ctx +
        " reviewer.promptBudgetKey must be a dotted config key or null " +
        "(got: " +
        describeValue(r.promptBudgetKey) +
        ")",
    );
  }
  ...
  if (r.handler !== null && !VALID_LANE_HANDLERS.has(r.handler)) {
    errors.push(
      ctx +
        " reviewer.handler must be null or one of: " +
        [...VALID_LANE_HANDLERS].join(", ") +
        " (got: " +
        describeValue(r.handler) +
        "). Handlers are first-party module NAMES, " +
...
function validateLaneProbe(ctx, probe) {
  ...
  if (typeof probe !== "object" || probe === null || Array.isArray(probe)) {
    errors.push(
      ctx +
        ' reviewer.probe must be an object with a "kind" from: ' +
        [...VALID_LANE_PROBE_KINDS].join(", "),
    );
    return errors;
  }

  const kindErrors = validateEnumField(
    ctx,
    "reviewer.probe.kind",
    probe.kind,
  ...
  if (probe.kind === "command-exists" || probe.kind === "command-capability") {
    if (typeof probe.binary !== "string" || probe.binary.length === 0) {
      errors.push(
        ctx + ' reviewer.probe.binary must be a non-empty string for kind "' + probe.kind + '"',
      );
    }
  ...
  if (probe.kind === "command-capability") {
    if (typeof probe.needle !== "string" || probe.needle.length === 0) {
      errors.push(
        ctx + ' reviewer.probe.needle must be a non-empty string for kind "command-capability"',
      );
    }
  ...
  if (probe.kind === "http-reachable") {
    if (typeof probe.hostConfigKey !== "string" || probe.hostConfigKey.length === 0) {
      errors.push(
        ctx + ' reviewer.probe.hostConfigKey must be a non-empty string for kind "http-reachable"',
      );
    }
    if (typeof probe.path !== "string" || probe.path.length === 0) {
      errors.push(
        ctx + ' reviewer.probe.path must be a non-empty string for kind "http-reachable"',
      );
    }
  } else {
    if (probe.hostConfigKey !== undefined) {
      errors.push(
        ctx + ' reviewer.probe.hostConfigKey is not permitted for kind "' + probe.kind + '"',
      );
    }
  ...
  if (needsBound) {
    if (!isPositiveIntegerMs(probe.timeoutMs)) {
      errors.push(
        ctx +
          ' reviewer.probe.timeoutMs must be a positive integer of milliseconds for kind "' +
          probe.kind +
          '" — an unbounded probe hangs every /gsd-review invocation ' +
          "(got: " +
          describeValue(probe.timeoutMs) +
          ")",
      );
    }
    ...
    errors.push(
      ctx +
        ' reviewer.probe.timeoutMs is not permitted for kind "command-exists" — ' +
...
function validateLaneInvoke(ctx, transport, invoke) {
  ...
  if (hasSpawnField && hasHttpField) {
    errors.push(
      ctx +
        " reviewer.invoke mixes spawn-only fields (" +
        SPAWN_ONLY_INVOKE_FIELDS.join(", ") +
        ") with openai-http-only fields (" +
        HTTP_ONLY_INVOKE_FIELDS.join(", ") +
  ...
  if (transport === "spawn") {
    ...
    for (const f of SPAWN_ONLY_INVOKE_FIELDS) {
      if (invoke[f] !== undefined) {
        errors.push(
          ctx + " reviewer.invoke." + f + ' is not permitted for transport "openai-http"',
        );
      }
    }
...
function validateSpawnInvoke(ctx, invoke) {
  ...
  if (!Array.isArray(invoke.args)) {
    errors.push(
      ctx + " reviewer.invoke.args must be an array (use [] when the lane takes no arguments)",
    );
  } else {
  ...
  errors.push(
    ...validateEnumField(
      ctx,
      "reviewer.invoke.promptChannel",
      invoke.promptChannel,
      VALID_PROMPT_CHANNELS,
    ),
  );

  const outputChannelErrors = validateEnumField(
    ctx,
    "reviewer.invoke.outputChannel",
  ...
  if (outputChannelErrors.length > 0) {
    ...
    if (typeof invoke.outputArg !== "string" || invoke.outputArg.length === 0) {
      errors.push(
        ctx +
          ' reviewer.invoke.outputArg is required (non-empty string) when outputChannel is "file-arg"',
      );
    }
    ...
    errors.push(
      ctx +
        ' reviewer.invoke.outputArg is only permitted when outputChannel is "file-arg" ' +
        "(got outputChannel: " +
        describeValue(invoke.outputChannel) +
        ")",
    );
  }
  ...
  if (
    invoke.modelArg !== null &&
    (typeof invoke.modelArg !== "string" || invoke.modelArg.length === 0)
  ) {
    errors.push(
      ctx +
        " reviewer.invoke.modelArg must be a non-empty string or null " +
        "(got: " +
        describeValue(invoke.modelArg) +
        ")",
    );
  }
  ...
  if (invoke.env !== undefined) {
    if (typeof invoke.env !== "object" || invoke.env === null || Array.isArray(invoke.env)) {
      errors.push(
        ctx +
          " reviewer.invoke.env must be an object of environment name/value pairs " +
          "(got: " +
          describeValue(invoke.env) +
          ")",
      );
    } else {
      for (const key of Object.keys(invoke.env)) {
        ...
        if (DENIED_LANE_ENV_KEYS.has(key.toUpperCase())) {
          ...
          errors.push(
            ctx +
              ' reviewer.invoke.env key "__proto__" is not permitted ' +
...
function validateHttpInvoke(ctx, invoke) {
  ...
  if (typeof invoke.hostConfigKey !== "string" || invoke.hostConfigKey.length === 0) {
    errors.push(
      ctx +
        " reviewer.invoke.hostConfigKey must be a non-empty dotted config key " +
  ...
  if (typeof invoke.path !== "string" || invoke.path.length === 0) {
    errors.push(
      ctx +
        ' reviewer.invoke.path must be a non-empty string for transport "openai-http" (e.g. "/v1/chat/completions")',
    );
  }
  ...
  errors.push(
    ...validateEnumField(
      ctx,
      "reviewer.invoke.modelDiscovery",
  ...
  if (invoke.effortChannel !== "none") {
    errors.push(
      ctx +
        ' reviewer.invoke.effortChannel must be "none" for transport "openai-http" ' +
        "(got: " +
        describeValue(invoke.effortChannel) +
        ")",
    );
  }
      ...
      if (abs !== capRoot && !abs.startsWith(capRoot + path.sep)) {
        errors.push(
          cap.id +
            ...
            "[" +
            i +
            "].fragment.path escapes capability directory: " +
            fragment.path,
        );
        continue;
      }
        ...
        if (body === null) {
          ...
          errors.push(
            cap.id +
              ...
              "[" +
              i +
              "].fragment.path could not be read (missing, non-regular " +
              "(FIFO/device), or exceeds the size cap): " +
              fragment.path,
          );
          continue;
        }
...
function validateStep(step, prefix, declaredSkills, declaredAgents) {
  ...
  if (typeof step.ref !== "object" || step.ref === null) {
    ...
    if (dispatchCount === 0) {
      ...
      errors.push(
        prefix + '.ref must have exactly one of "skill", "agent", or "command", not multiple',
      );
    }
    if (hasSkill && typeof step.ref.skill !== "string") {
      ...
      typeof step.ref.skill === "string" &&
      declaredSkills !== null &&
      !declaredSkills.has(step.ref.skill)
    ) {
      ...
      errors.push(
        prefix +
          '.ref.skill "' +
          step.ref.skill +
          "\" is not declared in this capability's skills: [" +
          [...declaredSkills].join(", ") +
          "]",
      );
    }
    if (hasAgent && typeof step.ref.agent !== "string") {
      ...
      hasAgent &&
      typeof step.ref.agent === "string" &&
      declaredAgents !== null &&
      !declaredAgents.has(step.ref.agent)
    ) {
      ...
      errors.push(
        prefix +
          '.ref.agent "' +
          step.ref.agent +
          "\" is not declared in this capability's agents: [" +
          [...declaredAgents].join(", ") +
          "]",
      );
    }
...
function validateGate(gate, prefix) {
  ...
  if (typeof gate.check !== "object" || gate.check === null) {
    ...
    if (hasAgentVerdict && gate.blocking === true) {
      errors.push(
        prefix +
          ".check.agentVerdict forces blocking: false (non-deterministic checks may not halt the loop)",
      );
    }
  }
...
function validateAgainstContract(cap, capId) {
    ...
    if (contract && !contract.agentRoles.includes(contrib.into)) {
      errors.push(
        prefix +
          ...
          '" at point "' +
          contrib.point +
          "\" is not in the step's agentRoles [" +
          contract.agentRoles.join(", ") +
          "]",
      );
    }
  }
  ...
  for (const step of cap.steps) {
    if (step.pointFrom !== undefined) {
      ...
      const slice =
        typeof cap.config === "object" && cap.config !== null
          ? cap.config[step.pointFrom]
          : undefined;
      if (!slice || typeof slice !== "object") {
        errors.push(
          prefix +
            ' step.pointFrom "' +
            step.pointFrom +
            '" is not defined in capability config keys',
        );
      } else if (slice.type !== "enum") {
        errors.push(
          prefix +
            ' step.pointFrom "' +
            step.pointFrom +
            '" must reference an enum config key (got type: ' +
            slice.type +
            ")",
        );
        ...
        errors.push(
          prefix +
            ' step.pointFrom "' +
            step.pointFrom +
            '" enum values do not include this step\'s own point "' +
            step.point +
            '" — the step could never activate',
        );
      }
    }
  }
...
function validateConsumesGlobal(capMap) {
  ...
  for (const [artifact, postIdx] of Object.entries(HOST_ARTIFACT_EARLIEST_POINT_IDX)) {
    if (artifact === "__proto__" || artifact === "constructor" || artifact === "prototype")
      continue;
  ...
  for (const artifact of Object.keys(capHookProducers)) {
    ...
    for (const pointIdxStr of Object.keys(byPoint)) {
      ...
      if (distinctProducers.size >= 2) {
        ...
        throw new Error(
          'duplicate-producer invariant violated: artifact "' +
            artifact +
            '" is produced by ' +
            'two or more capability steps at the same Loop Extension Point "' +
            pointName +
            '" ' +
            "(capabilities: " +
            capIds +
...
function validateCrossCapability(capMap, centralKeys, centralPatterns = []) {
  ...
  for (const [capId, cap] of capMap) {
    ...
    if (Array.isArray(cap.commands)) {
      for (const cmd of cap.commands) {
        ...
        if (
          cmd.family === "__proto__" ||
          cmd.family === "constructor" ||
          cmd.family === "prototype"
        )
          continue;
      ...
      if (collidingPattern) {
        errors.push(
          'config key "' +
            ...
            '" is declared in capability "' +
            capId +
            '" AND is matched by central config-schema pattern /' +
            collidingPattern.source +
  ...
  for (const [capId, cap] of capMap) {
    ...
    for (const req of cap.requires) {
      if (!capMap.has(req)) {
        errors.push('capability "' + capId + '" requires "' + req + '" which does not exist');
      }
    }
  }
  ...
  for (const [capId, cap] of capMap) {
    if (
      cap.role !== "feature" ||
      typeof cap.runtimeCompat !== "object" ||
      cap.runtimeCompat === null
    )
      continue;
      ...
      for (const runtimeId of Object.keys(cap.runtimeCompat.notes)) {
        ...
        if (!runtimeIds.has(runtimeId)) {
          errors.push(
            'capability "' +
              capId +
              '" runtimeCompat.notes references unknown runtime "' +
              runtimeId +
              '"',
          );
        }
  ...
  for (const [capId, cap] of capMap) {
    ...
    for (const req of cap.requires) {
      ...
      if (reqRank > myRank) {
        errors.push(
          'tier-monotone violation: capability "' +
            capId +
            '" (tier: ' +
            cap.tier +
            ') requires "' +
...
function topoSortHookEntries(entries, hookKey, hookKind) {
  ...
  if (result.length < n) {
    ...
    throw new Error(
      "produces/consumes cycle detected in " +
        hookKind +
        ' at point "' +
    ...
    if (covered.size === 0) {
      errors.push(
        'capability "' +
          ...
          '].point "' +
          point +
          '" has `loop render-hooks ' +
          point +
          "` call site(s), but their dispatch text covers NO " +
    ...
    if (!covered.has(kind)) {
      errors.push(
        'capability "' +
          ...
          '" registers a ' +
          kind +
          " hook, but the host call site's dispatch text never " +
          'covers `kind == "' +
          kind +
          '"` (it covers: ' +
          [...covered].sort().join(", ") +
...
function classifyCrossErrors(crossErrors) {
  ...
  const collisionRe =
    /config key "([^"]+)" is declared in capability "([^"]+)" AND exists in the central config-schema/;
...
function runConfigFormatParityGate(capMap) {
  ...
  for (const [capId, cap] of capMap) {
    ...
    if (expectedConfigFormat === undefined) {
      ...
      throw new Error(
        'configFormat parity gate: runtime "' +
          capId +
          '" has installSurface "' +
          installSurface +
    ...
    if (r.configFormat !== expectedConfigFormat) {
      throw new Error(
        'configFormat parity gate FAILED for runtime "' +
          capId +
          '":\n' +
          "  installSurface:       " +
          installSurface +
