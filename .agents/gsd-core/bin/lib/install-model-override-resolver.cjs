...
const { resolveTierEntry: gsdResolveTierEntry, resolveModelPolicy: gsdResolveModelPolicy } =
  modelResolverModule;
/**
...
function readGsdEffectiveAgentTools(targetDir = null, options = {}) {
  const home = options.homedir ? options.homedir() : node_os_1.default.homedir();
  const globalConfig = _readGsdConfigFile(
    node_path_1.default.join(home, ".gsd", "defaults.json"),
    "global defaults",
  );
...
function readGsdRuntimeProfileResolver(targetDir = null) {
  const homeDefaults = _readGsdConfigFile(
    node_path_1.default.join(node_os_1.default.homedir(), ".gsd", "defaults.json"),
    "~/.gsd/defaults.json",
  );
  ...
  const merged = {
    runtime:
      (projectConfig && projectConfig.runtime) || (homeDefaults && homeDefaults.runtime) || null,
    model_profile:
  ...
  return {
    ...
    resolve(agentName) {
      ...
      const policyForRuntime = merged.model_policy ? { ...merged.model_policy, runtime } : null;
