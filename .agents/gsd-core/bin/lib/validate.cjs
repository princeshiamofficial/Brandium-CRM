...
exports.BRACKET_PHASE_DIR_RE =
  exports.MILESTONE_ARCHIVE_DIR_RE =
  exports.PHASE_TOKEN_FROM_DIR_RE =
  exports.phaseDirNameRe =
    void 0;
...
exports.phaseDirNameRe = new RegExp(
...
exports.PHASE_TOKEN_FROM_DIR_RE = new RegExp(
  `^(${CASE_FLEXIBLE_PROJECT_CODE_PREFIX_SOURCE}` +
...
exports.BRACKET_PHASE_DIR_RE = new RegExp(
...
function isPhaseDirName(dirName, convention) {
  ...
  if (convention === "bracket" && exports.BRACKET_PHASE_DIR_RE.test(name)) return true;
...
function phaseVariants(phase) {
  ...
  if (milestoneHeadMatch) {
    ...
    const paddedSubs = subSegs
      .slice(1)
      .split("-")
      .map((s) => s.padStart(2, "0"))
      .join("-");
    const unpaddedSubs = subSegs
      .slice(1)
      .split("-")
...
function buildRoadmapPhaseVariants(roadmapContent, convention) {
  ...
  const phasePattern = new RegExp(
    `#{2,4}\\s*${phaseHeadingPrefixSrcFor(PHASE_HEADING_BASELINE.ANY_BRACKET, convention, capturing)}([\\w][\\w.-]*)(?:\\s*\\([^)\\n]{0,200}\\))?\\s*:`,
    "gi",
  );
  let m;
  ...
  const checklistPattern = new RegExp(
    `-\\s*\\[[ xX]\\]\\s*\\*{0,2}${phaseHeadingPrefixSrcFor(PHASE_HEADING_BASELINE.LABEL_ONLY, convention, capturing)}([\\w][\\w.-]*)\\s*:`,
    "gi",
  );
  let cm;
...
function buildNotStartedPhaseVariants(roadmapContent, convention) {
  ...
  const uncheckedPattern = new RegExp(
    `-\\s*\\[\\s\\]\\s*\\*{0,2}${phaseHeadingPrefixSrcFor(PHASE_HEADING_BASELINE.LABEL_ONLY, convention)}([\\w][\\w.-]*)[:\\s*]`,
    "gi",
  );
  let um;
...
function checkBracketCoherence(roadmapContent) {
  ...
  const bracketPhaseRe = new RegExp(
    `^\\[(${BRACKET_ID_SRC})\\][ \\t]*(?:Phase\\s+)?(${PHASE_NUMBER_TOKEN_SOURCE})\\s*:`,
    "i",
  );
  ...
  const legacyPhaseRe = new RegExp(
    `^Phase\\s+(${PHASE_NUMBER_TOKEN_SOURCE}|\\d+(?:-\\d+)+[A-Z]?(?:\\.\\d+)*)\\s*:`,
    "i",
  );
