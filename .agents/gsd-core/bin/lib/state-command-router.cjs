...
function routeStateCommand({ state, args, cwd, raw, error }) {
  ...
  routeHubCommandFamily({
    ...
    subcommands: [
      "load",
      "complete-phase",
      ...command_aliases_cjs_1.STATE_SUBCOMMANDS.filter((s) => s !== "load"),
    ],
    ...
    unknownMessage: (subcommand, available) =>
      `Unknown state subcommand: "${subcommand}". Available: ${available.join(", ")}`,
    handlers: {
      ...
      "record-metric": () => {
        const a = (0, command_arg_projection_cjs_1.parseNamedArgsOrExit)(
          args,
          { valueFlags: ["phase", "plan", "duration", "tasks", "files"], positionals: 2 },
          error,
        );
      ...
      "add-decision": () => {
        const a = (0, command_arg_projection_cjs_1.parseNamedArgsOrExit)(
          args,
          {
            valueFlags: ["phase", "summary", "summary-file", "rationale", "rationale-file"],
            positionals: 2,
          },
          error,
        );
      ...
      "add-blocker": () => {
        const a = (0, command_arg_projection_cjs_1.parseNamedArgsOrExit)(
          args,
          { valueFlags: ["text", "text-file"], positionals: 2 },
          error,
        );
        state.cmdStateAddBlocker(
          cwd,
          { text: strArg(a, "text"), text_file: strArg(a, "text-file") },
          raw,
        );
      },
      "add-roadmap-evolution": () => {
        const a = (0, command_arg_projection_cjs_1.parseNamedArgsOrExit)(
          args,
          {
            valueFlags: ["phase", "action", "after", "note", "note-file"],
            booleanFlags: ["urgent"],
            positionals: 2,
          },
          error,
        );
        state.cmdStateAddRoadmapEvolution(
          cwd,
          {
            phase: strArg(a, "phase"),
      ...
      "record-session": () => {
        const a = (0, command_arg_projection_cjs_1.parseNamedArgsOrExit)(
          args,
          { valueFlags: ["stopped-at", "resume-file"], positionals: 2 },
          error,
        );
        ...
        state.cmdStateRecordSession(
          cwd,
          { stopped_at: strArg(a, "stopped-at"), resume_file: strArg(a, "resume-file") },
          raw,
        );
      },
      "begin-phase": () => {
      ...
      "signal-waiting": () => {
        const a = (0, command_arg_projection_cjs_1.parseNamedArgsOrExit)(
          args,
          { valueFlags: ["type", "question", "options", "phase"], positionals: 2 },
          error,
        );
      ...
      validate: () => {
        ...
        const a = (0, command_arg_projection_cjs_1.parseNamedArgsOrExit)(
          args,
          { booleanFlags: ["strict"], positionals: 2 },
          error,
        );
      ...
      sync: () => {
        const a = (0, command_arg_projection_cjs_1.parseNamedArgsOrExit)(
          args,
          { booleanFlags: ["verify"], positionals: 2 },
          error,
        );
      ...
      prune: () => {
        const a = (0, command_arg_projection_cjs_1.parseNamedArgsOrExit)(
          args,
          { valueFlags: ["keep-recent"], booleanFlags: ["dry-run"], positionals: 2 },
          error,
        );
        state.cmdStatePrune(
          cwd,
          { keepRecent: strArg(a, "keep-recent") || "3", dryRun: a["dry-run"] === true },
          raw,
        );
      },
      rebuild: () => {
        const a = (0, command_arg_projection_cjs_1.parseNamedArgsOrExit)(
          args,
          { booleanFlags: ["dry-run", "verbose"], positionals: 2 },
          error,
        );
        state.cmdStateRebuild(
          cwd,
          { dryRun: a["dry-run"] === true, verbose: a["verbose"] === true },
          raw,
        );
      },
      ...
      "milestone-switch": () => {
        const a = (0, command_arg_projection_cjs_1.parseNamedArgsOrExit)(
          args,
          { valueFlags: ["milestone", "name"], positionals: 2 },
          error,
        );
