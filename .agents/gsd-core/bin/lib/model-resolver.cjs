...
const { MODEL_PROFILES, AGENT_TO_PHASE_TYPE, AGENT_DEFAULT_TIERS, VALID_AGENT_TIERS, nextTier } =
  modelProfiles;
...
function resolveActiveRuntime(config) {
  return (
    (0, runtime_name_policy_cjs_1.resolveRuntimeNameFromCandidates)(
      ...
      config["runtime"],
      (0, runtime_slash_cjs_1.readInstallRuntimeMarker)(),
    ) || "claude"
  );
}
  ...
  for (const dir of layers) {
    try {
      const parsed = JSON.parse(
        node_fs_1.default.readFileSync(node_path_1.default.join(dir, "config.json"), "utf8"),
      );
...
const CLAUDE_POLICY_ID_TO_ALIAS = {
  ...Object.fromEntries(
    Object.entries(model_catalog_cjs_1.MODEL_ALIAS_MAP)
...
function mapClaudeOverrideForRuntime(override, configRuntime, agentType) {
  ...
  if (model_catalog_cjs_1.CLAUDE_AGENT_ALIASES.has(override)) return override;
...
function resolveModelPolicy(policy, tier) {
  ...
  const budget =
    policy["budget"] && typeof policy["budget"] === "string" ? policy["budget"] : "medium";
...
function computeProfileTier(config, agentType) {
  ...
  const agentModels = Object.hasOwn(modelProfilesMap, agentType)
    ? modelProfilesMap[agentType]
    : undefined;
  const phaseType = AGENT_TO_PHASE_TYPE[agentType];
  const configModels = config["models"];
  const phaseTypeTier =
    phaseType && configModels && typeof configModels === "object"
    ...
    : profile === "inherit"
      ...
      : agentModels
        ? // Own-property guard: `profile` is a config-supplied string
          ...
          (Object.hasOwn(agentModels, profile) ? agentModels[profile] : undefined) ||
...
function resolveTierFromConfig(config, agentType) {
  ...
  const override =
    modelOverrides && Object.hasOwn(modelOverrides, agentType)
  ...
  if (profileTier && profileTier !== "inherit") {
    ...
    if (policyModel) {
      ...
      if (model_catalog_cjs_1.CLAUDE_AGENT_ALIASES.has(policyModel)) return policyModel;
      return "unknown";
    }
  }
...
function resolveModelInternal(cwd, agentType) {
  ...
  const agentModels = Object.hasOwn(modelProfilesMapForModel, agentType)
    ? modelProfilesMapForModel[agentType]
    : undefined;
  ...
  if (
    config["resolve_model_ids"] === "omit" &&
    (projectExplicitlySetsOmit(cwd) ||
      !RUNTIMES_WITH_NATIVE_ALIASES.has(resolveActiveRuntime(config)))
  ) {
    return "";
  }
...
function resolveGranularityInternal(cwd, phaseType, override) {
  ...
  const perPhase =
    phaseType && configGranularities && typeof configGranularities === "object"
...
function assertValidGranularityOverride(override, fail) {
  if (
    ...
    override !== null &&
    override !== "" &&
    !VALID_GRANULARITIES.has(override)
  ) {
...
function resolveModelForTier(cwd, agentType, attempt) {
  ...
  const maxEscalations =
    Number.isInteger(dr["max_escalations"]) && dr["max_escalations"] >= 0
      ? dr["max_escalations"]
      : 1;
...
function resolveEffortInternal(cwd, agentType, opts) {
  ...
  const config = loadConfig(cwd);
  const effortCfg =
    config["effort"] && typeof config["effort"] === "object" && !Array.isArray(config["effort"])
      ? config["effort"]
      : null;
  ...
  if (effortCfg) {
    ...
    const mao =
      canonicalEffort && typeof canonicalEffort === "object"
  ...
  if (agentTier) {
    ...
    const merged = (0, model_catalog_cjs_1.mergeEffortTierDefaults)(
      manifestDefaults,
...
function resolveFastModeInternal(cwd, agentType, opts) {
  ...
  const fmCfg =
    config["fast_mode"] &&
    typeof config["fast_mode"] === "object" &&
    !Array.isArray(config["fast_mode"])
      ? config["fast_mode"]
      : null;
  // Step 2: agent_overrides
  if (fmCfg) {
  ...
  if (agentTier) {
    if (
      ...
      const manifestDefaults =
        canonicalFm && typeof canonicalFm === "object"
...
module.exports = {
  ...
  _resetInstallRuntimeMarkerCacheForTests:
    runtime_slash_cjs_1._resetInstallRuntimeMarkerCacheForTests,
  VALID_GRANULARITIES,
