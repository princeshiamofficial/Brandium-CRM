...
function transliterateForSlug(text) {
  ...
  for (const ch of lowered) {
    out += CYRILLIC_TRANSLITERATION_KEYS.includes(ch) ? CYRILLIC_TRANSLITERATION[ch] : ch;
  }
  return out;
}
/**
...
function getPhaseFileStats(phaseDir) {
  ...
  return {
    ...
    hasVerification: scopedFiles.some(
...
function summaryCandidates(plan) {
  ...
  const candidates = [dir + base.replace(/PLAN/i, "SUMMARY") + ".md", dir + base + "-SUMMARY.md"];
