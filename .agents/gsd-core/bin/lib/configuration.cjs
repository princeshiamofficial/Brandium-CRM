...
exports.DYNAMIC_KEY_PATTERNS =
  exports.RUNTIME_STATE_KEYS =
  exports.VALID_CONFIG_KEYS =
...
function planningDir(cwd, workstream) {
  if (!workstream) return (0, node_path_1.join)(cwd, ".planning");
...
function hoistLegacyKey(result, normalizations, skipped, legacyKey, section, field) {
  ...
  result[section] =
    existing[field] === undefined ? { ...existing, [field]: value } : { ...existing };
...
function normalizeLegacyKeys(parsed) {
  ...
  if (
    Object.prototype.hasOwnProperty.call(result, "depth") &&
    !Object.prototype.hasOwnProperty.call(result, "granularity")
  ) {
...
function migrateOnDisk(cwd, workstream, configPathOverride) {
  ...
  const configPath =
    configPathOverride ?? (0, node_path_1.join)(planningDir(cwd, workstream), "config.json");
  let raw;
  try {
