      ...
      case "proportional-truncate": {
        ...
        if (total() > contentBudget) {
          const members = working.filter(
            (m) =>
              m.strategy.kind === "proportional-truncate" &&
              (m.group ?? m.id) === groupKey &&
              !m.isolate,
          );
          ...
          if (groupBudget > 0 && groupBudget < groupContentTotal) {
            ...
            for (const m of members) {
              const floorChars =
                m.strategy.kind === "proportional-truncate" ? m.strategy.floorChars : 0;
              const share =
                originalChars > 0
                  ? Math.floor((m.originalLength / originalChars) * charsBudget)
                  : 0;
