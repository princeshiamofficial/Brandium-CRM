...
const CASE_FLEXIBLE_PROJECT_CODE_PREFIX_SOURCE = OPTIONAL_PROJECT_CODE_PREFIX_SOURCE.replaceAll(
  "A-Z",
  "A-Za-z",
);
const CASE_FLEXIBLE_PHASE_NUMBER_TOKEN_SOURCE = PHASE_NUMBER_TOKEN_SOURCE.replaceAll(
  "A-Z",
  "A-Za-z",
);
...
const BRACKET_ID_PREFIX_RE = new RegExp(
...
const BRACKET_QUALIFIED_KEY_RE = new RegExp(
  `^(${BRACKET_PROJECT_CODE_SRC})\\.(${BRACKET_MILESTONE_NUMERIC_SRC})-(\\d+(?:\\.\\d+)?)(?=-|$)`,
  "i",
);
...
function phaseHeadingPrefixSrcFor(baseline, convention, capturing = false) {
  const base =
    baseline === PHASE_HEADING_BASELINE.ANY_BRACKET
...
function normalizePhaseName(phase) {
  ...
  if (milestoneMatch) {
    ...
    const subSegments = milestoneMatch[2]
      .slice(1)
      .split("-")
...
function getPhaseDirFromPhaseId(phaseId, phaseName, projectCode) {
  ...
  const slug = phaseName
    ? (require("./core-utils.cjs").generateSlugInternal(phaseName, null) ?? "")
    : "";
...
function phaseMarkdownRegexSource(phaseNum) {
  ...
  if (!match) return (0, pattern_cjs_1.escapeRegex)(String(phaseNum));
...
function isPhaseArtifact(fileName, phaseDirName) {
  ...
  const strippedToken =
    strippedDir !== phaseDirName ? extractPhaseToken(strippedDir) : literalToken;
  const leadingRunMatch = strippedDir.match(LEADING_DIGIT_RUN_RE);
  const rawCandidates = [literalToken, strippedToken, leadingRunMatch?.[1]].filter((t) =>
    Boolean(t),
  );
  ...
  const candidates = new Set(
    rawCandidates.flatMap((t) => [t, normalizePhaseName(t), depad(t)]).map((t) => t.toUpperCase()),
  );
...
function parsePhaseFromProse(value) {
  ...
  const name = rawName && !STATUS_WORD_RE.test(rawName.trim()) ? rawName.trim() : null;
  return {
