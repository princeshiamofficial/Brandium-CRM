...
function determinePhaseStatus(plans, summaries, phaseDir, defaultPending) {
  ...
  try {
    ...
    if (verificationFile) {
      ...
      const content =
        (0, shell_command_projection_cjs_1.platformReadSync)(verificationFilePath) || "";
...
function cmdListTodos(cwd, area, raw) {
  ...
  try {
    ...
    for (const file of files) {
      const content = (0, shell_command_projection_cjs_1.platformReadSync)(
        node_path_1.default.join(pendingDir, file),
      );
      ...
      todos.push({
        ...
        area: todoArea,
        path: toPosixPath(
          node_path_1.default.relative(cwd, node_path_1.default.join(pendingDir, file)),
        ),
...
function cmdListSeeds(cwd, statusFilter, raw) {
  ...
  for (const entry of files) {
    ...
    let safeFilePath;
    try {
      safeFilePath = (0, security_cjs_1.requireSafePath)(
        node_path_1.default.join(seedsDir, entry.name),
        planDir,
        "seed file",
        { allowAbsolute: true },
      );
...
function cmdVerifyPathExists(cwd, targetPath, raw) {
  ...
  const fullPath = node_path_1.default.isAbsolute(targetPath)
    ? targetPath
    : node_path_1.default.join(cwd, targetPath);
  try {
...
function cmdHistoryDigest(cwd, raw) {
  ...
  if (node_fs_1.default.existsSync(phasesDir)) {
    try {
      const currentDirs = node_fs_1.default
      ...
      for (const dir of currentDirs) {
        allPhaseDirs.push({
          name: dir,
          fullPath: node_path_1.default.join(phasesDir, dir),
          milestone: null,
        });
      }
    } catch {
...
function cmdResolveModel(cwd, agentType, raw) {
  ...
  const agentModels = Object.hasOwn(agentModelsMap, agentType)
    ? agentModelsMap[agentType]
    : undefined;
...
function cmdResolveExecution(cwd, agentType, raw, opts) {
  ...
  let model =
    opts.attempt !== undefined && opts.attempt !== null
  ...
  if (runtime === "claude") {
    try {
      ...
      if (
        !node_path_1.default
          .resolve(agentPath)
          .startsWith(node_path_1.default.resolve(agentsDirEff) + node_path_1.default.sep)
      ) {
      ...
      /* no frontmatter evidence — stay on the resolved value */
    }
  }
  // Own-property guard: agentType is an unvalidated CLI positional, so a
  // prototype-chain value ("toString", "constructor") would otherwise return
...
function setFrontmatterKeyLine(content, key, value) {
  ...
  const renderedValue = agentScalarNeedsDoubleQuoting(value)
    ? `"${escapeDoubleQuotedScalar(value)}"`
    : value;
  ...
  return (
    content.slice(0, closingStart) + `${key}: ${renderedValue}${eol}` + content.slice(closingStart)
  );
}
/**
...
function cmdEffortSync(cwd, raw, opts) {
    ...
    output(
      {
        ...
        changes: [],
        dry_run: dryRun,
        reason: `runtime '${runtime}' does not use effort: frontmatter`,
      },
      raw,
      "",
    );
  ...
  } = require("./install-effort-resolver.cjs");
  const effortCfg = readGsdEffectiveEffortConfig(cwd);
  const agentsDir = node_path_1.default.join(
    opts.configDir || getGlobalConfigDir(runtime),
    "agents",
  );
    ...
    if (universalEffort === "inherit") {
      ...
      if (!dryRun) {
        ...
        try {
          ...
          /* non-fatal: fall back to writing without an explicit mode */
        }
        try {
          ...
          try {
            ...
            /* non-fatal: proceed with default tmp-file mode */
          }
      ...
      changes.push({
        agent: agentName,
        from: effortMatchInherit ? effortMatchInherit[1] : "",
        to: null,
      });
      synced++;
      continue;
...
function cmdEffortSyncCodex(raw, dryRun, configDir) {
  ...
  for (const file of files) {
    ...
    let doc = parsed.doc;
    const stripModelNeeded =
      doc.model !== null && (0, model_catalog_cjs_1.isAnthropicFlavoredModel)(doc.model);
    ...
    const stripEffortNeeded =
      doc.reasoningEffort !== null && (stripModelNeeded || doc.model === null);
    ...
    if (!dryRun) {
      ...
      try {
        node_fs_1.default.writeFileSync(
          tmpPath,
          (0, codex_agent_toml_cjs_1.renderCodexAgentToml)(doc),
        ...
        // the content write is what matters, not the mode.
        try {
          if (originalMode !== undefined) node_fs_1.default.chmodSync(tmpPath, originalMode);
        } catch {
          /* non-fatal: proceed with default tmp-file mode */
        }
    ...
    if (!dryRun) {
      ...
      try {
        node_fs_1.default.writeFileSync(
          tmpPath,
          target === null
            ? removeFrontmatterKeyLine(content, "variant")
            : setFrontmatterKeyLine(content, "variant", target),
          originalMode !== undefined ? { mode: originalMode } : undefined,
        );
        // Not redundant with the `mode` option above: `mode` only applies
        // when the file is actually created (O_CREAT). A leftover tmp file
...
function cmdCommit(cwd, message, files, raw, amend, noVerify) {
  ...
  const commitDocsPolicy = resolveCommitDocsPolicy(config, detectPhaseNumberFromFiles(files), () =>
  ...
  if (branchingStrategy && branchingStrategy !== "none") {
    ...
    if (branchingStrategy === "phase") {
      ...
      const milestone =
        milestoneInfo.scope === SCOPE.COMPLETE || milestoneInfo.scope === SCOPE.TRUNCATED
          ? milestoneInfo.value
          : null;
    ...
    if (branchName) {
      const currentBranch = (0, shell_command_projection_cjs_1.execGit)(
      ...
      if (currentBranch.exitCode === 0 && currentBranch.stdout.trim() !== branchName) {
        ...
        const verify = (0, shell_command_projection_cjs_1.execGit)(
        ...
        if (verify.exitCode !== 0) {
          ...
          const create = (0, shell_command_projection_cjs_1.execGit)(
  ...
  const preStaged = new Set(
    (0, shell_command_projection_cjs_1.execGit)(["diff", "--cached", "--name-only"], { cwd })
      .stdout.split("\n")
      .map((s) => s.trim())
      .filter(Boolean),
  );
  for (const file of filesToStage) {
    ...
    if (!node_fs_1.default.existsSync(fullPath)) {
      ...
      const rmResult = (0, shell_command_projection_cjs_1.execGit)(
  ...
  const mergeHeadProbe = (0, shell_command_projection_cjs_1.execGit)(
  ...
  const cherryPickProbe = guardApplies
    ? (0, shell_command_projection_cjs_1.execGit)(
        ["rev-parse", "-q", "--verify", "CHERRY_PICK_HEAD"],
        { cwd },
      )
    : null;
  const partialCommitRefused =
    isMergeInProgress ||
    (0, shell_command_projection_cjs_1.isSpawnTimeout)(mergeHeadProbe) ||
    (cherryPickProbe !== null &&
      (cherryPickProbe.exitCode === 0 ||
        (0, shell_command_projection_cjs_1.isSpawnTimeout)(cherryPickProbe)));
  ...
  const assumeUnchangedWouldRecord = () => {
    const listed = (0, shell_command_projection_cjs_1.execGit)(
      ["ls-files", "-v", "--", ...stagedPaths],
      { cwd },
    );
    ...
    if (listed.exitCode === 0 && !listed.stdout.split("\n").some((line) => /^[a-z] /.test(line)))
      return false;
    const dryRun = (0, shell_command_projection_cjs_1.execGit)(
      [
        "commit",
        "--dry-run",
        "--porcelain",
  ...
  const canScope = explicitFiles && stagedPaths.length > 0 && !amend && !isMergeInProgress;
  ...
  const commitResult = (0, shell_command_projection_cjs_1.execGit)(commitArgs, {
    cwd,
    env: commitEnv,
  ...
  // Get short hash
  const hashResult = (0, shell_command_projection_cjs_1.execGit)(["rev-parse", "--short", "HEAD"], {
    cwd,
  });
...
function cmdCommitToSubrepo(cwd, message, files, raw) {
  ...
  for (const [repo, repoFiles] of Object.entries(grouped)) {
    ...
    const preStagedSub = new Set(
      (0, shell_command_projection_cjs_1.execGit)(["diff", "--cached", "--name-only"], {
        cwd: repoCwd,
      })
        .stdout.split("\n")
        .map((s) => s.trim())
    ...
    for (const file of repoFiles) {
      ...
      const addResult = (0, shell_command_projection_cjs_1.execGit)(["add", relativePath], {
        cwd: repoCwd,
      });
      if (addResult.exitCode === 0) {
    ...
    if (subStagingFailures.length > 0) {
      ...
      if (toUnstageSub.length > 0) {
        (0, shell_command_projection_cjs_1.execGit)(["reset", "-q", "--", ...toUnstageSub], {
          cwd: repoCwd,
        });
      }
    ...
    const isMergeInProgressSub =
      (0, shell_command_projection_cjs_1.execGit)(["rev-parse", "-q", "--verify", "MERGE_HEAD"], {
        cwd: repoCwd,
      }).exitCode === 0;
    ...
    // Get hash
    const hashResult = (0, shell_command_projection_cjs_1.execGit)(
...
function cmdPrSubrepo(cwd, repo, branch, commitMessage, raw) {
  ...
  const statusResult = (0, shell_command_projection_cjs_1.execGit)(
  ...
  const branchCheck = (0, shell_command_projection_cjs_1.execGit)(
  ...
  const prevBranchResult = (0, shell_command_projection_cjs_1.execGit)(
    ["rev-parse", "--abbrev-ref", "HEAD"],
    { cwd: repoCwd },
  );
  const prevBranchName = prevBranchResult.exitCode === 0 ? prevBranchResult.stdout.trim() : null;
  // 3. Create branch
  const checkoutResult = (0, shell_command_projection_cjs_1.execGit)(["checkout", "-b", branch], {
    cwd: repoCwd,
  });
  ...
  for (const file of filesToStage) {
    const addResult = (0, shell_command_projection_cjs_1.execGit)(["add", "--", file], {
      cwd: repoCwd,
    });
    if (addResult.exitCode !== 0) {
      rollback();
  ...
  const remoteResult = (0, shell_command_projection_cjs_1.execGit)(
  ...
  const pushResult = (0, shell_command_projection_cjs_1.execGit)(
    ["push", "--set-upstream", "origin", branch],
    { cwd: repoCwd, timeout: 60_000 },
  );
  if (pushResult.exitCode !== 0) {
    error(
      `Failed to push ${branch} in ${repo}: ${pushResult.stderr}\nBranch ${branch} was created locally — retry with: git -C ${repo} push --set-upstream origin ${branch}`,
    );
  }
  const result = {
    ok: true,
  ...
  try {
    ...
    for (const dir of dirs) {
      ...
      const status = determinePhaseStatus(
        plans,
        summaries,
        node_path_1.default.join(phasesDir, dir),
        "Pending",
      );
  ...
  if (phaseInfoDisk && phaseInfoDisk["found"]) {
    try {
      ...
      for (const pf of planFiles) {
        const planContent = (0, shell_command_projection_cjs_1.platformReadSync)(
          node_path_1.default.join(phaseDir, pf),
        );
...
function cmdTodoComplete(cwd, filename, options, raw) {
    ...
    output(
      {
        ...
        date: today,
        would_move: {
          source: node_path_1.default
            .relative(cwd, sourcePath)
            .split(node_path_1.default.sep)
            .join("/"),
          target: node_path_1.default
            .relative(cwd, node_path_1.default.join(completedDir, filename))
            .split(node_path_1.default.sep)
            .join("/"),
        },
  ...
  (0, shell_command_projection_cjs_1.platformWriteSync)(
    node_path_1.default.join(completedDir, filename),
    completedContent,
  );
  ...
  switch (type) {
    ...
    case "phase-dir": {
      ...
      output(
        {
          created: true,
          directory: toPosixPath(node_path_1.default.relative(cwd, dirPath)),
          path: dirPath,
        },
        raw,
        dirPath,
...
function cmdStats(cwd, format, raw) {
  ...
  try {
    ...
    const headingPattern =
      /#{2,4}\s*(?:\[[^\]]{1,200}\]\s*)?Phase\s+([A-Za-z]?\d+[A-Z]?(?:[.-]\d+)*)(?:\s*\([^)\n]{0,200}\))?\s*:\s*([^\n]+)/gi;
    let match;
  ...
  const percent =
    phaseScope === SCOPE.COMPLETE
      ? (0, phase_lifecycle_cjs_1.clampPercent)(completedPhases, phases.length)
      : null;
  // Requirements stats
  ...
  const commitCount = (0, shell_command_projection_cjs_1.execGit)(["rev-list", "--count", "HEAD"], {
    cwd,
  });
  if (commitCount.exitCode === 0) {
    gitCommits = parseInt(commitCount.stdout, 10) || 0;
  }
  const rootHash = (0, shell_command_projection_cjs_1.execGit)(
    ["rev-list", "--max-parents=0", "HEAD"],
    { cwd },
  );
  if (rootHash.exitCode === 0 && rootHash.stdout) {
    const firstCommit = rootHash.stdout.split("\n")[0].trim();
    const firstDate = (0, shell_command_projection_cjs_1.execGit)(
      ["show", "-s", "--format=%as", firstCommit],
      { cwd },
    );
...
function cmdCheckCommit(cwd, raw) {
  const config = loadConfig(cwd);
  const stagedResult = (0, shell_command_projection_cjs_1.execGit)(
  ...
  if (stagedResult.exitCode === 0) {
    ...
    if (planningFiles.length > 0) {
      ...
      output(
        {
          allowed: true,
          reason: policy.source === "phase" ? "phase_commit_docs_true" : "commit_docs_enabled",
        },
        raw,
        "allowed",
      );
...
function findRuntimeLauncherSnippet() {
  ...
  throw new Error(
    `commit-docs-guard: could not locate workflows/_runtime-launcher.snippet.sh from ${__dirname}`,
  );
}
/**
...
function buildCommitDocsGuardHookScript() {
  ...
  const preamble = (0, text_lines_cjs_1.normalizeEol)(
    node_fs_1.default.readFileSync(snippetPath, "utf8"),
  ).replace(/\n+$/, "");
  const lines = [
    "#!/usr/bin/env bash",
...
function resolveIndexLockPath(cwd) {
  const result = (0, shell_command_projection_cjs_1.execGit)(
    ["rev-parse", "--git-path", "index.lock"],
    { cwd },
  );
  if (result.exitCode !== 0) return node_path_1.default.join(cwd, ".git", "index.lock");
  const raw = result.stdout.trim();
  return raw
    ? node_path_1.default.isAbsolute(raw)
      ? raw
      : node_path_1.default.join(cwd, raw)
    : node_path_1.default.join(cwd, ".git", "index.lock");
}
...
function resolveCommitDocsGuardHooksDir(cwd) {
  const gitDirResult = (0, shell_command_projection_cjs_1.execGit)(["rev-parse", "--git-dir"], {
    cwd,
  });
  if (gitDirResult.exitCode !== 0) {
    return { ok: false, reason: "not_a_git_repo" };
  }
  const hooksPathResult = (0, shell_command_projection_cjs_1.execGit)(
  ...
  const hooksDir = node_path_1.default.isAbsolute(hooksDirRaw)
    ? hooksDirRaw
    : node_path_1.default.join(cwd, hooksDirRaw);
...
function cmdCommitDocsGuardEnable(cwd, raw) {
  ...
  if (!hooksDir.ok || !hooksDir.dir) {
    error(
      ...
      ERROR_REASON.COMMIT_DOCS_GUARD_NOT_A_REPO,
    );
    return;
  }
  ...
  const hooksPathConfig = (0, shell_command_projection_cjs_1.execGit)(
  ...
  if (hooksPathConfig.exitCode === 0 && hooksPathConfig.stdout.trim() !== "") {
    ...
    error(
      `core.hooksPath is set to "${configuredPath}"; a hook written to ${node_path_1.default.join(hooksDir.dir, "pre-commit")} ` +
