...
function cmdEvalScore(_cwd, args, raw) {
  ...
  if (
    ...
    process.stderr.write(
      "Usage: gsd-tools query eval.score --covered N --total N --infra a,b,c,d,e (each ok|partial|missing)\n",
    );
    process.exitCode = 1;
    return;
  }
  ...
  if (
    ...
    process.stderr.write(
      "Invalid eval.score domain: require integer counts with 0 <= covered <= total.\n",
    );
    process.exitCode = 1;
    return;
  }
  ...
  if (invalidInfra !== undefined) {
    process.stderr.write(
      `Invalid eval.score infra token: ${invalidInfra || "<empty>"}. Expected ok|partial|missing.\n`,
    );
    process.exitCode = 1;
    return;
  }
