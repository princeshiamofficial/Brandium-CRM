...
exports.CODEX_CONFIG_HOME_ENV =
  exports.CODEX_SESSION_ENV_SIGNALS =
  exports.CODEX_CONFIG_MARKER =
    void 0;
...
Object.defineProperty(exports, "CODEX_CONFIG_MARKER", {
  enumerable: true,
  get: function () {
    return update_context_cjs_1.CODEX_CONFIG_MARKER;
  },
});
...
function detectHostRuntime(deps) {
  try {
    ...
    if (typeof codexHome === "string" && codexHome.trim() !== "") {
      try {
        if (
          fileExists(node_path_1.default.join(codexHome, update_context_cjs_1.CODEX_CONFIG_MARKER))
        ) {
...
function resolveReportedRuntimeUnsafe(projectDir, deps) {
  const explicit = (0, runtime_slash_cjs_1.resolveExplicitRuntime)(
    projectDir,
