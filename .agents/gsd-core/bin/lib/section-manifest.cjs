 ...
 */
exports.WHEN_PREDICATES = Object.freeze(
  Object.assign(Object.create(null), {
    always: () => true,
    "flag:--wave": (facts) => hasFlag(facts, "--wave"),
    "state:gap-closure-phase": (facts) =>
...
for (const when of workflowFragments.WHEN_VOCABULARY) {
  if (!Object.hasOwn(exports.WHEN_PREDICATES, when)) {
    throw new Error(
      `section-manifest: WHEN_VOCABULARY entry "${when}" has no predicate in WHEN_PREDICATES`,
    );
  }
}
...
for (const predicateKey of Object.keys(exports.WHEN_PREDICATES)) {
  if (!VOCABULARY_SET.has(predicateKey)) {
    throw new Error(
      `section-manifest: WHEN_PREDICATES entry "${predicateKey}" has no matching WHEN_VOCABULARY entry`,
    );
  }
}
/**
  ...
  for (const section of sections) {
    if (!Object.hasOwn(exports.WHEN_PREDICATES, section.when)) {
      fail(
        ...
        `section "${section.id}" has unrecognized when= value "${section.when}"`,
      );
    }
