...
exports.UI_VALIDATORS =
  exports.UNCLASSIFIED_CATEGORY =
  exports.UI_TAXONOMY =
  exports.VALID_ELEMENT_KINDS =
  exports.UI_CUES =
    void 0;
...
exports.UI_CUES = {
  ...
  "list-collection":
    /\b(lists?|listing|tables?|grids?|collections?|rows?|items?|cards?|feed|results?)\b/i,
  ...
  "interactive-control":
    /\b(buttons?|toggles?|switch|switches|dropdowns?|sliders?|controls?|pickers?)\b/i,
...
function classifyElement(text) {
  ...
  for (const kind of Object.keys(exports.UI_CUES)) {
    if (exports.UI_CUES[kind].test(subject)) kinds.push(kind);
  }
  return kinds;
}
/**
...
exports.UI_TAXONOMY = [
  {
    id: "empty",
    name: "Empty / no data",
    elements: ["form", "list-collection", "media"],
    consideration:
      "What is shown when there is no data — zero items, an unfilled form, or absent media?",
  },
  {
    id: "loading",
    name: "Loading / in-flight",
    elements: ["form", "list-collection", "media", "nav", "interactive-control"],
    consideration:
      "What is shown while data or content is still loading (skeleton, spinner, progressive reveal)?",
  },
  {
    id: "error",
    name: "Error / failure",
    elements: ["form", "list-collection", "media", "nav", "interactive-control"],
    consideration:
      "What is shown when the load or submit fails (message, retry affordance, partial fallback)?",
  },
  {
    id: "populated",
    name: "Populated / happy path",
    elements: ["list-collection", "media"],
    consideration:
  ...
  },
  {
    ...
    elements: ["form", "list-collection"],
    consideration:
      "What is shown for partial or incomplete data — some fields or rows present, others missing?",
  },
  {
    id: "overflow",
    name: "Overflow / truncation",
    elements: ["list-collection", "nav", "static-content"],
    consideration:
      "What happens when content exceeds its container — scroll, clip, wrap, or truncate?",
  },
  {
    id: "zero-one-many",
    name: "Zero / one / many",
    ...
    consideration:
      "How does the layout read at zero, one, and many items (singular vs plural copy, spacing)?",
  },
  {
    id: "long-text",
    name: "Long text",
    elements: ["form", "static-content", "interactive-control", "nav"],
    consideration:
      "What happens with unusually long text — truncation, wrapping, ellipsis, or reflow?",
  },
];
  ...
  if (Array.isArray(element.elements)) {
    ...
    for (const k of element.elements) {
      if (typeof k !== "string" || !exports.VALID_ELEMENT_KINDS.has(k)) {
        throw new Error(
          `invalid element kind ${JSON.stringify(k)} for element ${element.id} — must be one of: ${[...exports.VALID_ELEMENT_KINDS].join(", ")}`,
        );
      }
    }
...
if (require.main === module) {
  ...
  runMain(() => {
    (0, probe_core_cjs_1.runProbeCli)(
      (elements, resolutions) => analyzeCoverage(elements, resolutions),
