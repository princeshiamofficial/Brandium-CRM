...
function classifyMarker(inner, sourcePath, lineNo) {
  ...
  if (inner.startsWith(CLOSE_TAG) && /^\s/.test(inner.slice(CLOSE_TAG.length))) {
    fail(
      sourcePath,
      lineNo,
      exports.REASON.CLOSE_WITH_ATTRIBUTES,
      "close marker must not carry attributes",
    );
  }
  ...
  if (attrs === null) {
    fail(
      sourcePath,
      lineNo,
      exports.REASON.MALFORMED_ATTRIBUTES,
      "malformed section marker attributes",
    );
  }
  ...
  if (extraKeys.length > 0) {
    fail(
      sourcePath,
      lineNo,
      exports.REASON.UNRECOGNIZED_ATTRIBUTE,
      `unrecognized attribute "${extraKeys[0]}" on section marker`,
    );
  }
  ...
  if (id === undefined) {
    fail(
      sourcePath,
      lineNo,
      exports.REASON.MISSING_ID,
      'section marker missing required "id" attribute',
    );
  }
  if (when === undefined) {
    fail(
      sourcePath,
      lineNo,
      exports.REASON.MISSING_WHEN,
      'section marker missing required "when" attribute',
    );
  }
  if (!ID_RE.test(id)) {
    fail(
      sourcePath,
      lineNo,
      exports.REASON.MALFORMED_ID,
      `section marker "id" value "${id}" does not match ${ID_RE}`,
    );
  }
  if (!exports.WHEN_VOCABULARY.includes(when)) {
    fail(
      sourcePath,
      lineNo,
      exports.REASON.UNKNOWN_WHEN,
      `section marker "when" value "${when}" is not in the frozen WHEN_VOCABULARY`,
    );
  }
...
function parseWorkflowSections(content, sourcePath) {
        ...
        if (classification.kind === "open") {
          if (currentOpen !== null) {
            fail(
              sourcePath,
              lineNo,
              exports.REASON.NESTED_SECTION,
              `nested gsd-section marker (already inside "${currentOpen.id}")`,
            );
          }
          if (seenIds.has(classification.id)) {
            fail(
              sourcePath,
              lineNo,
              exports.REASON.DUPLICATE_ID,
              `duplicate section id "${classification.id}"`,
            );
          }
          flushGapBefore(i);
          ...
          if (currentOpen === null) {
            fail(
              sourcePath,
              lineNo,
              exports.REASON.UNMATCHED_CLOSE,
              "unmatched /gsd-section close marker",
            );
          }
          sections.push({
            id: currentOpen.id,
  ...
  if (currentOpen !== null) {
    fail(
      sourcePath,
      currentOpen.startLineIndex + 1,
      exports.REASON.UNCLOSED_SECTION,
      `unclosed gsd-section marker "${currentOpen.id}"`,
    );
  }
