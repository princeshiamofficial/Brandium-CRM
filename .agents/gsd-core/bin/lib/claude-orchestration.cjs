...
function detectWorkflowBackend(input) {
  ...
  const floor =
    typeof floorRaw === "string" && isValidSemver(floorRaw)
      ? floorRaw
...
function emitWorkflowScript(input) {
  ...
  if (!isScriptableIdentifier(phaseDir)) {
    return {
      ok: false,
      reason: "phaseDir must be a non-empty string without newlines/quotes/backslash/control chars",
    };
  }
  if (!isScriptableIdentifier(runId)) {
    return {
      ok: false,
      reason: "runId must be a non-empty string without newlines/quotes/backslash/control chars",
    };
  }
  ...
  if (typeof executorModel === "string" && UNSCRIPTABLE_CHAR_RE.test(executorModel)) {
    return {
      ok: false,
      reason:
        "executorModel must not contain newlines/quotes/backslash/control/line-separator chars",
    };
  }
  ...
  for (let i = 0; i < waves.length; i++) {
    ...
    if (!isScriptableIdentifier(w.id)) {
      return {
        ok: false,
        reason: "waves[" + i + "].id must not contain newlines/quotes/backslash/control chars",
      };
    }
    if (seenWaveIds.has(w.id)) {
      return {
        ok: false,
        reason:
          'duplicate wave id "' + w.id + '" — wave ids must be unique (phase titles must map 1:1)',
      };
    }
    ...
    for (let j = 0; j < w.plans.length; j++) {
      ...
      if (
        ...
        !Array.isArray(p.files_modified)
      ) {
        return {
          ok: false,
          reason: "waves[" + i + "].plans[" + j + "] must be { id, brief, files_modified[] }",
        };
      }
      if (!isScriptableIdentifier(p.id)) {
        return {
          ...
          reason:
            ...
            "].plans[" +
            j +
            "].id must not contain newlines/quotes/backslash/control chars",
        };
      }
      if (p.use_worktree !== undefined && typeof p.use_worktree !== "boolean") {
        return {
          ok: false,
          reason: "waves[" + i + "].plans[" + j + "].use_worktree must be a boolean if present",
        };
      }
      if (seenIds.has(p.id)) {
      ...
      for (const f of p.files_modified) {
        if (typeof f !== "string" || f.length === 0) {
          return {
            ok: false,
            reason:
              "waves[" + i + "].plans[" + j + "].files_modified entries must be non-empty strings",
          };
        }
      }
    }
  ...
  const budgetTokens =
    ...
    Number.isFinite(input.budgetTokens) &&
  ...
  if (provenanceModel !== undefined) {
    lines.push(
      "// model: " +
        quoteString(provenanceModel) +
        " (resolved for gsd-executor, same source as the inline path)",
    );
  } else {
  ...
  lines.push(
    "// resume: pass " + quoteString(runId) + " as the Workflow tool's resumeFromRunId input",
  );
  ...
  for (let wi = 0; wi < waves.length; wi++) {
    ...
    for (let si = 0; si < stages.length; si++) {
      ...
      for (const p of stagePlans) {
        lines.push(
          "  () => agent(" + quoteString(p.brief) + ", " + agentOptions(p, executorModel) + "),",
        );
      }
      lines.push("])");
      ...
      for (let pi = 0; pi < stagePlans.length; pi++) {
        ...
        lines.push(
          "  { plan: " +
            quoteString(p.id) +
            ", expects_worktree: " +
            (p.use_worktree !== false) +
            ", metadata: gsdWorktreeMetadata(gsdStage_" +
            wi +
            "_" +
            si +
            "[" +
