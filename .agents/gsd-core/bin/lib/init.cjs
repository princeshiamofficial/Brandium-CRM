...
const { resolveModelInternal, resolveGranularityInternal, assertValidGranularityOverride } =
  modelResolver;
...
function roadmapPhaseMatchesExactPrefix(roadmapPhase, phase) {
  ...
  return new RegExp(
    `^#{2,4}\\s*Phase\\s+${(0, pattern_cjs_1.escapeRegex)(phase)}(?:\\b|\\s|:)`,
    "i",
  ).test(section);
}
...
function guardedFindPhase(cwd, phase, projectCode) {
  ...
  if (
    isForeignPrefixedPhaseQuery(phase, projectCode) &&
    !phaseInfoMatchesExactPrefix(phaseInfo, phase)
  ) {
    phaseInfo = null;
  }
  return phaseInfo;
...
function guardedGetRoadmapPhase(cwd, phase, projectCode) {
  ...
  if (
    isForeignPrefixedPhaseQuery(phase, projectCode) &&
    !roadmapPhaseMatchesExactPrefix(roadmapPhase, phase)
  ) {
    roadmapPhase = null;
  }
  return roadmapPhase;
...
function buildPhaseCompletionProjection(
  ...
  return {
    ...
    verification_stale_check_indeterminate:
      "staleCheckIndeterminate" in verificationStatus &&
...
function getInitGitState(cwd) {
  ...
  if (info["inside"]) {
    ...
    try {
      const prefixResult = (0, shell_command_projection_cjs_1.execGit)(
        ["rev-parse", "--show-prefix"],
        { cwd, timeout: 5000 },
      );
      if (prefixResult["exitCode"] === 0) {
        const prefix = (0, shell_command_projection_cjs_1.posixNormalize)(
          (typeof prefixResult["stdout"] === "string" ? prefixResult["stdout"] : "").trim(),
        );
  ...
  if (inNestedSubdir && typeof worktreeRoot === "string") {
    const toComparableRaw = (p) =>
      (0, shell_command_projection_cjs_1.posixNormalize)(p).replace(/\/+$/g, "").toLowerCase();
...
const _sectionManifestCandidatePath = () =>
...
function detectHasPriorPhases(cwd, phaseInfo) {
  ...
  const currentDirName = phaseInfo?.["directory"]
    ...
    : null;
  try {
    if (!node_fs_1.default.existsSync(phasesDir)) return false;
...
function detectPhaseMvpMode(cwd, phaseNumber) {
  ...
  try {
    ...
    if (!node_fs_1.default.existsSync(roadmapPath)) return false;
...
function detectUiPhaseActive(cwd, phaseInfo) {
  ...
  if (typeof rawDir === "string" && rawDir) {
    try {
      ...
      const files = node_fs_1.default.readdirSync(
        node_path_1.default.join(planningDir(cwd), "phases", dirName),
      );
...
function buildSectionManifestField(cwd, phaseInfo, options, workflow, overrides = {}) {
  ...
  const chunkedMode =
    flags.has("--chunked") || readConfigJsonBoolean(cwd, ["workflow", "plan_chunked"]);
  const facts = {
    flags,
    phaseNumber,
  ...
  try {
    ...
    return {
      ...
      read: selection.included.map((id) => readById.get(id)).filter((p) => typeof p === "string"),
    };
  } catch {
    return null;
  }
...
function cmdInitExecutePhase(cwd, phase, raw, options = {}) {
  ...
  const result = {
    ...
    state_exists: node_fs_1.default.existsSync(
      node_path_1.default.join(planningDir(cwd), "STATE.md"),
    ),
    roadmap_exists: node_fs_1.default.existsSync(
      node_path_1.default.join(planningDir(cwd), "ROADMAP.md"),
    ),
    config_exists: node_fs_1.default.existsSync(
      node_path_1.default.join(planningDir(cwd), "config.json"),
    ),
    ...
    requirements_path: node_fs_1.default.existsSync(requirementsPath)
      ? toPosixPath(requirementsPath)
      : null,
  };
  if (options["validate"]) {
    try {
      ...
      if (stateContent !== null) {
        ...
        if (
          phaseInfo?.["directory"] &&
          node_fs_1.default.existsSync(node_path_1.default.join(cwd, phaseInfo["directory"]))
        ) {
          const diskPlans = listPhasePlanFiles(
            node_path_1.default.join(cwd, phaseInfo["directory"]),
          ).length;
  ...
  if (!phaseDirPlan && phaseNumberPlan && phaseNamePlan) {
    ...
    if (slug) {
      ...
      expectedPhaseDirPlan = toPosixPath(
        node_path_1.default.join(planningPaths(cwd).phases, dirName),
      );
    }
  }
  ...
  const result = {
    ...
    phase_status: phaseDirPlan
      ? determinePhaseStatus(
          ...
          node_path_1.default.join(cwd, phaseDirPlan),
          "Pending",
        )
      : "Pending",
    ...
    try {
      ...
      if (verificationFile) {
        result["verification_path"] = toPosixPath(
          node_path_1.default.join(phaseDirFull, verificationFile),
        );
      }
...
function buildInitCompletenessFields(cwd) {
  ...
  const requirementsExists = node_fs_1.default.existsSync(
    node_path_1.default.join(dir, "REQUIREMENTS.md"),
  );
  const milestonesExists = node_fs_1.default.existsSync(
    node_path_1.default.join(dir, "MILESTONES.md"),
  );
  const coreComplete =
    node_fs_1.default.existsSync(node_path_1.default.join(dir, "PROJECT.md")) &&
    requirementsExists &&
...
function cmdInitNewProject(cwd, raw, options = {}) {
  ...
  const hasBraveSearch = !!(
    process.env["BRAVE_API_KEY"] || node_fs_1.default.existsSync(braveKeyFile)
  );
  const firecrawlKeyFile = node_path_1.default.join(homedir, ".gsd", "firecrawl_api_key");
  const hasFirecrawl = !!(
    process.env["FIRECRAWL_API_KEY"] || node_fs_1.default.existsSync(firecrawlKeyFile)
  );
  ...
  const result = {
    ...
    project_exists: pathExistsInternal(
      cwd,
      toPosixPath(
        node_path_1.default.relative(cwd, node_path_1.default.join(planningDir(cwd), "PROJECT.md")),
      ),
    ),
...
function cmdInitResume(cwd, raw) {
  ...
  const agentIdRaw = (0, shell_command_projection_cjs_1.platformReadSync)(
    node_path_1.default.join(planningRoot(cwd), "current-agent-id.txt"),
  );
...
function cmdInitTodos(cwd, area, raw) {
  ...
  const result = {
    ...
    planning_exists: node_fs_1.default.existsSync(planningDir(cwd)),
    todos_dir_exists: node_fs_1.default.existsSync(
      node_path_1.default.join(planningDir(cwd), "todos"),
    ),
    pending_dir_exists: node_fs_1.default.existsSync(
      node_path_1.default.join(planningDir(cwd), "todos", "pending"),
    ),
  };
  output(withProjectRoot(cwd, result), raw);
}
function cmdInitMilestoneOp(cwd, raw) {
  ...
  if (roadmapPhaseNumbers.length > 0) {
    ...
    for (const num of roadmapPhaseNumbers) {
      ...
      try {
        const hasSummary =
          listPhaseSummaryFiles(node_path_1.default.join(phasesDir, dirName)).length > 0;
    ...
    try {
      ...
      for (const dir of dirs) {
        try {
          const hasSummary =
            listPhaseSummaryFiles(node_path_1.default.join(phasesDir, dir)).length > 0;
          if (hasSummary) completedPhases++;
        } catch {
          /* intentionally empty */
        }
  ...
  const result = {
    ...
    archive_exists: node_fs_1.default.existsSync(
      node_path_1.default.join(planningRoot(cwd), "archive"),
    ),
    phases_dir_exists: node_fs_1.default.existsSync(
      node_path_1.default.join(planningDir(cwd), "phases"),
    ),
  };
  output(withProjectRoot(cwd, result), raw);
}
function cmdInitMapCodebase(cwd, raw) {
  ...
  const result = {
    ...
    planning_exists: pathExistsInternal(cwd, ".planning"),
    codebase_dir_exists: pathExistsInternal(
      cwd,
      toPosixPath(
        node_path_1.default.relative(cwd, node_path_1.default.join(planningDir(cwd), "codebase")),
      ),
    ),
  };
  output(withProjectRoot(cwd, result), raw);
...
function cmdInitManager(cwd, raw) {
  ...
  if (!node_fs_1.default.existsSync(paths.roadmap)) {
    error(
      `No ROADMAP.md found. Run ${(0, runtime_slash_cjs_1.formatGsdSlash)("new-milestone", _slashRuntime)} first.`,
    );
  }
  if (!node_fs_1.default.existsSync(paths.state)) {
    error(
      `No STATE.md found. Run ${(0, runtime_slash_cjs_1.formatGsdSlash)("new-milestone", _slashRuntime)} first.`,
    );
  }
  ...
  const _cbPattern = new RegExp(
    `-\\s*\\[(x| )\\]\\s*.*Phase\\s+(${PHASE_NUMBER_TOKEN_SOURCE})[:\\s]`,
    "gi",
  );
  let _cbMatch;
  ...
  for (const phase of phases) {
    if (!phase["depends_on"] || /^none$/i.test(phase["depends_on"].trim())) {
      ...
      const depNums =
        phase["depends_on"].match(new RegExp(`${PHASE_NUMBER_TOKEN_SOURCE}`, "gi")) || [];
  ...
  const result = {
    ...
    in_progress_count: phases.filter((p) =>
      ["executed", "partial", "planned", "discussed", "researched"].includes(p["disk_status"]),
    ).length,
...
function cmdInitProgress(cwd, raw, options = {}) {
  ...
  try {
    ...
    for (const dir of dirs) {
      ...
      const phaseDirRel = toPosixPath(
        node_path_1.default.relative(
          cwd,
          node_path_1.default.join(planningDir(cwd), "phases", dir),
        ),
      );
      const completion = buildPhaseCompletionProjection(
        cwd,
      ...
      if (
        !currentPhase &&
        (status === "executed" || status === "in_progress" || status === "researched")
      ) {
  ...
  let pausedAt = null;
  const state = (0, shell_command_projection_cjs_1.platformReadSync)(
    node_path_1.default.join(planningDir(cwd), "STATE.md"),
  );
  if (state !== null) {
  ...
  const result = {
    ...
    in_progress_count: phases.filter((p) => ["executed", "in_progress"].includes(p["status"]))
      .length,
...
function cmdInitListWorkspaces(cwd, raw) {
  ...
  if (node_fs_1.default.existsSync(defaultBase)) {
    ...
    for (const entry of entries) {
      ...
      if (!node_fs_1.default.existsSync(manifestPath)) continue;
      let repoCount = 0;
      ...
      hasProject = node_fs_1.default.existsSync(
        node_path_1.default.join(wsPath, ".planning", "PROJECT.md"),
      );
      workspaces.push({
        name: entry.name,
        path: wsPath,
...
function cmdInitRemoveWorkspace(cwd, name, raw) {
  ...
  for (const repo of repos) {
    ...
    if (!node_fs_1.default.existsSync(repoPath)) continue;
    const statusResult = (0, shell_command_projection_cjs_1.execGit)(["status", "--porcelain"], {
      cwd: repoPath,
      timeout: 5000,
    });
...
function buildAgentSkillsBlock(config, agentType, projectRoot, diagnostics) {
  ...
  for (const skillPath of skillPaths) {
    if (typeof skillPath !== "string") {
      warn(
        `[agent-skills] WARNING: Ignoring non-string skill entry (${typeof skillPath}) — skipping\n`,
      );
      continue;
    }
    if (skillPath.startsWith("global:")) {
      ...
      const pathCheck = (0, security_cjs_1.validatePath)(globalSkillMd, globalSkillsBase, {
        allowAbsolute: true,
      });
      if (!pathCheck["safe"]) {
        const acceptedViaTrustedRoot = trustedGlobalRoots.some((root) => {
          const rootCheck = (0, security_cjs_1.validatePath)(globalSkillMd, root, {
            allowAbsolute: true,
          });
        ...
        process.stderr.write(
          `[agent-skills] NOTE: Global skill "${skillName}" accepted via trusted_global_roots (resolves outside the default skills dir)\n`,
        );
      }
      validEntries.push({
        kind: "include",
    ...
    if (!node_fs_1.default.existsSync(skillMdPath)) {
      ...
      if (globalSkillsBase !== null) {
        ...
        if (
          globalDir &&
          node_fs_1.default.existsSync(node_path_1.default.join(globalDir, "SKILL.md"))
        ) {
...
function cmdAgentSkills(cwd, agentType, raw, jsonMode) {
    ...
    if (runtime !== "claude") {
      ...
      if (typeof agentsDir === "string" && agentsDir.length > 0) {
        ...
        try {
          ...
          /* agent file not found — fall through to empty block */
        }
      }
    }
  }
  ...
  const agentSkillsMap =
    config && config["agent_skills"] && typeof config["agent_skills"] === "object"
      ? config["agent_skills"]
      : {};
  ...
  if (jsonMode) {
    ...
    const resolution = (0, resolution_cjs_1.makeResolution)(
    ...
    : [
        ...
        },
        {
          ...
          path: node_path_1.default.join(
            (0, runtime_homes_cjs_1.getGlobalConfigDir)("codex"),
            "skills",
          ),
          scope: "global",
          kind: "skills",
