...
function isCommittedLedgerEntry(ledger, id, e) {
  ...
  if (Object.prototype.hasOwnProperty.call(e, "_pending")) return false; // intent ⇒ uncommitted.
...
function ledgerOverlayIds(ledger, rootDir) {
  ...
  try {
    ...
    /* missing/invalid/non-regular/oversized ledger — no pending, no committed (fail closed) */
  }
  return { pending, committed };
}
...
function loadRegistry(options = {}) {
    ...
    for (const ent of entries) {
      ...
      if (pendingIds.has(id)) {
        warnings.push({
          id,
          scope: root.scope,
          reason: "install/upgrade in progress (uncommitted) — deferred until reconciliation",
        });
        continue;
      }
      let cap;
      try {
        ...
        if (manifestRaw === null) {
          warnings.push({
            ...
            scope: root.scope,
            reason:
              "capability.json missing, non-regular (FIFO/device), or exceeds the size cap — skipped",
          });
          continue;
        }
        ...
        warnings.push({
          id,
          scope: root.scope,
          reason: "unreadable or invalid capability.json: " + errMessage(e),
        });
        continue;
      }
      ...
      try {
        ...
        const cfgKeys =
          cap.config && typeof cap.config === "object" && !Array.isArray(cap.config)
        ...
        const families = Array.isArray(cap.commands)
          ? cap.commands
              .map((c) =>
                c && typeof c === "object" && typeof c.family === "string" ? c.family : null,
              )
        ...
        if (root.scope === "project") {
          ...
          if (!consented) {
            ...
            warnings.push({
              ...
              scope: root.scope,
              kind: "unconsented",
              reason: "discovered — no user consent record (inactive)",
            });
            continue;
          }
        }
  ...
  try {
    ...
    const reason =
      "buildRegistry failed composing overlays: " + errMessage(e) + "; falling back to first-party";
