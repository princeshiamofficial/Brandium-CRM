...
exports.ANTIGRAVITY_FAILURE_MODE =
  exports.MODEL_VALUE_MAX =
  exports.BANNER_SCAN_LINES =
  exports.UNRESOLVED_MODEL =
  exports.MODEL_SOURCE =
    void 0;
...
function normalizeModelValue(raw) {
  ...
  if (value.length > exports.MODEL_VALUE_MAX) return null;
...
function checkEgressHost(consentedHost, currentHost) {
  const consented =
    typeof consentedHost === "string"
      ? (0, review_lane_invocation_cjs_1.normalizeHost)(consentedHost)
      : "";
...
async function probeLane(plan, deps) {
  ...
  switch (probe.kind) {
    ...
    case "http-reachable": {
      ...
      const base = (0, review_lane_invocation_cjs_1.normalizeHost)(
      ...
      if (!base) {
        return {
          ...
          detail: "no host resolved",
        };
      }
      const r = await deps.httpJson(`${base}${probe.path}`, {
        method: "GET",
...
function antigravityFailureMode(workspace, mark, deps) {
  const convId = resolveWorkspaceConvId(workspace, deps);
  if (!convId) return exports.ANTIGRAVITY_FAILURE_MODE.PRE_SESSION_STALL;
  // A different conv-id than the watermark saw = a fresh session this run, transcript or not.
  if (convId !== mark.convId) return exports.ANTIGRAVITY_FAILURE_MODE.SESSION_STARTED;
  const tx = transcriptPath(deps.homeDir, convId);
  if (!deps.exists(tx)) return exports.ANTIGRAVITY_FAILURE_MODE.PRE_SESSION_STALL;
  try {
    const lines = deps
      .readFile(tx)
      .split(/\r?\n/)
...
function stampBlindReview(review) {
  if ((0, review_lane_invocation_cjs_1.isEmptyReview)(review)) return review;
  const head = review.split(/\r?\n/).slice(0, 5).join("\n");
  const selfReported = head.includes("REVIEWED-WITHOUT-REPO-ACCESS");
  const scratchTell =
    /(workspace|working) (directory|dir)[\s\S]{0,40}antigravity-cli\/scratch/i.test(review);
...
async function runOpenAiCompatible(plan, promptText, deps) {
  ...
  try {
    ...
    if (servedModel.source !== exports.MODEL_SOURCE.UNKNOWN) served = servedModel;
  ...
  if (plan.transport === "openai-http") {
    ...
    if (!egress.allowed) {
      ...
      return {
        ...base,
        ok: false,
        reason: review_lane_invocation_cjs_1.LANE_UNAVAILABLE.EGRESS_HOST_CHANGED,
        detail,
      };
    }
  }
