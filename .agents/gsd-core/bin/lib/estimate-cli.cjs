...
function readCalibrationSamples(cwd) {
  let raw;
  try {
    raw = node_fs_1.default.readFileSync(
      node_path_1.default.join(planningDir(cwd), exports.CALIBRATION_FILENAME),
      "utf-8",
    );
  } catch {
    return [];
...
function collectCalibrationSamples(cwd) {
  ...
  for (const phase of phases) {
    ...
    for (const f of files) {
      ...
      if (m[2] === "PLAN") entry.plan = node_path_1.default.join(dir, f);
      else entry.summary = node_path_1.default.join(dir, f);
