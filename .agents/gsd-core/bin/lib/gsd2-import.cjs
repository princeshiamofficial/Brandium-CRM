...
function slugify(title) {
  ...
  return transliterateForSlug(title)
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-|-$/g, "");
}
// ─── GSD-2 Parser ───────────────────────────────────────────────────────────
...
function findGsd2Root(startPath) {
  if (
    node_path_1.default.basename(startPath) === ".gsd" &&
    node_fs_1.default.existsSync(startPath)
  ) {
    return startPath;
  }
  const candidate = node_path_1.default.join(startPath, ".gsd");
  if (
    node_fs_1.default.existsSync(candidate) &&
    node_fs_1.default.statSync(candidate).isDirectory()
  ) {
    return candidate;
  }
  return null;
...
function readTasksDir(tasksDir) {
  if (!node_fs_1.default.existsSync(tasksDir)) return [];
...
function parseGsd2(gsdDir) {
  ...
  if (!node_fs_1.default.existsSync(milestonesBase)) return data;
  const milestoneIds = node_fs_1.default
    .readdirSync(milestonesBase)
    .filter((d) =>
      node_fs_1.default.statSync(node_path_1.default.join(milestonesBase, d)).isDirectory(),
    )
    .sort();
  for (const mid of milestoneIds) {
    ...
    const slices = sliceInfos.map((info) => {
      ...
      return {
        ...
        summary: hasSDir
          ? readOptional(node_path_1.default.join(sDir, `${info.id}-SUMMARY.md`))
          : null,
        research: hasSDir
          ? readOptional(node_path_1.default.join(sDir, `${info.id}-RESEARCH.md`))
          : null,
        context: hasSDir
          ? readOptional(node_path_1.default.join(sDir, `${info.id}-CONTEXT.md`))
          : null,
...
function buildPreview(gsd2Data, artifacts, projectDir) {
  ...
  lines.push(
    `  - GSD-2 database state (rebuilt from files on first ${(0, runtime_slash_cjs_1.formatGsdSlash)("health", (0, runtime_slash_cjs_1.resolveRuntime)(projectDir))})`,
  );
...
function cmdFromGsd2(args, cwd, raw) {
  ...
  const projectDir =
    pathIdx >= 0 && args[pathIdx + 1] ? node_path_1.default.resolve(cwd, args[pathIdx + 1]) : cwd;
