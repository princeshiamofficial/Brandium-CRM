...
function expandTilde(p, home = node_os_1.default.homedir()) {
  if (!p) return p;
  if (p.startsWith("~/") || p === "~") return node_path_1.default.join(home, p.slice(1));
  return p;
}
/**
...
function detectAntigravityDirAmbiguity(opts = {}) {
  ...
  const candidates = ["antigravity", "antigravity-ide", "antigravity-cli"].map((c) =>
    node_path_1.default.join(base, c),
  );
  const presentDirs = candidates.filter((dir) => existsSyncFn(dir));
  const gsdMarkedDirs = candidates.filter((dir) =>
    existsSyncFn(node_path_1.default.join(dir, marker)),
  );
  return {
...
function getGlobalConfigDir(runtime, explicitDir) {
  ...
  if (runtime === "grok") {
    ...
    return env["GROK_AGENTS_HOME"]
      ? expandTilde(env["GROK_AGENTS_HOME"])
      : node_path_1.default.join(node_os_1.default.homedir(), ".agents");
  }
...
function getGlobalSkillsBase(runtime) {
  ...
  const globalSkillsKind = descriptor?.artifactLayout?.global?.find(
    (entry) => entry.kind === "skills",
  );
