 ...
 */
const DEFAULT_COMMANDS_DIR = node_path_1.default.resolve(
  __dirname,
  "..",
  "..",
  "..",
...
function loadSkillsManifest(
  ...
  if (!node_fs_1.default.existsSync(commandsDir)) return manifest;
  ...
  for (const entry of entries) {
    ...
    try {
      const content = node_fs_1.default.readFileSync(
        node_path_1.default.join(commandsDir, entry.name),
        "utf8",
      );
      manifest.set(stem, parseRequires(content));
      // Derive agent references from body text + the workflows it delegates to
      const agentRefs = [...parseCallsAgents(content), ...workflowAgentRefs(content, workflowsDir)];
...
function resolveProfile({ modes, manifest, _profilesOverride, registry } = {}) {
  ...
  const validModes = modesToResolve.filter((mode) =>
    Object.prototype.hasOwnProperty.call(profiles, mode),
  );
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      installFs().copyFileSync(
        node_path_1.default.join(srcDir, entry.name),
        node_path_1.default.join(stageDir, entry.name),
      );
    }
  } catch (err) {
    try {
  ...
  try {
    if (resolvedProfile.agents instanceof Set && resolvedProfile.agents.size > 0) {
      ...
      for (const entry of entries) {
        ...
        installFs().copyFileSync(
          node_path_1.default.join(srcAgentsDir, entry.name),
          node_path_1.default.join(stageDir, entry.name),
        );
      }
    }
...
function buildNamespaceBundleMap(srcCommandsDir) {
  ...
  for (const entry of installFs().readdirSync(srcCommandsDir, { withFileTypes: true })) {
    ...
    let children = [];
    try {
      children = parseRequires(
        installFs().readFileSync(node_path_1.default.join(srcCommandsDir, entry.name), "utf8"),
      );
    } catch {
      children = [];
    }
...
function isSafeCapabilitySkillStem(stem) {
  ...
  if (node_path_1.default.isAbsolute(stem)) return false;
  return true;
}
/**
...
function readInstalledCapabilitySkill(stem, registry) {
  ...
  if (!(0, external_descriptor_trust_cjs_1.isPathConfined)(relSkillPath, capDir)) return null;
  const skillPath = node_path_1.default.join(capDir, relSkillPath);
  try {
    if (!node_fs_1.default.statSync(skillPath).isFile()) return null;
...
function stageSkillsForRuntimeAsSkills(
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      const content = installFs().readFileSync(
        node_path_1.default.join(srcCommandsDir, entry.name),
        "utf8",
      );
      ...
      if (doNest && bundles.routerStems.has(stem)) {
        ...
        installFs().writeFileSync(
          node_path_1.default.join(destDir, "SKILL.md"),
          transformRouterBodyToNested(converted),
        );
        continue;
      }
    ...
    if (registry) {
      ...
      for (const stem of candidateStems) {
        ...
        const skillName = `${prefix}${stem}`;
        if (!(0, external_descriptor_trust_cjs_1.isPathConfined)(skillName, stageDir)) continue; // defense-in-depth
        const destDir = node_path_1.default.join(stageDir, skillName);
        installFs().mkdirSync(destDir, { recursive: true });
        ...
        installFs().writeFileSync(
          node_path_1.default.join(destDir, CAPABILITY_SKILL_MARKER),
          found.capId + "\n",
          "utf8",
        );
      }
...
function stageAgentsForRuntimeWithConverter(
  ...
  try {
    ...
    throw new Error(
      `stageAgentsForRuntimeWithConverter: could not resolve the shipped agents/ directory "${srcAgentsDir}" to stage — refusing to install (fail-closed, #2284/#2875): ${err.message}`,
    );
  }
  try {
    // Resolve cmdNames once per staging call (not per file) for performance.
    const cmdNames = agentCtx ? _readGsdCommandNames() : [];
    const agentTools = installModelOverrideResolver.readGsdEffectiveAgentTools(
      agentCtx?.projectDir ?? agentCtx?.targetDir ?? null,
    );
    for (const entry of entries) {
      if (!entry.isFile()) continue;
      if (!entry.name.endsWith(".md")) continue;
      ...
      const grants = [...(agentTools?.["*"] || []), ...(agentTools?.[agentName] || [])];
      if (agentCtx) {
        ...
        content = _applyAgentFrontmatterExtensions(content, {
          runtime: agentCtx.runtime,
          agentName,
          targetDir: agentCtx.targetDir,
        });
...
function writeActiveProfile(runtimeConfigDir, profileName) {
  (0, shell_command_projection_cjs_1.platformWriteSync)(
    node_path_1.default.join(runtimeConfigDir, PROFILE_MARKER_NAME),
    profileName + "\n",
  );
}
// ---------------------------------------------------------------------------
...
function stageSkillsForMode(srcDir, mode) {
  ...
  if (!node_fs_1.default.existsSync(srcDir)) return srcDir;
  const stageDir = node_fs_1.default.mkdtempSync(
    node_path_1.default.join(node_os_1.default.tmpdir(), "gsd-minimal-skills-"),
  );
  try {
