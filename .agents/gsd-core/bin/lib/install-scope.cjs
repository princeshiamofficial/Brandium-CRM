 ...
 * invent a fact for every non-the agent runtime that asked. Callers that
...
function expandTildeForExplicitDir(p, home) {
  ...
  if (p.startsWith("~/")) return node_path_1.default.join(resolvedHome, p.slice(2));
  return p;
}
/**
...
function resolveScopeConfigHome(id, descriptor, input) {
  ...
  return normalizeSeparators(
    (0, runtime_homes_cjs_1.resolveConfigHomeFromDescriptor)(descriptor.configHome, {
      env: input.env,
      home: input.home,
...
function resolveScope(input) {
  ...
  const registryEntry = typeof runtime === "string" ? getRegistry().runtimes[runtime] : undefined;
