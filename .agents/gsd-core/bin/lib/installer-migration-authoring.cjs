...
function validateStringArray(record, field, source) {
  ...
  if (!isNonEmptyStringArray(record[field])) {
    throw new Error(
      `migration record ${field} must be a non-empty string array when provided: ${source}`,
    );
  }
}
...
function requireActionEvidence(action, field, migration) {
  ...
  if (typeof v !== "string" || v.trim() === "") {
    throw new Error(
      `migration action ${getStr(action, "type")} must include ${field}: ${actionSource(migration, action)}`,
    );
  }
}
...
function validateInstallerMigrationActions(actions, migration) {
  ...
  for (const action of actions) {
    ...
    if (!actRelPath || actRelPath.trim() === "") {
      throw new Error(
        `migration action ${actType} must include a non-empty relPath: ${getStr(migration, "id")}`,
      );
    }
    ...
    if (
      actType === "remove-managed" ||
      actType === "rewrite-json" ||
      actType === "remove-empty-dir"
    ) {
    ...
    if (actType === "rewrite-json") {
      ...
      if (!rc || rc.trim() === "") {
        throw new Error(
          `migration action rewrite-json requires migration runtimeContract: ${actionSource(migration, act)}`,
        );
      }
    }
  }
