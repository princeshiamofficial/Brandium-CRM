...
function checkGraphifyInstalled() {
  const result = (0, shell_command_projection_cjs_1.execTool)("graphify", ["--help"], {
    timeout: 5000,
  });
  if (result.error) {
    return {
...
function checkGraphifyVersion() {
  ...
  const versionResult = (0, shell_command_projection_cjs_1.execTool)("graphify", ["--version"], {
    timeout: 2000,
  });
  let versionStr = null;
  ...
  if (!versionStr) {
    ...
    const pyVerify = (0, shell_command_projection_cjs_1.execTool)(
      "python3",
      ["-c", 'from importlib.metadata import version; print(version("graphifyy"))'],
      { timeout: 5000 },
    );
  ...
  if (!pyPackageConfirmed) {
    return {
      ...
      warning:
        "graphify version " +
        versionStr +
        " detected but the graphifyy Python package could not be confirmed — the `graphify` binary on PATH may be a different tool. Verify with: pip show graphifyy",
    };
  }
  const warning = versionInRange
    ? null
...
function applyBudget(result, budgetTokens, term = "") {
  ...
  const trimmedLabel = (dropped, unreachable) =>
  ...
  let estimate = wireEstimate(
    nodes,
    edges,
    trimmedLabel(omitted, result.nodes.length - nodes.length),
  );
...
function countCommitsBetween(cwd, from, to) {
  const r = (0, shell_command_projection_cjs_1.execGit)(["rev-list", "--count", `${from}..${to}`], {
    cwd,
  });
  if (r.exitCode !== 0) return null;
...
function resolveGraphLocation(cwd, planningDir) {
  ...
  return {
    graphPath: node_path_1.default.join(planningDir, "graphs", GRAPH_FILENAME),
    configured: false,
  };
}
/**
...
function graphifyStatus(cwd) {
  ...
  const lastBuildAutoUpdate = node_fs_1.default.existsSync(statusPath)
    ? safeReadJson(statusPath)
    : null;
...
function graphifyDiff(cwd) {
  ...
  const snapshotPath = node_path_1.default.join(
    node_path_1.default.dirname(graphPath),
    SNAPSHOT_FILENAME,
  );
