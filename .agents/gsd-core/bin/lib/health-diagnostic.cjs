  ...
  switch (action) {
    ...
    case REMEDY_ACTION.RESET_CONFIG: {
      (0, shell_command_projection_cjs_1.platformWriteSync)(
        configPath,
        JSON.stringify(defaultConfigPayload(), null, 2),
      );
    ...
    case REMEDY_ACTION.REGENERATE_STATE: {
      ...
      const priorState = node_fs_1.default.existsSync(statePath)
    ...
    case REMEDY_ACTION.ADD_AI_INTEGRATION_PHASE_KEY: {
      const key =
        action === REMEDY_ACTION.ADD_NYQUIST_KEY ? "nyquist_validation" : "ai_integration_phase";
