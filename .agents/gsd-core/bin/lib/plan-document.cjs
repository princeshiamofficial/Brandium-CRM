 ...
 * would be the *Generative Fix Divergence* class `GEMINI.md` names. A leaf owned
...
function parseXmlTasks(content) {
  ...
  return openings.map((match, i) => {
    ...
    const nextStart =
      i + 1 < openings.length ? (openings[i + 1].index ?? content.length) : content.length;
    ...
    const kind =
      type !== null && type.toLowerCase().startsWith("checkpoint")
