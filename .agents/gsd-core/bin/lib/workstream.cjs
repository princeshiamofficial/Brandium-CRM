...
const { getOtherActiveWorkstreamInventories, inspectWorkstream, listWorkstreamInventories } =
  workstreamInventory;
// ─── Migration ───────────────────────────────────────────────────────────────
/**
...
function migrateToWorkstreams(cwd, workstreamName) {
  try {
    (0, workstream_name_policy_cjs_1.assertValidActiveWorkstreamName)(
      workstreamName,
    ...
    for (const name of filesMoved) {
      try {
        (0, shell_command_projection_cjs_1.retryRenameSync)(
          node_path_1.default.join(wsDir, name),
          node_path_1.default.join(baseDir, name),
        );
      } catch {
        /* ignore */
      }
...
function cmdWorkstreamCreate(cwd, name, options, raw) {
  ...
  if (!node_fs_1.default.existsSync(baseDir)) {
    error(
      `.planning/ directory not found — run ${(0, runtime_slash_cjs_1.formatGsdSlash)("new-project", (0, runtime_slash_cjs_1.resolveRuntime)(cwd))} first`,
    );
  }
  const wsRoot = node_path_1.default.join(baseDir, "workstreams");
  const wsDir = node_path_1.default.join(wsRoot, slug);
  if (
    node_fs_1.default.existsSync(wsDir) &&
    node_fs_1.default.existsSync(node_path_1.default.join(wsDir, "STATE.md"))
  ) {
    output(
      {
        created: false,
        ...
        workstream: slug,
        path: toPosixPath(node_path_1.default.relative(cwd, wsDir)),
      },
      raw,
      undefined,
    );
  ...
  if (isFlatMode && options.migrate !== false) {
    const hasExistingWork =
      node_fs_1.default.existsSync(node_path_1.default.join(baseDir, "ROADMAP.md")) ||
...
function cmdWorkstreamStatus(cwd, name, raw) {
  ...
  try {
    (0, workstream_name_policy_cjs_1.assertValidActiveWorkstreamName)(
      name,
      workstream_name_policy_cjs_1.INVALID_ACTIVE_WORKSTREAM_NAME_MESSAGE,
    );
  } catch {
...
function cmdWorkstreamComplete(cwd, name, options, raw) {
  ...
  try {
    ...
    for (const entry of entries) {
      (0, shell_command_projection_cjs_1.retryRenameSync)(
        node_path_1.default.join(wsDir, entry.name),
        node_path_1.default.join(archivePath, entry.name),
      );
    ...
    for (const fname of filesMoved) {
      try {
        (0, shell_command_projection_cjs_1.retryRenameSync)(
          node_path_1.default.join(archivePath, fname),
          node_path_1.default.join(wsDir, fname),
        );
      } catch {
        /* ignore */
      }
  ...
  let remainingWs = 0;
  try {
    remainingWs = node_fs_1.default
      .readdirSync(wsRoot, { withFileTypes: true })
      .filter((e) => e.isDirectory()).length;
    if (remainingWs === 0) node_fs_1.default.rmdirSync(wsRoot);
  } catch {
    /* ignore */
  }
  output(
    ...
    output(
      {
        active: null,
        error: "invalid_name",
        message: "Workstream name must be alphanumeric, hyphens, underscores, or dots",
      },
      raw,
      undefined,
    );
...
function cmdWorkstreamGet(cwd, raw) {
  ...
  output(
    { active, mode: node_fs_1.default.existsSync(wsRoot) ? "workstream" : "flat" },
    raw,
    active || "none",
  );
}
function cmdWorkstreamProgress(cwd, raw) {
  ...
  output(
    { mode: "workstream", active: inventory.active, workstreams, count: workstreams.length },
    raw,
    undefined,
  );
}
