...
function eslintMessages(file, key) {
  return file && typeof file === "object" && Array.isArray(file[key]) ? file[key] : [];
}
/**
...
function defaultRunCheck(check, cwd, timeoutMs) {
  try {
    ...
    if (check.kind === "lint-rule") {
      ...
      let json = "";
      try {
        json = (0, node_child_process_1.execFileSync)(
          process.execPath,
      ...
      return {
        passed:
          lintedSomething && !eslintHasFatalError(json) && !eslintJsonHasRule(json, check.rule),
      };
    }
...
function runProhibitionEnforcement(prohibition, check, options = {}) {
  ...
  const validRule =
    !!c && (c.kind !== "lint-rule" || (typeof c.rule === "string" && c.rule.trim().length > 0));
  if (!c || !validKind || !validTarget || !validRule) {
    const disposition = (0, probe_core_cjs_1.dispositionForProhibition)(prohibition, {
      enforcementEvidence: [],
    });
  ...
  const runCheck =
    options.runCheck ??
    ((toRun) => defaultRunCheck(toRun, options.cwd ?? process.cwd(), options.timeoutMs));
  const proveFailFirst =
    options.proveFailFirst ??
    ((toProve) => defaultProveFailFirst(toProve, options.cwd ?? process.cwd(), options.timeoutMs));
...
function parseRequest(args) {
  ...
  try {
    ...
    const check = checkRaw && typeof checkRaw === "object" ? checkRaw : null;
...
function routeProhibitionEnforcement(args, raw) {
  ...
  const result = runProhibitionEnforcement(
    req.prohibition,
    req.check,
