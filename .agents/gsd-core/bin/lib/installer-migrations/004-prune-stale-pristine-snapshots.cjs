...
const REASON =
  "stale pristine snapshot from legacy get-shit-done/ dir, orphaned by rename migration 003 (#604, #934)"; // gsd-allow-legacy-name
const migration = {
  ...
  destructive: true,
  plan(ctx) {
    const pristineGsdRoot = node_path_1.default.join(
      ctx.configDir,
      "gsd-pristine",
      "get-shit-done",
    ); // gsd-allow-legacy-name
    // Idempotency: if the stale pristine subdir doesn't exist, nothing to do.
    if (!node_fs_1.default.existsSync(pristineGsdRoot)) return [];
    ...
    try {
      if (node_fs_1.default.lstatSync(pristineParent).isSymbolicLink()) return [];
    } catch {
      return [];
    }
    try {
      if (node_fs_1.default.lstatSync(pristineGsdRoot).isSymbolicLink()) return []; // gsd-allow-legacy-name
    } catch {
      return [];
    }
    ...
    walkPristineFiles(
      ctx.configDir,
      node_path_1.default.posix.join("gsd-pristine", "get-shit-done"),
      baseResolved,
      relPaths,
    ); // gsd-allow-legacy-name
    const actions = [];
