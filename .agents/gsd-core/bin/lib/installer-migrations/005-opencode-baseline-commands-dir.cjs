...
function walkFiles(root, relDir, files) {
  ...
  if (!node_fs_1.default.existsSync(dir)) return;
...
function scanCommandsSurface(configDir) {
  ...
  if (!node_fs_1.default.existsSync(fullPath)) return [];
...
const migration = {
  ...
  plan: ({ configDir, baselineScan, classifyArtifact }) => {
    ...
    for (const relPath of scanCommandsSurface(configDir)) {
      ...
      if (
        artifact.classification === "managed-pristine" ||
        artifact.classification === "managed-modified"
      ) {
        actions.push({
          type: "record-baseline",
          relPath,
          reason:
            "existing manifest-managed OpenCode commands/ file included in first-time migration baseline",
        });
        continue;
      }
      ...
      if (isStaleGsdLookingPath(relPath)) {
        actions.push({
          ...
          currentHash,
          prompt:
            "Choose whether to remove this stale-looking GSD artifact or keep it as user-owned.",
          choices: ["keep", "remove"],
        });
        continue;
      }
    ...
    return actions.sort(
      (left, right) =>
        baselineActionRank(left) - baselineActionRank(right) ||
