...
function isValidItem(item) {
  ...
  if (!exports.VALID_STATUS.includes(i.status)) return false;
...
function isValidReport(report) {
  ...
  if (
    typeof c.applicable !== "number" ||
    typeof c.resolved !== "number" ||
    typeof c.unresolved !== "number"
  ) {
    return false;
  }
...
function validateResolution(r, validators) {
  ...
  if (r.status !== "resolved" && r.verification != null) {
    throw new Error(
      `verification must be null unless status is "resolved" (got "${r.verification}") for ${key}`,
    );
  }
  ...
  if (r.status === "resolved") {
    ...
    if (tier == null) {
      throw new Error(
        `resolved requires a verification tier (one of: ${validators.verification.join(", ")}) for ${key}`,
      );
    }
    if (!validators.verification.includes(tier)) {
      throw new Error(
        `invalid verification "${tier}" for ${key} — must be one of: ${validators.verification.join(", ")}`,
      );
    }
...
function analyzeCoverage(items, resolutions = [], validators) {
  ...
  for (const item of items) {
    if (!validCategories.has(item.category)) {
      throw new Error(
        `item ${key(item)} has unknown category "${item.category}" — not one of: ${validators.categories.join(", ")}`,
      );
    }
  ...
  for (const k of resMap.keys()) {
    if (!itemKeys.has(k)) {
      throw new Error(
        `unknown resolution for ${k} — no matching proposed item (typo'd category or non-applicable shape?)`,
      );
    }
  }
...
function projectProhibitions(items) {
  ...
  for (const item of items) {
    ...
    if ((kind === "node-test" || kind === "lint-rule") && targetOk) {
      ...
      if (
        typeof p.check_violation_fixture === "string" &&
        p.check_violation_fixture.trim() !== ""
      ) {
...
function dispositionForUnverifiableTruth(truth, context = {}) {
  ...
  if (tier !== "backstop") {
    return {
      ...
      tier,
      reason:
        "inferable truth — verified normally (no abstention; ADR-550 D4 over-abstention guard)",
    };
  }
  ...
  return {
    ...
    tier,
    reason:
      "backstop truth confirmed by explicit evidence (a passing wired held-out/property test or directly-observed behavior)",
  };
}
/**
...
function runProbeCli(analyze, options) {
  ...
  try {
    ...
    if (!isValidReport(report)) {
      throw new Error(
        "adapter returned a structurally-invalid coverage report (expected { items[], coverage{ applicable, resolved, unresolved, byVerification } })",
      );
    }
