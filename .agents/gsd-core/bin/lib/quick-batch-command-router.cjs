...
function parseJsonArg(raw, label) {
  ...
  if (!parsed.ok) {
    return {
      ok: false,
      reason: `${label} is not valid JSON: ${parsed.error ?? "unknown parse error"}`,
    };
  }
    ...
    handlers: {
      ...
      create: () => {
        ...
        if (!filePath) {
          return makeInvalidArgs(
            "--file",
            "Usage: gsd-tools quick-batch create --file <path> [--base-revision <sha>] [--options <json>]",
            ERROR_REASON.USAGE,
          );
        }
        ...
        if (!parsed.ok) {
          return makeInvalidArgs(
            "--file",
            parsed.reason ?? "unable to parse task list",
            ERROR_REASON.USAGE,
          );
        }
      ...
      update: () => {
        ...
        if (!batchId) {
          return makeInvalidArgs(
            "--batch",
            "Usage: gsd-tools quick-batch update --batch <id> --updates <json>",
            ERROR_REASON.USAGE,
          );
        }
      ...
      resume: () => {
        ...
        if (!batchId) {
          return makeInvalidArgs(
            "--batch",
            "Usage: gsd-tools quick-batch resume --batch <id> [--current-base-revision <sha>]",
            ERROR_REASON.USAGE,
          );
        }
        ...
        emit(
          qb.resumeBatch(
            cwd,
            batchId,
            currentBaseRevision !== undefined ? { currentBaseRevision } : {},
          ),
        );
      },
      ...
      "effective-concurrency": () => {
        ...
        output(
          {
            concurrency: dispatch.computeEffectiveConcurrency({
              jobs,
              taskCount,
              capacity,
              isolation,
      ...
      "merge-eligible": () => {
        ...
        output(
          {
            mergeable: dispatch.computeMergeOrder(
              waveOrderResult.value,
              new Set(readyResult.value),
            ),
          },
          raw,
        );
      ...
      "spawn-plan": () => {
        ...
        output(
          dispatch.computeSpawnPlan({
            eligibleIds: eligibleResult.value,
            capacity,
            currentInFlight,
            refused,
          }),
      ...
      "cleanup-entry": () => {
        ...
        output(
          dispatch.buildCleanupManifestEntry({
