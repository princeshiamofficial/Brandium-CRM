...
function resolveCapabilityState(input) {
  ...
  for (const capId of Object.keys(capabilitiesMap)) {
    ...
    const skills = Array.isArray(skillsRaw) ? skillsRaw.filter((s) => typeof s === "string") : [];
    // ── installed ──────────────────────────────────────────────────────────────
    ...
    const activationKey =
      typeof capObj["activationKey"] === "string" && capObj["activationKey"].length > 0
        ? capObj["activationKey"]
        : undefined;
    const configActivation =
...
function _loadInstalledSkillsManifest(configDir) {
  ...
  if (!node_fs_1.default.existsSync(skillsDir)) return manifest;
  let entries;
  try {
...
function _loadFlatCommandsGsdManifest(commandsParentDir, workflowsDir) {
  ...
  const flatWorkflowsDir =
    workflowsDir || node_path_1.default.resolve(commandsParentDir, "..", "gsd-core", "workflows");
  const manifest = new Map();
  let entries;
  try {
  ...
  for (const entry of entries) {
    ...
    try {
      const content = node_fs_1.default.readFileSync(
        node_path_1.default.join(commandsParentDir, entry.name),
        "utf8",
      );
      manifest.set(stem, parseRequires(content));
      manifest.set(`_calls_agents_${stem}`, [
        ...new Set([...parseCallsAgents(content), ...workflowAgentRefs(content, flatWorkflowsDir)]),
      ]);
    } catch {
      manifest.set(stem, []);
      manifest.set(`_calls_agents_${stem}`, []);
...
function resolveCapabilityRuntimeState(cwd, runtimeConfigDir, configOverride, runtimeOverride) {
  ...
  try {
    ...
    surfacedSkills = surfaceResult.skills instanceof Set ? surfaceResult.skills : new Set();
  } catch (err) {
