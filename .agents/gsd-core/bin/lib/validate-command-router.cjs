...
function routeValidateCommand({ verify, args, cwd, raw, output: outputFn, error }) {
  routeCjsCommandFamily({
    ...
    unknownMessage: (_subcommand, available) =>
      `Unknown validate subcommand. Available: ${available.join(", ")}`,
    handlers: {
      ...
      context: () => {
        const opts = (0, command_arg_projection_cjs_1.parseNamedArgsOrExit)(
          args,
          { valueFlags: ["tokens-used", "context-window"], booleanFlags: ["json"], positionals: 2 },
          error,
        );
        ...
        const threadCmd = String(
          (0, runtime_slash_cjs_1.formatGsdSlash)(
            "thread",
        ...
        const RECOMMENDATIONS = {
          ...
          [context_utilization_cjs_1.STATES.WARNING]:
            `Context is approaching the fracture zone — consider ${threadCmd} to continue in a fresh window.`,
          [context_utilization_cjs_1.STATES.CRITICAL]:
            `Reasoning quality may degrade past 70% utilization (fracture point). Run ${threadCmd} now to preserve output quality.`,
        };
        let classified;
        try {
          classified = (0, context_utilization_cjs_1.classifyContextUtilization)(
