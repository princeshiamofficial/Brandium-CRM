...
function normalizeFallowReport(report) {
  ...
  const unusedExports = (
    Array.isArray(deadCodeRaw?.unused_exports) ? (deadCodeRaw?.unused_exports ?? []) : []
  ).filter((x) => x !== null && typeof x === "object");
  const unusedFiles = (
    Array.isArray(deadCodeRaw?.unused_files) ? (deadCodeRaw?.unused_files ?? []) : []
  ).filter((x) => x !== null && typeof x === "object");
  const circularDeps = (
    Array.isArray(deadCodeRaw?.circular_dependencies)
      ? (deadCodeRaw?.circular_dependencies ?? [])
      : []
  ).filter((x) => x !== null && typeof x === "object");
  const cloneGroups = (
    Array.isArray(duplicationRaw?.clone_groups) ? (duplicationRaw?.clone_groups ?? []) : []
  ).filter((x) => x !== null && typeof x === "object");
  const findings = [];
