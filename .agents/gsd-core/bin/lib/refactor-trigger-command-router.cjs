...
function disabledResponse() {
  return {
    disabled: true,
    message:
      "refactor-trigger is not enabled. Enable with: gsd config-set refactor.trigger_enabled true",
  };
}
// ─── Arg parsing ────────────────────────────────────────────────────────────
...
function resolveConfinedPath(cwd, relFile) {
  ...
  if (resolved !== root && !resolved.startsWith(root + node_path_1.default.sep)) return null;
  try {
    if (!node_fs_1.default.lstatSync(resolved).isFile()) return null;
  } catch {
    return null;
  }
  return resolved;
...
function readEvalConfig(cwd) {
  ...
  const threshold = resolveConfigKey("refactor.complexity_threshold", {
    config,
    cwd,
    registry,
  }).value;
  const jumpDelta = resolveConfigKey("refactor.complexity_jump_delta", {
    config,
    cwd,
    registry,
  }).value;
  ...
  const windowsEnforce = resolveConfigKey("workflow.windows_enforce", {
    config,
    cwd,
    registry,
  }).value;
...
function strictNotEnforcingWarning(complexity, ledgerRecorded, windowsEnforce) {
  if (!ledgerRecorded) {
    return {
      ...
      message:
        "refactor.trigger_strict is on, but the broken-windows capability is unavailable, so ship will not " +
  ...
  if (!windowsEnforce) {
    return {
      reason: complexity.REASON.REFACTOR_STRICT_NOT_ENFORCING,
      message:
        "refactor.trigger_strict is on, but workflow.windows_enforce is off, so ship will not actually be " +
...
function loadWindowsOrDegrade(cwd, windowsOverride) {
  ...
  try {
    ...
    return {
      ok: false,
      note: "broken-windows capability unavailable — proposal recorded locally only",
    };
  }
...
function recordStrictWindow(cwd, padded, target, windowsOverride) {
  ...
  try {
    ...
    return {
      recorded: false,
      note: `failed to record broken-windows entry: ${e instanceof Error ? e.message : String(e)}`,
    };
  }
}
/**
...
function resolveLedgerWindow(cwd, padded, file, line, kind, reasonText, windowsOverride) {
  ...
  if (!entry) {
    return {
      resolved: false,
      note: "no open broken-windows entry found for this phase/target — nothing to resolve",
    };
  }
  const now = new Date().toISOString();
  try {
    ...
    return {
      resolved: false,
      note: `failed to resolve broken-windows entry: ${e instanceof Error ? e.message : String(e)}`,
    };
  }
}
// ─── evaluate ────────────────────────────────────────────────────────────────
  ...
  for (const relFile of touched) {
    ...
    if (confined === null) {
      analyzed.push({
        file: relFile,
        ok: false,
        reason: complexity.REASON.REFACTOR_FILE_UNREADABLE,
      });
      continue;
    }
    let source;
...
function finalizeEvaluation(opts) {
  ...
  const nextBaselineValue = complexity.nextBaseline(baselineRead.baseline, analyzed, {
  ...
  if (
    evalConfig.strict &&
    evaluation.verdict === complexity.VERDICT.TRIGGERED &&
    evaluation.target
  ) {
    ...
    const warning = strictNotEnforcingWarning(
      complexity,
      ledgerRecorded,
      evalConfig.windowsEnforce,
    );
...
function handleEvaluate(args, cwd, raw, c, complexity, git, windowsOverride) {
  ...
  if (resolved === null) {
    c.output(
      {
        verdict: complexity.VERDICT.SKIPPED,
        reason: complexity.REASON.REFACTOR_INVALID_PHASE,
  ...
  if (sinceRef === null) {
    c.output(
      {
        verdict: complexity.VERDICT.SKIPPED,
        reason: complexity.REASON.REFACTOR_GIT_UNAVAILABLE,
        phase: padded,
      },
      raw,
    );
  ...
  if (touched.length === 0) {
    c.output(
      {
        verdict: complexity.VERDICT.BELOW_THRESHOLD,
        reason: complexity.REASON.REFACTOR_NO_TOUCHED_FILES,
        phase: padded,
      },
      raw,
    );
...
function scanProposals(cwd, complexity) {
  ...
  for (const dirName of dirs) {
    ...
    for (const fileName of files) {
      ...
      try {
        const text = node_fs_1.default.readFileSync(
          node_path_1.default.join(full, fileName),
          "utf8",
        );
...
function handleStatus(args, cwd, raw, c, complexity) {
  ...
  if (
    phaseFlag.duplicated ||
    phaseFlag.flagShaped ||
    trimmed === "" ||
    !PHASE_VALUE_RE.test(trimmed)
  ) {
...
function handleDisposition(kind, args, cwd, raw, c, complexity, windowsOverride) {
  ...
  const planningDirPath = planningDir(cwd);
  const baselineRead = complexity.readBaseline(planningDirPath);
  const nextBaselineValue = complexity.reanchorBaseline(baselineRead.baseline, key, liveScore, {
...
function routeRefactorTriggerCommand({
  args,
  cwd,
  raw,
  error,
  ...
  routeHubCommandFamily({
    ...
    handlers: {
      evaluate: () =>
        withActiveComplexity(cwd, raw, c, _complexity, (complexity) => {
          // eslint-disable-next-line @typescript-eslint/no-require-imports
      ...
      status: () =>
        withActiveComplexity(cwd, raw, c, _complexity, (complexity) =>
          handleStatus(args, cwd, raw, c, complexity),
        ),
      accept: () =>
        withActiveComplexity(cwd, raw, c, _complexity, (complexity) =>
          handleDisposition("accept", args, cwd, raw, c, complexity, _windows),
        ),
      decline: () =>
        withActiveComplexity(cwd, raw, c, _complexity, (complexity) =>
          handleDisposition("decline", args, cwd, raw, c, complexity, _windows),
        ),
    },
    unknownMessage: (subcommand, available) =>
