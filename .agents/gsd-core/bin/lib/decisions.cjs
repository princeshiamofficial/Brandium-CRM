 ...
 */
const bulletColonRe =
  /^\s*-\s+\*\*D-([A-Za-z0-9][A-Za-z0-9_-]*)(?:\s*\[([^\]]+)\])?[^:*]*:\*\*\s*(.*)$/;
/**
...
function extractDecisions(content) {
  ...
  const { text: stripped, unterminatedFence } = (0, markdown_sectionizer_cjs_1.stripFencedCode)(
    content,
  );
