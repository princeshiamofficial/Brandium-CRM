...
var UnicodeTables = class {
  ...
  static CATEGORIES = new LazyMap({
    ...
    Emoji_Component: () =>
      new UnicodeRangeTable(decodeRanges("jBHHGJB0+H2G2Gsp3B3+8B3+8BBYB8PEBxtBDBtzhY-CB", false)),
    ...
    Quotation_Mark: () =>
      new UnicodeRangeTable(decodeRanges("iBFFkEQQ96HHBaBBowDqOqOBCBOCBixzBDB+FFF7CBB", false)),
    ...
    White_Space: () =>
      new UnicodeRangeTable(decodeRanges("JEBTlDlDbgvFgvFgsCKBeBBGwBwBh9DAB", false)),
  });
  static get Upper() {
...
var Unicode = class Unicode {
  ...
  static isPrint(r) {
    if (r <= Unicode.MAX_LATIN1)
      return (r >= 32 && r < Unicode.MAX_ASCII) || (r >= 161 && r !== 173);
...
for (let i = 0; i < FAST_PATH_TABLE_SIZE; i++)
  WORD_RUNE_TABLE[i] =
    (97 <= i && i <= 122) || (65 <= i && i <= 90) || (48 <= i && i <= 57) || i === 95 ? 1 : 0;
    ...
    if (globalThis.TextEncoder) {
      ...
      for (let i = 0; i < str.length; i++) {
        ...
        if (c < 128) out[p++] = c;
          ...
          (c & 64512) === Unicode.MIN_HIGH_SURROGATE &&
          i + 1 < str.length &&
          (str.charCodeAt(i + 1) & 64512) === Unicode.MIN_LOW_SURROGATE
        ) {
          c =
...
var MachineUTF16Input = class extends MachineInputBase {
  ...
  step(pos) {
    ...
    if (c2 >= Unicode.MIN_LOW_SURROGATE && c2 <= Unicode.MAX_LOW_SURROGATE)
      return (
        (((c1 - Unicode.MIN_HIGH_SURROGATE) * 1024 +
...
var Machine = class Machine {
  ...
  step(runq, nextq, pos, nextPos, c, nextCond, anchor, atEnd) {
    ...
    for (let j = 0; j < runq.size; j++) {
      const pc = runq.densePcs[j];
      const capOffset = j * this.ncap;
      if (longest && this.matched && this.ncap > 0 && this.matchcap[0] < runq.denseCaps[capOffset])
        continue;
      const i = this.prog.inst[pc];
      let add = false;
      switch (i.op) {
...
var DFA = class DFA {
  ...
  evictCache() {
    ...
    for (const bucket of this.stateCache.values())
      for (let i = 0; i < bucket.length; i++) allStates.push(bucket[i]);
  ...
  step(state, charCode, anchor) {
    ...
    if (charCode <= Unicode.MAX_LATIN1)
      if (anchor === RE2Flags.UNANCHORED) state.nextLatin1[charCode] = nextState;
    ...
    switch (this.op) {
      ...
      case Regexp.Op.CONCAT:
        for (let sub of this.subs)
          if (sub.op === Regexp.Op.ALTERNATE) out += `(?:${sub.appendTo()})`;
          else out += sub.appendTo();
        break;
    ...
    switch (this.op) {
      ...
      case Regexp.Op.CHAR_CLASS:
        ...
        for (let i = 0; i < this.runes.length; i++)
          if (this.runes[i] !== that.runes[i]) return false;
        break;
      case Regexp.Op.ALTERNATE:
      case Regexp.Op.CONCAT:
        if (this.subs.length !== that.subs.length) return false;
        for (let i = 0; i < this.subs.length; ++i)
          if (!this.subs[i].equals(that.subs[i])) return false;
        break;
      case Regexp.Op.STAR:
      case Regexp.Op.PLUS:
      case Regexp.Op.QUEST:
      ...
      for (const sub of pf.subs) {
        ...
        if (s.type !== Prefilter.Type.NONE)
          if (s.type === Prefilter.Type.AND)
            for (let j = 0; j < s.subs.length; j++) newSubs.push(s.subs[j]);
          else newSubs.push(s);
      }
...
const code1 = [48, 57];
const code2 = [9, 10, 12, 13, 32, 32];
const code3 = [48, 57, 65, 90, 95, 95, 97, 122];
...
const code4 = [48, 57, 65, 90, 97, 122];
const code5 = [65, 90, 97, 122];
const code6 = [0, 127];
const code7 = [9, 9, 32, 32];
const code8 = [0, 31, 127, 127];
const code9 = [48, 57];
const code10 = [33, 126];
const code11 = [97, 122];
const code12 = [32, 126];
const code13 = [33, 47, 58, 64, 91, 96, 123, 126];
const code14 = [9, 13, 32, 32];
const code15 = [65, 90];
const code16 = [48, 57, 65, 90, 95, 95, 97, 122];
const code17 = [48, 57, 65, 70, 97, 102];
...
var Parser = class Parser {
  ...
  static ASCII_TABLE = new UnicodeRangeTable(new Uint32Array([0, 127, 1]));
  static ASCII_FOLD_TABLE = new UnicodeRangeTable(
    new Uint32Array([0, 127, 1, 383, 383, 1, 8490, 8490, 1]),
  );
    ...
    switch (re.op) {
      ...
      case Regexp.Op.CHAR_CLASS:
        for (let i = 0; i < re.runes.length; i += 2)
          if (re.runes[i] <= r && r <= re.runes[i + 1]) return true;
        return false;
      case Regexp.Op.ANY_CHAR_NOT_NL:
  ...
  repeat(op, min, max, beforePos, t, lastRepeatPos) {
    ...
    this.stack[n - 1] = re;
    this.checkLimits(re);
    if (op === Regexp.Op.REPEAT && (min >= 2 || max >= 2) && !this.repeatIsValid(re, 1e3))
  ...
  parsePerlFlags(t) {
    ...
    if (s.startsWith("(?P<") || s.startsWith("(?<")) {
      ...
      if (!Parser.isValidCaptureName(name))
        throw new RE2JSSyntaxException(Parser.ERR_INVALID_NAMED_CAPTURE, s.substring(0, end + 1));
...
var RE2 = class RE2 {
  ...
  findUTF8Submatch(b) {
    ...
    for (let i = 0; i < ret.length; i++)
      if (2 * i < a.length && a[2 * i] >= 0) ret[i] = b.slice(a[2 * i], a[2 * i + 1]);
    return ret;
  }
  /**
  ...
  findSubmatch(s) {
    ...
    const ret = new Array(1 + this.numSubexp).fill(null);
    for (let i = 0; i < ret.length; i++)
      if (2 * i < a.length && a[2 * i] >= 0) ret[i] = s.substring(a[2 * i], a[2 * i + 1]);
    return ret;
  }
  /**
   * Returns an array holding the index pairs identifying the leftmost match of this regular
  ...
  findAllUTF8Submatch(b, n) {
    const result = this.allMatches(MachineInput.fromUTF8(b), n, (match) => {
      ...
      for (let j = 0; j < slice.length; j++)
        if (match[2 * j] >= 0) slice[j] = b.slice(match[2 * j], match[2 * j + 1]);
      return slice;
    });
    if (result.length === 0) return null;
    return result;
  ...
  findAllSubmatch(s, n) {
    const result = this.allMatches(MachineInput.fromUTF16(s), n, (match) => {
      let slice = new Array((match.length / 2) | 0).fill(null);
      for (let j = 0; j < slice.length; j++)
        if (match[2 * j] >= 0) slice[j] = s.substring(match[2 * j], match[2 * j + 1]);
      return slice;
    });
    if (result.length === 0) return null;
    return result;
