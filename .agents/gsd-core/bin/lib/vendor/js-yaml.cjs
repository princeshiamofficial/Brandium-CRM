(function (global, factory) {
  ...
  function requireSnippet() {
    ...
    function makeSnippet(mark, options) {
      ...
      for (var _i = 1; _i <= options.linesAfter; _i++) {
        ...
        var _line2 = getLine(
          mark.buffer,
          lineStarts[foundLineNo + _i],
          lineEnds[foundLineNo + _i],
          mark.position - (lineStarts[foundLineNo] - lineStarts[foundLineNo + _i]),
          maxLineLength,
        );
        result +=
          common2.repeat(" ", options.indent) +
          padStart((mark.line + _i + 1).toString(), lineNoLength) +
          " | " +
          _line2.str +
          "\n";
      }
  ...
  function requireLoader() {
    ...
    function State(input, options) {
      ...
      this.maxTotalMergeKeys =
        typeof options["maxTotalMergeKeys"] === "number" ? options["maxTotalMergeKeys"] : 1e4;
    ...
    function composeNode(state, parentIndent, nodeContext, allowToSeek, allowCompact) {
      ...
      if (state.tag === null) {
        ...
        for (
          var typeIndex = 0, typeQuantity = state.implicitTypes.length;
        ...
        if (_hasOwnProperty.call(state.typeMap[state.kind || "fallback"], state.tag)) {
          ...
          for (
            var _typeIndex = 0, _typeQuantity = typeList.length;
            _typeIndex < _typeQuantity;
            _typeIndex += 1
          ) {
    ...
    function detectType(state, object, explicit) {
      ...
      for (var index = 0, length = typeList.length; index < length; index += 1) {
        ...
        if (
          ...
          (!type2.instanceOf ||
            (_typeof(object) === "object" && object instanceof type2.instanceOf)) &&
    ...
    function writeNode(state, level, object, block, compact, iskey, isblockseq) {
      ...
      if (duplicate && state.usedDuplicates[duplicateIndex]) {
        ...
        if (state.tag !== null && state.tag !== "?") {
          var tagStr = encodeURI(state.tag[0] === "!" ? state.tag.slice(1) : state.tag).replace(
            /!/g,
            "%21",
          );
          if (state.tag[0] === "!") {
  ...
  var Type = yaml.Type,
    Schema = yaml.Schema,
    FAILSAFE_SCHEMA = yaml.FAILSAFE_SCHEMA,
    JSON_SCHEMA = yaml.JSON_SCHEMA,
    CORE_SCHEMA = yaml.CORE_SCHEMA,
    DEFAULT_SCHEMA = yaml.DEFAULT_SCHEMA,
    load = yaml.load,
    loadAll = yaml.loadAll,
    dump = yaml.dump,
    YAMLException = yaml.YAMLException,
    types = yaml.types,
    safeLoad = yaml.safeLoad,
    safeLoadAll = yaml.safeLoadAll,
