...
const CANONICAL_HEADERS = {
  ...
  deferred: ["future work", "deferred", "future", "later", "follow-up", "next steps"],
  dependencies: [
    "dependencies",
    "depends on",
    "prerequisites",
  ...
  consequences: ["consequences", "implications", "impact", "what this means", "result"],
};
...
function main(argv) {
  ...
  const safePath = (0, security_cjs_1.requireSafePath)(
    opts.input,
    node_path_1.default.resolve(opts.projectDir),
    "ADR input path",
    { allowAbsolute: true },
  );
  const content = node_fs_1.default.readFileSync(safePath, "utf8");
  const parsed = parseAdrMarkdown(content, {
    sourcePath: opts.input ?? undefined,
    format: opts.format,
  });
