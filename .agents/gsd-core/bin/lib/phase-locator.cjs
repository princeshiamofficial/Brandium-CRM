...
const { computeHaltPropagation, buildSummaryFileIndex, isSummaryFileHalted } =
  planDependencyGraphModule;
// eslint-disable-next-line @typescript-eslint/no-require-imports
...
function listArchiveVersionDirs(cwd, wsOverride) {
  ...
  // preserved untouched.
  const milestonesDir = node_path_1.default.join(
    planningDir(cwd, wsOverride ?? undefined),
    "milestones",
  );
  const out = [];
  const seen = new Set();
  ...
  for (const archiveName of flat) {
    pushVersion(
      archiveName.match(/^(v[\d.]+)-phases$/)[1],
      node_path_1.default.join(milestonesDir, archiveName),
    );
  }
...
function listMilestonePhaseDirs(phasesDir, opts = {}) {
  ...
  if (!node_fs_1.default.existsSync(phasesDir)) return { value: [], scope };
  let names;
  try {
...
function listAllPhaseDirs(phasesDir, opts) {
  ...
  if (!node_fs_1.default.existsSync(phasesDir)) return { value: [], scope: SCOPE.COMPLETE };
  let names;
  try {
    names = node_fs_1.default
      .readdirSync(phasesDir, { withFileTypes: true })
...
function getAllArchivedPhaseDirs(cwd) {
  ...
  try {
    const wsEntries = node_fs_1.default
