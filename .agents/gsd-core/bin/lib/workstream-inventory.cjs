...
function countRoadmapPhases(roadmapPath, fallbackCount, cwd, ws, versionOverride) {
  try {
    if (!node_fs_1.default.existsSync(roadmapPath)) return fallbackCount;
...
function parseRoadmapProgressRows(roadmapPath) {
  ...
  const table =
    (0, markdown_table_cjs_1.findTableWithColumns)(content, ["Phase", "Milestone"]) ??
...
function readCurrentMilestoneVersion(statePath, roadmapPath) {
  try {
    const m = node_fs_1.default
      ...
      .match(/^milestone:\s*["']?(v\d+(?:\.\d+)+)["']?/m);
    if (m) return m[1];
  } catch {
    /* no state */
  }
  try {
    const rm = node_fs_1.default
...
function currentMilestoneHeadingShipped(roadmapPath, version) {
  try {
    return isMilestoneShippedInRoadmap(
      node_fs_1.default.readFileSync(roadmapPath, "utf-8"),
      version,
    );
  } catch {
...
function legacyMilestoneShipped(roadmapPath, planningBase) {
  ...
  try {
    if (/SHIPPED/i.test(node_fs_1.default.readFileSync(roadmapPath, "utf-8"))) return true;
  } catch {
    /* no roadmap */
  }
  return false;
...
function renameVerificationLedgerWithRetry(tmpPath, finalPath) {
  ...
  for (let attempt = 0; attempt < LEDGER_RENAME_MAX_ATTEMPTS; attempt++) {
    try {
      ...
      if (
        code &&
        LEDGER_RENAME_RETRY_ERRNOS.has(code) &&
        attempt < LEDGER_RENAME_MAX_ATTEMPTS - 1
      ) {
...
function writeVerificationLedger(wsDir, ledger) {
  ...
  try {
    ...
    try {
      ...
      /* nothing to clean up, or cleanup itself failed — not fatal */
    }
  }
}
function readStateProjection(statePath) {
  try {
    ...
    return {
      status:
        (0, state_document_cjs_1.stateFieldValue)(fm, body, "status", "Status").value || "unknown",
      current_phase: (0, state_document_cjs_1.stateFieldValue)(
        fm,
        body,
        "current_phase",
        "Current Phase",
      ).value,
      last_activity: (0, state_document_cjs_1.stateFieldValue)(
        fm,
        body,
        "last_activity",
        "Last Activity",
...
function workstreamShippedSignal(roadmapPath, planningBase, currentVersion) {
  ...
  const snapshot = node_path_1.default.join(
    planningBase,
    "milestones",
    `${currentVersion}-ROADMAP.md`,
  );
  if (node_fs_1.default.existsSync(snapshot)) return "snapshot";
...
function inspectWorkstream(cwd, name, options = {}) {
  ...
  if (!node_fs_1.default.existsSync(wsDir)) return null;
  const activeWorkstreamName =
    options.active === undefined ? getActiveWorkstream(cwd) : options.active;
  ...
  const currentMilestoneDeclaredEmpty =
    ...
    !headingScoped &&
    (headingFilter.versionSectionFound ||
      headingFilter.missingExplicitVersion ||
      claimedElsewhere.size > 0);
  ...
  const ledgerWinnerByKey = (0, workstream_inventory_builder_cjs_1.pickRollupWinners)(
    rawPhaseEntries,
  ...
  let fallbackPhaseCount = countRoadmapPhases(
    p.roadmap,
    phaseDirNames.length,
    cwd,
    name,
    currentVersion,
  );
...
function getOtherActiveWorkstreamInventories(cwd, excludeWs) {
  return listWorkstreamInventories(cwd)
    .workstreams.filter((inventory) => inventory.name !== excludeWs)
    .filter(
      (inventory) =>
        !(0, workstream_inventory_builder_cjs_1.isCompletedInventory)(inventory.status),
    );
}
module.exports = {
  countPhaseFiles,
