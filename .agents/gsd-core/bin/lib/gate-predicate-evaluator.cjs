...
function evaluateCommandExitZero(predicate, ctx, deps) {
  ...
  if (command.length > COMMAND_MAX_LENGTH) {
    throw new Error(
      `command-exit-zero predicate "command" exceeds max length ${COMMAND_MAX_LENGTH}`,
    );
  }
  ...
  if (rawTimeout !== undefined) {
    if (typeof rawTimeout !== "number" || !Number.isFinite(rawTimeout) || rawTimeout <= 0) {
      throw new Error(
        'command-exit-zero predicate "timeout" must be a positive finite number (seconds)',
      );
    }
  ...
  if (res.timedOut) {
    return {
      block: true,
      message: trimToMax(
        `command timed out after ${Math.round(timeoutMs / 1000)}s: ${res.stderr || interpolated}`,
      ),
...
function evaluateArtifactFrontmatterEquals(predicate, ctx, deps) {
  ...
  const expectedStr =
    typeof expectedValue === "object" ? JSON.stringify(expectedValue) : String(expectedValue);
  // eslint-disable-next-line @typescript-eslint/no-base-to-string
  const actualStr =
    typeof actualValue === "object" ? JSON.stringify(actualValue) : String(actualValue);
  const matches =
    actualValue === expectedValue ||
    (actualValue !== undefined && actualValue !== null && actualStr === expectedStr);
  if (matches) {
    return {
      block: false,
...
function evaluatePredicate(predicate, context, deps) {
  ...
  if (!handler) {
    throw new Error(
      `Unknown predicate kind: "${kind}". Known kinds: ${EVALUATOR_KINDS.join(", ")}`,
    );
  }
