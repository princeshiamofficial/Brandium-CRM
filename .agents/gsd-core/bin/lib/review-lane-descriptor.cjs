...
exports.DOCS_PARITY_VIOLATION =
  exports.LANE_SLUG_RE =
  exports.PARITY_VIOLATION =
  exports.REVIEWER_LANES =
  exports.ARGV_PLACEHOLDER =
    void 0;
...
function mergeReviewerLanes(firstParty, registry) {
  ...
  for (const cap of Object.values(capabilities)) {
    ...
    if (!slug || !exports.LANE_SLUG_RE.test(slug)) continue;
...
function checkReviewerLaneParity(input) {
  ...
  for (const raw of rawLanes) {
    ...
    if (seenSlug.has(slug)) add(exports.PARITY_VIOLATION.DUPLICATE_SLUG, slug);
    seenSlug.add(slug);
    const flags = Array.isArray(lane.flags) ? lane.flags : [];
    for (const flag of flags) {
      if (typeof flag !== "string") continue;
      if (seenFlag.has(flag)) add(exports.PARITY_VIOLATION.DUPLICATE_FLAG, flag);
      seenFlag.add(flag);
    }
    ...
    if (section !== null) {
      if (seenSection.has(section)) add(exports.PARITY_VIOLATION.DUPLICATE_SECTION, section);
      seenSection.add(section);
    }
  }
  // --- descriptor <-> roster ---
  const rosterSet = new Set(
    (Array.isArray(roster) ? roster : []).filter((x) => typeof x === "string"),
  );
  for (const slug of rosterSet) {
    if (!seenSlug.has(slug)) add(exports.PARITY_VIOLATION.ROSTER_SLUG_UNDECLARED, slug);
  }
  for (const slug of seenSlug) {
    if (!rosterSet.has(slug)) add(exports.PARITY_VIOLATION.DESCRIPTOR_LANE_NOT_IN_ROSTER, slug);
  }
  // --- descriptor <-> registry ---
  //
  ...
  const registrySet = new Set(
    (Array.isArray(input.registry) ? input.registry : []).filter((x) => typeof x === "string"),
  );
  for (const slug of registrySet) {
    if (!seenSlug.has(slug)) add(exports.PARITY_VIOLATION.REGISTRY_LANE_UNDECLARED, slug);
  }
  for (const slug of seenSlug) {
    if (!registrySet.has(slug)) add(exports.PARITY_VIOLATION.DESCRIPTOR_LANE_NOT_IN_REGISTRY, slug);
  }
...
function checkReviewerDocsParity(input) {
  ...
  const rawLanes = Array.isArray(input?.descriptor) ? input.descriptor : [];
  for (const raw of rawLanes) {
    if (raw === null || typeof raw !== "object") {
  ...
  const rawDocs = input?.docs !== null && typeof input?.docs === "object" ? input.docs : {};
