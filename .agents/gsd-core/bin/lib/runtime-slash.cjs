...
function resolveExplicitRuntime(projectDir, env = process.env) {
  const envRuntime = (0, runtime_name_policy_cjs_1.resolveRuntimeNameFromCandidates)(
    env["GSD_RUNTIME"],
  );
    ...
    try {
      ...
      if (node_fs_1.default.existsSync(configPath)) {
        ...
        if (parsed && typeof parsed === "object" && "runtime" in parsed) {
          const configRuntime = (0, runtime_name_policy_cjs_1.resolveRuntimeNameFromCandidates)(
            parsed["runtime"],
          );
...
function resolveRuntime(projectDir) {
  ...
  const markerRuntime = (0, runtime_name_policy_cjs_1.resolveRuntimeNameFromCandidates)(
