...
function scanPhasePlans(phaseDir) {
  ...
  const supersededPlanFiles = allPlanFiles.filter((f) =>
    isPlanSuperseded((0, node_path_1.join)(phaseDir, f)),
  );
  const planFiles =
    supersededPlanFiles.length === 0
      ? allPlanFiles
  ...
  const countableSummaryFiles = summaryFiles.filter(
    (f) => !isSummaryFileBlocked((0, node_path_1.join)(phaseDir, f)),
  );
