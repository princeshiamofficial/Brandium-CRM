...
function checkW021(content) {
  ...
  const UNPREFIXED_PHASE_RE =
    /^#{2,4}\s*(?:\[[^\]]{1,200}\]\s*)?Phase\s+(\d+[A-Za-z]?(?:\.\d+)*)(?:\s*\([^)\n]{0,200}\))?\s*:/i;
...
function routeRoadmapCommand({ roadmap, args, cwd, raw, error }) {
  routeCjsCommandFamily({
    ...
    unknownMessage: (_subcommand, available) =>
      `Unknown roadmap subcommand. Available: ${available.join(", ")}`,
    handlers: {
      ...
      validate: () => {
        ...
        if (contentAfterBom.startsWith("---")) {
          ...
          if (!closeMatch) {
            warnings.push({
              code: "V003",
              message: "ROADMAP.md frontmatter is malformed (unterminated --- fence)",
            });
          }
        }
        ...
        if (!hasPhaseEntry && !warnings.some((w) => w.code === "V002")) {
          warnings.push({
            code: "V004",
            message:
              "ROADMAP.md contains no recognizable phase entries (no phase headings, bullet entries, or table rows)",
          });
        }
