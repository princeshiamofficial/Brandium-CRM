...
const FAST_CODEBASE_MAP_FILES = ["STACK.md", "INTEGRATIONS.md", "ARCHITECTURE.md", "STRUCTURE.md"];
...
function hasCodeFilesInternal(dir, depth = 0) {
  ...
  for (const entry of entries) {
    if (entry.isFile() && CODE_EXTENSIONS.has(node_path_1.default.extname(entry.name))) return true;
    if (entry.isDirectory() && !CODE_SCAN_SKIP_DIRS.has(entry.name)) {
      if (hasCodeFilesInternal(node_path_1.default.join(dir, entry.name), depth + 1)) return true;
    }
  }
  return false;
}
...
function listPlanningDocCandidates(cwd) {
  ...
  for (const root of roots) {
    ...
    if (node_fs_1.default.existsSync(full)) visit(full, root, 0);
  }
...
function listCodebaseMapFiles(cwd) {
  ...
  if (!node_fs_1.default.existsSync(codebaseDir)) return [];
  return REQUIRED_CODEBASE_MAP_FILES.filter((file) =>
    node_fs_1.default.existsSync(node_path_1.default.join(codebaseDir, file)),
  );
}
...
function nextAction(params) {
  if (params.isBrownfield && params.needsOnboardCodebaseMap) {
    return {
      kind: "map-codebase",
      command: params.fastMode
        ? params.handoffCommands.map_codebase_fast
        : params.handoffCommands.map_codebase,
  ...
  if (params.fastMode && params.mapReadiness === "fast" && !params.projectExists) {
    return {
      ...
      reason:
        "The fast map is enough for lightweight onboarding, but project setup still requires the complete codebase map.",
    };
  }
...
function buildOnboardProjection(cwd, options) {
  ...
  const projectExists =
    node_fs_1.default.existsSync(projectRootPath) ||
    node_fs_1.default.existsSync(projectScopedPath);
  const requirementsExists = node_fs_1.default.existsSync(
    node_path_1.default.join(planningDir(cwd), "REQUIREMENTS.md"),
  );
  const roadmapExists = node_fs_1.default.existsSync(
    node_path_1.default.join(planningDir(cwd), "ROADMAP.md"),
  );
  const stateExists = node_fs_1.default.existsSync(
    node_path_1.default.join(planningDir(cwd), "STATE.md"),
  );
  const onboardingSummaryPath = node_path_1.default.join(
    planningRoot(cwd),
    "onboarding",
    "SUMMARY.md",
  );
  ...
  return {
    ...
    codebase_dir_exists: node_fs_1.default.existsSync(
      node_path_1.default.join(planningRoot(cwd), "codebase"),
    ),
    ...
    project_path: toPosixPath(
      node_fs_1.default.existsSync(projectRootPath) ? projectRootPath : projectScopedPath,
    ),
