...
function buildWorkstreamInventory(inputs) {
  ...
  const shippedSignal =
    milestoneShippedSignal !== undefined
      ? milestoneShippedSignal
      : milestoneShipped
        ? "legacy"
        : null;
  ...
  for (const dir of [...phaseDirNames].sort()) {
    ...
    const status =
      (counts?.complete ?? false) ? "complete" : planCount > 0 ? "in_progress" : "pending";
  ...
  const status = useDerived ? "milestone complete" : artifactOverride ? "in_progress" : fieldStatus;
