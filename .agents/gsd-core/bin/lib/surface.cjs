...
function writeSurface(runtimeConfigDir, surfaceState) {
  (0, shell_command_projection_cjs_1.platformWriteSync)(
    node_path_1.default.join(runtimeConfigDir, SURFACE_FILE_NAME),
...
function resolveSurface(runtimeConfigDir, manifest, clusterMap, registry, surfaceOverride) {
  ...
  if (baseResolved.skills === "*") {
    ...
    if (
      registry &&
      registry.capabilityClusters &&
      typeof registry.capabilityClusters === "object"
    ) {
...
function applySurface(runtimeConfigDir, layout, manifest, clusterMap, registry, opts, deps = {}) {
  if (
    node_path_1.default.resolve(runtimeConfigDir) !== node_path_1.default.resolve(layout.configDir)
  ) {
  ...
  const hasCandidateState =
    opts !== undefined && Object.prototype.hasOwnProperty.call(opts, "surfaceState");
  ...
  const _resolvedTarget = (0, shell_command_projection_cjs_1.posixNormalize)(
    node_path_1.default.resolve(layout.configDir),
  );
  ...
  const _pathPrefix = runtimeArtifactConversion._computePathPrefix({
    isGlobal: _isGlobal,
    isOpencode: _isOpencode,
    isWindowsHost: _isWindowsHost,
    resolvedTarget: _resolvedTarget,
    homeDir: _homeDir,
  });
  const _attribution = opts?.resolveAttribution
    ? opts.resolveAttribution(layout.runtime)
    : undefined;
  ...
  const _surfaceState =
    candidateState === undefined ? readSurface(layout.configDir) : candidateState;
  const _baseProfileName =
    _surfaceState && _surfaceState.baseProfile
      ? _surfaceState.baseProfile
      : readActiveProfile(layout.configDir) || "full";
  const _hasSurfaceMods =
    !!_surfaceState &&
  ...
  try {
    ...
    if (hasCandidateState) {
      if (candidateState === null) {
        node_fs_1.default.rmSync(node_path_1.default.join(runtimeConfigDir, SURFACE_FILE_NAME), {
          force: true,
        });
...
function pruneSkillDirs(skillsDir, retainedNames, prefix, manifest) {
  if (!node_fs_1.default.existsSync(skillsDir)) return;
  ...
  for (const entry of node_fs_1.default.readdirSync(skillsDir)) {
    ...
    if (!node_fs_1.default.statSync(entryPath).isDirectory()) continue;
    let isGsdOwned;
    if (prefix !== "") {
      ...
      if (canonicalStems && canonicalStems.has(stem)) {
        ...
        node_fs_1.default.existsSync(node_path_1.default.join(entryPath, CAPABILITY_SKILL_MARKER))
      ) {
...
function _syncGsdDir(stagedDir, destDir, kind, manifest, runtime) {
  if (!node_fs_1.default.existsSync(stagedDir)) return;
  ...
  if (kindName === "skills") {
    ...
    const stagedDirs = new Set(
      node_fs_1.default.readdirSync(stagedDir).filter((entry) => {
    ...
    for (const dirName of stagedDirs) {
      ...
      node_fs_1.default.cpSync(node_path_1.default.join(stagedDir, dirName), destSubDir, {
        recursive: true,
      });
    }
    ...
    const kindDestSubpath =
      typeof kind === "object" && kind !== null && kind.destSubpath ? kind.destSubpath : "";
    ...
    for (const file of stagedFiles) {
      ...
      node_fs_1.default.copyFileSync(
        node_path_1.default.join(stagedDir, file),
