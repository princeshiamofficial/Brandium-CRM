...
const RULE_W026 = {
  ...
  severity: SEVERITY.WARNING,
  description:
    "STATE says milestone complete but ROADMAP lists an unstarted phase for that milestone",
  repairable: false,
  check: (snapshot) => {
    ...
    for (const phaseId of snapshot.currentMilestoneRoadmapPhaseIds.value) {
      ...
      const hasDirectory =
        matchPhaseDirs(snapshot.allPhaseDirNames.value, normalized, snapshot.phaseIdConvention)
          .matches.length > 0;
