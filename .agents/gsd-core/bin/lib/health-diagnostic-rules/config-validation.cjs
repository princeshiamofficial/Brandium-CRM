    ...
    return [
      {
        ...
        remedy: {
          action: REMEDY_ACTION.ADD_AI_INTEGRATION_PHASE_KEY,
    ...
    for (const [phaseType, tierValue] of Object.entries(configModels)) {
      if (!model_catalog_cjs_1.VALID_PHASE_TYPES.has(phaseType)) {
        diagnostics.push({
          ...
          remedy: adviseRemedy(
            `Valid phase types: ${[...model_catalog_cjs_1.VALID_PHASE_TYPES].join(", ")}`,
          ),
        });
...
const RULES = [
  ...
  },
  {
    code: "E005",
    severity: SEVERITY.ERROR,
    description: "config.json parse error",
    repairable: false,
    check: checkE005,
  },
  {
    code: "W004",
    severity: SEVERITY.WARNING,
    description: "config.json invalid field value",
    repairable: false,
    check: checkW004,
  },
  {
  ...
  },
  {
    ...
    severity: SEVERITY.WARNING,
    description:
      "config.json models entry malformed (unknown phase type, invalid tier, or non-object value)",
    repairable: false,
    check: checkW022,
  },
  {
    ...
    severity: SEVERITY.WARNING,
    description:
      ".planning/ matches a gitignore rule but is still tracked by git (gitignore has no effect on already-tracked files)",
    repairable: false,
    check: checkW029,
  },
];
