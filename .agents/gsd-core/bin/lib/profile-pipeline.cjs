...
function getSessionsDir(overridePath) {
  const dir =
    overridePath || node_path_1.default.join(node_os_1.default.homedir(), ".claude", "projects");
  if (!node_fs_1.default.existsSync(dir)) return null;
  return dir;
}
...
async function cmdExtractMessages(projectArg, options, raw, overridePath) {
  ...
  ensureGsdTempDir();
  const tmpDir = node_fs_1.default.mkdtempSync(
    node_path_1.default.join(GSD_TEMP_DIR, "gsd-pipeline-"),
  );
...
async function cmdProfileSample(overridePath, options, raw) {
  ...
  ensureGsdTempDir();
  const tmpDir = node_fs_1.default.mkdtempSync(
    node_path_1.default.join(GSD_TEMP_DIR, "gsd-profile-"),
  );
