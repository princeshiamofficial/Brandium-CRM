  ...
  if (trimmed.startsWith("{")) {
    try {
      ...
      if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
        ...
        const pidVal = p["pid"];
        const pid =
          typeof pidVal === "number" && Number.isInteger(pidVal) && pidVal > 0 ? pidVal : null;
...
function acquireLock(lockPath, opts) {
  ...
  const maxAttempts =
    opts && Number.isInteger(opts.maxAttempts) && opts.maxAttempts > 0
      ? opts.maxAttempts
      : LOCK_MAX_ATTEMPTS;
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      ...
      try {
        ...
        /* best-effort — release falls back to the token check alone */
      }
    ...
    if (isSameHost(parsed) && parsed.pid !== null) {
      ...
      if (holderVerifiedLive(parsed)) {
        // provably-live same-host holder — blocked.
        const b = blocked(attempt);
        if (b === "retry") continue;
        return b;
      }
...
function releaseLock(handle) {
  ...
  try {
    ...
    /* already gone / stale-stolen / unreadable — nothing of ours to release */
  }
}
module.exports = {
  acquireLock,
