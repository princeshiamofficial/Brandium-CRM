...
const { checkAgentsInstalled, checkCodexModelPosture, checkCodexSandboxPosture } =
  agentInstallCheck;
...
function verifySummaryCore(cwd, summaryPath, checkFileCount, opts) {
  ...
  const isFutureMention = (line) =>
    /\b(?:will(?:\s+(?:add|create|build|land))?(?:[^.])?|(?:next|later|future)\s+phase|planned?|would\s+(?:be|add|create|build)|to\s+be\s+(?:added|created|built)|eventually|not\s+yet)\b/i.test(
      line,
    );
  const patterns = [
    /`([^`]+\.[a-zA-Z]+)`/g,
...
function scanFileWideNegativeGateConflict(content) {
  ...
  const looksLikePath = (token) =>
    !token.includes("*") && (token.includes("/") || /\.[a-zA-Z]{1,6}$/.test(token));
        ...
        while ((gm = grepArgRe.exec(seg)) !== null) {
          ...
          const rawPat =
            gm[2] !== undefined
              ...
              : gm[3] !== undefined
                ? gm[3]
                : gm[4] !== undefined && plausibleBare(gm[4])
                  ? gm[4]
                  : null;
          if (!rawPat) continue;
...
function scanQuantitativeCriteria(content) {
  ...
  for (const zone of zones) {
    ...
    for (const seg of text.split("\n").flatMap(splitShellSegments)) {
      ...
      if (
        /\bwc\b/.test(seg) &&
        /\bgrep\s+(?:-{1,2}[A-Za-z]*x[A-Za-z]*|--line-regexp)\s+['"]?\d+['"]?/.test(seg)
      ) {
        record(
          errors,
          "R5",
...
function cmdVerifyPlanStructure(cwd, filePath, raw) {
  ...
  const fullPath = node_path_1.default.isAbsolute(filePath)
    ? filePath
    : node_path_1.default.join(cwd, filePath);
...
function cmdVerifyArtifacts(cwd, planFilePath, raw) {
  ...
  const fullPath = node_path_1.default.isAbsolute(planFilePath)
    ? planFilePath
    : node_path_1.default.join(cwd, planFilePath);
  const content = (0, shell_command_projection_cjs_1.platformReadSync)(fullPath);
  if (!content) {
...
function collectPromisedFilesAtOrAfterWave(phaseDir, minWave) {
  ...
  for (const planFile of planFiles) {
    ...
    const wave =
      typeof waveRaw === "string"
        ? parseInt(waveRaw, 10)
        : typeof waveRaw === "number"
          ? waveRaw
          : NaN;
...
function cmdVerifyKeyLinks(cwd, planFilePath, raw) {
  ...
  const currentWave =
    typeof currentWaveRaw === "string"
      ? parseInt(currentWaveRaw, 10)
      : typeof currentWaveRaw === "number"
        ? currentWaveRaw
        : 1;
  ...
  function getPromisedFiles() {
    if (promisedFiles === null) {
      promisedFiles = collectPromisedFilesAtOrAfterWave(
        phaseDir,
        isNaN(currentWave) ? 1 : currentWave,
      );
    }
  ...
  for (const link of keyLinks) {
    ...
    if (!sourceContent) {
      ...
      if (isPromised) {
        check["pending"] = true;
        check["detail"] =
          "Source file not yet created — declared in files_modified of a same-or-later-wave plan";
        pendingCount++;
      } else {
        check["detail"] =
      ...
      if (pat.neutralized !== null) {
        ...
        switch (pat.neutralized) {
          ...
          case "unsupported":
            reason =
              "pattern is not valid RE2 syntax — backreferences and look-around are not supported";
            break;
        }
          ...
          if (pat.test(sourceContent)) {
            ...
            if (toPath !== "") {
              ...
              if (!toCheck.safe) {
                ...
                check["detail"] =
                  `Pattern "${link["pattern"]}" not found in source; target path rejected — resolves outside the project directory`;
              } else {
                targetContent = (0, shell_command_projection_cjs_1.platformReadSync)(
                  toCheck.resolved,
                );
              }
            }
...
function resolvePhaseDirByToken(phasesDir, phaseArg) {
  ...
  if (matched) return node_path_1.default.join(phasesDir, matched);
  const check = (0, security_cjs_1.validatePath)(phaseArg, phasesDir);
  if (check.safe && node_fs_1.default.existsSync(check.resolved)) return check.resolved;
  return null;
}
/**
  ...
  if (
    staleArtifacts.some(
      (f) =>
        ...
        (f.endsWith("-SPEC.md") && !f.endsWith("-AI-SPEC.md") && !f.endsWith("-UI-SPEC.md")),
    )
  ) {
    parts.push(
      "Regenerate or manually reconcile VALIDATION.md / SPEC.md against the current decisions.",
    );
  }
  parts.push(
    "Do not hand-inject the newer decisions into a prompt as a substitute for regenerating — that carries the staleness forward.",
  );
  return parts.join(" ");
}
function cmdVerifyContextDrift(cwd, phaseArg, raw) {
  ...
  try {
    const rawCfg = JSON.parse(
      node_fs_1.default.readFileSync(node_path_1.default.join(pDir, "config.json"), "utf-8"),
    );
    wf = rawCfg["workflow"];
  } catch {
    wf = undefined;
  ...
  const message =
    staleArtifacts.length > 0 ? buildContextDriftMessage(staleArtifacts, phaseArg) : "";
  output(
    {
      block,
      skipped: false,
...
function cmdVerifySchemaDrift(cwd, phaseArg, skipFlag, raw) {
  ...
  if (!node_fs_1.default.existsSync(phasesDir)) {
    output(
      { block: false, drift_detected: false, blocking: false, message: "No phases directory" },
      raw,
    );
    return;
  }
  ...
  for (const sf of summaryFiles) {
    executionLog +=
      node_fs_1.default.readFileSync(node_path_1.default.join(phaseDir, sf), "utf-8") + "\n";
  }
  const gitLog = (0, shell_command_projection_cjs_1.execGit)(["log", "--oneline", "--all", "-50"], {
    cwd,
  });
  if (gitLog.exitCode === 0) {
    executionLog += "\n" + gitLog.stdout;
  }
  const result = (0, schema_detect_cjs_1.checkSchemaDrift)(allFiles, executionLog, {
    skipCheck: !!skipFlag,
  });
...
function cmdVerifyCodebaseDrift(cwd, raw) {
  ...
  try {
    ...
    const diff = (0, shell_command_projection_cjs_1.execGit)(
    ...
    const threshold =
      Number.isInteger(wf?.drift_threshold) && wf?.drift_threshold >= 1 ? wf?.drift_threshold : 3;
