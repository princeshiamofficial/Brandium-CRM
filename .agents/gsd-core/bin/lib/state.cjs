...
const { getFieldClassification, getFrontmatterBodySource, frontmatterKeyForBodyField } =
  stateTransitionMod;
...
const STATE_PROGRESS_RESYNC_FIELDS = new Set(["Progress", "Total Plans in Phase", "Total Phases"]);
...
function cmdStateLoad(cwd, raw) {
  ...
  const stateRaw =
    (0, shell_command_projection_cjs_1.platformReadSync)(
      node_path_1.default.join(planDir, "STATE.md"),
    ) || "";
  const configExists = node_fs_1.default.existsSync(
    node_path_1.default.join(planDir, "config.json"),
  );
  const roadmapExists = node_fs_1.default.existsSync(
    node_path_1.default.join(planDir, "ROADMAP.md"),
  );
...
function cmdStatePatch(cwd, patches, raw) {
  ...
  try {
    ...
    readModifyWriteStateMd(
      ...
      cwd,
      { resync: shouldResync, divergedFields, explicitProgressField: shouldResync, preWriteState },
    );
...
function explainUpdateFailure(field) {
  ...
  if (bodySource) {
    ...
    return (
      `Field "${field}" is a body-derived frontmatter key and is not directly writable. ` +
  ...
  if (owningKey) {
    ...
    return (
      `Field "${field}" not found in STATE.md. It is the body source for frontmatter key ` +
  ...
  try {
    ...
    readModifyWriteStateMd(
      ...
      (content) => {
        const result = transitionCore(
          content,
          { kind: "update", field: field, value: value },
          { clock: clock_cjs_1.realClock },
        );
      ...
      cwd,
      {
        ...
        divergedFields,
        authoritativeFm,
        explicitProgressField: shouldResync,
        preWriteState,
      },
    );
    ...
    const reconciled = reconcileReportedFields(
      statePath,
      preWriteState,
      updated ? [field] : [],
      divergedFields,
    ...
    if (updated) {
      ...
      if (!wroteFrontmatter) {
        ...
        const ownLabels = new Set(
          (getFrontmatterBodySource(field) ?? []).map((l) => l.toLowerCase()),
        );
        output(
          {
            updated: true,
...
function scanOutstanding(phasesDir, dir) {
  ...
  const countableSummaries = scan.summaryFiles.filter(
    (f) => !planDependencyGraphMod.isSummaryFileBlocked(node_path_1.default.join(phaseDirPath, f)),
  );
  const outstanding = coreUtilsMod.findUnsummarizedPlans(scan.planFiles, countableSummaries);
  return {
    dir,
    outstanding,
    planCount: scan.planFiles.length,
    summaryCount: countableSummaries.length,
  };
}
...
function cmdStateAdvancePlan(cwd, raw) {
  ...
  const wrote = readModifyWriteStateMd(
    statePath,
    (content) => {
      ...
      const positionPhase = parseProsePhaseField(
        (0, state_document_cjs_1.stateExtractField)(positionScope, "Phase"),
      ).phase;
  ...
  if (!resultData || resultData["error"]) {
    ...
    if (resultData && resultData["reason"] === "ambiguous_position_phase") {
      output(
        {
          error:
            "Current Position section contains more than one Phase: entry — refusing to silently advance the first. Resolve the section to a single current entry and re-run.",
    ...
    if (resultData && resultData["reason"] === "ambiguous_plan_position") {
      output(
        {
          error:
            "STATE.md carries two plan positions with different numbers — refusing to advance either. Resolve them to a single current plan and re-run.",
          reason: resultData["reason"],
    ...
    const diskFacts =
      positionToken !== null ? unsummarizedPlansForPositionPhase(cwd, positionToken) : null;
    if (diskFacts === null) {
      ...
      output(
        { error: advancePlanShapeError(), reason: "plan_position_unreadable" },
        raw,
        undefined,
      );
      return;
...
function cmdStateRecordMetric(cwd, options, raw) {
  ...
  readModifyWriteStateMd(
    ...
    (content) => {
      ...
      // regex.
      const metricsSection = (0, markdown_sectionizer_cjs_1.collectSection)(content, (h) =>
        /^performance metrics$/i.test(h.text.trim()),
      );
      ...
      for (let i = 0; i < lines.length - 1; i++) {
        ...
        const isMetricsHeader =
          normalized.length === METRICS_HEADER.length &&
...
function computeUpdateProgressPreview(statePath, cwd) {
  ...
  const storedMilestone =
    typeof existingFm["milestone"] === "string" ? existingFm["milestone"] : null;
  const builtFm = buildStateFrontmatter(
    preBody,
    cwd,
    storedMilestone,
    readStoredTotalPhases(existingFm),
    readStoredCompletedPhases(existingFm),
    readStoredTotalPlans(existingFm),
    readStoredCompletedPlans(existingFm),
  );
  const progress = builtFm["progress"];
  const percent = progress && typeof progress["percent"] === "number" ? progress["percent"] : null;
  const completedPlans =
    progress && typeof progress["completed_plans"] === "number"
      ? progress["completed_plans"]
      : null;
  const totalPlans =
    progress && typeof progress["total_plans"] === "number" ? progress["total_plans"] : null;
  ...
  if (percent === null || completedPlans === null || totalPlans === null) {
    return {
      withheld: true,
      reason: "progress percent withheld by buildStateFrontmatter — STATE.md left unchanged",
    };
  }
...
function cmdStateUpdateProgress(cwd, raw) {
  ...
  if (preview.withheld) {
    declineNoOp(
      ...
      "updated",
      preview.reason,
      `state update-progress skipped — ${preview.reason}`,
    );
    return;
  }
...
function cmdStateAddDecision(cwd, options, raw) {
  ...
  readModifyWriteStateMd(
    ...
    (content) => {
      ...
      const decisionsPred = (lv, text) =>
        (lv === 2 || lv === 3) &&
        /^(?:Decisions|Decisions Made|Accumulated.*Decisions)$/i.test(text);
      const sectionBody = (() => {
      ...
      if (sectionBody !== null) {
        ...
        _added = true;
        return (
          content.slice(0, sectionBody.bodyStart) + newBody + content.slice(sectionBody.bodyEnd)
        );
      }
  ...
  readModifyWriteStateMd(
    ...
    (content) => {
      ...
      if (sectionSpan !== null) {
        ...
        _added = true;
        return (
          content.slice(0, sectionSpan.bodyStart) + sectionBody + content.slice(sectionSpan.bodyEnd)
        );
      }
...
function cmdStateAddRoadmapEvolution(cwd, options, raw) {
  ...
  readModifyWriteStateMd(
    ...
    (content) => {
      ...
      const accIdx = accHs.findIndex(
        (h) => h.level === 2 && /^accumulated\s+context$/i.test(h.text),
      );
      if (accIdx !== -1) {
        ...
        const subIdx = subHs.findIndex(
          (h) => h.level === 3 && /^roadmap\s+evolution$/i.test(h.text),
        );
        if (subIdx !== -1) {
  ...
  readModifyWriteStateMd(
    ...
    (content) => {
      ...
      const i = hs.findIndex(
        (h) =>
          (h.level === 2 || h.level === 3) &&
          /^(?:Blockers|Blockers\/Concerns|Concerns)$/i.test(h.text),
      );
...
function cmdStateRecordSession(cwd, options, raw) {
  ...
  readModifyWriteStateMd(
    ...
    (content) => {
      ...
      if (options.stopped_at) {
        result = (0, state_document_cjs_1.stateReplaceField)(
          content,
          "Stopped At",
          options.stopped_at,
        );
      ...
      if (options.resume_file !== undefined && options.resume_file !== null) {
        ...
        const newRf = (0, state_document_cjs_1.stateReplaceFieldIfTemplate)(
          content,
          "Resume File",
          resumeFileDefaults,
          "None",
        ...
        if (newRf !== content) {
          ...
          const newRfAlt = (0, state_document_cjs_1.stateReplaceFieldIfTemplate)(
            content,
            "Resume file",
            resumeFileDefaults,
            "None",
      ...
      const callerSuppliedValues = !!(
        options.stopped_at ||
        (options.resume_file !== undefined && options.resume_file !== null)
      );
      ...
      const needsResumeFile =
        options.resume_file !== undefined &&
        options.resume_file !== null &&
        !updated.includes("Resume File");
  ...
  // preservation restored that this transform never touched (#3345's
  // direction).
  const reconciledUpdated = reconcileReportedFields(
    statePath,
    preWriteState,
    updated,
    divergedFields,
...
function matchSessionSection(body) {
  ...
  const isSessionContinuity = (h) =>
    h.level === 2 && h.text.trim().toLowerCase() === "session continuity";
  const section =
    (0, markdown_sectionizer_cjs_1.collectSection)(body, isSession, { levelBounded: true }) ??
    (0, markdown_sectionizer_cjs_1.collectSection)(body, isSessionContinuity, {
      levelBounded: true,
    });
...
function resolveStatePhase(fm, body) {
  ...
  const frontmatterRaw = (0, state_document_cjs_1.stateFieldValue)(
    fm,
    body,
    "current_phase",
    null,
  ).value;
  const legacyRaw = (0, state_document_cjs_1.stateFieldValue)(
    fm,
    currentPositionScope,
    null,
    "Current Phase",
  ).value;
  const currentPositionRaw = (0, state_document_cjs_1.stateFieldValue)(
    fm,
    currentPositionScope,
    null,
    "Phase",
  ...
  return {
    ...
    name:
      (0, state_document_cjs_1.stateFieldValue)(fm, body, "current_phase_name", null).value ??
      (0, state_document_cjs_1.stateFieldValue)(
        fm,
        currentPositionScope,
        null,
        "Current Phase Name",
  ...
  const prosePhase =
    positionSection !== null
      ? parseProsePhaseField(
          (0, state_document_cjs_1.stateFieldValue)(fm, positionSection, null, "Phase").value,
        ).phase
      : null;
  return (
...
function cmdStateSnapshot(cwd, raw) {
  ...
  const totalPhasesRaw = (0, state_document_cjs_1.stateFieldValue)(
    fm,
    body,
    "total_phases",
    "Total Phases",
  ).value;
  const currentPlan = (0, state_document_cjs_1.stateFieldValue)(
    fm,
    body,
    "current_plan",
    "Current Plan",
  ).value;
  const totalPlansRaw = (0, state_document_cjs_1.stateFieldValue)(
    fm,
    body,
  ...
  const progressRaw = (0, state_document_cjs_1.stateFieldValue)(
    fm,
    body,
    "progress",
    "Progress",
  ).value;
  const rawLastActivity =
    (0, state_document_cjs_1.stateFieldValue)(fm, body, null, "Last Activity").value ??
    (0, state_document_cjs_1.stateFieldValue)(fm, body, null, "Last activity").value;
  const proseLastActivity = parseProseLastActivityField(rawLastActivity);
  const lastActivity =
    (0, state_document_cjs_1.stateFieldValue)(fm, body, "last_activity", null).value ??
    proseLastActivity.date ??
    rawLastActivity;
  const lastActivityDesc =
    ...
    ).value ?? proseLastActivity.description;
  ...
  const pausedAt = (0, state_document_cjs_1.stateFieldValue)(
    fm,
    sessionScope,
    "paused_at",
    "Paused At",
  ...
  const decisions = [];
  const decisionsSection = (0, markdown_sectionizer_cjs_1.collectSection)(body, (h) =>
    /^decisions made$/i.test(h.text.trim()),
  );
  const decisionsTable = decisionsSection
    ? (0, markdown_table_cjs_1.parseMarkdownTable)(decisionsSection.body)
    : null;
  ...
  const blockers = [];
  const blockersSection = (0, markdown_sectionizer_cjs_1.collectSection)(
    body,
    (h) => h.level === 2 && h.text.trim().toLowerCase() === "blockers",
    { levelBounded: true },
  );
  if (blockersSection) {
...
function isMilestoneBounded(roadmapRaw, milestone, convention) {
  ...
  const bracketMilestoneHeadingRe = new RegExp(
    `^${bracketMilestoneIntroSrcFor(milestoneInt)}`,
    "i",
  );
  ...
  return (0, markdown_sectionizer_cjs_1.tokenizeHeadings)(roadmapRaw).some(
...
function extractRetiredPhaseNumbers(scope, convention) {
  ...
  const scanScope =
    convention === "bracket" ? (0, markdown_sectionizer_cjs_1.stripFencedCode)(scope).text : scope;
...
function countRoadmapPhaseHeadings(
  ...
  convention,
  retiredPhaseNums,
  applyConventionTokenSentinelRules,
) {
  let count = 0;
  ...
  const phaseHeadingPattern = new RegExp(
    `#{2,4}\\s*${phaseHeadingPrefixSrcFor(PHASE_HEADING_BASELINE.LABEL_ONLY, convention, true)}([\\w][\\w.-]*)(?:\\s*\\([^)\\n]{0,200}\\))?\\s*:`,
    "gi",
  );
  let m;
...
function buildStateFrontmatter(
  ...
  const prosePhase = parseProsePhaseField(
    (0, state_document_cjs_1.stateExtractField)(currentPositionScope, "Phase"),
  );
  const currentPhase =
    (0, state_document_cjs_1.stateExtractField)(bodyContent, "Current Phase") ?? prosePhase.phase;
  const currentPhaseName =
    (0, state_document_cjs_1.stateExtractField)(bodyContent, "Current Phase Name") ??
    prosePhase.name;
  ...
  const totalPlansRaw = (0, state_document_cjs_1.stateExtractField)(
    bodyContent,
    "Total Plans in Phase",
  );
  ...
  const rawLastActivity =
    (0, state_document_cjs_1.stateExtractField)(bodyContent, "Last Activity") ??
    (0, state_document_cjs_1.stateExtractField)(bodyContent, "Last activity");
  const proseLastActivity = parseProseLastActivityField(rawLastActivity);
  const lastActivity = proseLastActivity.date ?? rawLastActivity;
  const lastActivityDesc =
    (0, state_document_cjs_1.stateExtractField)(bodyContent, "Last Activity Description") ??
    proseLastActivity.description;
  ...
  const stoppedAt =
    (0, state_document_cjs_1.stateExtractField)(sessionBodyScope, "Stopped At") ||
    (0, state_document_cjs_1.stateExtractField)(sessionBodyScope, "Stopped at");
      ...
      if (node_fs_1.default.existsSync(phasesDir)) {
        ...
        if (!cached) {
          ...
          try {
            ...
            /* fall through: no roadmap scope → no retired exclusion */
          }
          ...
          const { value: allMatchingDirs, scope: phaseDirScope } = listMilestonePhaseDirs(
            phasesDir,
            {
              cwd,
          ...
          for (const dir of allMatchingDirs) {
            ...
            if (
              retiredPhaseNums.size > 0 &&
              retiredPhaseNums.has(phaseKeyFromDir(dir, phaseConvention))
            )
              continue;
          ...
          cached = (() => {
            ...
            if (assertedMilestoneVersion && roadmapRaw !== null) {
              ...
              milestoneBounded = isMilestoneBounded(
                roadmapRaw,
                String(assertedMilestoneVersion).trim(),
                phaseConvention,
              );
            }
            ...
            const roadmapHasAnyMilestoneSection =
              roadmapRaw !== null && hasAnyMilestoneSection(roadmapRaw);
            const safeToUseRoadmapCount =
              milestoneBounded || (roadmapPhaseCount > 0 && !roadmapHasAnyMilestoneSection);
  ...
  let progressPercent = (0, state_document_cjs_1.computeProgressPercent)(
    completedPlans,
    totalPlans,
    completedPhases,
    totalPhases,
  ...
  if (
    ...
    !milestoneUnbounded &&
    typeof completedPhases === "number" &&
    Number.isFinite(completedPhases) &&
    typeof totalPhases === "number" &&
    Number.isFinite(totalPhases) &&
    totalPhases > 0 &&
...
function readStateHeadFreshness(cwd, stateHead) {
  ...
  if (stamp && head && cwd) {
    ...
    const ancestry = (0, shell_command_projection_cjs_1.execGit)(
      ["merge-base", "--is-ancestor", stamp, head],
      { cwd },
    );
    if (ancestry.exitCode === 0) {
      const r = (0, shell_command_projection_cjs_1.execGit)(
        ["rev-list", "--count", `${stamp}..${head}`],
        { cwd },
      );
      if (r.exitCode === 0) {
...
function syncStateFrontmatter(content, cwd, authoritativeFm, sanctionedPermanentEmptyFallback) {
  ...
  const derivedFm = buildStateFrontmatter(
    body,
    cwd,
    storedMilestone,
    readStoredTotalPhases(existingFm),
  ...
  const derivedLooksLikeName =
    typeof derivedName === "string" &&
  ...
  if (sanctionedPermanentEmptyFallback) {
    ...
    if (!derivedFm["progress"] && existingFm["progress"]) {
      derivedFm["progress"] = (0, state_document_cjs_1.normalizeProgressNumbers)(
        existingFm["progress"],
      );
    }
  }
  ...
  for (const key of Object.keys(existingFm)) {
    ...
    if (
      ...
      classification &&
      (classification.preservation === "preserve-when-unchanged" ||
        classification.preservation === "preserve-always")
    )
      continue;
...
function acquireStateLock(statePath, clock) {
  if (clock === undefined) clock = clock_cjs_1.realClock;
  ...
  while (true) {
    ...
    try {
      const fd = node_fs_1.default.openSync(
        lockPath,
        node_fs_1.default.constants.O_CREAT |
          node_fs_1.default.constants.O_EXCL |
      ...
      try {
        ...
        if (steal) {
          ...
          const sameInstance =
            ...
            confirmStat.dev === stat.dev &&
            confirmStat.ino === stat.ino &&
          ...
          const stolen =
            lockPath + ".stale-" + process.pid + "-" + clock.now() + "-" + _stateStealSeq++;
          let renamed = false;
          try {
        ...
        if (err?.lockBudgetExceeded) throw err;
...
function applyPostSyncPreservation(
  ...
  const preBodyStoppedAt =
    (0, state_document_cjs_1.stateExtractField)(preSessionScope, "Stopped At") ||
    (0, state_document_cjs_1.stateExtractField)(preSessionScope, "Stopped at");
  ...
  const preBodyCurrentPhase =
    (0, state_document_cjs_1.stateExtractField)(preBody, "Current Phase") ??
    parseProsePhaseField(
      (0, state_document_cjs_1.stateExtractField)(preCurrentPositionScope, "Phase"),
    ).phase;
  const preBodyPausedAt = (0, state_document_cjs_1.stateExtractField)(preSessionScope, "Paused At");
  const preBodyLastActivityRaw =
    (0, state_document_cjs_1.stateExtractField)(preBody, "Last Activity") ??
    (0, state_document_cjs_1.stateExtractField)(preBody, "Last activity");
  const preBodyLastActivityDesc =
    (0, state_document_cjs_1.stateExtractField)(preBody, "Last Activity Description") ??
  ...
  const postBodyStoppedAt =
    (0, state_document_cjs_1.stateExtractField)(postSessionScope, "Stopped At") ||
    (0, state_document_cjs_1.stateExtractField)(postSessionScope, "Stopped at");
  ...
  const postBodyCurrentPhase =
    (0, state_document_cjs_1.stateExtractField)(postBody, "Current Phase") ??
    parseProsePhaseField(
      (0, state_document_cjs_1.stateExtractField)(postCurrentPositionScope, "Phase"),
    ).phase;
  const postBodyPausedAt = (0, state_document_cjs_1.stateExtractField)(
    postSessionScope,
    "Paused At",
  );
  const postBodyLastActivityRaw =
    (0, state_document_cjs_1.stateExtractField)(postBody, "Last Activity") ??
    (0, state_document_cjs_1.stateExtractField)(postBody, "Last activity");
  const postBodyLastActivityDesc =
    (0, state_document_cjs_1.stateExtractField)(postBody, "Last Activity Description") ??
  ...
  const persistedPercent = (0, state_document_cjs_1.toFiniteNumber)(
    preservation.postFm["progress"] && preservation.postFm["progress"]["percent"],
  );
...
function reconcileReportedFields(statePath, preWriteState, reported, divergedFields) {
  ...
  const isDeclaredDerivedLeaf = (candidate) =>
    candidate.includes(".") &&
    Object.prototype.hasOwnProperty.call(FIELD_CLASSIFICATION, candidate);
  const changed = (field) => {
  ...
  const changedFrontmatterFields = computeChangedFrontmatterFields(
    snapshotFm,
    persistedFm,
    preWriteState.bodyDeltas,
  );
...
function cmdStateJson(cwd, raw) {
  ...
  const storedMilestoneJson =
    typeof existingFm["milestone"] === "string" ? existingFm["milestone"] : null;
  const built = buildStateFrontmatter(
    body,
    cwd,
    storedMilestoneJson,
    readStoredTotalPhases(existingFm),
  ...
  if (existingFm) {
    ...
    const positionScope = matchCurrentPositionSection(body) ?? body;
    const bodyStoppedAt =
      (0, state_document_cjs_1.stateExtractField)(sessionScope, "Stopped At") ||
      (0, state_document_cjs_1.stateExtractField)(sessionScope, "Stopped at");
    ...
    const bodyCurrentPhase =
      (0, state_document_cjs_1.stateExtractField)(body, "Current Phase") ??
      parseProsePhaseField((0, state_document_cjs_1.stateExtractField)(positionScope, "Phase"))
        .phase;
    ...
    const bodyLastActivityRaw =
      (0, state_document_cjs_1.stateExtractField)(body, "Last Activity") ??
      (0, state_document_cjs_1.stateExtractField)(body, "Last activity");
    const bodyLastActivityDesc =
      (0, state_document_cjs_1.stateExtractField)(body, "Last Activity Description") ??
  ...
  if (
    existingFm &&
    (0, state_document_cjs_1.shouldPreserveExistingProgress)(
      existingFm["progress"],
      built["progress"],
    )
  ) {
...
function cmdStateBeginPhase(cwd, phaseNumber, phaseName, planCount, raw) {
  ...
  const wrote = readModifyWriteStateMd(
    ...
    (content) => {
      ...
      if (milestoneConflict) {
        milestoneLockMod.warnMilestoneConflict(
          milestoneConflict,
          `state.begin-phase ${phaseNumber}`,
        );
      }
      const result = transitionCore(content, intent, deps);
      precomputedUpdated = result.updated;
...
function cmdSignalWaiting(cwd, type, question, options, phase, raw) {
  const gsdDir = node_fs_1.default.existsSync(node_path_1.default.join(cwd, ".gsd"))
    ? node_path_1.default.join(cwd, ".gsd")
    : planningDir(cwd);
  ...
  try {
    ...
    (0, shell_command_projection_cjs_1.platformWriteSync)(
      waitingPath,
...
function updatePerformanceMetricsSection(content, cwd, phaseNum, planCount, summaryCount) {
  ...
  if (tableStart !== null) {
    ...
    const canonCell = /^\d+$/.test(phaseNumStr)
      ? `0*${Number(phaseNumStr)}`
      : (0, pattern_cjs_1.escapeRegex)(phaseNumStr);
    ...
    let rowExists = false;
    const existsProbe = (0, markdown_table_cjs_1.updateTableCell)(
      tableText,
      rowMatch,
      "Phase",
      (current) => {
    ...
    if (rowExists) {
      ...
      const phaseResult = (0, markdown_table_cjs_1.updateTableCell)(
        tableText,
        rowMatch,
        "Phase",
        ` ${phaseNum} `,
      ...
      const plansResult = (0, markdown_table_cjs_1.updateTableCell)(
        tableText,
        rowMatch,
        "Plans",
        ` ${summaryCount} `,
      ...
      const totalResult = (0, markdown_table_cjs_1.updateTableCell)(
        tableText,
        rowMatch,
        "Total",
        " - ",
      ...
      const avgResult = (0, markdown_table_cjs_1.updateTableCell)(
        tableText,
        rowMatch,
        "Avg/Plan",
        " - ",
      ...
      const withoutPlaceholder = (0, markdown_table_cjs_1.deleteTableRow)(
        tableText,
        placeholderRow,
      );
  ...
  const result =
    updated.length === 0
      ? {
          ...
          warning:
            "STATE.md Current Position has no recognized labels — transition was a no-op. Verify STATE.md uses the canonical labeled format (Status:, Total Plans in Phase:, etc.).",
        }
...
function cmdStateValidate(cwd, raw, opts = {}) {
    ...
    emit({
      ...
      warnings: [
        stateDiagnostic(
          ...
          SEVERITY.ERROR,
          encErr,
          "Re-save STATE.md as UTF-8 text with the embedded NUL byte(s) removed",
        ),
      ],
    });
    return;
  ...
  const status =
    (0, state_document_cjs_1.stateFieldValue)(fm, body, "status", "Status").value || "";
  const resolvedPhase = resolveStatePhase(fm, body);
  const currentPhase = resolvedPhase.phase;
  const totalPlansRaw = (0, state_document_cjs_1.stateFieldValue)(
    fm,
  ...
  if (
    Object.values(resolvedPhase.sources).some(
  ...
  try {
    ...
    const phaseDir = entries.find(
      (entry) =>
        entry.isDirectory() && phaseKeyFromDir(entry.name, validateConvention) === selectedPhaseKey,
    );
    if (!phaseDir) {
      warnings.push(
        stateDiagnostic(
  ...
  try {
    ...
    for (const vf of verificationFiles) {
      try {
        const vContent = node_fs_1.default.readFileSync(
          node_path_1.default.join(phaseDirPath, vf),
          "utf-8",
        );
        ...
        /* best-effort (#2245 audit): cmdStateValidate is a diagnostic
  ...
  const lastActivity = (0, state_document_cjs_1.stateFieldValue)(
    fm,
    body,
    "last_activity",
    "Last activity",
  ...
  if (!(0, state_document_cjs_1.isUnfilledFieldValue)(lastActivity)) {
    ...
    const fromFrontmatter = (0, state_document_cjs_1.stateFieldValue)(
      fm,
      "",
      "last_activity",
      "Last activity",
    ...
    const dropped =
      fromFrontmatter !== null
        ? null
        : (0, state_document_cjs_1.stateFieldContinuation)(body, "Last activity");
    if (dropped !== null) {
      warnings.push(
        stateDiagnostic(
          "S009",
...
function cmdStateSync(cwd, options, raw) {
  ...
  try {
    const roadmapRaw = (0, shell_command_projection_cjs_1.platformReadSync)(
      node_path_1.default.join(planningDir(cwd), "ROADMAP.md"),
    );
    if (roadmapRaw !== null) {
  ...
  const syncMilestonePhaseDirs =
    syncConvention === "bracket"
  ...
  try {
    entries = node_fs_1.default
      ...
      .filter(
        (name) =>
          !(
            syncRetiredPhaseNums.size > 0 &&
            syncRetiredPhaseNums.has(phaseKeyFromDir(name, syncConvention))
          ),
      )
  ...
  const roadmapPhaseCount =
    syncRoadmapScope !== null
      ? countRoadmapPhaseHeadings(
          syncRoadmapScope,
          syncConvention,
          syncRetiredPhaseNums,
          syncConvention === "bracket",
        )
      : 0;
  ...
  if (!milestoneBounded) {
    changes.push(
      `Progress: skipped — milestone ${versionStr} cannot be bounded to a versioned ROADMAP phase set (#1761)`,
    );
  } else {
    ...
    if (syncScope !== SCOPE.COMPLETE) {
      changes.push(
        `Progress: skipped — milestone phase scope is "${syncScope}", not COMPLETE (#3217)`,
      );
    } else {
      const p = (0, state_document_cjs_1.computeProgressPercent)(
        totalDiskSummaries,
        totalDiskPlans,
        diskCompletedPhases,
        syncTotalPhases,
  ...
  const syncResult = transitionCore(
    ...
    {
      kind: "sync",
      totalPlansInPhase: highestIncompletePhase ? highestIncompletePhaseplanCount : null,
      percent,
    },
    { clock: clock_cjs_1.realClock },
  );
  ...
  if (syncWillWrite) {
    ...
    writeStateMd(
      statePath,
      modified,
      stateTransitionMod.rebuildStateTransaction({
...
function cmdStatePrune(cwd, options, raw) {
  ...
  const archivePath = node_path_1.default.join(
    node_path_1.default.dirname(statePath),
    "STATE-ARCHIVE.md",
  );
  const archived = [];
...
function cmdStateRebuild(cwd, options, raw) {
  ...
  const phaseInventoryProvider = () => {
    try {
      ...
      if (
        !node_fs_1.default.existsSync(phasesDir) ||
        !node_fs_1.default.statSync(phasesDir).isDirectory()
      )
    ...
    emit(
      {
        ...
        note: mutated
          ...
          : scanFailed
            ? scanFailureNote(data.phase_inventory_scan_reason)
            : "Nothing to rebuild",
      },
      raw,
      mutated ? "true" : "false",
  ...
  emit(
    {
      ...
      note: capturedMutated
        ...
        : capturedScanFailed
          ? scanFailureNote(capturedScanReason)
          : "Nothing to rebuild",
    },
    raw,
    capturedMutated ? "true" : "false",
  );
}
/**
...
function cmdStateCompletePhase(cwd, raw, overridePhase) {
  ...
  if (scope !== SCOPE.COMPLETE) {
    output(
      {
        error:
          "Unable to read STATE.md frontmatter; refusing to run complete-phase to avoid a destructive rollback (#3489). Fix or remove the malformed frontmatter and retry.",
      },
      raw,
      undefined,
    );
  ...
  if (!resolvedPhase || /^phase$/i.test(resolvedPhase)) {
    output(
      {
        error:
          "Unable to resolve current phase. Pass an explicit phase: state complete-phase --phase <N>",
      },
      raw,
      undefined,
    );
    ...
    output(
      {
        ...
        phase: resolvedPhase,
        idempotent: true,
        note: "phase already superseded; no-op",
      },
      raw,
      "false",
    );
      ...
      // Mirrors /(##\s*Current Position\s*\n)([\s\S]*?)(?=\n##|$)/i
      {
        ...
        if (cpIdx !== -1) {
          ...
          if (/^Phase:/m.test(posBody)) {
            ...
            const replaced = (0, state_document_cjs_1.stateReplaceField)(
              posBody,
              "Phase",
          ...
          if (/^Last activity:/im.test(posBody)) {
            ...
            const replaced =
              (0, state_document_cjs_1.stateReplaceField)(
                posBody,
                "Last Activity",
                activityValue,
              ) ??
  ...
  const reconciled = [
    ...sectionEntries,
    ...reconcileReportedFields(statePath, preWriteState, fieldEntries, divergedFields),
  ];
  output(
