...
function resolveServerVersion() {
  ...
  try {
    const v = node_fs_1.default
      .readFileSync(node_path_1.default.join(__dirname, "..", "..", "VERSION"), "utf8")
      .trim();
...
let cachedCatalog;
function getCatalog() {
  if (cachedCatalog === undefined) cachedCatalog = (0, mcp_catalog_cjs_1.buildCatalog)();
...
function catalogErrorCode(err) {
  return catalogErrorReason(err) === mcp_catalog_cjs_1.REASON.READ_FAILED
    ? INTERNAL_ERROR
    : INVALID_PARAMS;
}
  ...
  try {
    if (name === "gsd_invoke_command") {
      ...
      if (!family || !subcommand) {
        return {
          isError: true,
          content: [
            { type: "text", text: 'gsd_invoke_command requires string "family" and "subcommand".' },
          ],
        };
      }
      const res = dispatchGsdCommand({
        family,
        subcommand,
      ...
      if (!res.ok) {
        return {
          ...
          content: [
            {
              type: "text",
              text: res.stderr || res.stdout || `dispatch failed (exit ${res.code})`,
            },
          ],
        };
      }
    ...
    if (name === "gsd_read_state") {
      ...
      if (!p)
        return {
          isError: true,
          content: [{ type: "text", text: 'gsd_read_state requires string "path".' }],
        };
    ...
    if (name === "gsd_write_state") {
      ...
      if (!p || content === null)
        return {
          isError: true,
          content: [
            { type: "text", text: 'gsd_write_state requires string "path" and "content".' },
          ],
        };
      const io = stateIo.createStateIO({ io: "filesystem" });
      io.write(p, content);
  ...
  for await (const chunk of input) {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        output.write(
          JSON.stringify(
            errorResponse(null, INTERNAL_ERROR, e instanceof Error ? e.message : "Internal error."),
          ) + "\n",
        );
      }
    }
