...
exports.TABLE_SCHEMAS = {
  ...
  RequirementsTraceability: [{ label: "default", columns: ["Requirement", "Phase", "Status"] }],
  QuickTasks: [
...
function updateTableCell(tableText, match, column, newValue) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const candidateColumns = candidateRanges.map((r) =>
      unescapeCellText(tableText.slice(r.start, r.end)),
    );
    const delimiterCells = splitTableRow(delimiterLine);
    if (!isDelimiterRow(delimiterCells)) {
      recordMalformed("missing delimiter row");
  ...
  for (let i = headerIdx + 2; i < lines.length; i++) {
    ...
    for (let c = 0; c < presentCount; c++) {
      record[columns[c]] = unescapeCellText(
        tableText.slice(cellRanges[c].start, cellRanges[c].end),
      );
    }
  ...
  return {
    ok: true,
    value:
      tableText.slice(0, selectedRange.start) + replacement + tableText.slice(selectedRange.end),
  };
}
...
function insertTableRow(tableText, valueFor, fallback = "-") {
  ...
  if (lastLineIdx + 1 < lines.length) {
    ...
    const eol = tableText.slice(
      lines[lastLineIdx].start + lines[lastLineIdx].line.length,
      insertAt,
    );
    return {
      ok: true,
      value: tableText.slice(0, insertAt) + newRow + eol + tableText.slice(insertAt),
    };
  }
...
function selectQuickTasksSection(stateContent) {
  const candidates = (0, markdown_sectionizer_cjs_1.collectSections)(
    stateContent,
    isQuickTasksHeading,
  );
  if (candidates.length === 0) return null;
  const boundedOf = (cand) =>
    (0, markdown_sectionizer_cjs_1.collectSection)(
      stateContent,
      (h) => h.offset === cand.heading.offset,
    );
...
function appendQuickTaskRow(stateContent, fields) {
  ...
  const cellFor = (col) => {
    switch (col) {
      case "#":
        return escapeCell(fields.quickId ?? String(rowNumber));
      case "Description":
        return escapeCell(fields.description);
      case "Date":
        return escapeCell(fields.date);
      case "Commit":
        return escapeCell(fields.commit);
      case "Status":
        return escapeCell(fields.status ?? "—");
      case "Directory":
        return escapeCell(fields.directory ?? "—");
      default:
        return "—";
    }
  };
...
function migrateQuickTasksTable(stateContent) {
  ...
  const targets = parsed.value.columns.map(
    (c) => QUICK_TASKS_COLUMN_ALIASES[c.trim().toLowerCase()] ?? null,
  );
  ...
  while (
    lastTableLineIdx + 1 < lines.length &&
    lines[lastTableLineIdx + 1].trim().startsWith("|")
  ) {
    lastTableLineIdx++;
  }
...
function resetQuickTaskRows(stateContent) {
  ...
  const newLines = [...lines.slice(0, headerIdx + 2), ...lines.slice(lastTableLineIdx + 1)];
  const newBody = newLines.join(eol);
  const content = (0, markdown_sectionizer_cjs_1.replaceSection)(stateContent, section, newBody);
