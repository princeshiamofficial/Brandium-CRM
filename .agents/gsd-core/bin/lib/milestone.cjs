...
const { getMilestonePhaseFilter, extractCurrentMilestone, getMilestoneInfo, sliceMilestoneWindow } =
  roadmapParserMod;
// eslint-disable-next-line @typescript-eslint/no-require-imports
...
function cmdRequirementsMarkComplete(cwd, reqIdsRaw, raw) {
  ...
  for (const reqId of reqIds) {
    ...
    const doneCheckbox = new RegExp(`-\\s*\\[x\\]\\s*\\*\\*${reqEscaped}\\*\\*`, "i").test(
      reqContent,
    );
...
function cmdRequirementsReadyIds(cwd, args, raw) {
  ...
  const isNestedPlanPath =
    node_path_1.default.basename(node_path_1.default.dirname(planAbsPath)) === "plans";
  const phaseDir = isNestedPlanPath
    ? node_path_1.default.dirname(node_path_1.default.dirname(planAbsPath))
    : node_path_1.default.dirname(planAbsPath);
  const currentRelative = isNestedPlanPath
    ? `plans/${node_path_1.default.basename(planAbsPath)}`
    : node_path_1.default.basename(planAbsPath);
  ...
  for (const reqId of reqIds) {
    ...
    for (const siblingFile of siblingPlanFiles) {
      ...
      const siblingDeclaresId = siblingReqIds.some(
        (id) => id.toLowerCase() === reqId.toLowerCase(),
      );
...
function cmdMilestoneComplete(cwd, version, options, raw) {
  ...
  if (!ARCHIVE_VERSION_LABEL_RE.test(version)) {
    error(
      `milestone complete: version "${version}" is invalid — a milestone version label may contain only letters, digits, '.', '-' and '_', and must not contain path separators or "..".`,
    );
  }
  ...
  if (!options.force) {
    try {
      ...
      try {
        const stateRaw = node_fs_1.default.existsSync(statePath)
      ...
      // the lookup.
      const scopedContent =
        sliceMilestoneWindow(roadmapContent, version) ??
  ...
  const { value: milestonePhaseDirs, scope: milestonePhaseScope } = listMilestonePhaseDirs(
    phasesDir,
    {
      cwd,
  ...
  const phasesArchiveSkippedForScope =
    options.archivePhases !== false && milestonePhaseScope === SCOPE.UNREADABLE;
    ...
    const dryRunResult = {
      ...
      would_archive: {
        roadmap: node_fs_1.default.existsSync(roadmapPath)
          ? {
              source: node_path_1.default
                .relative(cwd, roadmapPath)
                .split(node_path_1.default.sep)
                .join("/"),
              target: node_path_1.default
                .relative(cwd, node_path_1.default.join(archiveDir, `${version}-ROADMAP.md`))
                .split(node_path_1.default.sep)
                .join("/"),
            }
          : null,
        requirements: node_fs_1.default.existsSync(reqPath)
          ? {
              source: node_path_1.default
                .relative(cwd, reqPath)
                .split(node_path_1.default.sep)
                .join("/"),
              target: node_path_1.default
                .relative(cwd, node_path_1.default.join(archiveDir, `${version}-REQUIREMENTS.md`))
                .split(node_path_1.default.sep)
                .join("/"),
            }
          : null,
        audit: node_fs_1.default.existsSync(
          node_path_1.default.join(planningBase, `${version}-MILESTONE-AUDIT.md`),
        )
          ? {
              source: node_path_1.default
                .relative(
              ...
              target: node_path_1.default
                .relative(
                  cwd,
                  node_path_1.default.join(archiveDir, `${version}-MILESTONE-AUDIT.md`),
                )
                .split(node_path_1.default.sep)
                .join("/"),
            }
      ...
      would_update: {
        milestones_md: node_path_1.default
          .relative(cwd, milestonesPath)
          .split(node_path_1.default.sep)
          .join("/"),
        state_md: node_fs_1.default.existsSync(statePath)
          ? node_path_1.default.relative(cwd, statePath).split(node_path_1.default.sep).join("/")
          : null,
      },
    };
  ...
  if (node_fs_1.default.existsSync(roadmapPath)) {
    ...
    (0, shell_command_projection_cjs_1.platformWriteSync)(
      node_path_1.default.join(archiveDir, `${version}-ROADMAP.md`),
      roadmapContent,
    );
  }
  ...
  if (node_fs_1.default.existsSync(reqPath)) {
    ...
    const reqDisplay = node_path_1.default
      .relative(cwd, reqPath)
      .split(node_path_1.default.sep)
      .join("/");
    const archiveHeader = `# Requirements Archive: ${version} ${milestoneName}\n\n**Archived:** ${today}\n**Status:** SHIPPED\n\nFor current requirements, see \`${reqDisplay}\`.\n\n---\n\n`;
    (0, shell_command_projection_cjs_1.platformWriteSync)(
      node_path_1.default.join(archiveDir, `${version}-REQUIREMENTS.md`),
  ...
  const milestonesBefore = node_fs_1.default.existsSync(milestonesPath)
  ...
  const milestonesAfter = node_fs_1.default.existsSync(milestonesPath)
    ? node_fs_1.default.readFileSync(milestonesPath, "utf-8")
    : null;
  ...
  if (node_fs_1.default.existsSync(statePath)) {
    withStateLock(statePath, () => {
      ...
      const result = (0, state_transition_cjs_1.transitionCore)(
        originalStateContent,
        {
          kind: "milestoneComplete",
          version,
          nextMilestoneCommand: (0, runtime_slash_cjs_1.formatGsdSlash)(
            "new-milestone",
            (0, runtime_slash_cjs_1.resolveRuntime)(cwd),
          ),
        },
    ...
    try {
      ...
      for (const dir of milestonePhaseDirs) {
        (0, shell_command_projection_cjs_1.retryRenameSync)(
          ...
          node_path_1.default.join(phaseArchiveDir, dir),
        );
        archivedCount++;
      }
    } catch {
  ...
  const result = {
    ...
    accomplishments,
    archived: {
      roadmap: node_fs_1.default.existsSync(
        node_path_1.default.join(archiveDir, `${version}-ROADMAP.md`),
      ),
      requirements: node_fs_1.default.existsSync(
        node_path_1.default.join(archiveDir, `${version}-REQUIREMENTS.md`),
      ),
      audit: node_fs_1.default.existsSync(
        node_path_1.default.join(archiveDir, `${version}-MILESTONE-AUDIT.md`),
...
function cmdPhasesClear(cwd, raw, args) {
  ...
  if (avIndex !== -1) {
    ...
    if (
      typeof rawArchiveVersion !== "string" ||
      rawArchiveVersion.startsWith("--") ||
      rawArchiveVersion.trim() === ""
    ) {
    ...
    if (!ARCHIVE_VERSION_LABEL_RE.test(trimmed)) {
      error(
        `--archive-version "${trimmed}" is invalid — a milestone version label may contain only letters, digits, '.', '-' and '_', and must not contain path separators or "..".`,
      );
    }
  ...
  if (node_fs_1.default.existsSync(phasesDir)) {
    ...
    if (dirs.length > 0 && !force) {
      ...
      let gitStatusOutput = "";
      try {
        const gitResult = (0, shell_command_projection_cjs_1.execGit)(
          ["status", "--porcelain", relPhasesDir],
          { cwd, timeout: 10_000 },
        );
...
function archivePhaseDirectories(cwd, phasesDir, dirs, archiveVersionOverride = null) {
  ...
  const safeOverride =
    archiveVersionOverride && ARCHIVE_VERSION_LABEL_RE.test(archiveVersionOverride.trim())
  ...
  if (!archiveVersion) {
    try {
      ...
      archiveVersion =
        liveVersion && ARCHIVE_VERSION_LABEL_RE.test(liveVersion) ? liveVersion : null;
    } catch {
  ...
  const archivePhasesDir = node_path_1.default.join(
    planningPaths(cwd).planning,
    "milestones",
    `${archiveVersion}-phases`,
  );
...
function buildQuickArchiveIndex(archiveQuickDir) {
  ...
  const entries = dirNames.map((dirName) => {
    ...
    if (summaryPath) {
      const relSummary = node_path_1.default
        .relative(archiveQuickDir, summaryPath)
        .split(node_path_1.default.sep)
        .join("/");
...
function writeQuickArchiveReadme(archiveQuickDir) {
  try {
    ...
    (0, shell_command_projection_cjs_1.platformWriteSync)(
      node_path_1.default.join(archiveQuickDir, "README.md"),
      index.render(),
    );
  } catch {
...
function listQuickTaskDirsForArchive(cwd) {
  ...
  for (const entry of sourceEntries) {
    ...
    try {
      (0, security_cjs_1.requireSafePath)(
        node_path_1.default.join(quickDir, entry.name),
        planningBase,
        "quick task dir",
        { allowAbsolute: true },
      );
  ...
  try {
    ...
    for (const name of dirNames) {
      ...
      try {
        ...
        safeSrc = (0, security_cjs_1.requireSafePath)(src, planningBase, "quick task dir", {
          allowAbsolute: true,
        });
      } catch {
...
function cmdQuickArchive(cwd, version, options, raw) {
  ...
  if (!ARCHIVE_VERSION_LABEL_RE.test(version)) {
    error(
      `milestone.archive-quick: version "${version}" is invalid — a milestone version label may contain only letters, digits, '.', '-' and '_', and must not contain path separators or "..".`,
    );
  }
  const statePath = planningPaths(cwd).state;
  const planningBase = planningPaths(cwd).planning;
  const toPosixRel = (p) =>
    node_path_1.default.relative(cwd, p).split(node_path_1.default.sep).join("/");
    ...
    output(
      {
        ...
        archive_dir: toPosixRel(
          node_path_1.default.join(planningBase, "milestones", `${version}-quick`),
        ),
      },
      raw,
    );
