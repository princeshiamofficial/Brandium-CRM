...
const INTERFACE_POINTS = Object.freeze([
  "command",
  "dispatch",
  "model",
  "hooks",
...
function degradationFor(point, axes) {
  ...
  switch (point) {
    ...
    case "dispatch": {
      ...
      const isUnbounded =
        typeof disp.maxDepth === "number" && Number.isFinite(disp.maxDepth) && disp.maxDepth < 0;
      const depth =
        typeof disp.maxDepth === "number" && Number.isFinite(disp.maxDepth) ? disp.maxDepth : 0;
      ...
      if (isFullDepth) {
        ...
        return {
          level: "degraded",
          fallback: "restricted/undocumented subagent toolkit — limited dispatch surface",
        };
      }
      // flat (maxDepth===1)
...
function negotiateHostCapabilities(host, engine = DEFAULT_ENGINE) {
  ...
  if (
    h.protocolVersion !== undefined &&
    (typeof h.protocolVersion !== "number" || !Number.isFinite(h.protocolVersion))
  ) {
    warnings.push(
      `host protocolVersion is not a finite number — using engine version ${engine.protocolVersion}`,
    );
  }
  const hostPV =
    typeof h.protocolVersion === "number" && Number.isFinite(h.protocolVersion)
  ...
  // ---------------------------------------------------------------------------
  const hostDispatch = typeof h.dispatch === "object" && h.dispatch !== null ? h.dispatch : null;
  ...
  if (hostDispatch === null) {
    ...
    effectiveBackgroundDispatch =
      hostDispatch.backgroundDispatch === true && engineDispatch.backgroundDispatch;
    ...
    effectiveSubagentToolkit =
      hostToolkit === "read-only" || engineToolkit === "read-only" ? "read-only" : "full";
    ...
    effectiveIsolation =
      typeof hostIso === "string" && HOST_INTEGRATION_AXES.isolation.includes(hostIso)
        ? hostIso
        : "none";
...
function shouldFlattenDispatch(dispatch) {
  ...
  const depth =
    typeof dispatch.maxDepth === "number" && Number.isFinite(dispatch.maxDepth)
      ? dispatch.maxDepth
      : 0;
...
const DISPATCH_TYPE_SUFFIX_MAP = Object.freeze([
  ...
  [
    /-?(researcher|mapper|checker|verifier|auditor|analyzer|synthesizer|profiler|curator|classifier|reviewer)$/i,
    "explore",
  ],
]);
// Names that are already generic (not gsd-*) and should map to the
// general-purpose built-in on built-in-only runtimes.
const GENERIC_NAMES_TO_CODER = Object.freeze(
  new Set(["general-purpose", "general", "default", "sonnet", "opus", "haiku"]),
);
...
function resolveOrchestratorExec(orchestratorExec, cwd, prompt, model) {
  if (
    !orchestratorExec ||
    typeof orchestratorExec !== "object" ||
    Array.isArray(orchestratorExec)
  ) {
  ...
  if (
    oe.args !== undefined &&
    (!Array.isArray(oe.args) || !oe.args.every((a) => typeof a === "string"))
  ) {
  ...
  let args = baseArgs;
  if (
    typeof oe.modelFlag === "string" &&
    oe.modelFlag.length > 0 &&
    typeof model === "string" &&
    model.length > 0
  ) {
  ...
  args =
    typeof oe.cwdFlag === "string" && oe.cwdFlag.length > 0 ? [...args, oe.cwdFlag, cwd] : args;
