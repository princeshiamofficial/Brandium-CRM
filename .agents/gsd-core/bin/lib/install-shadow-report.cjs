...
const { resolveRuntimeArtifactLayout, resolveRuntimeArtifactLayoutFromRegistry } =
...
function isGenuinelyShadowed(t, scopeRecords, prefixFor) {
  ...
  return (
    (winnerRecord?.stems ?? []).includes(winnerStem) &&
    (shadowedRecord?.stems ?? []).includes(shadowedStem)
  );
}
// ── Report builder ──────────────────────────────────────────────────────
/**
...
function buildShadowReport(runtime, opts = {}) {
  ...
  const shadowedSurfaces = surface.triggers.filter((t) =>
    isGenuinelyShadowed(t, scopeRecords, prefixFor),
  );
  ...
  for (const record of surface.scopes) {
    if (
      record.declaredRuntimeMatchesProbe === false ||
      record.declaredScopeMatchesProbe === false
    ) {
      mismatches.push({
        scope: record.scope,
...
function renderShadowReport(report, opts = {}) {
  if (
    report.reason !== exports.SHADOW_REASON.SCOPE_SHADOWED ||
    report.winner === null ||
    report.shadowedSide === null
  ) {
    return [];
  }
  ...
  for (const t of sample) {
    lines.push(
      `  - ${t.trigger}: ${t.shadowedScope}/${t.shadowedKind} shadowed by ${t.winnerScope}/${t.winnerKind}`,
    );
  }
