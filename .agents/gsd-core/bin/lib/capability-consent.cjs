...
const PYCACHE_DIR_BASENAMES = [Buffer.from("__pycache__"), Buffer.from(".pytest_cache")];
/** FILE-name suffixes excluded ONLY when the file's parent directory basename is `__pycache__` (see below). */
const PYCACHE_FILE_SUFFIXES = [Buffer.from(".pyc"), Buffer.from(".pyo")];
...
function hasPycacheFileSuffix(name) {
  for (const suffix of PYCACHE_FILE_SUFFIXES) {
    if (
      ...
      Buffer.compare(name.subarray(name.length - suffix.length), suffix) === 0
    ) {
      return true;
    }
  }
...
function collectBundleEntries(absDir, relDir, dirBasename, acc, total, count) {
  ...
  try {
    ...
    throw new Error(
      `bundleContentHash: cannot read directory "${absDir.toString("utf8")}": ${err.message}`,
    );
  }
  ...
  try {
    for (;;) {
      ...
      if (count.n > BUNDLE_MAX_FILES) {
        throw new Error(
          `bundleContentHash: bundle entry count exceeds ${BUNDLE_MAX_FILES} (refusing)`,
        );
      }
  ...
  for (const ent of levelEntries) {
    ...
    if (st.isSymbolicLink()) {
      ...
      throw new Error(
        `bundleContentHash: refusing to hash a symlink in the bundle: "${abs.toString("utf8")}"`,
      );
    }
    if (st.isDirectory()) {
    ...
    if (!st.isFile()) {
      throw new Error(
        `bundleContentHash: refusing to hash a non-regular file in the bundle: "${abs.toString("utf8")}"`,
      );
    }
    ...
    if (total.bytes > BUNDLE_MAX_TOTAL_BYTES) {
      throw new Error(
        `bundleContentHash: bundle size exceeds ${BUNDLE_MAX_TOTAL_BYTES} bytes (refusing)`,
      );
    }
...
function bundleContentHash(capDir) {
  ...
  const rootBasename = Buffer.from(
    node_path_1.default.basename(node_path_1.default.resolve(capDir)),
  );
  const entries = [];
  ...
  for (const ent of entries) {
    ...
    if (content === null) {
      throw new Error(
        `bundleContentHash: file vanished during hash: "${ent.abs.toString("utf8")}"`,
      );
    }
...
function consentLockPath(gsdHome) {
  return node_path_1.default.join(
    node_path_1.default.dirname(consentStorePath(gsdHome)),
    ".consent.lock",
  );
}
/**
...
function acquireConsentLock(dir) {
  ...
  return lockMod.acquireLock(node_path_1.default.join(dir, ".consent.lock"), {
    maxAttempts: CONSENT_LOCK_MAX_ATTEMPTS,
    waitForFresh: true,
  });
}
...
function fsyncContainingDir(dest) {
  ...
  try {
    ...
    if (code !== undefined && !DIR_FSYNC_TOLERATED_ERRNOS.has(code)) {
      throw new Error(
        `Directory fsync of "${node_path_1.default.dirname(dest)}" failed (${code}); durability of the consent ` +
...
function recordProjectConsent(args) {
  const { gsdHome, projectRoot, id, integrity, disclosureSignature, contentHash, reviewerHost } =
    args;
  ...
  try {
    ...
    if (
      !Object.prototype.hasOwnProperty.call(store.records, key) &&
