...
function versionFile(dir) {
  return node_path_1.default.join(dir, "gsd-core", "VERSION");
}
function markerFile(dir) {
  return node_path_1.default.join(dir, "gsd-core", "workflows", "update.md");
}
...
function inferPreferredRuntime({ fs, env, preferredConfigDir }) {
  if (preferredConfigDir) {
    if (
      fs.exists(node_path_1.default.join(preferredConfigDir, "kilo.json")) ||
      fs.exists(node_path_1.default.join(preferredConfigDir, "kilo.jsonc"))
    )
      return "kilo";
    if (
      fs.exists(node_path_1.default.join(preferredConfigDir, "opencode.json")) ||
    ...
    const known = exports.RUNTIME_DIRS.find(([, reldir]) =>
      resolved.endsWith(node_path_1.default.sep + reldir.split("/").join(node_path_1.default.sep)),
    );
