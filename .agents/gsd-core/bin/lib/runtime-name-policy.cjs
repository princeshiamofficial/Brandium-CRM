...
function getProjectInstructionFile(runtime) {
  ...
  if (canonical === "claude") return ".agents/GEMINI.md";
  ...
  const declared = canonical
    ? runtimes[canonical]?.runtime?.hostBehaviors?.projectInstructionFile
    : undefined;
...
function getDirName(runtime) {
  ...
  if (entry?.configHome?.kind === "none") return exports.NO_LOCAL_CONFIG_DIR_SENTINEL;
  return ".claude";
}
/**
...
function runtimeIdToFlagName(id) {
  return (
    "is" + id.charAt(0).toUpperCase() + id.slice(1).replace(/-([a-z])/g, (_m, c) => c.toUpperCase())
  );
}
/**
