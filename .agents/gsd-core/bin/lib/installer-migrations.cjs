...
function evaluateRemoveEmptyDir(configDir, fullPath) {
  ...
  if (
    ...
    !resolvedTarget.startsWith(resolvedRoot + node_path_1.default.sep)
  ) {
...
function readInstallState(configDir) {
  const state = readJsonIfPresent(
    node_path_1.default.join(configDir, INSTALL_STATE_NAME),
    STRICT_JSON,
  );
...
function readJson(configDir, relPath) {
  ...
  try {
    return {
      exists: true,
      value: JSON.parse(node_fs_1.default.readFileSync(fullPath, "utf8")),
      error: null,
    };
  } catch (error) {
...
function normalizeRelPath(relPath) {
  ...
  if (
    ...
    node_path_1.default.win32.isAbsolute(normalized)
  ) {
...
function classifyArtifact(configDir, relPath, manifest) {
  ...
  if (!installFs().existsSync(fullPath)) {
    return {
      classification: originalHash ? "managed-missing" : "missing",
      originalHash,
      currentHash: null,
    };
  }
...
function discoverInstallerMigrations({ migrationsDir }) {
  if (!migrationsDir || !node_fs_1.default.existsSync(migrationsDir)) return [];
  return node_fs_1.default
    ...
    .flatMap((fileName) => {
      ...
      return records.map((record) =>
        (0, installer_migration_authoring_cjs_1.validateInstallerMigrationRecord)(record, source),
      );
    });
}
        ...
        if (lockData !== null) {
          ...
          if (isSameProcess || isDeadProcess) {
            ...
            try {
              ...
              /* unlink failed — fall through to timeout path */
            }
            if (reclaimed) continue;
          }
        }
        if (clock.now() - started >= timeoutMs) {
          const holderInfo = lockData
            ? ` (held by pid ${lockData.pid} since ${lockData.acquiredAt})`
            : "";
...
function planInstallerMigrations({
  ...
  const validatedMigrations = migrations.map((migration) =>
    (0, installer_migration_authoring_cjs_1.validateInstallerMigrationRecord)(migration),
  );
  ...
  for (const migration of pending) {
    ...
    (0, installer_migration_authoring_cjs_1.validateInstallerMigrationActions)(
      plannedActions,
      migration,
    );
...
function rollbackAppliedMigrationResult({
  configDir,
  journal,
  journalPath,
  rollbackRoot,
  ...
  for (const action of [...journal.actions].reverse()) {
    ...
    if (action.backupRelPath) {
      try {
        node_fs_1.default.rmSync(node_path_1.default.join(configDir, action.backupRelPath), {
          force: true,
        });
      } catch {
  ...
  try {
    if (previousInstallStateBytes === null) {
      node_fs_1.default.rmSync(node_path_1.default.join(configDir, INSTALL_STATE_NAME), {
        force: true,
      });
    } else {
...
function applyInstallerMigrationPlan({ configDir, plan, now = () => new Date().toISOString() }) {
  ...
  const rollbackRootRelPath = node_path_1.default.posix.join(
    "gsd-migration-journal",
    `${runId}-rollback`,
  );
  const rollbackRoot = node_path_1.default.join(configDir, rollbackRootRelPath);
  const backupRootRelPath = node_path_1.default.posix.join(
    "gsd-migration-journal",
    `${runId}-backups`,
  );
  ...
  try {
    ...
    for (const action of plan.actions) {
      ...
      if (action.type === "record-baseline" || action.type === "baseline-preserve-user") {
        journal.actions.push(
          journalAction(action, action.type === "record-baseline" ? "recorded" : "preserved"),
        );
        continue;
      }
      ...
      if (action.type === "rewrite-json") {
        if (action.deleteIfEmpty && isStructurallyEmpty(action.value)) {
          ...
          journal.actions.push(
            journalAction(action, "rewritten", {
              rollbackRelPath: node_path_1.default.posix.join(rollbackRootRelPath, normalized),
            }),
          );
        }
      ...
      if (action.type === "backup-and-remove") {
        const backupRelPath =
          action.backupRelPath || node_path_1.default.posix.join(backupRootRelPath, normalized);
    ...
    return {
      ...
      journalRelPath,
      rollback: () =>
        rollbackAppliedMigrationResult({
          configDir,
          journal,
          journalPath,
          rollbackRoot,
    ...
    if (rollbackFailures.length > 0) {
      const rollbackError = new Error(
        `migration apply failed and rollback incomplete: ${error.message}`,
      );
