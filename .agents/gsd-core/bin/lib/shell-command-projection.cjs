...
// broken hook (#3393).
function buildLocalShellHookCommand({
  localPrefix,
  hookFile,
  bashRunner,
  runtime = "generic",
...
function projectLocalHookPrefix({ runtime: _runtime = "claude", dirName, hookPathStyle }) {
  ...
  return hookPathStyle === "raw" ? dirName : `"$CLAUDE_PROJECT_DIR"/${dirName}`;
}
...
function projectManagedHookCommand({
  absoluteRunner,
  scriptPath,
  runtime = "generic",
  platform = process.platform,
...
const MANAGED_HOOK_BASENAMES_BY_SURFACE = {
  ...
  "codex-toml": new Set(["gsd-check-update.js"]),
};
...
const LEGACY_MANAGED_HOOK_ALIASES_BY_SURFACE = {
  "codex-toml": new Set(["gsd-update-check.js"]),
  "codex-hooks-json": new Set(["gsd-update-check.js"]),
};
function managedHookSurfaceSet(surface = "settings-json") {
  return (
    MANAGED_HOOK_BASENAMES_BY_SURFACE[surface] || MANAGED_HOOK_BASENAMES_BY_SURFACE["settings-json"]
  );
}
...
function managedHookCommandSurfaceSet(surface = "settings-json", includeLegacyAliases = false) {
  const base =
    MANAGED_HOOK_COMMAND_BASENAMES_BY_SURFACE[surface] ||
...
function projectPathActionProjection({ mode = "repair", targetDir, platform = process.platform }) {
  ...
  if (platform === "win32" && String(targetDir).includes('"'))
    return {
      shellActions: [],
      actionLines: [],
      reason: exports.PATH_ACTION_REASON.WIN32_RESERVED_QUOTE,
    };
...
function _isFile(candidate, requireExecutable = false, platform = process.platform) {
  try {
    if (!node_fs_1.default.statSync(candidate).isFile()) return false;
...
function resolveExecutableBinary(name, opts = {}) {
  ...
  const exts = String(envGet(env, "PATHEXT") || DEFAULT_PATHEXT)
    .split(";")
    .filter(Boolean);
...
function execTool(program, args, opts = {}) {
  ...
  const result = node_child_process_1.default.spawnSync(
    mediated ? invocation.command : program,
    mediated ? invocation.args : args,
    {
      cwd: opts.cwd,
      env: spawnEnv,
      encoding: "utf-8",
...
function probeTty(opts = {}) {
  ...
  if (platform === "win32") return null;
  try {
    const ttyPath = node_child_process_1.default
      .execFileSync("tty", [], {
        encoding: "utf-8",
