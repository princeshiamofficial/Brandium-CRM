...
const { readGsdEffectiveEffortConfig, resolveInstallTimeEffort, _getGsdEffortCatalog } =
  installEffortResolver;
...
function resolveVersionFrom(libDir) {
  try {
    const v = node_fs_1.default
      .readFileSync(node_path_1.default.join(libDir, "..", "..", "VERSION"), "utf8")
      .trim();
  ...
  try {
    ...
    /* runtime root has no package.json (e.g. Codex) */
  }
  return "";
}
let cachedVersion;
...
function mapClaudeToolsToKimiTools(claudeTools, options = {}) {
  ...
  const seen = new Set();
  const source =
    options && Object.prototype.hasOwnProperty.call(options, "source") ? options.source : null;
...
function convertClaudeCommandToClaudeSkill(content, skillName, runtime = null, cmdNames = null) {
  ...
  if (_hostBehaviors(runtime).skillPriorityFrontmatter) {
    const stem =
      typeof skillName === "string" && skillName.startsWith("gsd-")
        ? skillName.slice(4)
        : skillName;
...
const WORKFLOW_SPEC_ROOT_INCLUDE_RE =
  /^@~\/\.claude\/gsd-core\/workflows\/([A-Za-z0-9._-]+)\.md[ \t]*(\r?)$/gm;
/**
...
function resolveSpecRootReference(body) {
  ...
  if (!body.includes("@.agents/gsd-core/workflows/")) return body;
  ...
  const fenceRanges = (0, markdown_sectionizer_cjs_1.scanFencedBlocks)(lines).map(
    ({ openLineIdx, closeLineIdx }) => {
...
function convertGsdCommandReferencesToKimiSkillInvocations(content, cmdNames) {
  ...
  const colonPattern = new RegExp(
    `(?<![A-Za-z0-9_/:.-])/?gsd-(${commandGroup})(?=[^A-Za-z0-9_-]|$)`,
    "g",
  );
...
function appendAgentTools(content, grants) {
  ...
  if (inlineValue.trim()) {
    existing.push(
      ...splitToolScalars(inlineValue)
        .map(decodeToolScalar)
...
function buildKimiAgentArtifacts({
  ...
  for (const source of subagentSources) {
    ...
    const fallbackName = parsed.path
      ? node_path_1.default.basename(parsed.path, node_path_1.default.extname(parsed.path))
      : null;
  ...
  const requested =
    Array.isArray(requestedSubagents) && requestedSubagents.length > 0
      ? requestedSubagents
      : [...subagentMap.keys()];
  ...
  const rootTools = mapKimiAgentContractTools(
    parseFrontmatterTools(rootFrontmatter),
    diagnostics,
    "gsd",
  );
...
function convertClaudeCommandToWindsurfWorkflow(content, commandName) {
  ...
  if (
    ...
    !/^(?:gsd-)?[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/.test(commandName)
  ) {
    const preview =
      typeof commandName === "string"
        ? JSON.stringify(commandName.slice(0, 60))
        : String(commandName);
    throw new Error(
  ...
  const effectiveDescription = truncateWindsurfWorkflowDescription(
    singleLineDescription || `Run ${commandName}.`,
  );
...
function convertClaudeCommandToClineSkill(content, skillName, _runtime = null, cmdNames = null) {
  ...
  const shortDescription = cp.length > 1024 ? cp.slice(0, 1021).join("") + "..." : description;
  const fm = `---\nname: ${yamlIdentifier(skillName)}\ndescription: ${yamlQuote(shortDescription)}\n---`;
  return `${fm}\n${clineBody}`;
}
...
function convertClaudeToOpencodeFrontmatter(
  content,
  { isAgent = false, modelOverride = null, variant = null } = {},
) {
  ...
  for (const line of lines) {
    ...
    if (
      isAgent &&
      /^(skills|color|memory|maxTurns|permissionMode|disallowedTools):/.test(trimmed)
    ) {
      inSkippedArray = true;
      continue;
    }
    ...
    if (trimmed.startsWith("tools:")) {
      if (isAgent) {
        ...
        if (toolsValue && !toolsValue.startsWith("#")) {
          const tools = splitToolScalars(toolsValue)
            .map(decodeToolScalar)
...
function convertClaudeAgentToCopilotAgent(content, isGlobal = false) {
  ...
  const toolsArray = uniqueTools.length > 0 ? "['" + uniqueTools.join("', '") + "']" : "[]";
...
function convertClaudeAgentToQwenAgent(content) {
  ...
  let converted = content;
  if (_b["GEMINI.md"]) converted = converted.replace(/CLAUDE\.md/g, _b["GEMINI.md"]);
  // #2284(b): skips <runtime_compatibility> comparison-table content (protected region).
  if (_b["Claude Code"]) converted = applyClaudeCodeBrandSwap(converted, _b["Claude Code"]);
  if (_b[".agents/"]) converted = converted.replace(/\.claude\//g, _b[".agents/"]);
  const { frontmatter, body } = extractFrontmatterAndBody(converted);
  if (!frontmatter) return converted;
  const name = extractFrontmatterField(frontmatter, "name") || "unknown";
  const description = extractFrontmatterField(frontmatter, "description") || "";
...
function convertClaudeAgentToZcodeAgent(content) {
  ...
  while (i < fmEnd) {
    ...
    if (inlineTools && !commentOnlyHeader) {
      const grants = splitToolScalars(inlineTools[1])
        .map((tool) => tool.trim())
        .filter((tool) => tool !== "");
...
function convertClaudeCommandToOpencodeSkill(content, skillName) {
  return convertClaudeCommandToOpencodeFamilySkill(content, skillName, (c) =>
    convertClaudeToOpencodeFrontmatter(c),
  );
}
/**
...
function convertClaudeCommandToKiloSkill(content, skillName) {
  return convertClaudeCommandToOpencodeFamilySkill(content, skillName, (c) =>
    convertClaudeToKiloFrontmatter(c),
  );
}
...
function computePathPrefix({
  ...
  const posixHome = homeDir
    ? (0, shell_command_projection_cjs_1.posixNormalize)(String(homeDir))
    : homeDir;
 ...
 */
function _applyRuntimeRewrites(
  content,
  runtime,
  pathPrefix,
  isGlobal = false,
  ...
  switch (runtime) {
    ...
    case "trae":
      ...
      content = content.replace(
        new RegExp("~/" + (0, pattern_cjs_1.escapeRegex)(dirName) + "/", "g"),
        pathPrefix,
      );
      content = processAttribution(content, attribution);
      break;
 ...
 */
function applyRuntimeContentRewritesInPlace(
  stagedDir,
  runtime,
  pathPrefix,
  isGlobal = false,
 ...
 */
function applyRuntimeContentRewritesForCommandsInPlace(
  stagedDir,
  runtime,
  pathPrefix,
  isGlobal = false,
  ...
  try {
    for (const entry of installFs().readdirSync(stagedDir, { withFileTypes: true })) {
      ...
      let content = installFs().readFileSync(
...
function rewriteStagedSkillBodies(stagedDir, opts) {
  ...
  if (!installFs().existsSync(stagedDir)) return;
  const resolvedTarget = (0, shell_command_projection_cjs_1.posixNormalize)(
...
function rewriteStagedCommandBodies(stagedDir, opts) {
  ...
  const attribution = resolveAttribution ? resolveAttribution(runtime) : undefined;
  return applyRuntimeContentRewritesForCommandsInPlace(
    stagedDir,
    runtime,
    pathPrefix,
    isGlobal,
...
function applyAgentFrontmatterExtensions(content, { runtime, agentName, targetDir }) {
  ...
  if (universalEffort !== "inherit") {
    const renderedEffort = _getGsdEffortCatalog().renderEffortForRuntime(
      runtime,
      universalEffort,
    ).value;
