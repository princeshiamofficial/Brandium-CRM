...
const { resolveCapabilityRuntimeState, _resolveManifest, _resolveCommandsGsdDir } =
  capabilityStateMod;
// eslint-disable-next-line @typescript-eslint/no-require-imports
...
function setCapabilityState(cwd, runtimeConfigDir, desired, opts) {
  ...
  const capabilitiesMap =
    ...
    typeof registry["capabilities"] === "object" &&
  ...
  for (const cap of after.capabilities) {
    ...
    if (cap.enabled === true && cap.hooks.length > 0 && cap.hooks.every((h) => !h.configured)) {
      warnings.push(
...
function cmdCapabilitySet(cwd, runtimeConfigDir, capId, options, raw) {
  ...
  if (raw) {
    ...
    coreOutput(
      { capabilities: result.capabilities, warnings: result.warnings, errors: result.errors },
      true,
    );
  ...
  if (result.errors.length > 0) {
    process.stderr.write(
      `Error: capability set: ${String(result.errors.length)} error(s) — see above\n`,
    );
    throw new ExitError(1);
  }
