      ...
      if (items.length > 0 || headingsSeen > 0) {
        const entry = {
          ...
          file_path: toPosixPath(
            node_path_1.default.relative(cwd, node_path_1.default.join(phaseDir, file)),
          ),
          type: "uat",
          status,
    ...
    if (files.includes(deferredFile)) {
      ...
      if (items.length > 0) {
        results.push({
          ...
          file: deferredFile,
          file_path: toPosixPath(
            node_path_1.default.relative(cwd, node_path_1.default.join(phaseDir, deferredFile)),
          ),
          type: "deferred",
          status: "unresolved",
          archived_milestone: milestone,
...
function cmdRenderCheckpoint(cwd, options = {}, raw) {
  ...
  const resolvedPath = (0, security_cjs_1.requireSafePath)(filePath, cwd, "UAT file", {
    allowAbsolute: true,
  });
  ...
  const responseLanguage =
    typeof config.response_language === "string" ? config.response_language : undefined;
...
function parseFirstPendingTest(content) {
  ...
  const testsSection = collectSection(content, (h) => /^tests$/i.test(h.text) && h.level === 2, {
    levelBounded: true,
  });
  if (!testsSection) {
    return null;
 ...
 * specific key.
 */
const ANY_KEY_SCALAR_HEADER_LINE_RE = new RegExp(
  String.raw`^[A-Za-z_][\w-]*:${SCALAR_HEADER_BODY}$`,
);
/**
...
function extractScalarField(block, key) {
  ...
  const blockMatch =
    block.match(new RegExp(`${opener}([\\s\\S]*?)(?=^\\w[\\w-]*:\\s)`, "m")) ||
...
function isolateCheckpointFrameText(text, frame) {
  return frame.direction === "rtl" ? `${RTL_ISOLATE}${text}${POP_DIRECTIONAL_ISOLATE}` : text;
}
...
function parseUatItemsWithStats(content) {
  ...
  const allHeadings = tokenizeHeadings(blankIndentedFenceDelimiters(content)).filter((h) =>
    isColumnZeroHeading(content, h),
  );
...
function parseGapsItems(content) {
  const gapsSection = collectSection(content, (h) => /^gaps$/i.test(h.text) && h.level === 2, {
    levelBounded: true,
  });
...
function parseDeferredItemsWithStatus(content) {
  ...
  items.push(
    ...parseDeferredTableItems(sectionBody).map((item) => ({ name: item.name, status: "" })),
  );
  return items;
}
...
function acknowledgeDeferredItem(content, targetText) {
  ...
  if (headingEntries !== null) {
    return acknowledgeHeadingShapedEntry({
      content,
      sectionBody,
      deferredSection,
      headingEntries,
  ...
  const statusLineIdx = entryFieldLines(matchedLines, DEFERRED_BULLET_MARKERS).findIndex(
  ...
  let newMatchedLines;
  if (statusLineIdx === -1) {
    const bulletIndentMatch = matchedLines[0]
      .replace(/\r$/, "")
      .match(DEFERRED_BULLET_MARKERS.strip);
    ...
    const eofScope =
      sectionBody.slice(deferredSection ? 0 : entries[0].start, start) ||
      sectionBody.slice(0, start);
    const crlf =
      matchedLines.length > 1
        ? line0HadCr
        : content.startsWith("\r\n", spanEnd) || (spanEnd >= content.length && crlfAtEof(eofScope));
    newMatchedLines = [
  ...
  // runtime assertion.
  const newContent =
    content.slice(0, matchIndexInContent) +
    newMatchedLines.join("\n") +
 ...
 */
function acknowledgeHeadingShapedEntry({
  content,
  sectionBody,
  deferredSection,
  headingEntries,
  ...
  if (statusLineIdx === -1) {
    ...
    const prevCr = followsLast
      ? content.startsWith("\r\n", spanEnd) ||
        (spanEnd >= content.length && crlfAtEof(sectionBody.slice(0, entry.start)))
    ...
    newRawLines.splice(
      last + 1,
      0,
      `${indent}status: acknowledged${!followsLast && prevCr ? "\r" : ""}`,
    );
  } else {
    ...
    const field = parseGapEntryFieldLine(
      reader,
      DEFERRED_BULLET_MARKERS,
      stripsMarkerAt(statusLineIdx, entry.opener),
    );
...
class ListRuns {
  byIndent = new Set();
  at(indent) {
    return this.byIndent.has(indent);
  }
  opened(indent) {
...
function splitDeferredHeadingEntriesDetailed(sectionBody) {
  ...
  const flushPending = () => {
    if (pendingStartLine !== -1) {
      ...
      for (const { lines: entryLines, opener, start, end } of splitGapsEntriesCore(
        region,
        DEFERRED_BULLET_MARKERS,
      )) {
        entries.push({
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      scan.unterminatedFrom !== -1 &&
      i > scan.unterminatedFrom &&
      topLevelItemShape(line, DEFERRED_BULLET_MARKERS, bodyBase)
    ) {
...
function splitGapsEntriesCore(sectionBody, markers = HYPHEN_BULLET_MARKERS) {
  ...
  rawLines.forEach((rawLine, idx) => {
    ...
    if (
      scan.unterminatedFrom !== -1 &&
      idx > scan.unterminatedFrom &&
      topLevelItemShape(line, markers, baseIndent)
    ) {
    ...
    if (!markers.blockStructure) {
      ...
      if (
        bulletMatch &&
        isSeparatorShaped(line, bulletMatch[0].length) &&
...
function entryFieldLines(entryLines, markers = HYPHEN_BULLET_MARKERS, openerFlags) {
  ...
  return entryLines.map((rawLine, idx) =>
    fenced.has(idx)
      ? null
      : parseGapEntryFieldLine(rawLine, markers, stripsMarkerAt(idx, openerFlags)),
  );
}
/**
...
function entryFencedLines(entryLines, markers, openerFlags) {
  if (!markers.blockStructure) return new Set();
...
function parseGapEntryFieldLine(rawLine, markers = HYPHEN_BULLET_MARKERS, stripMarker = true) {
  ...
  const headLen = bulletStripped
    ? line.length - bare.length
...
function rawGapEntryText(entryLines, markers = HYPHEN_BULLET_MARKERS, openerFlags) {
  return (
    entryLines
      ...
      .map((l, i) =>
        i === 0 && stripsMarkerAt(0, openerFlags) ? l.replace(markers.strip, "$2") : l.trim(),
      )
      .join(" ")
      .trim()
  );
  ...
  if (hvSection) {
    ...
    for (const line of lines) {
      ...
      if (tableCells) {
        ...
        const hasPassResult = tableCells
          .slice(2)
          .some((c) => /^pass$/i.test(c) || /^resolved$/i.test(c));
