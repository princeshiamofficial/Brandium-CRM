...
function findProjectRoot(startDir) {
  ...
  try {
    ...
    if (
      node_fs_1.default.existsSync(ownPlanningDir) &&
      node_fs_1.default.statSync(ownPlanningDir).isDirectory()
    ) {
      return startDir;
    }
  } catch {
  ...
  function isInsideGitRepo(candidateParent) {
    ...
    while (d !== fsRoot) {
      try {
        if (node_fs_1.default.existsSync(d + node_path_1.default.sep + ".git")) return true;
      } catch {
        // ignore
      }
  ...
  function nearestGitRoot(from, upTo) {
    ...
    while (d !== fsRoot) {
      if (d === upTo) break;
      try {
        if (node_fs_1.default.existsSync(d + node_path_1.default.sep + ".git")) return d;
      } catch {
        // ignore
      }
      const next = node_path_1.default.dirname(d);
  ...
  while (dir !== fsRoot && depth < FIND_PROJECT_ROOT_MAX_DEPTH) {
    ...
    try {
      parentPlanningIsDir =
        node_fs_1.default.existsSync(parentPlanning) &&
        node_fs_1.default.statSync(parentPlanning).isDirectory();
    } catch {
    ...
    if (parentPlanningIsDir) {
      ...
      if (isInsideGitRepo(parent)) {
        ...
        while (
          ancestor !== fsRoot &&
          ancestor !== home &&
          ancestorDepth < FIND_PROJECT_ROOT_MAX_DEPTH
        ) {
          const ancestorPlanning = ancestor + node_path_1.default.sep + ".planning";
          try {
            if (
              node_fs_1.default.existsSync(ancestorPlanning) &&
              node_fs_1.default.statSync(ancestorPlanning).isDirectory()
            ) {
              const ancestorConfig =
                ancestor +
                node_path_1.default.sep +
                ".planning" +
                node_path_1.default.sep +
                "config.json";
  ...
  while (dir2 !== fsRoot && depth2 < FIND_PROJECT_ROOT_MAX_DEPTH) {
    ...
    try {
      ...
      if (
        node_fs_1.default.existsSync(candidatePlanning) &&
        node_fs_1.default.statSync(candidatePlanning).isDirectory()
      ) {
        // #2843: do not cross a git-repo boundary. If the caller is inside its
