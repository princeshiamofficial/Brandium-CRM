...
function assertFetchableUrl(rawUrl) {
  ...
  if (host.startsWith("::ffff:")) {
    ...
    if (/^\d{1,3}(?:\.\d{1,3}){3}$/.test(suffix)) {
      ...
      if (
        hextets.length === 2 &&
        /^[\da-f]{1,4}$/.test(hextets[0]) &&
        /^[\da-f]{1,4}$/.test(hextets[1])
      ) {
...
function copyDirRecursive(src, dest, budget = { entries: 0, bytes: 0 }) {
  ...
  try {
    ...
    try {
      ...
      /* best-effort: no fd leak per opened Dir */
    }
  }
}
/**
...
function assertSafeTarMembers(execTar, tgzPath) {
  ...
  for (const line of listing.stdout.split("\n")) {
    ...
    if (
      member.startsWith("/") ||
      node_path_1.default.isAbsolute(member) ||
...
function parseSpec(spec) {
  ...
  if (
    ...
    if (ref !== undefined && (SHELL_METACHAR_RE.test(ref) || ref.startsWith("-"))) {
      ...
      throw new Error(
        `Unsafe git ref (shell metacharacters or leading dash not allowed): "${ref}"`,
      );
    }
...
function resolveLocal(parsed, opts, gsdHome, hostVersion) {
  ...
  return stageValidated({
    ...
    skipEnginesGate: opts.skipEnginesGate,
  });
}
function resolveGit(parsed, opts, gsdHome, hostVersion) {
  ...
  if (opts.integrity) {
    throw new Error(
      "integrity pinning is not supported for git sources; pin the commit with #sha:<commit>",
    );
  }
  const cloneDir = node_fs_1.default.mkdtempSync(
    node_path_1.default.join(node_os_1.default.tmpdir(), "gsd-cap-git-"),
  );
  try {
    // Clone (copy only — no hooks execute on clone, no npm install).
    const cloneResult = execGit(["clone", "--depth", "1", "--", parsed.target, cloneDir], {
      timeout: 60_000,
    });
    ...
    if (parsed.ref) {
      const checkoutResult = execGit(["-C", cloneDir, "checkout", parsed.ref, "--"], {
        timeout: 60_000,
      });
...
function resolveNpm(parsed, opts, gsdHome, hostVersion) {
  ...
  const tmpPackDir = node_fs_1.default.mkdtempSync(
    node_path_1.default.join(node_os_1.default.tmpdir(), "gsd-cap-npm-pack-"),
  );
  const extractDir = node_fs_1.default.mkdtempSync(
    node_path_1.default.join(node_os_1.default.tmpdir(), "gsd-cap-npm-ext-"),
  );
  try {
    ...
    const sourceDir = node_fs_1.default.existsSync(
      node_path_1.default.join(packageDir, "capability.json"),
    )
      ? packageDir
      : extractDir;
...
async function resolveTarball(parsed, opts, gsdHome, hostVersion) {
  ...
  const extractDir = node_fs_1.default.mkdtempSync(
    node_path_1.default.join(node_os_1.default.tmpdir(), "gsd-cap-tar-"),
  );
...
function peekLatestVersion(source, opts = {}) {
  ...
  switch (parsed.kind) {
    case "git": {
      ...
      if (refKind.kind === "sha" || refKind.kind === "tag") {
        return {
          status: "pinned",
          version: null,
          reason: `git source pinned to ${refKind.kind} "${refKind.value}"; update will not move it`,
        };
      }
      if (refKind.kind === "bare") {
        ...
        if (resolved === "tag") {
          ...
          return {
            status: "pinned",
            version: null,
            reason: `git source ref "${refKind.value}" resolves to an immutable tag; update will not move it`,
          };
        }
      ...
      if (!r || r.exitCode !== 0 || r.signal) {
        const reason =
          r && r.signal
            ? `git ls-remote timed out (signal ${r.signal})`
    ...
    case "npm": {
      ...
      if (selector !== "" && NPM_VERSION_RE.test(selector)) {
        return {
          status: "pinned",
          version: selector,
          reason: `npm source pinned to exact version "${selector}"; update will not move it`,
        };
      }
      const execNpm = opts.execOverrides?.npm ?? shellSeam.execNpm;
      let r;
      ...
      if (!r || r.exitCode !== 0 || r.signal) {
        const reason =
          r && r.signal
            ? `npm view timed out (signal ${r.signal})`
      ...
      if (version === null) {
        return {
          status: "unknown",
          version: null,
          reason: `npm view returned no matching semver version: ${(r.stdout || "").trim() || "(empty)"}`,
        };
      }
    ...
    case "local": {
      ...
      let cap;
      try {
        const manifestPath = node_path_1.default.join(
          node_path_1.default.resolve(parsed.target),
          "capability.json",
        );
        cap = readManifestBounded(
          manifestPath,
          `local capability.json not readable: ${parsed.target}`,
        );
      } catch (err) {
      ...
      if (!version)
        return {
          status: "unknown",
          version: null,
          reason: "local capability.json missing version",
        };
      return { status: "ok", version };
    }
    case "tarball":
      ...
      return {
        status: "manual",
        version: null,
        reason: "tarball sources cannot be auto-checked; re-install from a new URL",
      };
    case "registry":
      ...
      return {
        status: "unsupported",
        version: null,
        reason: "registry source kind is not yet implemented",
      };
    default: {
      const _never = parsed.kind;
