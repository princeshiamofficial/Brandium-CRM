...
function sameDirectory(a, b) {
  if (node_path_1.default.resolve(a) === node_path_1.default.resolve(b)) return true;
  const ia = identify(a);
  const ib = identify(b);
...
function assertTestHomeSandboxed(operation, runtime, kinds, deps = {}) {
  ...
  if (realHome.kind === "ok") {
    ...
    for (const kind of overriding) {
      const declared = node_path_1.default.resolve(
        node_path_1.default.join(kind.home, kind.destSubpath ?? ""),
      );
      ...
      if (derivesFromSandboxedHome(dest, effectiveHome, realHome, passwdHome)) continue;
      throw refusal(
...
function resolveThroughLinks(dest) {
  ...
  for (;;) {
    try {
      ...
      if (parent === cur) return node_path_1.default.resolve(dest);
