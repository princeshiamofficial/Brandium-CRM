...
function parseQuickBatchArgs(args) {
  ...
  if (args.includes("--discuss")) {
    return {
      ok: false,
      reason:
        "quick-batch does not support --discuss in v1 — use /gsd-quick --discuss per item instead",
    };
  }
  ...
  if (jobsIdx !== -1) {
    ...
    if (raw === undefined || raw.startsWith("--")) {
      return {
        ok: false,
        reason: 'quick-batch --jobs requires a value ("auto" or a positive integer)',
      };
    }
    if (raw === "auto") {
      jobs = "auto";
      ...
      if (!Number.isFinite(n) || !Number.isInteger(n) || n <= 0) {
        return {
          ok: false,
          reason: `quick-batch --jobs must be "auto" or a positive integer, got ${JSON.stringify(raw)}`,
        };
      }
      jobs = n;
    }
...
function computeEffectiveConcurrency(input) {
  let concurrency =
    input.jobs === "auto" ? input.capacity : Math.min(input.taskCount, input.jobs, input.capacity);
...
function routeVerificationOutcome(status) {
  switch (status) {
    ...
    case "gaps_found":
      return {
        action: "fail",
        failureReason: "verification reported gaps_found (no automatic gap-fix retry in v1)",
      };
  }
}
/**
