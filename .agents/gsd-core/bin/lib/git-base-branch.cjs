      ...
      try {
        ...
        if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
          ...
          config = {
            ...
            allow_default_branch_commits: _readGitNested(
              normalized,
        ...
        /* malformed direct edit contributes no policy values */
      }
    }
  } else {
    try {
      ...
      /* configuration loading is fail-soft for branch resolution */
    }
  }
  const rawBaseBranch = config.base_branch;
  const baseBranch =
    typeof rawBaseBranch === "string" && rawBaseBranch.trim() ? rawBaseBranch.trim() : null;
...
function trySymbolicRef(cwd, execGit) {
  try {
    const r = execGit(["symbolic-ref", "--quiet", "--short", "refs/remotes/origin/HEAD"], {
      cwd,
      timeout: 5_000,
    });
...
function phaseStartCommit(cwd, phaseDir, execGit) {
  ...
  try {
    ...
    const r = git(["log", "--format=%H", "--diff-filter=A", "-1", "--", pathspec], {
      cwd,
      timeout: 15_000,
    });
    if (r.exitCode !== 0 || !r.stdout) return null;
