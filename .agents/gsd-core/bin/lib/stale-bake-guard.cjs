...
function resolveAgentDir(runtime, { env = process.env, homedir = os.homedir } = {}) {
  if (runtime === "opencode") {
    const base =
      (env.OPENCODE_CONFIG_DIR && String(env.OPENCODE_CONFIG_DIR).trim()) ||
  ...
  if (runtime === "kilo") {
    const base =
      (env.KILO_CONFIG_DIR && String(env.KILO_CONFIG_DIR).trim()) ||
      path.join(homedir(), ".config", "kilo");
    return path.join(base, "agents");
  }
  if (runtime === "codex") {
    const base =
      (env.CODEX_HOME && String(env.CODEX_HOME).trim()) || path.join(homedir(), ".codex");
    return path.join(base, "agents");
  }
  return null;
}
...
function findOldestAgentMtime(
  ...
  for (const entry of entries) {
    ...
    const isAgentFile =
      ((runtime === "opencode" || runtime === "kilo") && entry.name.endsWith(".md")) ||
