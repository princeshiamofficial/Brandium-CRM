...
// ─── Constants ───────────────────────────────────────────────────────────────
const DEFAULT_STORE_DIR = node_path_1.default.join(
...
function learningsWrite(entry, opts) {
  ...
  if (opts && opts.dedupeIndex) {
    ...
    (0, shell_command_projection_cjs_1.platformWriteSync)(
      node_path_1.default.join(dir, `${id}.json`),
...
function learningsDelete(id, opts) {
  ...
  const dir = getStoreDir(opts);
  const filePath = node_path_1.default.join(dir, `${id}.json`);
  if (!node_fs_1.default.existsSync(filePath)) return false;
...
function learningsCopyFromProject(planningDir, opts) {
  ...
  const sourceProject =
    (opts && opts.sourceProject) ||
    node_path_1.default.basename(node_path_1.default.resolve(planningDir, ".."));
...
function learningsPrune(olderThan, opts) {
  ...
  const dir = getStoreDir(opts);
  if (!node_fs_1.default.existsSync(dir)) return { removed: 0, kept: 0 };
  const files = node_fs_1.default.readdirSync(dir).filter((f) => f.endsWith(".json"));
  let removed = 0;
  let kept = 0;
  for (const file of files) {
