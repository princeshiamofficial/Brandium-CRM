...
function validateEntry(entry, index, seenIds) {
  ...
  if (!isPlainObject(entry)) {
    errors.push({
      index,
      id: null,
      code: ERROR_CODE.MALFORMED_ENTRY,
    ...
    v.forEach((ve, vi) => {
      if (!isPlainObject(ve)) {
        push(
          ...
          "verification item is not a mapping",
          `verification[${vi}]`,
        );
        return;
      }
      if (typeof ve.kind !== "string" || !VALID_KINDS.includes(ve.kind)) {
        push(
          ...
          `verification kind must be one of ${VALID_KINDS.join(", ")}`,
          `verification[${vi}].kind`,
        );
      }
      if (typeof ve.status !== "string" || !VALID_STATUSES.includes(ve.status)) {
        push(
          ERROR_CODE.INVALID_STATUS,
          `verification status must be one of ${VALID_STATUSES.join(", ")}`,
          `verification[${vi}].status`,
        );
      }
    ...
    return legacyResult(summaryFile, [
      {
        ...
        message:
          "coverage block is present but could not be parsed into entries; falling back to prose extraction",
      },
    ]);
  }
...
function cmdClassify(cwd, options = {}, raw) {
  ...
  let resolvedPath;
  try {
    resolvedPath = (0, security_cjs_1.requireSafePath)(filePath, cwd, "SUMMARY file", {
      allowAbsolute: true,
    });
  } catch (e) {
  ...
  const result = classifyContent(
    content,
    toPosixPath(node_path_1.default.relative(cwd, resolvedPath)),
  );
