...
const { withPlanningLock, planningDir, planningRoot, planningPaths, quickDirFrom } =
  planningWorkspace;
// eslint-disable-next-line @typescript-eslint/no-require-imports
...
function dateAndStartBlock(instant) {
  ...
  const secondsSinceMidnight =
    instant.getHours() * 3600 + instant.getMinutes() * 60 + instant.getSeconds();
...
function allocateIdsGivenUsed(dateStr, startBlock, count, used) {
  ...
  while (result.length < count) {
    if (block > MAX_TIME_BLOCK) {
      throw new Error(
        `quick-batch: exhausted collision-free quick ids for ${dateStr} (advanced past block ${MAX_TIME_BLOCK})`,
      );
    }
...
function collectExistingBatchQuickIds(cwd, used) {
  ...
  for (const name of entries) {
    ...
    const parsed = (0, security_cjs_1.safeJsonParse)(raw, {
      maxLength: 1048576,
      label: "sibling BATCH.json",
    });
...
function parseTaskList(text) {
  ...
  if (items.length < 2) {
    return {
      ok: false,
      reason: `quick-batch requires at least 2 explicit tasks, found ${items.length}`,
    };
  }
...
function parseTaskListFromFile(cwd, filePath) {
  ...
  let safePath;
  try {
    safePath = (0, security_cjs_1.requireSafePath)(filePath, root, "quick-batch --file", {
      allowAbsolute: true,
    });
  } catch (err) {
    return { ok: false, reason: err instanceof Error ? err.message : String(err) };
  ...
  try {
    ...
    return {
      ok: false,
      reason: `unable to stat --file path: ${err instanceof Error ? err.message : String(err)}`,
    };
  }
  if (!stat.isFile()) {
  ...
  try {
    ...
    return {
      ok: false,
      reason: `unable to read --file path: ${err instanceof Error ? err.message : String(err)}`,
    };
  }
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    for (const ref of deps) {
      ...
      if (idx === null) {
        return {
          ok: false,
          reason: `item ${i} (${items[i].description}) declares an unknown dependency reference: ${JSON.stringify(ref)}`,
        };
      }
      if (idx === i) {
        return {
          ok: false,
          reason: `item ${i} (${items[i].description}) declares a dependency on itself`,
        };
      }
      adj[i].push(idx);
    }
    ...
    return withPlanningLock(
      ...
      () => {
        ...
        const items = itemsInput.map((input, i) => ({
          ...
          planned_files: (input.plannedFiles ?? []).map(
            shell_command_projection_cjs_1.posixNormalize,
          ),
        ...
        for (const it of items) it.wave = waveOf.get(it.quick_id);
        const manifest = {
          schema_version: 1,
          batch_id: batchId,
...
function validateBatchSchema(parsed, batchId) {
  ...
  if (obj.schema_version !== 1) {
    return {
      ok: false,
      reason: `BATCH.json for batch ${batchId} has an unsupported or missing schema_version`,
    };
  }
  if (typeof obj.batch_id !== "string" || obj.batch_id !== batchId) {
    return {
      ok: false,
      reason: `BATCH.json for batch ${batchId} has a mismatched or missing batch_id`,
    };
  }
  ...
  if (
    obj.base_revision !== null &&
    obj.base_revision !== undefined &&
    typeof obj.base_revision !== "string"
  ) {
    return {
      ok: false,
      reason: `BATCH.json for batch ${batchId} has an invalid base_revision field`,
    };
  }
  ...
  for (const raw of obj.items) {
    ...
    if (typeof it.quick_id !== "string" || !QUICK_ID_RE.test(it.quick_id)) {
      return {
        ok: false,
        reason: `BATCH.json for batch ${batchId} has an item with an invalid quick_id`,
      };
    }
    if (seenIds.has(it.quick_id)) {
      return {
        ok: false,
        reason: `BATCH.json for batch ${batchId} has a duplicate quick_id: ${it.quick_id}`,
      };
    }
    ...
    if (
      typeof it.worktree === "string" &&
      it.worktree !== "" &&
      !node_fs_1.default.existsSync(it.worktree)
    ) {
      return {
        ok: false,
        reason: `item ${it.quick_id} references a worktree that does not exist on disk: ${it.worktree}`,
      };
    }
    ...
    if (
      it.dispatched_worktree !== null &&
      it.dispatched_worktree !== undefined &&
      typeof it.dispatched_worktree !== "string"
    ) {
      return { ok: false, reason: `item ${it.quick_id} has an invalid dispatched_worktree field` };
    }
    if (
      it.dispatched_branch !== null &&
      it.dispatched_branch !== undefined &&
      typeof it.dispatched_branch !== "string"
    ) {
      return { ok: false, reason: `item ${it.quick_id} has an invalid dispatched_branch field` };
    }
    if (
      it.dispatched_base !== null &&
      it.dispatched_base !== undefined &&
      typeof it.dispatched_base !== "string"
    ) {
      return { ok: false, reason: `item ${it.quick_id} has an invalid dispatched_base field` };
    }
    if (
      it.wave !== undefined &&
      (typeof it.wave !== "number" || !Number.isInteger(it.wave) || it.wave < 0)
    ) {
    ...
    if (
      ...
      it.failure_reason !== undefined &&
      typeof it.failure_reason !== "string"
    ) {
    ...
    items.push({
      ...
      dispatched_worktree:
        typeof it.dispatched_worktree === "string" ? it.dispatched_worktree : null,
  ...
  for (const it of items) {
    for (const dep of it.depends_on) {
      if (!seenIds.has(dep)) {
        return {
          ok: false,
          reason: `item ${it.quick_id} depends on ${dep}, which is not present in this batch`,
        };
      }
    }
  }
...
function loadBatch(cwd, batchId) {
  ...
  if (!parsed.ok) {
    return {
      ok: false,
      reason: `BATCH.json for batch ${batchId} is not valid JSON: ${parsed.error ?? "unknown parse error"}`,
    };
  }
...
function hasQuickTaskRow(stateContent, quickId) {
  ...
  const sections = (0, markdown_sectionizer_cjs_1.collectSections)(stateContent, (h) =>
    QUICK_TASKS_HEADING_RE.test(h.text.trim()),
  );
  ...
  try {
    return withPlanningLock(
      ...
      () => {
        ...
        const stateContent = node_fs_1.default.existsSync(statePath)
      ...
      () => {
        ...
        for (const update of updates) {
          ...
          if (update.dependsOn !== undefined) {
            for (const dep of update.dependsOn) {
              if (dep === update.quickId) {
                return {
                  ok: false,
                  reason: `item ${update.quickId} declares a dependency on itself`,
                };
              }
              if (!byId.has(dep)) {
                return {
                  ok: false,
                  reason: `item ${update.quickId} declares an unknown dependency reference: ${JSON.stringify(dep)}`,
                };
              }
            }
          ...
          if (update.plannedFiles !== undefined) {
            item.planned_files = update.plannedFiles.map(
              shell_command_projection_cjs_1.posixNormalize,
            );
          }
        ...
        for (const it of manifest.items) it.wave = waveOf.get(it.quick_id);
        (0, shell_command_projection_cjs_1.platformWriteSync)(
          batchManifestPath(cwd, batchId),
          JSON.stringify(manifest, null, 2) + "\n",
        );
      ...
      () => {
        ...
        const eligible = manifest.items
          .filter(
            (it) =>
              it.status === "pending" &&
              it.depends_on.every((d) => byId.get(d)?.status === "complete"),
          )
          .map((it) => it.quick_id);
