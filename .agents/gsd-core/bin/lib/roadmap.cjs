...
const { planningPaths, withPlanningLock, findContextMdIn, resolvePhaseIdConvention } =
  planningWorkspace;
...
function searchPhaseInContent(content, escapedPhase, phaseNum, convention) {
  ...
  const headerMatch =
    headingIndex === -1 ? null : headings[headingIndex].text.match(headingPattern);
  if (!headerMatch) {
...
function cmdRoadmapGetPhase(cwd, phaseNum, raw) {
  ...
  try {
    ...
    for (const source of roadmapPhaseLookupSources(phaseNum)) {
      ...
      if (!malformed)
        malformed = milestoneResult?.error
          ? milestoneResult
          : fullResult?.error
            ? fullResult
            : null;
    ...
    const tableHit =
      collectTablePhaseRows(milestoneContent).find(
        (tr) => stripPad(tr.id) === stripPad(phaseNum),
      ...
      output(
        {
          found: true,
          phase_number: phaseNum,
          phase_name: tableHit.name ?? `Phase ${tableHit.id}`,
          goal: null,
          section: tableHit.row.trim(),
        },
        raw,
        tableHit.row.trim(),
      );
...
function collectAnalyzePhases(content, phasesDir, phaseDirNames, convention) {
  ...
  const phasePattern = new RegExp(
    `#{2,4}\\s*${phaseHeadingPrefixSrcFor(PHASE_HEADING_BASELINE.ANY_BRACKET, convention, true)}([A-Za-z]?\\d+[A-Z]?(?:[.-]\\d+)*)(?:\\s*\\([^)\\n]{0,200}\\))?\\s*:\\s*([^\\n]+)`,
    "gi",
  );
  ...
  while ((match = phasePattern.exec(content)) !== null) {
    ...
    const nextHeader = restOfContent.match(
      new RegExp(
        `\\n#{2,4}\\s+${phaseHeadingPrefixSrcFor(PHASE_HEADING_BASELINE.ANY_BRACKET, convention)}[A-Za-z]?\\d[\\d.-]*`,
        "i",
      ),
    );
    ...
    const checkboxPattern = new RegExp(
      `-\\s*\\[(x| )\\]\\s*.*${phaseHeadingPrefixSrcFor(PHASE_HEADING_BASELINE.LABEL_ONLY, convention)}${phaseMarkdownRegexSource(phaseNum)}${OPTIONAL_PHASE_TAG_SOURCE}[:\\s]`,
      "i",
    );
  ...
  for (const tr of collectTablePhaseRows(content)) {
    ...
    if (dirMatchA) {
      ...
      tHasContext = node_fs_1.default.existsSync(
        node_path_1.default.join(phasesDir, dirMatchA, "CONTEXT.md"),
      );
      tHasResearch = node_fs_1.default.existsSync(
        node_path_1.default.join(phasesDir, dirMatchA, "RESEARCH.md"),
      );
...
function cmdRoadmapAnalyze(cwd, raw) {
  ...
  if (phases.length === 0 && scope !== SCOPE.COMPLETE && _phaseDirNames.length > 0) {
    ...
    const fallbackCollection = collectAnalyzePhases(
      fallbackContent,
      phasesDir,
      _phaseDirNames,
      convention,
  ...
  // phase-id-owner: uses the [.-] (dot-or-dash) separator variant, not the canonical dot-only token; a swap to PHASE_NUMBER_TOKEN_SOURCE would drop hyphenated phase-id matches.
  const checklistPattern = new RegExp(
    `-\\s*\\[[ x]\\]\\s*\\*\\*${phaseHeadingPrefixSrcFor(PHASE_HEADING_BASELINE.LABEL_ONLY, convention, true)}([A-Za-z]?\\d+[A-Z]?(?:[.-]\\d+)*)`,
    "gi",
  );
  ...
  const missingDetails = [
    ...new Set(
      checklistOccurrences
        .filter(
          (o) =>
            !detailKeys.has(occurrenceKey(o.token, o.bracketId)) &&
  ...
  try {
    ...
    /* progressScope stays the pessimistic SCOPE.UNREADABLE default */
  }
  const progressPercent =
    progressScope === SCOPE.COMPLETE
...
function cmdRoadmapMilestoneScope(cwd, raw) {
  ...
  if (!node_fs_1.default.existsSync(roadmapPath)) {
    output(
      { error: "ROADMAP.md not found", scope: SCOPE.UNREADABLE, phases: [], phase_count: 0 },
      raw,
      undefined,
    );
    return;
  ...
  const { value: window, scope } = extractCurrentMilestoneScoped(
    rawContent,
    cwd,
    undefined,
    phaseIdConvention,
...
function cmdRoadmapUpdatePlanProgress(cwd, phaseNum, raw) {
  ...
  withPlanningLock(cwd, () => {
    ...
    roadmapContent = replaceInCurrentMilestone(
      roadmapContent,
      planCountPattern,
      (_match, label, existingCount, trailing) => {
        if (existingCount) {
    ...
    if (missingPlans.length > 0) {
      ...
      if (lastDetailsClose === -1) {
        ...
        withRows =
          regionA !== activeRegion ? regionA : scopedReplace(activeRegion, insertRowsPatternB);
      } else {
...
function cmdRoadmapAnnotateDependencies(cwd, phaseNum, raw) {
  ...
  if (!node_fs_1.default.existsSync(roadmapPath)) {
    declineNoOp(
      ...
      "updated",
      "ROADMAP.md not found",
      "roadmap annotate-dependencies skipped — ROADMAP.md not found.",
    );
    return;
  }
  const phaseInfo = findPhaseInternal(cwd, phaseNum);
  ...
  for (const planFile of phaseInfo.plans) {
    const planPath = node_path_1.default.join(
      node_path_1.default.resolve(cwd, phaseInfo.directory),
      planFile,
    );
    try {
  ...
  withPlanningLock(cwd, () => {
    ...
    const phaseHeaderPattern = new RegExp(
      `(#{2,4}\\s*Phase\\s+${phaseEscaped}${OPTIONAL_PHASE_TAG_SOURCE}:[^\\n]*)`,
      "i",
    );
    ...
    const listLines = (0, text_lines_cjs_1.splitLines)(existingList).filter((l) =>
      /^\s*-\s*\[/.test(l),
    );
