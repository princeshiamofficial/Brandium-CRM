...
exports.RUNTIMES_WITH_FAST_MODE =
  exports.EFFORT_ARGV =
  exports.EFFORT_RENDERING =
  exports.CLAUDE_AGENT_ALIASES =
  exports.KNOWN_PROVIDERS =
  exports.CODEX_MODEL_EFFORT =
  exports.PROVIDER_PRESETS =
  exports.RUNTIMES_WITH_REASONING_EFFORT =
  exports.KNOWN_RUNTIMES =
  exports.RUNTIME_PROFILE_MAP =
  exports.MODEL_ALIAS_MAP =
  exports.AGENT_DEFAULT_TIERS =
  exports.AGENT_TO_PHASE_TYPE =
  exports.MODEL_PROFILES =
  exports.ADAPTIVE_TIER_VALUES =
  exports.VALID_TIERS =
  exports.VALID_AGENT_TIERS =
  exports.VALID_PHASE_TYPES =
  exports.VALID_PROFILES =
  exports.catalog =
    void 0;
...
const _catalogCandidates = [
  ...
  ...(process.env["GSD_MODEL_CATALOG"]
    ? [node_path_1.default.resolve(process.env["GSD_MODEL_CATALOG"])]
    : []),
];
let catalog = null;
...
exports.MODEL_PROFILES = Object.fromEntries(
  Object.entries(_catalog.agents).map(([agent, meta]) => [
    agent,
    {
      quality: meta.golden,
...
exports.AGENT_TO_PHASE_TYPE = Object.fromEntries(
  Object.entries(_catalog.agents).map(([agent, meta]) => [agent, meta.phaseType]),
);
exports.AGENT_DEFAULT_TIERS = Object.fromEntries(
  Object.entries(_catalog.agents).map(([agent, meta]) => [agent, meta.routingTier]),
);
exports.MODEL_ALIAS_MAP = Object.fromEntries(
  Object.entries(_catalog.runtimeTierDefaults["claude"] ?? {}).map(([tier, entry]) => [
    tier,
    entry?.model,
  ]),
);
...
exports.RUNTIMES_WITH_REASONING_EFFORT = new Set(
...
exports.CODEX_MODEL_EFFORT = Object.fromEntries(
...
const CODEX_EFFORT_BASELINE =
  exports.CODEX_MODEL_EFFORT["_baseline"] && exports.CODEX_MODEL_EFFORT["_baseline"].size > 0
...
function advertisedCodexEffort(model) {
  ...
  return Object.prototype.hasOwnProperty.call(exports.CODEX_MODEL_EFFORT, model)
    ? exports.CODEX_MODEL_EFFORT[model]
    : CODEX_EFFORT_BASELINE;
}
...
exports.KNOWN_PROVIDERS = new Set(
  Object.entries(exports.PROVIDER_PRESETS)
    .filter(([, tiers]) =>
      Object.values(tiers).some(
        (budgets) => budgets && Object.values(budgets).some((entry) => entry && entry.model),
      ),
    )
    .map(([name]) => name),
);
...
function getAgentToModelMapForProfile(normalizedProfile) {
  const profile = exports.VALID_PROFILES.includes(normalizedProfile)
    ? normalizedProfile
    : "balanced";
  const out = {};
...
function renderEffortForRuntime(runtime, universalEffort, model) {
  ...
  if (runtime === "codex") {
    ...
    if (universalEffort === "ultra") {
      return {
        ...
        clamped: false,
        reason:
          "'ultra' turns on Codex's automatic task delegation, which would let Codex spawn agents underneath GSD's own orchestration (#2167); GSD rejects it regardless of what the model advertises.",
      };
    }
  ...
  return {
    ...
    reason:
      value !== universalEffort
        ? `requested '${universalEffort}' clamped to '${value}' for ${runtime}.`
        : null,
  };
}
/**
...
function mergeEffortTierDefaults(manifest, override, isValid) {
  ...
  if (override && typeof override === "object" && !Array.isArray(override)) {
    for (const [tier, value] of Object.entries(override)) {
      ...
      if (isValid(value)) merged[tier] = value;
    }
  }
  return merged;
}
