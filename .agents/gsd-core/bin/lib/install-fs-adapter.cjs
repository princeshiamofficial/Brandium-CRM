...
const REAL_ADAPTER = {
  ...
  readdirSync: (p, opts) =>
    opts ? node_fs_1.default.readdirSync(p, opts) : node_fs_1.default.readdirSync(p),
  readFileSync: (p, encoding) =>
    encoding ? node_fs_1.default.readFileSync(p, encoding) : node_fs_1.default.readFileSync(p),
  ...
  readSync: (fd, buffer, offset, length, position) =>
    node_fs_1.default.readSync(fd, buffer, offset, length, position),
...
function mkInstallTempDir(prefix) {
  if (current === REAL_ADAPTER) {
    return node_fs_1.default.mkdtempSync(
      node_path_1.default.join(node_os_1.default.tmpdir(), prefix),
    );
  }
  const dir = node_path_1.default.join(
    node_os_1.default.tmpdir(),
    `${prefix}${node_crypto_1.default.randomBytes(8).toString("hex")}`,
  );
