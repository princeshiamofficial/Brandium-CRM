...
function planningDir(cwd, ws, project) {
  ...
  if (project) base = node_path_1.default.join(base, project);
  if (ws) base = node_path_1.default.join(base, "workstreams", ws);
  return base;
}
...
function resolvePhaseIdConvention(cwd, ws) {
  const readFrom = (dir) => {
    ...
    if (!node_fs_1.default.existsSync(configPath)) return null;
    try {
...
function listAvailableWorkstreams(cwd) {
  try {
    return node_fs_1.default
      .readdirSync(node_path_1.default.join(planningRoot(cwd), "workstreams"), {
        withFileTypes: true,
      })
      ...
      if (nodeErr.code === "EEXIST") {
        ...
        try {
          ...
          if (stealable) {
            ...
            const sameInstance =
              typeof decisionStat.dev === "number" &&
              typeof decisionStat.ino === "number" &&
              confirmStat.dev === decisionStat.dev &&
              confirmStat.ino === decisionStat.ino &&
              decisionBody !== null &&
              confirmBody === decisionBody;
            if (!sameInstance) {
            ...
            const stolen =
              lockPath + ".stale-" + process.pid + "-" + clock.now() + "-" + _planningStealSeq++;
            let renamed = false;
            try {
