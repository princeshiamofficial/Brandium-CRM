...
exports.WindowsError =
  exports.KINDS =
  exports.REASON =
  exports.SCHEMA_VERSION =
  exports.LEDGER_FILE_NAME =
    void 0;
...
function validateKind(kind) {
  if (typeof kind !== "string" || !KIND_SET.has(kind)) {
    throw new WindowsError(
      exports.REASON.WINDOWS_INVALID_KIND,
      `Invalid window kind: ${JSON.stringify(kind)}. Allowed: ${exports.KINDS.join(", ")}.`,
    );
  }
}
function validateDescription(description) {
  if (typeof description !== "string" || description.trim() === "") {
    throw new WindowsError(
      exports.REASON.WINDOWS_APPEND_MISSING_FIELD,
...
function rejectBacktickRun(value, field) {
  if (value.includes(FORBIDDEN_BACKTICK_RUN)) {
    throw new WindowsError(
      exports.REASON.WINDOWS_INVALID_TEXT,
...
function validateFile(file) {
  ...
  if (typeof file !== "string") {
    throw new WindowsError(
      exports.REASON.WINDOWS_INVALID_FILE,
...
function assertOpen(entry) {
  if (entry.status !== "open") {
    throw new WindowsError(
      exports.REASON.WINDOWS_ALREADY_RESOLVED,
...
function markWaived(ledger, id, reason, opts = { now: new Date().toISOString() }) {
  if (typeof reason !== "string" || reason.trim() === "") {
    throw new WindowsError(
      exports.REASON.WINDOWS_WAIVE_REASON_EMPTY,
  ...
  const entries = ledger.entries.map((e) =>
    e.id === id ? { ...e, status: newStatus, reason, resolved_at: opts.now } : e,
  );
...
function markFixed(ledger, id, opts = { now: new Date().toISOString() }) {
  ...
  const entries = ledger.entries.map((e) =>
    e.id === id ? { ...e, status: newStatus, resolved_at: opts.now } : e,
  );
  return recomputeCounts({ ...ledger, entries, last_updated: opts.now });
}
// ─── Pure: parse / render ──────────────────────────────────────────────────
...
function parseFrontmatterStrict(raw) {
  if (!raw.startsWith("---\n") && !raw.startsWith("---\r\n")) {
    throw new WindowsError(
      exports.REASON.WINDOWS_LEDGER_MALFORMED,
...
function parseJsonBlock(raw, expectedTotal) {
  ...
  try {
    ...
    throw new WindowsError(
      exports.REASON.WINDOWS_LEDGER_MALFORMED,
      `Ledger JSON block failed to parse: ${e.message}`,
    );
  }
...
function validateEntryShape(e, i) {
  ...
  const phaseStr =
    typeof o.phase === "string"
      ...
      : o.phase == null
        ? ""
        : typeof o.phase === "number" || typeof o.phase === "boolean"
          ? String(o.phase)
          : "";
  const recordedStr =
    typeof o.recorded_at === "string"
      ...
      : o.recorded_at == null
        ? ""
        : typeof o.recorded_at === "number" || typeof o.recorded_at === "boolean"
          ? String(o.recorded_at)
          : "";
  const resolvedStr =
    typeof o.resolved_at === "string"
      ? o.resolved_at
      : o.resolved_at == null
        ? null
        : typeof o.resolved_at === "number" || typeof o.resolved_at === "boolean"
...
function parseLedger(raw) {
  ...
  if (fm.schema_version !== exports.SCHEMA_VERSION) {
    throw new WindowsError(
      exports.REASON.WINDOWS_LEDGER_MALFORMED,
      `Ledger schema_version must be ${exports.SCHEMA_VERSION}; got ${JSON.stringify(fm.schema_version)}.`,
    );
  }
  ...
  const entries = parseJsonBlock(
    raw,
    typeof fm.total_count === "number" ? fm.total_count : undefined,
  );
  const ledger = {
    schema_version: exports.SCHEMA_VERSION,
...
function readLedgerOrNull(cwd) {
  ...
  try {
    ...
    throw new WindowsError(
      exports.REASON.WINDOWS_LEDGER_MALFORMED,
      `Could not read ledger at ${p} (${code || "unknown fs error"}): ${e.message}.`,
    );
  }
...
function writeLedgerAtomic(cwd, ledger) {
  ...
  if (existing !== null) {
    ...
    try {
      ...
      throw new WindowsError(
        exports.REASON.WINDOWS_LEDGER_MALFORMED,
        `Ledger frontmatter in ${p} could not be parsed (${e.message}); refusing ` +
    ...
    if (actualTable === null) {
      throw new WindowsError(
        exports.REASON.WINDOWS_LEDGER_TABLE_DRIFT,
...
function cmdWindowsStatus(cwd, opts = {}) {
  ...
  try {
    ...
    throw new WindowsError(
      exports.REASON.WINDOWS_LEDGER_MALFORMED,
      `Unexpected error reading ledger: ${e.message}`,
    );
  }
...
function parseIdOrThrow(raw) {
  ...
  if (!Number.isInteger(n) || n < 1) {
    throw new WindowsError(
      exports.REASON.WINDOWS_INVALID_ID,
