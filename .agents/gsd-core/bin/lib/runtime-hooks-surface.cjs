...
function atomicWriteFileSync(target, data, options) {
  ...
  try {
    ...
    /* no pre-existing target: default creation mode applies */
  }
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    try {
      ...
      if (priorMode !== undefined) node_fs_1.default.chmodSync(tmp, priorMode);
...
function parseTomlValue(text, i) {
  ...
  const numMatch = text
    .slice(i)
    .match(
      /^[+-]?(?:0x[0-9a-fA-F_]+|0o[0-7_]+|0b[01_]+|[0-9][0-9_]*(?:\.[0-9_]+)?(?:[eE][+-]?[0-9_]+)?|inf|nan)/,
    );
  if (numMatch) {
...
function resolveNodeRunner(opts) {
  const execPath =
    (opts && opts.execPath) || (typeof process.execPath === "string" ? process.execPath : "");
...
function resolveBashRunner(opts) {
  ...
  if (env["ProgramFiles(x86)"])
    candidates.push(
      node_path_1.default.win32.join(env["ProgramFiles(x86)"], "Git", "bin", "bash.exe"),
    );
  if (env.SystemDrive) {
    candidates.push(
      node_path_1.default.win32.join(env.SystemDrive, "Program Files", "Git", "bin", "bash.exe"),
    );
    candidates.push(
      node_path_1.default.win32.join(
        env.SystemDrive,
        "Program Files (x86)",
        "Git",
        "bin",
...
const RESOLVER_RUNNER_COMMAND =
  /^(?:"[^"]*bash(\.exe)?"|bash)\s+"[^"]*gsd-node-runner\.sh"\s+"[^"]*"\s+\S/;
...
function rewriteLegacyCodexHookBlock(content, absoluteRunner, opts) {
  ...
  const updated = content.replace(
    ...
    (full, prefix, scriptToken, suffix) => {
      ...
      const currentCommand = `${prefix}${scriptToken}${suffix}`.replace(
        /^(command\s*=\s*")|("\s*)$/g,
        "",
      );
...
function reconcileCodexHooksJsonEvent(targetDir, eventName, opts = {}) {
  ...
  if (node_fs_1.default.existsSync(hooksJsonPath)) {
    ...
    if (raw.trim()) {
      try {
        ...
        throw new Error(
          `hooks.json parse failed: ${err && err.message ? err.message : String(err)}`,
        );
      }
    }
  }
  ...
  const hookTable = usesNestedHooksObject ? parsed["hooks"] : {};
...
function ensureCodexHooksJsonSessionStart(targetDir, opts = {}) {
  ...
  const scriptPath = shellCmdProjection.posixNormalize(
    node_path_1.default.resolve(targetDir, "hooks", "gsd-check-update.js"),
  );
  ...
  if (platform === "win32") {
    ...
    try {
      ...
      const reason =
        shimWriteErr && shimWriteErr.message ? shimWriteErr.message : String(shimWriteErr);
      console.warn(
...
function ensureCodexHooksJsonEvent(targetDir, eventName, opts = {}) {
  ...
  if (!absoluteRunner) return { changed: false, wrote: false, path: hooksJsonPath };
  const scriptPath = shellCmdProjection.posixNormalize(
    node_path_1.default.resolve(targetDir, "hooks", "gsd-context-monitor.js"),
  );
  let managedCommand;
  if (platform === "win32") {
    const shimIR = buildCodexHookWindowsShimIR(scriptPath, absoluteRunner);
...
function writeClineArtifacts(targetDir, isGlobalInstall) {
  ...
  node_fs_1.default.writeFileSync(
    node_path_1.default.join(clinerulesDir, "gsd.md"),
    buildClineRulesBody(),
  );
  ...
  if (node_fs_1.default.existsSync(hooksJsonPath)) {
    ...
    if (raw.trim()) {
      try {
        ...
        throw new Error(
          `Cursor hooks.json parse failed: ${err && err.message ? err.message : String(err)}`,
        );
      }
    }
  }
...
function writeCursorHooksJson(targetDir, src, opts) {
  ...
  stageTransitiveHookLibs({
    seedSources: [...installedScripts].map((script) =>
      node_fs_1.default.readFileSync(node_path_1.default.join(hooksDir, script), "utf8"),
    ),
...
function removeCursorHooksJson(targetDir) {
  const hooksJsonPath = node_path_1.default.join(targetDir, "hooks.json");
  if (!node_fs_1.default.existsSync(hooksJsonPath)) return { changed: false };
  ...
  if (node_fs_1.default.existsSync(hooksJsonPath)) {
    ...
    if (raw.trim()) {
      try {
        ...
        throw new Error(
          `Windsurf hooks.json parse failed: ${err && err.message ? err.message : String(err)}`,
        );
      }
    }
  }
...
function writeWindsurfHooksJson(targetDir, src, opts) {
  ...
  for (const ev of WINDSURF_HOOK_EVENTS) {
    ...
    commands[ev] =
      script && installedScripts.has(script) ? buildHookCommand(targetDir, script, hookOpts) : null;
  }
...
function writeCopilotHookConfig(targetDir) {
  ...
  node_fs_1.default.writeFileSync(
    hookPath,
    JSON.stringify(buildCopilotHookConfig(), null, 2) + "\n",
  );
  return hookPath;
}
  ...
  if (hooksSurface !== "none" && hooksSurface !== "kimi-hooks-toml") {
    ...
    for (const entries of Object.values(settings.hooks)) {
      ...
      for (const entry of entries) {
        if (!entry || !Array.isArray(entry.hooks)) continue;
        for (const h of entry.hooks) {
          if (blockingGuardNames.some((name) => referencesHook(h, name)) && h.timeout === 5) {
    ...
    const hasGsdUpdateHook = settings.hooks.SessionStart.some(
      (entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, "gsd-check-update")),
    );
    ...
    const hasContextMonitorHook = settings.hooks[postToolEvent].some(
      (entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, "gsd-context-monitor")),
    );
    const contextMonitorFile = node_path_1.default.join(
      targetDir,
      "hooks",
      "gsd-context-monitor.js",
    );
    if (
      !hasContextMonitorHook &&
      node_fs_1.default.existsSync(contextMonitorFile) &&
      contextMonitorCommand
    ) {
    ...
    const hasPromptGuardHook = settings.hooks[preToolEvent].some(
      (entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, "gsd-prompt-guard")),
    );
    const promptGuardFile = node_path_1.default.join(targetDir, "hooks", "gsd-prompt-guard.js");
    if (
      !hasPromptGuardHook &&
      node_fs_1.default.existsSync(promptGuardFile) &&
      promptGuardCommand
    ) {
    ...
    const hasReadGuardHook = settings.hooks[preToolEvent].some(
      (entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, "gsd-read-guard")),
    );
    ...
    const readInjectionScannerFile = node_path_1.default.join(
      targetDir,
      "hooks",
    ...
    if (
      ...
      node_fs_1.default.existsSync(readInjectionScannerFile) &&
      readInjectionScannerCommand
    ) {
      settings.hooks[postToolEvent].push({
        matcher: "Read",
    ...
    const workflowGuardHookEntry = settings.hooks[preToolEvent].find(
      (entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, "gsd-workflow-guard")),
    );
    ...
    if (hasWorkflowGuardHook && workflowGuardHookEntry.matcher !== workflowGuardMatcher) {
      ...
      !hasWorkflowGuardHook &&
      node_fs_1.default.existsSync(workflowGuardFile) &&
    ...
    const hasWorktreePathGuardHook = settings.hooks[preToolEvent].some(
      (entry) =>
        entry.hooks && entry.hooks.some((h) => referencesHook(h, "gsd-worktree-path-guard")),
    );
    const worktreePathGuardFile = node_path_1.default.join(
      targetDir,
      "hooks",
      "gsd-worktree-path-guard.js",
    );
    if (
      !hasWorktreePathGuardHook &&
      node_fs_1.default.existsSync(worktreePathGuardFile) &&
      worktreePathGuardCommand
    ) {
      settings.hooks[preToolEvent].push({
      ...
      console.warn(
        `  ${yellow}⚠${reset}  Skipped worktree path guard hook — gsd-worktree-path-guard.js not found at target`,
      );
    }
    ...
    const hasAgentIsolationGuardHook = settings.hooks[preToolEvent].some(
      (entry) =>
        entry.hooks && entry.hooks.some((h) => referencesHook(h, "gsd-agent-isolation-guard")),
    );
    const agentIsolationGuardFile = node_path_1.default.join(
      targetDir,
      "hooks",
      "gsd-agent-isolation-guard.js",
    );
    if (
      !hasAgentIsolationGuardHook &&
      node_fs_1.default.existsSync(agentIsolationGuardFile) &&
      agentIsolationGuardCommand
    ) {
      settings.hooks[preToolEvent].push({
      ...
      console.warn(
        `  ${yellow}⚠${reset}  Skipped agent isolation guard hook — gsd-agent-isolation-guard.js not found at target`,
      );
    }
    ...
    const hasWriteGuardHook = settings.hooks[preToolEvent].some(
      (entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, "gsd-write-guard")),
    );
    ...
    if (!hasWriteGuardHook && node_fs_1.default.existsSync(writeGuardFile) && writeGuardCommand) {
      ...
      console.log(
        `  ${green}✓${reset} Configured write guard hook (catastrophic-shrink protection)`,
      );
    } else if (!hasWriteGuardHook && !node_fs_1.default.existsSync(writeGuardFile)) {
      console.warn(
        `  ${yellow}⚠${reset}  Skipped write guard hook — gsd-write-guard.js not found at target`,
      );
    }
    ...
    const hasSecretReadGuardHook = settings.hooks[preToolEvent].some(
      (entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, "gsd-secret-read-guard")),
    );
    const secretReadGuardFile = node_path_1.default.join(
      targetDir,
      "hooks",
      "gsd-secret-read-guard.js",
    );
    if (
      !hasSecretReadGuardHook &&
      node_fs_1.default.existsSync(secretReadGuardFile) &&
      secretReadGuardCommand
    ) {
      settings.hooks[preToolEvent].push({
      ...
      console.log(
        `  ${green}✓${reset} Configured secret read guard hook (.env / .secrets read protection)`,
      );
    } else if (!hasSecretReadGuardHook && !node_fs_1.default.existsSync(secretReadGuardFile)) {
      console.warn(
        `  ${yellow}⚠${reset}  Skipped secret read guard hook — gsd-secret-read-guard.js not found at target`,
      );
    }
    ...
    const hasValidateCommitHook = settings.hooks[preToolEvent].some(
      (entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, "gsd-validate-commit")),
    );
    ...
    const validateCommitFile = node_path_1.default.join(
      targetDir,
      "hooks",
      "gsd-validate-commit.sh",
    );
    if (
      !hasValidateCommitHook &&
      node_fs_1.default.existsSync(validateCommitFile) &&
      ...
      console.warn(
        `  ${yellow}⚠${reset}  Skipped commit validation hook — Bash executable path unavailable (#3393)`,
      );
    }
    ...
    const hasGraphifyUpdateHook = settings.hooks[postToolEvent].some(
      (entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, "gsd-graphify-update")),
    );
    const graphifyUpdateFile = node_path_1.default.join(
      targetDir,
      "hooks",
      "gsd-graphify-update.sh",
    );
    if (
      !hasGraphifyUpdateHook &&
      node_fs_1.default.existsSync(graphifyUpdateFile) &&
      graphifyUpdateCommand
    ) {
      settings.hooks[postToolEvent].push({
        matcher: "Bash",
        hooks: [
      ...
      console.log(
        `  ${green}✓${reset} Configured graphify auto-update hook (opt-in via graphify.auto_update)`,
      );
    } else if (!hasGraphifyUpdateHook && !node_fs_1.default.existsSync(graphifyUpdateFile)) {
      console.warn(
        `  ${yellow}⚠${reset}  Skipped graphify auto-update hook — gsd-graphify-update.sh not found at target`,
      );
    } else if (!hasGraphifyUpdateHook && !graphifyUpdateCommand) {
      console.warn(
        `  ${yellow}⚠${reset}  Skipped graphify auto-update hook — Bash executable path unavailable (#3393)`,
      );
    }
    ...
    const hasSessionStateHook = settings.hooks.SessionStart.some(
      (entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, "gsd-session-state")),
    );
    const sessionStateFile = node_path_1.default.join(targetDir, "hooks", "gsd-session-state.sh");
    if (
      !hasSessionStateHook &&
      node_fs_1.default.existsSync(sessionStateFile) &&
      sessionStateCommand
    ) {
      settings.hooks.SessionStart.push({
        hooks: [
      ...
      console.warn(
        `  ${yellow}⚠${reset}  Skipped session state hook — Bash executable path unavailable (#3393)`,
      );
    }
    ...
    const hasPhaseBoundaryHook = settings.hooks[postToolEvent].some(
      (entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, "gsd-phase-boundary")),
    );
    const phaseBoundaryFile = node_path_1.default.join(targetDir, "hooks", "gsd-phase-boundary.sh");
    if (
      !hasPhaseBoundaryHook &&
      node_fs_1.default.existsSync(phaseBoundaryFile) &&
      phaseBoundaryCommand
    ) {
      settings.hooks[postToolEvent].push({
        matcher: "Write|Edit",
        hooks: [
      ...
      console.warn(
        `  ${yellow}⚠${reset}  Skipped phase boundary hook — Bash executable path unavailable (#3393)`,
      );
    }
    ...
    if (
      ...
      console.log(
        `  ${green}✓${reset} Reconciled managed .sh hook commands to current format (#3329)`,
      );
    }
    ...
    // so this loop is a no-op for every runtime that doesn't list SubagentStart.
    {
      ...
      for (const event of ["SubagentStop", "Stop", "PreCompact", "SubagentStart"]) {
        ...
        const alreadyHasContextMonitor = settings.hooks[event].some(
          (entry) =>
            entry.hooks && entry.hooks.some((h) => referencesHook(h, "gsd-context-monitor")),
        );
        if (
          !alreadyHasContextMonitor &&
          node_fs_1.default.existsSync(contextMonitorFile) &&
          contextMonitorCommand
        ) {
          ...
          console.log(
            `  ${green}✓${reset} Configured ${event} context monitor hook (${runtimeLabel})`,
          );
        } else if (!alreadyHasContextMonitor && !node_fs_1.default.existsSync(contextMonitorFile)) {
          console.warn(
            `  ${yellow}⚠${reset}  Skipped ${event} hook — gsd-context-monitor.js not found at target`,
          );
        }
      }
    }
    ...
    for (const extendedEvent of ["BeforeAgent", "AfterAgent", "BeforeModel"]) {
      ...
      const alreadyHasContextMonitor = settings.hooks[extendedEvent].some(
        (entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, "gsd-context-monitor")),
      );
      if (
        !alreadyHasContextMonitor &&
        ...
        console.warn(
          `  ${yellow}⚠${reset}  Skipped ${extendedEvent} hook — gsd-context-monitor.js not found at target`,
        );
      }
    }
    ...
    if (extendedEvents.includes("FileChanged")) {
      ...
      const alreadyHasConfigReload = settings.hooks.FileChanged.some(
        (entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, "gsd-config-reload")),
      );
      if (
        !alreadyHasConfigReload &&
        node_fs_1.default.existsSync(configReloadFile) &&
        configReloadCommand
      ) {
        ...
        console.warn(
          `  ${yellow}⚠${reset}  Skipped FileChanged hook — gsd-config-reload.js not found at target`,
        );
      } else if (!alreadyHasConfigReload && !configReloadCommand) {
        console.warn(
          `  ${yellow}⚠${reset}  Skipped FileChanged hook — Node executable path unavailable`,
        );
      }
    }
...
// ---------------------------------------------------------------------------
const KIMI_HOOKS_TOML_MARKER_BEGIN =
  "# GSD Hooks BEGIN — managed by GSD, do not edit between these markers";
...
function buildKimiHooksTomlBlock(targetDir, opts) {
  ...
  const specs = [
    ...
    {
      event: "PreToolUse",
      command: cmd("gsd-prompt-guard.js"),
      matcher: "WriteFile|StrReplaceFile",
      timeout: 5,
    },
    {
      event: "PreToolUse",
      command: cmd("gsd-read-guard.js"),
      matcher: "WriteFile|StrReplaceFile",
      timeout: 5,
    },
    {
      event: "PreToolUse",
      command: cmd("gsd-worktree-path-guard.js"),
      matcher: "WriteFile|StrReplaceFile",
      timeout: 5,
    },
    ...
    {
      event: "PreToolUse",
      command: cmd("gsd-secret-read-guard.js"),
      matcher: "ReadFile|Grep|Shell",
      timeout: 5,
    },
    {
      event: "PreToolUse",
      command: cmd("gsd-workflow-guard.js"),
    ...
    {
      event: "PostToolUse",
      command: cmd("gsd-phase-boundary.sh"),
      matcher: "WriteFile|StrReplaceFile",
      timeout: 5,
    },
    {
      event: "PostToolUse",
      command: cmd("gsd-read-injection-scanner.js"),
      matcher: "ReadFile",
      timeout: 5,
    },
  ...
  const entries = specs.map(buildKimiHookEntryToml).filter((entry) => entry !== null);
...
function writeKimiHooksToml(configPath, targetDir, opts) {
  const existing = node_fs_1.default.existsSync(configPath)
    ? node_fs_1.default.readFileSync(configPath, "utf8")
    : "";
  ...
  if (!block) {
    ...
    if (stripped.trim() === "") {
      if (node_fs_1.default.existsSync(configPath)) node_fs_1.default.unlinkSync(configPath);
    } else {
...
function removeKimiHooksToml(configPath) {
  if (!node_fs_1.default.existsSync(configPath)) return { changed: false };
...
function referencesHook(h, hookName) {
  ...
  const url = h["url"];
  return (
    (typeof cmd === "string" && cmd.includes(hookName)) ||
