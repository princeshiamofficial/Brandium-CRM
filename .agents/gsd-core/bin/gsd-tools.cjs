...
} = require("./lib/active-workstream-store.cjs");
...
const {
  ...
  comparePhaseStatus,
} = require("./lib/plan-drift-guard.cjs");
  ...
  if (!mod || !Object.prototype.hasOwnProperty.call(mod, entry.router)) {
    error(
      'capability command "' +
        ...
        '" router "' +
        entry.router +
        '" is not an own export of module "' +
        entry.module +
        '"',
    );
    return true; // consumed — don't emit "Unknown command"
  ...
  if (_result && typeof _result.then === "function") {
    error(
      'capability command "' +
        ...
        '" in module "' +
        entry.module +
        '" must be synchronous (returned a Promise); async capability routers are not supported.',
      ERROR_REASON.SDK_FAIL_FAST,
    );
  }
  return true;
...
function defaultRequireFromInstallRoot(installRoot, m) {
  ...
  if (realResolved !== realRoot && !realResolved.startsWith(realRoot + path.sep)) {
    throw new Error(
      "capability module resolves outside its install root (symlink): " + JSON.stringify(m),
    );
  }
 ...
 */
function dispatchOverlayCapabilityCommand({
  command,
  args,
  cwd,
  raw,
  ...
  try {
    const load =
      loadRegistry !== undefined
        ? loadRegistry
        : require("./lib/capability-loader.cjs").loadRegistry;
  ...
  try {
    ...
    error(
      'capability command "' +
        ...
        '" module "' +
        entry.module +
        '" failed to load from its install root',
    );
    return true; // consumed — don't emit "Unknown command"
  }
...
function routeResolveGranularity({ args, cwd, raw }) {
  ...
  for (let i = 0; i < granArgs.length; i++) {
    ...
    if (
      a === "--granularity" &&
      granArgs[i + 1] !== undefined &&
      !granArgs[i + 1].startsWith("--")
    ) {
      if (granOverride === undefined) {
        granOverride = granArgs[++i];
      } else {
        ++i;
      }
    } else {
...
function routeResolveExecution({ args, cwd, raw }) {
  ...
  for (let i = 0; i < execArgs.length; i++) {
    ...
    if (a.startsWith("--attempt=")) {
      ...
      if (!Number.isInteger(n) || n < 0)
        error("--attempt requires a non-negative integer", ERROR_REASON.USAGE);
      attempt = n;
      continue;
    }
    ...
    if (a === "--effort") {
      ...
      if (val === undefined || val.startsWith("--"))
        error("Missing value for --effort", ERROR_REASON.USAGE);
      effortOverride = val;
      i++;
      continue;
    }
    if (a === "--fast-mode") {
      const val = execArgs[i + 1];
      if (val === undefined || val.startsWith("--"))
        error("Missing value for --fast-mode", ERROR_REASON.USAGE);
    ...
    if (a === "--attempt") {
      const val = execArgs[i + 1];
      if (val === undefined || val.startsWith("--"))
        error("Missing value for --attempt", ERROR_REASON.USAGE);
    ...
    if (a === "--failure-class") {
      const val = execArgs[i + 1];
      if (val === undefined || val.startsWith("--"))
        error("Missing value for --failure-class", ERROR_REASON.USAGE);
      setFailureClass(val);
      i++;
      continue;
    }
    if (a === "--host") {
      const val = execArgs[i + 1];
      if (val === undefined || val.startsWith("--"))
        error("Missing value for --host", ERROR_REASON.USAGE);
      host = val;
      i++;
      continue;
    }
  ...
  if (positionals.length > 1)
    error(
      `resolve-execution requires exactly one agent-type argument; got: ${positionals.join(", ")}`,
      ERROR_REASON.USAGE,
    );
...
function routeConfigGet({ args, cwd, raw, defaultValue }) {
  const configGetSdkArgs =
    defaultValue !== undefined ? [args[1], "--default", defaultValue] : args.slice(1);
  const handled = _dispatchNonFamily({
...
function routeResearchStore({ args, cwd, raw }) {
  ...
  if (subcommand === "get") {
    ...
    if (!researchStore.isValidResearchKey(key)) {
      error(
        "research-store: <key> must be a 64-char sha256 hex (use research-plan to obtain keys)",
        ERROR_REASON.USAGE,
      );
    }
    ...
    if (!key || key.startsWith("--")) {
      error(
        "Usage: gsd-tools research-store put <key> --content <str> --source <s> --provider <p> --confidence <c> --kind <k>",
        ERROR_REASON.USAGE,
      );
    }
    if (!researchStore.isValidResearchKey(key)) {
    ...
    const entry = researchStore.putResearch(
      cwd,
      key,
    ...
    error(
      "Unknown research-store subcommand. Available: get, put",
      ERROR_REASON.SDK_UNKNOWN_COMMAND,
    );
  }
}
...
function routeResearchPlan({ args, cwd, raw }) {
  ...
  const homeDir = process.env.HOME || require("os").homedir();
  const plan = researchProvider.planResearch({
    questions,
    ecosystem,
    config: planConfig,
    cwd,
...
function routeCommitDocsGuard({ args, cwd, raw, error }) {
  ...
  if (subcommand === "enable") {
    ...
    error(
      "Unknown commit-docs-guard subcommand. Available: enable, disable",
      ERROR_REASON.SDK_UNKNOWN_COMMAND,
    );
  }
}
...
function routePrSubrepo({ args, cwd, raw, error }) {
  ...
  const { repo, branch } = parseNamedArgsOrExit(
    args,
    { valueFlags: ["repo", "branch"], positionals: messagePresent ? 2 : 1 },
    error,
  );
...
function routeTemplate({ args, cwd, raw, error }) {
  ...
  if (subcommand === "select") {
    ...
    const {
      ...
      wave,
      fields: fieldsRaw,
    } = parseNamedArgsOrExit(
      args,
      { valueFlags: ["phase", "plan", "name", "type", "wave", "fields"], positionals: 3 },
      error,
    );
    let fields = {};
    if (fieldsRaw) {
...
function routeFrontmatter({ args, cwd, raw, error }) {
  ...
  if (subcommand === "get") {
    frontmatter.cmdFrontmatterGet(
      cwd,
      file,
      parseNamedArgsOrExit(args, { valueFlags: ["field"], positionals: 3 }, error).field,
      raw,
    );
  } else if (subcommand === "set") {
    const { field, value } = parseNamedArgsOrExit(
      args,
      { valueFlags: ["field", "value"], positionals: 3 },
      error,
    );
    ...
    frontmatter.cmdFrontmatterMerge(
      cwd,
      file,
      parseNamedArgsOrExit(args, { valueFlags: ["data"], positionals: 3 }, error).data,
      raw,
    );
    ...
    frontmatter.cmdFrontmatterValidate(
      cwd,
      file,
      parseNamedArgsOrExit(args, { valueFlags: ["schema"], positionals: 3 }, error).schema,
      raw,
    );
  } else {
    error(
...
function routeProjectInstructionFile({ args, cwd, raw, error }) {
  ...
  for (let i = 0; i < pifArgs.length; i++) {
    const a = pifArgs[i];
    if (a === "--runtime" && pifArgs[i + 1] !== undefined) {
      pifRuntime = pifArgs[++i];
      continue;
    }
    if (a.startsWith("--runtime=")) {
      pifRuntime = a.slice("--runtime=".length);
      continue;
    }
    // First positional that isn't a flag also works (lenient); otherwise ignore unknown flags.
    if (!a.startsWith("-") && !pifRuntime) {
      pifRuntime = a;
    }
  }
...
function routeQuickTasksAppend({ args, cwd, raw, error }) {
  ...
  if (!qtaTask) {
    error(
      "quick-tasks-append requires --task <description> (or a positional description)",
      ERROR_REASON.USAGE,
    );
  }
  ...
  const qtaDirectory =
    qtaParsed["directory"] ||
    (qtaQuickId && qtaSlug
      ? `[${qtaQuickId}-${qtaSlug}](./quick/${qtaQuickId}-${qtaSlug}/)`
      : undefined);
...
async function routeReviewLane({ args, cwd, raw, error }) {
  ...
  if (!["plan", "invoke", "sections", "flags"].includes(sub)) {
    error(
      "Usage: review-lane <plan|invoke|sections|flags> [--selected a,b] [--run-dir D] [--repo-root R]",
    );
    return;
  }
  ...
  let resolved = {};
  try {
    resolved = (cfgLoader.loadConfigResolved(cwd) || {}).config || {};
  } catch {
    resolved = {};
  }
  const configGet = (key) => {
  ...
  const plans = chosen.map((slug) => {
    ...
    if (!lane)
      return { slug, ok: false, reason: "malformed_lane", detail: "no such declared lane" };
    ...
    try {
      ...
      return {
        slug,
        ok: false,
        reason: "malformed_lane",
        detail: `resolver threw: ${e && e.message ? e.message : String(e)}`,
      };
    }
    return r.ok
      ? {
  ...
  if (!slug) {
    error("review-lane invoke requires --slug");
    return;
  }
    ...
    output(
      {
        slug,
        ok: false,
        reason: entry ? entry.reason : "malformed_lane",
        detail: entry ? entry.detail : "unknown lane",
      },
      raw,
    );
    return;
  ...
  const deps = {
    spawn: (binary, argv, opts) => {
      ...
      } = projectSpawnInvocation(binary, argv);
...
function routeDispatchShouldFlatten({ args, cwd, raw, error }) {
  ...
  try {
    ...
    const runtimeEntry = registry.runtimes != null ? registry.runtimes[runtimeId] : null;
    ...
    const shouldFlat = dispatch !== null ? hostIntegration.shouldFlattenDispatch(dispatch) : true; // fail-closed: unknown runtime → inline
...
function routeDispatchIsolation({ args, cwd, raw, error }) {
  ...
  const phaseArg =
    phaseIdx !== -1 && args[phaseIdx + 1] && !args[phaseIdx + 1].startsWith("--")
      ? args[phaseIdx + 1]
      : null;
  const planIdx = args.indexOf("--plan");
  const planArg =
    planIdx !== -1 && args[planIdx + 1] && !args[planIdx + 1].startsWith("--")
      ? args[planIdx + 1]
      : null;
...
function resolveDispatchModelPin(agentName, rawValue) {
  ...
  if (isAnthropicFlavoredModel(trimmed)) {
    _warnDispatchModelPinDropped(
      agentName,
      rawValue,
      "is an Anthropic-flavored model/alias, not a valid Codex model",
    );
    return undefined;
  }
  if (LEADING_DASH_RE.test(trimmed)) {
    _warnDispatchModelPinDropped(
      agentName,
      rawValue,
      'looks like a flag/option, not a model id (leading "-")',
    );
    return undefined;
  }
  if (!MODEL_ID_CHARSET_RE.test(trimmed)) {
    _warnDispatchModelPinDropped(
      agentName,
      rawValue,
      "does not look like a model id (unsafe characters)",
    );
    return undefined;
  }
  if (trimmed.length > MODEL_ID_MAX_LENGTH) {
    _warnDispatchModelPinDropped(
      agentName,
      rawValue,
      `exceeds the maximum model id length (${MODEL_ID_MAX_LENGTH} characters)`,
    );
    return undefined;
  }
  return trimmed;
...
const DISPATCH_ISOLATION_VOCABULARY = new Set([
  "harness-worktree",
  ...
  try {
    ...
    if (isolation === "orchestrator-worktree") {
      ...
      if (cwdTarget) {
        ...
        // existed.
        const declaresModelFlag =
          typeof runtimeEntry?.runtime?.orchestratorExec?.modelFlag === "string" &&
        ...
        if (declaresModelFlag) {
          ...
          const pinned = resolveAgentModelOverride(
            "gsd-executor",
            readGsdEffectiveModelOverrides(cwd),
            null,
          );
...
function routeInspectDispatchIsolation({ args, cwd, raw }) {
  ...
  const { runtimeId, isolation, exec, harnessFlag } = resolveDispatchIsolationDecision({
    args,
    cwd,
  });
  if (args.indexOf("--json") !== -1) {
 ...
 */
function writeDispatchIsolationSentinel(
  cwd,
  { isolation, harnessFlag = null, phase = null, plan = null },
) {
  ...
  try {
    ...
    return {
      recorded: false,
      path: ".gsd/dispatch-isolation-sentinel.json",
...
function routeRecordDispatchIsolation({ args, cwd, raw, error }) {
  ...
  if (flagEqArg) {
    ...
    harnessFlag =
      flagIdx !== -1 && args[flagIdx + 1] && !args[flagIdx + 1].startsWith("--")
        ? args[flagIdx + 1]
        : null;
  }
  const phaseIdx = args.indexOf("--phase");
...
function routeResolveAgent({ args, cwd, raw, error }) {
  ...
  try {
    ...
    if (jsonIdx !== -1) {
      output(
        { runtime: runtimeId, requested: requested || null, resolved, fallback: fellBack },
        raw,
      );
    } else {
      process.stdout.write(String(resolved));
...
function routeAssumptionDelta({ args, cwd, raw, error }) {
  ...
  if (sub === "scan") {
    ...
    if (!phaseNum || phaseNum.startsWith("-")) {
      error(
        "Usage: assumption-delta scan <phase> [--terms <csv>]",
        ERROR_REASON.SDK_UNKNOWN_COMMAND,
      );
      return;
    }
  ...
  error(
    `Unknown assumption-delta subcommand: ${sub}. Available: scan`,
    ERROR_REASON.SDK_UNKNOWN_COMMAND,
  );
}
...
function routeRequirements({ args, cwd, raw, error }) {
  ...
  if (subcommand === "mark-complete") {
    ...
    error(
      "Unknown requirements subcommand. Available: mark-complete, ready-ids, revert-phase",
      ERROR_REASON.SDK_UNKNOWN_COMMAND,
    );
  }
}
...
function routeMilestone({ args, cwd, raw, error }) {
  ...
  if (subcommand === "complete") {
    ...
    milestone.cmdMilestoneComplete(
      cwd,
      args[2],
      { name: milestoneName, archivePhases, force, dryRun, archiveQuick, confirm },
      raw,
    );
    ...
    error(
      "Unknown milestone subcommand. Available: complete, archive-quick",
      ERROR_REASON.SDK_UNKNOWN_COMMAND,
    );
  }
}
...
function routeUat({ args, cwd, raw, error }) {
  ...
  if (subcommand === "render-checkpoint") {
    ...
    const options = parseNamedArgsOrExit(
      args,
      { valueFlags: ["summary", "file"], positionals: 2 },
      error,
    );
    coverage.cmdClassify(cwd, options, raw);
  } else {
    error(
      "Unknown uat subcommand. Available: render-checkpoint, classify-coverage",
      ERROR_REASON.SDK_UNKNOWN_COMMAND,
    );
  }
}
...
function routeTodo({ args, cwd, raw, error }) {
  ...
  if (subcommand === "complete") {
    ...
    error(
      "Unknown todo subcommand. Available: complete, match-phase",
      ERROR_REASON.SDK_UNKNOWN_COMMAND,
    );
  }
}
...
function routeLoop({ args, cwd, raw, error }) {
  ...
  if (loopSubcommand === "render-hooks") {
    ...
    if (activeCapEqArg) {
      ...
      if (!value)
        error(
          "Missing value for --active-cap (e.g. --active-cap tdd)",
...
function routeWorktree({ args, cwd, raw, error }) {
  ...
  if (subcommand === "cleanup-wave") {
    ...
    error(
      "Unknown worktree subcommand. Available: cleanup-wave, record-agent, reap-orphans, base-check, set-baseref, create",
      ERROR_REASON.SDK_UNKNOWN_COMMAND,
    );
  }
}
...
function routeLearnings({ args, cwd, raw, error }) {
  ...
  if (subcommand === "list") {
    ...
    if (!olderThan)
      error("Usage: gsd-tools learnings prune --older-than <duration>", ERROR_REASON.USAGE);
    ...
    error(
      "Unknown learnings subcommand. Available: list, query, copy, prune, delete",
      ERROR_REASON.SDK_UNKNOWN_COMMAND,
    );
  }
}
...
async function routeDetectCustomFiles({ args, cwd, raw, error }) {
  ...
  const GSD_WHOLE_MANAGED_DIRS = ["gsd-core", path.join("commands", "gsd")];
...
const RESTORE_GSD_PATH_RE =
  /gsd-core\/[A-Za-z0-9._-]+(?:\/[A-Za-z0-9._-]+)*\.(?:md|cjs|js|json|sh)/g;
...
function scanRestoreCompatibility(srcPath, relPath, configDir) {
  ...
  const isFrontmatterSurface =
    base === "SKILL.md" || relPath.startsWith("agents/") || relPath.startsWith("commands/");
...
function routeRestoreCustomFiles({ args, error }) {
  ...
  if (configDir === null || configDir.trim() === "" || configDir.startsWith("--")) {
    error(
      "Usage: gsd-tools restore-custom-files --config-dir <path> [--apply]",
      ERROR_REASON.USAGE,
    );
  }
  const resolvedConfigDir = path.resolve(configDir);
  ...
  for (const found of backupFound ? collectBackupEntries(backupDir, backupDir, []) : []) {
    ...
    if (manifestFound && manifestKeys.has(relPath)) {
      ...
      entries.push({
        path: relPath,
        outcome: RESTORE_OUTCOME.SKIPPED_DESTINATION_MANAGED,
        warnings,
      });
      continue;
    }
    ...
    if (destDiffers) {
      ...
      entries.push({
        path: relPath,
        outcome: RESTORE_OUTCOME.SKIPPED_DESTINATION_EXISTS,
        warnings,
      });
      continue;
    }
...
function routeContextPredicates({ args, cwd, raw, error }) {
  ...
  try {
    ...
    error(
      `context-predicates: cannot read ${contextMdPath}: ${err && err.message}`,
      ERROR_REASON.USAGE,
    );
    return;
  }
...
function routeUpdateContext({ args, cwd, raw, error }) {
  ...
  for (let i = 0; i < ucArgs.length; i++) {
    ...
    if (a.startsWith("--config-dir=")) {
      preferredConfigDir = a.slice("--config-dir=".length);
      continue;
    }
    if (a.startsWith("--runtime=")) {
      preferredRuntime = a.slice("--runtime=".length);
      continue;
    }
    if (a === "--config-dir") {
      const v = ucArgs[i + 1];
      if (v === undefined || v.startsWith("--"))
        error("Missing value for --config-dir", ERROR_REASON.USAGE);
      preferredConfigDir = v;
      i++;
      continue;
    }
    if (a === "--runtime") {
      const v = ucArgs[i + 1];
      if (v === undefined || v.startsWith("--"))
        error("Missing value for --runtime", ERROR_REASON.USAGE);
      preferredRuntime = v;
      i++;
      continue;
    }
...
async function routeClassifyConfidence({ args, cwd, raw, error }) {
  ...
  if (!provider || provider.startsWith("--")) {
    error(
      "Usage: gsd-tools query classify-confidence --provider <id> [--package <name> --ecosystem <npm|pypi|crates>] [--verified]",
      ERROR_REASON.USAGE,
    );
  }
  ...
  const confidence = researchProvider.classifyConfidence({
    provider,
    verifiedAgainstOfficial: verified,
    legitimacyVerdict,
  });
  output(
    {
...
async function routePackageLegitimacy({ args, cwd, raw, error }) {
  ...
  if (subcommand !== "check") {
    error(
      "Unknown package-legitimacy subcommand. Available: check",
      ERROR_REASON.SDK_UNKNOWN_COMMAND,
    );
  }
  const ecoIdx = args.indexOf("--ecosystem");
  ...
  if (!ecosystem || !VALID_ECOSYSTEMS.has(ecosystem)) {
    error(
      "Usage: gsd-tools package-legitimacy check --ecosystem <npm|pypi|crates> <pkg1> ...",
      ERROR_REASON.USAGE,
    );
  }
  ...
  if (packages.length === 0) {
    error(
      "Usage: gsd-tools package-legitimacy check --ecosystem <eco> <pkg1> <pkg2> ...",
      ERROR_REASON.USAGE,
    );
  }
  let pkgResults;
  try {
    ...
    error(
      `package-legitimacy: ${pkgErr && pkgErr.message ? pkgErr.message : String(pkgErr)}`,
...
function routeEffort({ args, cwd, raw, error }) {
  ...
  if (subcommand === "sync") {
    ...
    for (let i = 0; i < effortSyncArgs.length; i++) {
      ...
      if (a.startsWith("--config-dir=")) {
        effortSyncConfigDir = a.slice("--config-dir=".length);
        continue;
      }
      if (a === "--config-dir") {
        const v = effortSyncArgs[i + 1];
        if (!v || v.startsWith("--")) error("Missing value for --config-dir", ERROR_REASON.USAGE);
        effortSyncConfigDir = v;
        i++;
        continue;
      }
      if (a.startsWith("--runtime=")) {
        effortSyncRuntime = a.slice("--runtime=".length);
        continue;
      }
      if (a === "--runtime") {
        const v = effortSyncArgs[i + 1];
    ...
    commands.cmdEffortSync(cwd, raw, {
      dryRun,
      configDir: effortSyncConfigDir,
      runtime: effortSyncRuntime,
    });
  } else {
...
function routeUserStory({ args, cwd, raw, error }) {
  ...
  if (subcommand !== "validate") {
    error(
      `Unknown user-story subcommand: ${subcommand || "(none)"}. Available: validate`,
      ERROR_REASON.SDK_UNKNOWN_COMMAND,
    );
    return;
  }
  ...
  const story =
    storyIdx !== -1 && args[storyIdx + 1] && !args[storyIdx + 1].startsWith("--")
      ? args[storyIdx + 1]
      : "";
  ...
  if (!trimmed) {
    errors.push(
      'Story is empty. Required format: "As a [role], I want to [capability], so that [outcome]."',
    );
  } else {
    ...
    if (errors.length === 0) {
      ...
      if (!m) {
        errors.push(
          'Story does not match the canonical format: "As a [role], I want to [capability], so that [outcome]."',
        );
      } else {
...
function routeDriftGuard({ args, cwd, raw, error }) {
  ...
  if (subcommand === "severity") {
    ...
    if (!statusVal || statusVal.startsWith("--")) {
      error(
        "drift-guard severity requires --status <VERIFIED|MISSING|AMBIGUOUS|UNCHECKABLE>",
        ERROR_REASON.SDK_UNKNOWN_COMMAND,
      );
      return;
    }
    ...
    const authorityForClassify =
      authVal && !authVal.startsWith("--") ? authVal : effectiveAuthority;
...
const HOST_COMMAND_ROUTERS = {
  ...
  init: (ctx) => {
    try {
      warnIfStaleBake(ctx.cwd);
    } catch {
      /* guard must never break init */
    }
  ...
  "estimate-calibration": ({ args, cwd, raw }) =>
    estimateCli.cmdEstimateCalibration(cwd, args.slice(1), raw),
  "estimate-calibrate": ({ args, cwd, raw }) =>
    estimateCli.cmdEstimateCalibrate(cwd, args.slice(1), raw),
...
function runWithTimeout(argv) {
  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      process.stderr.write(
        `run-with-timeout: ${cmd}: ${err && err.message ? err.message : "failed to start"}\n`,
      );
...
const TOP_LEVEL_USAGE =
  "Usage: gsd-tools <command> [args] [--raw] [--pick <field>] [--cwd <path>] [--project-dir <path>] [--ws <name>] [--json-errors] [--exit-contract=<v>]\n" +
...
function resolveMainWorktreeCwd(cwd, deps = {}) {
  ...
  const resolveWorktreeRoot =
    deps.resolveWorktreeRoot || require("./lib/worktree-safety.cjs").resolveWorktreeRoot;
...
async function main() {
  ...
  if (projectDirEqArg) {
    ...
    if (!projectDirValue || projectDirValue.startsWith("--"))
      error("Missing value for --project-dir", ERROR_REASON.USAGE);
  ...
  if (projectDirValue !== undefined) {
    ...
    if (!fs.existsSync(resolvedProjectDir) || !fs.statSync(resolvedProjectDir).isDirectory()) {
      error(
        `Invalid --project-dir: ${resolvedProjectDir} (path does not exist or is not a directory)`,
        ERROR_REASON.USAGE,
      );
    }
    const resolvedProjectDirPlanning = path.join(resolvedProjectDir, ".planning");
    if (
      !fs.existsSync(resolvedProjectDirPlanning) ||
      !fs.statSync(resolvedProjectDirPlanning).isDirectory()
    ) {
      error(
        `Invalid --project-dir: ${resolvedProjectDir} (no .planning/ directory found — --project-dir must name the project root itself, not an ancestor to walk up from)`,
        ERROR_REASON.USAGE,
      );
    }
  ...
  if (pickIdx !== -1) {
    ...
    if (!pickField || pickField.startsWith("--"))
      error("Missing value for --pick", ERROR_REASON.USAGE);
  ...
  if (pickField) {
    ...
    try {
      ...
      error(
        `--pick ${formatDiagnosticToken(pickField)}: command output was not JSON`,
        ERROR_REASON.PICK_OUTPUT_NOT_JSON,
      );
      return;
    }
    ...
    if (!found) {
      ...
      error(
        `--pick ${formatDiagnosticToken(pickField)}: field not found; ${rootDescription}`,
        ERROR_REASON.PICK_FIELD_ABSENT,
      );
      return;
    }
  ...
  return Promise.resolve()
    ...
    .then(
      ...
      (err) => {
        ...
        if (captured) {
          try {
            originalWriteSync.call(fs, 1, resolveAtFileOutput(captured));
          } catch {
            /* best-effort flush */
          }
        }
