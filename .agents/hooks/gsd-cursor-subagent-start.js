...
} = require("./lib/isolation-sentinel.js");
...
const MSG_ABSENT = "GSD: Subagent session started — no .planning/ workflow found.";
...
function resolveFallbackIsolation(root, configPath) {
  ...
  let declaredIsolation =
    typeof declared === "string" && VALID_ISOLATION.has(declared) ? declared : "none";

  if (
    declaredIsolation === "harness-worktree" &&
    parsedConfig &&
    typeof parsedConfig === "object" &&
    parsedConfig.workflow &&
    typeof parsedConfig.workflow === "object" &&
    parsedConfig.workflow.use_worktrees === false
  ) {
 ...
 */
function evaluateRootIsolation(
  root,
  subagentType,
  { clock = Date, dispatchIds = null, realpath = fs.realpathSync } = {},
) {
  ...
  try {
    ...
    declaredIsolation =
      sentinel.present && !sentinel.stale && sentinelAppliesToDispatch(sentinel, dispatchIds)
        ? sentinel.isolation
...
function main() {
  ...
  process.stdin.on("end", () => {
    ...
    process.stdout.write(
      JSON.stringify(additionalContext !== null ? { additional_context: additionalContext } : {}),
    );
  });
}
