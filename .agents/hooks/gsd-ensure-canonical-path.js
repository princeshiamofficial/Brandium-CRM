...
function ensureCanonicalPath(opts = {}) {
  ...
  try {
    canonicalStat = fs.lstatSync(canonicalDir);
  } catch (_) {
    canonicalStat = null;
  }
  ...
  try {
    ...
    return {
      status: "error",
      reason: `mkdir-canonical: ${e.code || e.message}`,
      canonicalDir,
      bundledTree,
      ...empty,
    };
  ...
  for (const sub of MANAGED_SUBDIRS) {
    ...
    try {
      ...
      targetIsDir = fs.statSync(targetReal).isDirectory() && targetReal.startsWith(bundledWithSep);
    } catch (_) {
      targetIsDir = false;
    }
    if (!targetIsDir) {
