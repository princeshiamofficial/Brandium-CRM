  ...
  function parseScalar(key) {
    ...
    for (const line of lines) {
      ...
      if (
        ...
        if (val === "") {
          throw new Error(
            fileName + ': gsd:loop-host block field "' + key + '" must be a non-empty string',
          );
        }
        return val;
      }
...
function crossCheckRoles(content, agentRoles, fileName) {
  ...
  for (const role of agentRoles) {
    ...
    const agentRe = new RegExp("(^|[^\\w-])" + escapeRegExp(agentName) + "($|[^\\w-])");
    if (!agentRe.test(content)) {
      errors.push(
        fileName +
          ...
          '" but "' +
          agentName +
          '" is not referenced anywhere in the workflow file',
      );
    }
  }
  return errors;
...
function buildContract(workflowsDir) {
  ...
  for (const { file, step, auxiliaryHosts = [] } of STEP_WORKFLOWS) {
    ...
    if (entry.step !== step) {
      allErrors.push(
        file +
          ': gsd:loop-host block declares step "' +
          entry.step +
          '" but expected "' +
          step +
          '"',
    ...
    for (const auxiliary of auxiliaryHosts) {
      ...
      try {
        ...
        allErrors.push(
          "Could not read auxiliary host " + auxiliary.file + ": " + String(err.message),
        );
        continue;
      }
      ...
      const wiredKinds =
        scanWiredKinds(auxiliaryContent, auxiliary.into).get(auxiliary.point) || new Set();
      for (const kind of auxiliary.kinds) {
        if (!wiredKinds.has(kind)) {
          allErrors.push(
            auxiliary.file +
              ': auxiliary host point "' +
              auxiliary.point +
  ...
  if (allErrors.length > 0) {
    throw new Error(
      "Loop host contract generation failed:\n" + allErrors.map((e) => "  " + e).join("\n"),
    );
  }
  ...
  if (pointErrors.length > 0) {
    throw new Error(
      "Loop host contract points coverage failed:\n" + pointErrors.map((e) => "  " + e).join("\n"),
    );
  }
  ...
  for (const line of region.split(/\r?\n/)) {
    ...
    for (const segment of line.split(/(?<=[.;])\s+/)) {
      ...
      for (const kind of HOOK_KINDS) {
        if (new RegExp(`kind\\s*={2,3}\\s*["']${kind}["']`).test(segment))
          kindDiscriminators.push(kind);
      }
      if (kindDiscriminators.length === 0) {
        ...
        if (!expectedInto && deferralRe.test(segment))
          for (const kind of HOOK_KINDS) covered.add(kind);
        continue;
      }
      if (negationRe.test(segment)) continue;
      const narrowed =
        identityNarrowingRe.test(segment) ||
        (expectedInto ? !expectedIntoRe.test(segment) : intoNarrowingRe.test(segment));
...
function scanWiredKinds(text, expectedInto) {
  ...
  for (let i = 0; i < sites.length; i++) {
    const regionEnd =
      i + 1 < sites.length
        ? sites[i + 1].start
        : Math.min(text.length, sites[i].start + REGION_CAP);
...
function getWiredLoopPoints(repoRoot) {
  ...
  for (const relPath of HOST_LOOP_FILES) {
    ...
    try {
      ...
      throw new Error(
        "getWiredLoopPoints: cannot read host-loop file " + absPath + ": " + err.message,
      );
    }
