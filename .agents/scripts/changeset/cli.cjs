...
function injectEmptyPlaceholder(headerOnlyBlock) {
  return headerOnlyBlock.replace(/^(##\s+\[[^\]]+\][^\n]*)\n+/, "$1\n\n_No notable changes._\n\n");
}
...
function cmdRender(opts) {
  ...
  const alreadyPromoted = existingReleases.some((rel) => rel.version === version && rel.date);
  if (alreadyPromoted) {
    if (fragments.length === 0) {
...
function main() {
  ...
  const { exitCode, report } =
    opts.cmd === "render" ? cmdRender(opts) : cmdGithubReleaseNotes(opts);
  if (opts.json) {
    process.stdout.write(JSON.stringify(report, null, 2) + "\n");
