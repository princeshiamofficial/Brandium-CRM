...
const USER_FACING_PREFIXES = ["bin/", "gsd-core/", "src/", "agents/", "commands/", "hooks/"];
...
function main() {
  ...
  try {
    ...
    const out = cp.execFileSync("git", ["diff", "--name-only", `origin/${base}...HEAD`], {
      encoding: "utf8",
    });
  ...
  if (process.argv.includes("--json")) {
    ...
    process.stderr.write(`\nERROR changeset-lint: ${verdict.reason}\n`);
    process.stderr.write(
      `The following .changeset fragment(s) have a stale \`pr:\` field (DEFECT.CHANGESET-PR-FIELD-DRIFT):\n`,
    );
    for (const d of verdict.drift) {
      process.stderr.write(`  ${d.file}: pr: ${d.found}, expected pr: ${d.expected}\n`);
    }
    process.stderr.write(
      `Backfill \`pr:\` with this PR's real number (see .changeset/README.md), then push again.\n`,
    );
  } else {
    process.stderr.write(`\nERROR changeset-lint: ${verdict.reason}\n`);
    process.stderr.write(
      `PR touches user-facing files but does not include a .changeset/*.md fragment.\n`,
    );
    process.stderr.write(
      `Run \`npm run changeset\` to create one, or add the \`${OPT_OUT_LABEL}\` label\n`,
    );
    process.stderr.write(
      `if this PR genuinely has no user-facing impact (test refactor, CI tweak, etc.).\n`,
    );
  }
