...
} = require("../../gsd-core/bin/lib/package-identity.cjs");
...
const FIXED_GROUPS = [
  {
    ...
    pattern:
      /\b(verifier|verification|verify|probe|probes|debt|tbd|fixme|xxx|detect-custom-files|review|summary|blocker|critical)\b/i,
  },
  {
    title: "State, planning & execution",
    pattern:
      /\b(state|planning|planner|plan-phase|phase|roadmap|execute|executor|worktree|worktrees|resolve-model|init\.progress|model override|human_needed|ship preflight)\b/i,
  },
  {
    title: "Install & runtime conversion",
    pattern:
      /\b(install|installer|runtime|windows|powershell|codex|gemini|antigravity|hook|hooks|gsd-sdk|sdk readiness|cjs|model-catalog|path|shim)\b/i,
  },
];
...
function formatBullet(fragment) {
  if (!Number.isInteger(fragment.pr) || fragment.pr <= 0) {
    throw new Error(
      `Fragment ${fragment.slug || fragment.file || "<unknown>"} missing valid pr field`,
    );
  }
