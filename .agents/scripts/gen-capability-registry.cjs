...
function loadCentralConfigKeys(schemaPath = CONFIG_SCHEMA_PATH) {
  ...
  try {
    ...
    process.stderr.write(
      "  ERROR  Config-schema manifest at " +
        schemaPath +
        " is broken (JSON parse error): " +
        err.message +
        "\n",
    );
...
function deriveProfileMembership(capMap) {
  ...
  for (const [capId, cap] of capMap) {
    ...
    if (tierIdx === -1) {
      throw new Error(
        'deriveProfileMembership: capability "' +
          capId +
          '" tier "' +
          cap.tier +
...
function runConsistencyGate(capabilityClusters, profileMembership, capMap) {
  ...
  for (const capId of Object.keys(capabilityClusters)) {
    ...
    if (mismatch) {
      throw new Error(
        'capability-cluster consistency gate FAILED for capId "' +
          capId +
          '":\n' +
          "  derived set:      [" +
          [...derivedSet].sort().join(", ") +
          "]\n" +
          "  hand-authored set: [" +
          [...handAuthoredSet].sort().join(", ") +
          "]\n" +
          "The capability's skills array must match the hand-authored CLUSTERS[\"" +
          capId +
          '"] at cutover.',
      );
    }
  ...
  for (const capId of Object.keys(capabilityClusters)) {
    ...
    for (const skill of cap.skills) {
      ...
      if (missingProfiles.length > 0) {
        warnings.push(
          "⚠ pending-reconciliation: capability '" +
            capId +
            "' (tier " +
            membership.tier +
            ")" +
            " skill '" +
            skill +
            "' not yet in hand-authored profile(s): <" +
...
function loadAndValidate(centralKeys, capabilitiesDir, centralPatterns) {
  ...
  const resolvedCentralPatterns =
    centralPatterns !== undefined ? centralPatterns : loadCentralConfigPatterns();
  ...
  const folderEntries = fs
    .readdirSync(resolvedCapDir, { withFileTypes: true })
  ...
  for (const folderId of folderEntries) {
    ...
    for (const w of collectReviewerWarnings(cap))
      warnings.push(folderId + "/capability.json: " + w);
...
function buildRegistry(capMap) {
  ...
  for (const [capId, cap] of capMap) {
    ...
    for (const cmd of cap.commands) {
      ...
      if (cmd.family === "__proto__" || cmd.family === "constructor" || cmd.family === "prototype")
        continue;
...
function serializeRegistry(registry, capMap) {
  ...
  lines.push(
    "const capabilityClusters = " + JSON.stringify(sortedCapabilityClusters, null, 2) + ";",
  );
  lines.push("");
...
function main() {
  ...
  if (flag === "--check") {
    ...
    if (
      normalizeEol(stripGeneratedComment(committed)) !== normalizeEol(stripGeneratedComment(live))
    ) {
      process.stderr.write(
...
module.exports = {
  ...
  get PROFILE_RANK() {
    return getInstallProfiles().PROFILE_RANK;
  },
  get CLUSTERS() {
